...
- **PR #6b ✅ DONE (#179, 7f38fba)** — match-to-game attach flow. Age-gate → HealthKit auth → time window (matchDate + kickoffTime) → queryWorkouts → sanity-clamp → confirm/picker → queryRoute + trimRoute → saveMatchHealthSummary → refresh. `dbToMatch` kickoffTime additive field. DARK behind `VITE_HEALTH_KIT_ENABLED`. Remaining gates: G2 (Apple Developer), G3 (Mac/Xcode), G4 (App Store submit), G5 (real-device walk), then set `VITE_HEALTH_KIT_ENABLED=true`.
- **PR #7 ◻ IN PR (#182)** — indoor/outdoor venue preference per recurring game. `venue-preference.js` (localStorage `venue_pref_<teamId>`). Written from `doSave` after successful attach. `dbToMatch` teamId additive field. Phase-2 watchOS auto-start consumes this. DARK. **Loop STOPS: operator merges #182 when ready. Epic COMPLETE on merge.**
