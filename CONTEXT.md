...
*Last updated: Jun 29 2026 (session 230 — UNIVERSAL AI AGENT foundation (Pillar D) shipped: mig 454 = `ai_agent_access` opt-in canary/cost-ceiling table + `resolve_agent_caller` unified identity resolver across all 5 auth models (→ one caller-context jsonb, phase=1 answer-only) + `resolveAgentCaller` core wrapper (authenticated/anon client). EV 10/10 + leak-0, rpc-security PASS, live smoke PASS (team_demo→casual_admin, opt-in gate off); applied + PR #150 (feat/agent-caller-foundation). Grants gotcha fixed: project ALTER DEFAULT PRIVILEGES auto-grants anon+authenticated on new tables/functions → REVOKE FROM PUBLIC alone is insufficient, REVOKE from the named roles. Prior-Gaffer audit: built s33, NEVER wired live (0 ai_briefings rows); new Gaffer is grounded-data-only — navigation ("where do I X") is the ARCHIVED _archived_chatbot.jsx's job, slated for Pillar B / Phase 2. NEXT = Stage 1 casual-canary wiring (ENABLE_GAFFER→VITE_GAFFER_ENABLED env-gate + GafferCard placement on admin home + sonnet-4-5→4-6 bump + fix mis-wired App.jsx:1984 call site), a SEPARATE PR. Next free mig = 455.)*

## SESSION 171 (Jun 21 2026) — UNIFIED LOGIN SHIPPED (migs 376–377), LIVE on main

One account sign-in for **admins AND players**. Before, the same person reached the app
via three token-in-URL routes (`/admin/<admin_token>`, `/p/<player_token>`, `/join/<code>`).
Now: sign in once → land straight in your team (admin powers on if you're an admin); a
**"YOUR TEAMS"** chooser appears only for people in multiple teams. Old token links still
work as a web fallback; nobody is logged out.

- **mig 376** `get_my_admin_teams()` (authenticated-only SECDEF, read-only) — every team the
  account is a verified admin of (`team_admins.user_id=auth.uid() AND revoked_at IS NULL`),
  WITH its `admin_token`, so the account landing opens the admin view without the secret
  `/admin/<token>` URL. Admin WRITE RPCs UNCHANGED (still authorise via `p_admin_token`) — a
  bridge, not a new admin system, reusing the existing `team_admins.user_id` table.
- **mig 377** `claim_team_admin(p_admin_token)` — a signed-in user opening a valid `/admin/<token>`
  link is auto-enrolled as a real account-admin (INSERT `team_admins` role `'team_admin'`,
  `person_id=ensure_person(uid)`; idempotent; audit `admin_self_enrolled`, actor_type
  `'team_admin'`). Mig 377 also adds the partial unique index
  `team_admins_team_user_active_uniq` (one active admin row per team+user — see SCHEMA section).
- Account landing changes ONLY the `squad_only` path; multi/parent/club_member home types
  unchanged; non-admin routing byte-identical (empty admin list → existing `/p/<token>`).
  AuthCallback hardened to clear stale resume breadcrumbs on a generic sign-in. Apple
  "Hide My Email" safety net for relay-email + no-team users.
- **Data backfill (one-off):** Footy Tuesdays (`team_KPaoX8oJYMQ`) four vice-captain admins
  (Bidz, gbains2010=Gurnam, rockybram=Rocky, Tarny) pre-enrolled into `team_admins` so they
  land straight in the admin view (audit `admin_backfill_enrolled`).
- Commits `28b71d9` / `31fb00d` / `22a15c2` (reverted `5962813` then re-applied `46807eb` —
  login-loop scare, see BUGS.md s171) / `57d563e` / `7a228a8`. EV for `claim_team_admin`
  7/7 + leak 0; persona walk on a Footy-Tuesdays-shaped throwaway mirror (real logins, torn
  down, leak 0) proved all paths. ⛔ OWED: native real-device launch/landing/auth walk.
  **Next free mig = 378.** See FEATURES.md / DECISIONS.md / RPCS.md / BUGS.md / GO_LIVE_ISSUES.md s171.
...
Design, `WATCH_DESIGN_HANDOFF.md` in progress); (2) **Phase 1 identity backend, full cycle.**
Net-new, additive, invisible until the watch ships. **Two identity arms, one resolver:** LEAGUE/
club-fixture arm = the OFFICIAL model (`match_officials.user_id`, claimed by verified email or
operator-bound) → `fixtures.official_id` → `ref_token`; CASUAL arm = the PLAYER model (REUSES the
existing `players.user_id` link — NO new claim RPC; the squad member IS the identity) → per-game
`matches.ref_player_id` → reserved `matches.ref_token`. Claim model = option A (operator-assign +
self-claim) **applied to both arms** per operator s162 — but casual's self-claim half is the
already-built player link. Cohort officiating folds into the fixture arm (`club_cohorts.
primary_official_id` = default-official convenience; `club_sessions` refereeing deferred to Phase 6).
Mig 369: 4 additive columns + 5 SECDEF RPCs (`get_my_next_assignment` resolver [LOCKED shape, in-
progress > soonest kickoff > role priority, "N games" chooser; authenticated-only] + `ref_link_self_
to_official` + `venue_link_official_to_user` + `assign_casual_match_ref` + `club_admin_assign_cohort_
official`). JS wrappers + barrel (additive; unused in inorout yet). VERIFY: rpc-security PASS (anon
revoked on the 2 account-scoped fns), ephemeral-verify 9/9 + leak 0, build clean, casual-regression
additive-diff PASS. Native Phases 0/2/3+ still gated on iOS approval (need Mac + Apple Watch). Note:
`xcode-select` now points at full Xcode.app (watchOS prep) → Apple's git CLI needs a one-time
`sudo xcodebuild -license accept` (operator action); CLT git used this session. Next free mig = **370**.

**Session 161 (Jun 19 2026) — WATCHOS COMPANION APP SCOPE-LOCKED + FULLY PLANNED (planning session, no code).** Took the s160 logged-only watchOS idea to a complete, scope-locked plan. Native SwiftUI watch target added inside the existing Capacitor iOS Xcode project (`apps/inorout/ios`) → one App Store record, the watch is the only native code, runs independently (watchOS 10+); iPhone app untouched. Full app, all contexts, football-first: league ref (fixtures) + casual ref (one squad member assigned per game, net-new "casual ref slot") + club cohort officiating (net-new cohort→official binding). Identity-first resolver `get_my_next_assignment` keyed on `auth.uid()` (net-new ref/official identity linking `match_officials`↔user) returns the next relevant game + its `ref_token`; disambiguates multi-role same-day games + a "you have N games" chooser. Provider-agnostic watch auth (users are on Google/email/company-SSO/Apple = one Supabase pool): WatchConnectivity session-handoff from phone (primary) → email-OTP (fallback) → native Sign in with Apple (bonus); no direct Google/company OAuth on watchOS. HealthKit refs-only, auto-start on watch "Start Match" / auto-stop on "Full Time" (no manual controls; warn health only runs if started from the watch); metrics mirror Apple Outdoor Football (duration, active energy, distance, avg+max HR) + watchOS-27 HR Workout Zones (live + post-match, fallback to avg/max HR on watchOS 26); summary stored in new `match_health_sessions` (UK-GDPR special category → summary-only + MUST cascade into `api/delete-account.js` same migration). 2026/watchOS-27 features committed to v1: Always-On display, haptics, Double Tap, complication + Smart Stack widget, Live Activity + Dynamic Island on iPhone (live score/clock/HR via existing realtime). Tech: supabase-swift direct to existing `ref_*` RPCs (port the `apps/ref` offline/idempotency engine verbatim). Phases 0–7, migrations from 369, build AFTER iOS App Store approval, run solo. ETA recalibrated to AI-build velocity (bottleneck = operator real-device testing + Apple review, NOT code): MVP ~1–2 wks, full app ~2–4 wks + review. Docs written this session: full plan `~/.claude/plans/once-the-ios-app-dapper-marshmallow.md`, `WATCH_DESIGN_BRIEF.md` (+ handoff-back contract), FEATURES.md + DECISIONS.md (s161) + memory `project_watchos_companion`. Design track runs in parallel with Phase 1 backend. No migrations applied, no code touched.

**Session 149 (Jun 18 2026) — CLASSES OPEN/FREE/TRIAL ACCESS SHIPPED (mig 360).** Brought the gym PT booking's two levers to classes: an ACCOUNT (auth.uid→member_profiles) is always required; a paid MEMBERSHIP is optional per class type via the new `venue_class_types.members_only` flag (additive boolean NOT NULL DEFAULT true → existing classes byte-identical). members_only=false + price 0 = free open/trial; +price>0 = paid drop-in (door now, Stripe prepay dormant). Audit found the handoff's "three gate sites" was off — the live `member_claim_waitlist_spot` has no membership check, and packs stay member-only (operator) — so ONLY `member_book_class_session` changed (membership EXISTS wrapped in `IF COALESCE(members_only,true)`). Flag plumbed through venue_create/update/list_class_type (+11th arg on create, old 10-arg DROPped) + member_list_class_sessions; operator toggle + "Open" pill in ClassesView; member "Open to all"/Free label in ClassesTimetable. Gates: rpc-security PASS (5 RPCs), EV 5/5 + leak 0 (non-member books open-paid→£10 door charge; open-free→waived/no charge; rejected on member-only; member books member-only; column default true), casual-regression PASS (additive-diff), boot smoke clean, build inorout+venue + hygiene 7/7. ⛔ real-iPhone PWA walk OWED. Next free mig = 361. NEXT PLANNED (operator sequence s149): domain migration (DOMAIN_MIGRATION.md) → app-store submission (Capacitor native wrap). Domain migration PARKED s149 — BLOCKED on operator Phase 1 (app.in-or-out.com confirmed NXDOMAIN this session); Claude runs Phases 2+4+5.1 once app. is live. Status banner + paste-ready next-session prompt added to DOMAIN_MIGRATION.md "0bis. STATUS"; next free mig = 361.
*Prev: Jun 21 2026 (session 171 — UNIFIED LOGIN SHIPPED (migs 376–377), LIVE on main: one account sign-in for admins AND players. Two authenticated-only SECDEF RPCs — get_my_admin_teams (mig 376, read-only; verified admin's teams + admin_token so the landing opens admin view without the /admin/<token> URL; admin WRITE RPCs unchanged, bridge over existing team_admins.user_id) + claim_team_admin (mig 377, auto-enrols a signed-in admin-link holder; idempotent; audit admin_self_enrolled actor_type 'team_admin'). Mig 377 adds partial unique index team_admins_team_user_active_uniq (one active admin row per team+user). Sign-in lands you straight in your team; "YOUR TEAMS" chooser only for multi-team people; old token links still work; nobody logged out. Footy Tuesdays 4 VC admins backfilled. Login-loop scare resolved as a stale Safari breadcrumb (not a code regression) + AuthCallback hardening. Next free mig = 378. See SESSION 171 at top. PRIOR session 147 — GYM/BOXING VERTICAL Phase 3 SHIPPED (mig 358): PT / 1-on-1 appointment booking. Operator confirmed the timing gate (today = pilot day) + 3 build answers, then a full AUDIT→EXECUTE→VERIFY→COMMIT cycle. 3 RLS-walled tables: venue_trainers (admin_id NULLABLE = staff login OR no-login coach card), venue_trainer_availability (recurring weekly windows sliced into slots), venue_appointments (partial-unique (trainer_id,starts_at) WHERE status<>'cancelled' = one live booking per slot). 11 RPCs: operator venue_upsert_trainer / venue_set_trainer_availability / venue_list_trainers / venue_list_appointments / venue_pt_checkin (clone of venue_class_checkin) / venue_mark_appointment_completed (no-show bumps member_profiles.no_show_count + KEEPS charge); member member_list_trainers / member_list_trainer_slots (availability minus booked, future-only, Europe/London tz) / member_book_appointment (writes venue_charges source_type='pt', door) / member_cancel_appointment (per-trainer cutoff) / member_list_my_appointments. DEDICATED appointments model (NOT capacity=1 classes). TWO LEVERS: an account is ALWAYS required (auth.uid→member_profiles); per-trainer members_only adds the active-membership requirement — members_only=false + price 0 = a free open session ("A, but B for trials/one-offs", operator s147). Cancellation cutoff per-trainer (0=free). Money on the shared venue_charges ledger (settlement DORMANT); venue_charges_source_type_check extended to allow 'pt' (mig 358b — caught by EV: first priced booking failed without it). Operator: new apps/venue TrainersView (Trainers + Appointments tabs; ClassCheckinScanner generalised with an optional `checkin` cb for venuePtCheckin). Member: new /book route + BookPT view + ClubNavBar Train tab, gated on disciplineLabels.hasPT (gym/boxing/martial_arts/fitness → casual football byte-identical). EV 9/9 + leak 0, rpc-security PASS (11 RPCs: SECDEF/search_path/overload=1/grants), casual-regression PASS (additive-diff — no casual view touched, App.jsx purely additive), Playwright /book boot smoke 0 console errors, build inorout+venue + hygiene clean. OPT-IN follow-up: retrofit members_only+price-0 to the classes epic (free/trial classes) if the operator wants it — not bundled into 358. ⛔ real-iPhone PWA walk OWED (/book Train tab + booking + operator QR check-in; needs an authed member on a PT-discipline club). Next free mig = 359; Phase 4 (fight record) is the last phase. See DECISIONS s147. | PRIOR session 146 — GYM/BOXING VERTICAL Phase 2 SHIPPED (mig 357): grading / belt progression. 3 RLS-walled tables (venue_grading_schemes / venue_grades / member_grades append-only, "current = latest per member+scheme", monotonic awarded_seq tie-break); 5 RPCs (writes gated manage_facility + audited): venue_create_grading_scheme / venue_add_grade / venue_award_grade (caps stripes at grade max, returns at_max) + venue_list_grading_schemes / member_get_grade_history; get_member_pass extended with current grades[], venue_list_members extended with club_id+discipline (additive). Operator: Grading sub-tab in MembershipsView (per-club scheme+grade setup, gated to martial-arts clubs) + Award grade on member cards. Member: rank chip on MemberPass + Progression history on MemberProfile, gated on disciplineLabels.hasGrading (martial_arts only → casual football byte-identical). Operator decisions: award authority = existing manage_facility cap (1A); ladder = named+coloured grades + per-grade max_stripes + capped stripes, age bands = SEPARATE schemes (2B + BJJ/TKD research) — see DECISIONS s146. EV 11/11 + leak 0 (caught the awarded_seq same-txn tie-break bug pre-commit, fixed by the identity column), rpc-security PASS (6 RPCs), casual-regression PASS (additive-diff + Playwright boot smoke 0 app errors), build inorout+venue + hygiene clean. ⛔ real-iPhone PWA walk OWED (Pass chip + Profile history; martial-arts club). Next free mig = 358. Phase 3 (PT booking) is next. See SESSION 146 entry below. PRIOR session 145 — GYM/BOXING VERTICAL Phase 1 SHIPPED (mig 356): sparring / open-mat availability. `venue_class_types.is_sparring` flag (a class type is EITHER technical OR sparring; operator picks at create). NO new write RPC — booking reuses the class-session model wholesale; threaded `is_sparring` through `venue_create_class_type` (signature change → DROP old 9-arg + re-grant) / `venue_update_class_type` / `venue_list_class_types` / `member_list_class_sessions`, + `discipline` added to `get_member_pass` (Pass-surface nav). Operator toggle in ClassesView; member surface = new `/classes` route → new `ClassesScreen` (club+venue picker) rendering the reused `ClassesTimetable` (sparring-badged); `ClubNavBar` Classes tab lit for non-football disciplines only (football byte-identical). EV 5/5 + leak 0, rpc-security PASS both write RPCs, casual-regression PASS (additive-diff proof), build inorout+venue + hygiene clean, Playwright `/classes` boot smoke 0 errors; real-iPhone PWA walk OWED. Next free mig = 357. See SESSION 145 entry below. PRIOR session 144 — GYM/BOXING VERTICAL Phase 0 SHIPPED (mig 355): club `discipline` identity (text+CHECK pick-list, default football, zero casual footprint) + member-app `disciplineLabels.js` + gated/audited `venue_set_club_discipline` RPC, threaded `deriveClubContext`→`ClubNavBar`. Operator settled scope: discipline-as-pick-list, NOT a generic config engine (DECISIONS s144); boxing=fight-record not grades. EV 7/7 + leak 0, rpc-security/casual-regression/build/hygiene PASS; real-iPhone walk OWED. Next free mig = 356. See SESSION 144 entry below. PRIOR session 143 — CLUB/MEMBERSHIP SYSTEM END-TO-END AUDIT + 3 FIXES, all merged to main. (1) BST recurring-session timezone bug (mig 353, PR #13): venue_create_class_series / club_create_session_series / club_manager_create_session_series built session timestamps with a bare `(date+time)::timestamptz` cast → stored the UK wall-clock as UTC → every recurring class/training displayed & triggered 1h late during BST (booking cutoffs, QR check-in, conflict detection all off). Fixed to `AT TIME ZONE 'Europe/London'` (pattern from migs 143/181; same class as the mig-207 game-time fix); one-off creators take a client timestamptz so were untouched; 0 existing future series rows → no data correction. (2) Multi-club nav residual club[0] sites (PR #14, no mig): the s141 `?club=` fix missed MemberProfile (hardcoded active_clubs[0].pass_token) + ClubNavBar (Sessions/Profile tabs dropped ?club=) — both now thread the selected club. (3) MemberPass nav club context (mig 354, PR #15): added `club_id` to get_member_pass return + clubEntry on MemberPass → all 3 multi-club nav sites now keep the club. AUDIT VERDICT: memberships / class scheduling / member booking-cancel-waitlist-checkin-packages / club sessions+attendance / context routing all verified wired live; multi-sport is real schema but DORMANT logic (nothing branches on `sport`). Migs 353+354 applied + source-synced; next free mig = 355. OWED: real-iPhone walk for the nav changes (flag-gated). See SESSION 143 entry below. PRIOR session 142 — MULTI-CONTEXT NAV Phase 1 cleanup + Phase 2 guided tours, BOTH MERGED to main (PR #10 then #11). Phase 1 cleanup: MemberPass now renders the club nav bar for the pass OWNER only (the Pass tab no longer strands; a reception scan / shared link stays bar-less); new `lib/lastContext.js` gives a structured, all-context resume point (full path incl. ?club=, 30-day freshness) written on every resumable route and PREFERRED over the squad-only `ioo_last_visited` breadcrumb in the root resume bridge — so a multi-context member reopens where they actually were, not a dormant squad (squad-only users byte-identical). Classes nav tab still deferred (no /classes route). Phase 2: revived the dormant FirstTimeHint into a SPOTLIGHT GUIDED-TOUR engine — `components/Tour.jsx` (full dim + glow-ring from getBoundingClientRect, poll-until-mounted with graceful skip of absent targets, recompute on scroll/resize/orientation rAF-throttled, scroll-into-view, prefers-reduced-motion, auto-advance-on-tap [off for admin tiles that navigate away], Skip/Next/counter, SEEN-ON-SHOW, suppressed while any [data-tour-suppress] overlay open + resumes on clear → SquadReady→install→tour ordering free, portals to body) + `lib/tourRegistry.js` (tours per (context type, screen): casual/comp myview+stats, admin dash, club sessions/classes/pass/profile, guardian home, switcher) + `components/TourProvider.jsx` (flag gate for the inline coachmarks, no prop-threading). FirstTimeHint revived (git 0a1e759) as the inline-coachmark layer for deep-screen one-offs; the 2 tour-covered sites (PlayerView status, StatsView H2H) became plain data-tour markers (no double-cover). Gating: squad tours on `teams.multi_context_nav`; club/guardian on `localStorage.ioo_tours_preview` (default off). Ships DARK; localStorage-only → NO migration (next free mig was 353 at s142; s143 consumed 353+354 → next free mig = 355). Switcher guide folded into the myview tour's final avatar step (a separate tour would collide on the same avatar). Verified: inorout build clean, hygiene 7/7 on all 20 files, Playwright dev-server smoke proved the engine end-to-end (spotlight, graceful skip, centred card, Next, seen-on-show survives reload, suppression hides+resumes) AND flag-OFF = zero change on the casual surface; caught+fixed a framer-motion transform-clobber (card off-screen). OWED: real-iPhone PWA walk (HR#13) before enabling for any team. | session 141 — MULTI-CONTEXT NAV Phase 1 BUILT (migs 349–351, all read-only/additive; NOT yet committed to git — real-iPhone walk owed before commit per Hard Rule #13). DB: mig 349 adds context fields (team_type/is_competitive/club_id/club_name) to both team-state RPCs + mappers; mig 350 new `guardian_list_children_sessions` read RPC (children × all upcoming training+matches across all their clubs + RSVP); mig 351 `teams.multi_context_nav` kill-switch column + `get_team_feature_flags` RPC + `member_get_self.active_clubs[].pass_token`. All 3 applied live + verified (mig 350 full seed-and-rollback EV PASS + leak 0; flag default off). FRONTEND (apps/inorout): new `lib/deriveContext.js` (pure descriptor), config-driven `NavBar` (legacy fallback → footballer's bar byte-identical), `ContextSwitcher` overlay (header-avatar, squad routes, flag-gated; routes every squad to /p/<token> so ADMINS get the Admin tab — they used to hit the dead multi-team block, now → /feed when flag on), `ClubNavBar` (Sessions·Pass·Profile) on SessionsScreen+MemberProfile (unstrands club/guardian users), guardian Home rebuilt child-first on the new RPC with In/Out per fixture + child chips, /feed install manifest (?feed=1, path-relative — inherits BASE_URL for the domain move), multi-club `?club=` bug fix. Two pre-existing bugs fixed (multi-club selection, stranded users). Everything squad-route ships dark behind the per-team flag (default off) = casual flow unchanged; build 8/8 apps clean; hygiene 7/7 every file. Phase 2 (guided tours) deferred to a later session. mig 352 (added during browser smoke-test): fixed pre-existing get_user_relationships squad join (p.team_id→team_players, p.player_token→p.token) that 400'd for every authed user. Browser smoke test (Playwright) verified squad routes + guardian /parent-home + /sessions + Pass deep-link; caught+fixed a hooks-order crash + mig 352. Next free mig = 353. ⛔ OWED: operator real-iPhone PWA install walk (App.jsx routing/PlayerView/NavBar). session 137 — PAYMENT INFRASTRUCTURE PHASE 4: STRIPE TEST CLOCK LIFECYCLE. No mig, no code changes. Pure Stripe-side + state machine verification. Connected Express account: `acct_1TiiBMETFLuZs6P2`. Test clock: `clock_1Tir9kETFLuZs6P26l5r435Y`. Stripe CLI installed (brew, v1.42.13) and authenticated. Test fixtures created on connected account: product `prod_UiHjiqiUIDdLLI` (Full Adult), price `price_1TirAAETFLuZs6P2rY72vGEM` (£40/month), customer `cus_UiHj1DLWH1Wjop`, subscription `sub_1TirAhETFLuZs6P24yTW6YJP`. Test DB row seeded via `stripe_complete_member_enrolment` RPC (membership_id `1fd5fbd3`, pass_token `m_a229ac0a3b6c481f83cffb7d448c31b4`, member: Tarnbir Athwal, club: Finbar's FC). **All 5 state-machine scenarios PASS:** (A) clock advanced +1 month → Stripe auto-billed renewal invoice `in_1TirE9ETFLuZs6P2NfFbiyJD` → subscription `active` → `apply_membership_subscription_status('active')` → `payment_state=current` + member pass clean. (B) `past_due` → `payment_state=past_due` → `get_member_pass` returns `payment_state:past_due` → amber banner in `MemberPass.jsx`. (C) `unpaid` → `payment_state=suspended` → red banner. (D) Recovery: `active` → `payment_state=current`. (E bonus) `canceled` → `payment_state=suspended` AND `status=cancelled` — the ONLY Stripe status that also flips the membership `status` column. **Reconciliation cron PROVEN LIVE:** `membershipReconciliationJob` (cron.js:803) ran at 03:00 UTC (= 04:00 BST) and logged `membership_payment_state` audit event on real E2E membership `e438727e` — confirms cron fetches live subscription status from Stripe and applies it via the RPC. Webhook-not-needed resilience confirmed. **Auto-renewals CONFIRMED:** Stripe billing engine auto-creates and charges renewal invoices. `invoice.paid` fires. Stripe natively generates invoice objects (no custom PDF receipts needed). **Key findings:** (1) `platform-clubmanager` (not `inorout`) is the live serving project — has STRIPE_SECRET_KEY + SUPABASE_SERVICE_ROLE_KEY + all env vars; both projects map to in-or-out.com. (2) `inorout` Vercel project only has STRIPE_WEBHOOK_SECRET — webhook handler would return 503 if served from that project. (3) Test clock events from Express connected accounts have latency to platform Connect webhooks; `billing_events` stayed 0 during test — expected, reconciliation cron is the truth source. (4) Declining test cards (`tok_chargeDeclined`, `tok_chargeDeclinedExpiredCard`) fail at customer attach time; direct RPC calls used for state machine verification instead. **Test row restored to clean state** (`status:active, payment_state:current`) after Scenario E. **Phase 4 status: LIFECYCLE PROVEN. Go-live gate: operator must sign DECISIONS.md MONEY-FLOW GATE + swap test keys for live keys in `platform-clubmanager` Vercel project (NOT `inorout`).** Next mig = 337.) (session 136 — PAYMENT INFRASTRUCTURE PHASE 3 AUDIT ONLY (no mig, no code). Phase 3 (Stripe member enrolment + webhooks, mig 331) fully scoped and designed. Blocked on operator adding 4 Stripe env vars to inor-out Vercel project (confirmed zero env vars via `vercel env ls`). Agreed scope for mig 331: (1) `stripe_customer_id text` column on `venue_memberships`; (2) UNIQUE constraint on `venue_memberships.stripe_subscription_id`; (3) `get_venue_signup_tiers` extended with `stripe_connected: boolean`; (4) `get_member_pass` extended with `payment_state`; (5) new `stripe_complete_member_enrolment` RPC (service_role only, idempotent on `stripe_subscription_id`) — creates membership row from webhook context with Stripe IDs. New file: `api/stripe-member-checkout.js` (POST: verifies auth JWT, creates Stripe Customer + Checkout Session on connected account, returns checkout_url). `api/stripe-webhook.js` extended with `checkout.session.completed` arm. `MembershipSignup.jsx` forks at StepEnrol: paid + stripe_connected → checkout redirect; free or no Stripe → existing `member_enrol_membership` direct. `MemberPass.jsx` adds PaymentStateBanner (amber for past_due, red for suspended). `supabase.js` mappers extended. Operator must also register webhook endpoint in Stripe Dashboard: `https://in-or-out.com/api/stripe-webhook` with events checkout.session.completed, customer.subscription.*, invoice.paid/payment_failed, account.updated. EV plan = 7 assertions. Next mig = 331. Phase 3 execute ready to run as soon as keys are confirmed set.) (session 133 — PAYMENT INFRASTRUCTURE PHASE 2 (mig 330, commits 69cdf65 + b38fb2b). POST-DEPLOY COMPLETE. Stripe Connect venue OAuth flow activated. `venue_stripe_disconnect(p_venue_token)` RPC (SECDEF, anon+authenticated, idempotent, EV 5/5 PASS, leak-clean). `api/stripe-connect.js` unblocked with CORS for platform-venue.vercel.app (OPTIONS preflight handled; origin controlled via STRIPE_CONNECT_ALLOWED_ORIGIN env). `IntegrationsView.jsx`: Connect Stripe button → POST /api/stripe-connect {action:'onboard'} → redirect to Stripe Express onboarding; return URL handler (?connect=done/refresh auto-calls refresh action + reloads billing); disconnect button → venueStripeDisconnect RPC; graceful 503 message if keys absent. `Dashboard.jsx`: useState initialiser auto-switches to integrations view when ?connect=* in URL. `venueStripeDisconnect` wrapper + barrel export. Venue app redeployed prebuilt-static (bundle index-B3nNwqj1.js live at platform-venue.vercel.app — confirmed stripe-connect + venue_stripe_disconnect in bundle). Operator still needed: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, STRIPE_CONNECT_RETURN_URL=https://platform-venue.vercel.app/?connect=done, STRIPE_CONNECT_REFRESH_URL=https://platform-venue.vercel.app/?connect=refresh in inorout Vercel project before Phase 3. VITE_INOROUT_API_URL now baked into venue bundle. Next mig = 331. Phase 3 (member enrolment + webhooks) blocked on operator providing Stripe test keys.) (session 130 — BUG FIX (mig 328, commit b2495fd): mig 326 write RPCs club_admin_set_performance_config / club_admin_add_performance_event / club_admin_record_result all had audit_events INSERTs missing entity_type + entity_id (both NOT NULL since mig 003), throwing a NOT NULL constraint violation on every call. Fixed via CREATE OR REPLACE in mig 328 with correct INSERT pattern — entity_type/entity_id now written for each RPC ('tournament_event'/p_tournament_event_id, 'performance_event'/v_event_id, 'performance_result'/v_result_id). EV 9/9 PASS, leak-clean. Security sweep 3/3 PASS. Event OS not yet live so no users affected. Next mig = 329.) (session 130 — EVENT OS PHASE 7 COMMERCIAL (mig 327). Sponsors, branding, Player of Tournament, equipment hire bundle. 9 new SECDEF RPCs (authenticated-only). tournament_sponsors table. get_tournament_public + club_admin_get_tournament extended. Director UI in SessionsScreen + public page branding/sponsor/POT in TournamentScreen. Bug caught by EV: all 6 write RPCs had missing entity_type/entity_id in audit_events INSERTs — fixed before commit. 13/13 EV PASS. Event OS all phases complete. Next mig = 328.) (session 129 — EVENT OS PHASE 6 PERFORMANCE EVENTS (mig 326, commit bc46b7b). Athletics/sports-day model: disciplines, judge result entry (UPSERT), best-of-N ranking, per-event leaderboard, overall sports day standings, configurable points locked per tournament. 6 new SECDEF RPCs (authenticated-only). get_tournament_public extended (4th version) with performance_events[] + performance_standings[]. Director UI in SessionsScreen + public view in TournamentScreen. Phases 0–7D + Phase 6 complete. Next mig = 327. Phase 7 Commercial is next.) (session 128 — EVENT OS PHASE 7D DOUBLE ELIMINATION (mig 325, commit ebe1972). Phases 0–7D complete. Next mig = 326. Phase 6 Performance Events is next.) (session 127 — EVENT OS PHASE 7C CLASSIFICATION BRACKETS (mig 324, commit 6f40e11).) (session 126 — EVENT OS PHASE 7B CARD TRACKING + AUTO-SUSPENSION (mig 323, commit 926f561); also fixed club_admin_get_standings crash — club_admins table never existed, replaced with club_team_managers guard.) (session 125 — EVENT OS PHASE 7A H2H TIEBREAKER (mig 322, commit f26d7c9).) (session 124 — EVENT OS PHASE 6 PUBLIC TOURNAMENT PAGE (mig 321, commit 312ec78).) (session 123 — EVENT OS PHASE 5 TOURNAMENT MATCH DAY OPS (mig 320, commit b3d9f98).) (session 122 — EVENT OS PHASE 4 SCHEDULING + DAY OPS (mig 319, commit from s122).) (session 121 — EVENT OS PHASE 3 INVITATIONS + REGISTRATION part 2 (mig 318).) (session 120 — EVENT OS PHASE 2 + PHASE 3 part 1 (mig 317).) (session 119 — EVENT OS PHASE 1 SCHEMA APPLIED + RPCS + UI (commits e326b40 + 84c22c4). (1) Applied mig 315 to live DB: 3 new tables (tournament_events / performance_events / performance_results) + 4 ALTERs (competitions.tournament_event_id, competition_teams.waitlist_position, league_config.ref_ui_config, playing_areas.sport_types) — all confirmed via pg_proc. (2) mig 316 — 3 club_admin_* RPCs: club_admin_create_tournament (9 params: club_id, venue_id, name, slug, event_date + opt end_date/fee/payer/deadline; validates club, club_venues association, slug format, date order; inserts tournament_events + audit_events; slug UNIQUE constraint caught at DB level), club_admin_list_tournaments (list for a club, ordered by event_date DESC), club_admin_get_tournament (by slug, returns nested performance_events[] + competitions[]). Auth pattern: auth.uid() → member_profiles.auth_user_id → club_team_managers.member_profile_id + club_teams.club_id; same uid()-based pattern as mig 314. All SECDEF/search_path locked/authenticated-only/1 overload each. 3 JS wrappers (clubAdminCreateTournament/clubAdminListTournaments/clubAdminGetTournament) in supabase.js + barrel exports. RPC security sweep 3/3 PASS. (3) Phase 1 UI — SessionsScreen.jsx: Tournaments tab (manager-only, tab bar replaces sessions/tournaments toggle for isManager users; loaded on tab switch via clubAdminListTournaments; tournament cards link to /tournament/[slug]; create modal with 4 fields: name, slug auto-gen from name, event date, venue ID placeholder; handleCreateTournament with double-fire guard); TournamentScreen.jsx stub (public, no auth, /tournament/:slug placeholder page); App.jsx: tournament route + TournamentScreen import. Build PASS. Hygiene 7/7 PASS. Build PASS. Next mig = 317.) (session 118 — EVENT OS PHASE 0 UI + PHASE 1 SCHEMA (commit 5873d91). Track A (apps/inorout): App.jsx now calls getUserRelationships() on auth load; derives homeScreenType (squad_only|club_member|parent|multi) as a computed const — null when authUser or relationships absent = zero-footprint for all squad-only/unauthenticated users. Added 3 routes (parent-home, feed, follow-live/:profileId) + 3 new screens: UnifiedFeedScreen.jsx (14-day feed, live events in amber, squad_game/club_session/child_event tap handlers, BottomNav), ParentHomeScreen.jsx (per-child ChildCard with next session + Follow Live CTA), FollowLiveView.jsx (getChildLiveMatch + venue_live realtime channel, ScoreBoard + EventRow components). Zero-footprint confirmed: squad-only unauthenticated users never have authUser → relationships stays null → homeScreenType null → no redirect fires. Track B (schema): rls_migrations/315_phase1_event_os_schema.sql written — 3 new tables tournament_events/performance_events/performance_results + 4 ALTERs (competitions.tournament_event_id, competition_teams.waitlist_position, league_config.ref_ui_config, playing_areas.sport_types). Type corrections applied vs plan: clubs.id=text so club_id=text; players.id=text so athlete_id=text; playing_areas.id=uuid so surface_id=uuid. RLS enabled + REVOKE ALL from anon/authenticated on all 3 new tables. Down migration written. CRITICAL OWED: mig 315 SQL file written and committed but NOT yet applied in Supabase SQL editor (Hard Rule #1). Must apply before any Phase 2 work. Build PASS. Hygiene PASS. Next mig = 316.) (session 117 — EVENT OS PHASE 0 RPCS SHIPPED (mig 314, commit 58f2d1f). Four SECURITY DEFINER read-only RPCs: `get_user_relationships()` (routing oracle — squads/club_memberships/guardian_of/competitions/admin_roles via auth.uid()); `get_unified_home_feed()` (14-day chronological feed, 4 event types: squad_game/club_session/competition_fixture/child_event, max 30); `get_guardian_home_feed()` (per-child next session; competition fixtures deferred to Phase 2); `get_child_live_match(p_player_profile_id uuid)` (Follow Live — guardian ownership guard via member_guardians edge; returns ok:false when no live match, not an error). All authenticated-only, anon revoked, search_path locked, 1 overload each. Security sweep 4/4 PASS. JS wrappers getUserRelationships/getUnifiedHomeFeed/getGuardianHomeFeed/getChildLiveMatch in packages/core. Build clean. OWED: Phase 0 UI — App.jsx homeScreenType routing, UnifiedFeedScreen, ParentHomeScreen, FollowLiveView. Next mig = 315.) (session 116 — DEMO DATA RESEED COMPLETE (migs 310–313, commit ff1becf). Four migrations: (310) Casual — 10 in / 4 reserve / 4 out on team_demo; game date rolled to Jun 18 19:00; bibs enabled, bib_holder=p_demo_03; 48 POTM votes across 6 matches; bib_history enriched; payment_ledger rows added (correct values: type='game_fee', status='paid'/'unpaid', method='cash'/'bank_transfer'; all with match_id to satisfy partial-unique index). Gotcha: `payment_ledger_uniq_without_match` fires if match_id IS NULL — must include match_id. (311) Venue — stale null-name past bookings deleted; 4 new named upcoming bookings added (Thornton Dynamos / Lunchtime Casuals FC / Sunday Rovers confirmed; Office United pending); June 17 booking named; cohort_ids assigned to all club_demo venue_memberships. (312) Club OS — 2 club_teams (First Team + U12 Falcons; deterministic UUIDs: c0-prefix NOT 0t — 0t is invalid hex); 6 team_member assignments; 2 team_managers; 2 enhanced DBS records (club_staff_dbs.recorded_by FK → auth.users, NOT a public users table — used tarny's real auth UID 11e35b81-5fa7-4bee-b57d-f6e70449b013); 7 RSVPs; past session + 7 attendance rows; 3 sent announcements; 4 merchandise items. (313) League/Ref/Display — 3 in_progress fixtures rolled to current_date; 4 upcoming fixtures to current_date+7; 18 match_events seeded on 4 completed fixtures (recorded_by_type='referee' NOT 'ref'; local_timestamp NOT NULL on match_events); featured_fixture_id pinned on demo_venue.display_config. Next mig = 314. Pilot pitch Jun 18.) (session 115 — PHASE 9 CLUB MERCHANDISE SHIPPED (mig 309 + 309b fix, commit 6fc781f). Two new tables: `club_merchandise` (id, club_id, venue_id, name, description, category IN (kit|accessories|equipment|other), price_pence, stock_qty nullable=unlimited, active) and `club_purchases` (id, club_id, venue_id, member_profile_id FK→member_profiles, item_id FK→club_merchandise, quantity, unit_price_pence, status IN (pending_payment|pending|fulfilled|cancelled), notes, stripe_payment_intent_id nullable). `venue_charges.source_type` CHECK extended to include 'merchandise'. 8 SECURITY DEFINER RPCs: `venue_upsert_merchandise` (create/update item, returns {ok, is_new, item_id}), `venue_list_merchandise` (with per-item purchases_count + pending_count via inner subquery — nested aggregate bug caught by EV), `venue_list_purchases` (with member_name, item_name; status filter), `venue_fulfil_purchase` / `venue_cancel_purchase` (status transitions, return {ok: true}), `member_get_merchandise` (authenticated, active items only, excludes stock_qty=0), `member_purchase_merchandise` (authenticated, inserts as pending_payment Stripe scaffold, returns {ok, purchase_id, item_name, total_pence}), `member_list_my_purchases` (authenticated, excludes cancelled, returns {id, item_id, item_name, item_category, club_id, quantity, unit_price_pence, total_pence, status, created_at}). Stripe: dormant scaffold — purchases enter pending_payment until keys provided; same Connect account as memberships. UI: (1) Shop tab in MembershipsView.jsx (apps/venue) — CataloguePanel (table + upsert modal) + OrdersPanel (filter dropdown, Fulfil/Cancel with confirmation modal); (2) Club Shop section in SessionsScreen.jsx (apps/inorout) — zero-footprint until shopItems.length > 0; ShopItemRow with qty picker, stock warning, Order button; (3) My orders section in MemberProfile.jsx (apps/inorout) — zero-footprint until myOrders.length > 0; quantity/total/date/status chip. **Bug caught by EV:** `venue_list_merchandise` had nested aggregate (COUNT inside jsonb_agg) — PostgreSQL runtime error "aggregate function calls cannot be nested". Fixed with inner subquery (mig 309b applied via MCP). 8 JS wrappers + barrel exports. EV 14/14 PASS + leak=0. Security sweep 8/8 PASS. Hygiene 7/7 PASS. Both builds clean. ALL CLUB OS PHASES COMPLETE. Next mig = 310.) (session 113 — PHASE 8 MULTI-VENUE ACTIVATION SHIPPED (mig 308, commit 09ae92f). Schema: `venue_member_checkins.customer_id` → nullable; new `member_profile_id uuid FK→member_profiles`; index on member_profile_id. 4 RPC rewrites: `get_member_pass` LEFT JOINs both venue_customers+member_profiles, COALESCE names, adds `valid_venues[{venue_id,venue_name}]` (club footprint or single-element) + `member_profile_id` — V2 members (customer_id=NULL) now return ok=true. `member_check_in` multi-venue: if venue_id≠display_venue checks club_venues EXISTS; V2 fix LEFT JOINs both tables; INSERT writes nullable customer_id + member_profile_id. `member_get_self` adds `venues[]` to each active_clubs row (full club footprint). `venue_list_members` LEFT JOINs both tables, COALESCE, WHERE includes club-scoped members. 4 new RPCs: `venue_list_club_venues(token, club_id)` — recent_checkins 30d + is_self; `venue_add_club_venue(token, club_id, target_venue_id)` — trusted-network auth (caller must be in club), idempotent, audits club_venue_added; `venue_remove_club_venue(token, club_id, target_venue_id)` — guards last_venue_cannot_be_removed + venue_has_active_members, audits club_venue_removed; `venue_search(token, query, club_id?)` — ILIKE name+city, excludes already-linked, LIMIT 10, min query 2. 4 JS wrappers (venueListClubVenues/venueAddClubVenue/venueRemoveClubVenue/venueSearch) + barrel exports. Venue UI: VenuesSection per club card in MembershipsView (lazy-load, per-venue row with is_self + checkins + Remove, AddVenueModal with debounced search). MemberPass.jsx: ValidVenuesSection (collapsible list when >1 venue); single venue shows inline name. EV 14/14 PASS + leak=0. RPC security sweep PASS. Build clean. Next mig = 309. PHASE 8 COMPLETE.) (session 112 — PHASE 11 CLUB COMMS SHIPPED (mig 307, commit 62fc9e4). `club_announcements` table: club_id/venue_id/created_by/title/body/audience(club|cohort|team)/cohort_id/team_id/status(queued|sent|failed)/email_sent_count/sent_at; RLS enabled, REVOKE ALL FROM anon+authenticated. 3 RPCs: `club_send_announcement(token, club_id, title, body, audience, cohort_id?, team_id?)` — resolve_venue_caller+manage_memberships cap, club_venues ownership check, validates audience+cohort/team_id required/ownership, inserts queued row, audits `club_announcement_queued`; grants anon+authenticated. `get_pending_club_broadcasts()` — service_role only (REVOKE ALL FROM PUBLIC + REVOKE FROM anon,authenticated), returns all queued announcements with resolved recipient lists via 3-way UNION (club arm = venue_memberships WHERE club_id + status NOT IN cancelled + email NOT NULL; cohort arm = +cohort_id match; team arm = club_team_members WHERE team_id + is_active=true + email NOT NULL). `member_list_club_announcements(club_id)` — authenticated only (anon revoked), auth.uid()→member_profiles→active membership check, returns status='sent' announcements scoped to caller's visibility (club-wide always; cohort/team via EXISTS), LIMIT 20 DESC. Email delivery: new `clubBroadcastJob(base, results)` in cron.js (every tick, no time-window gate); calls get_pending_club_broadcasts; per-broadcast per-recipient loop; alreadyNotified dedup via notification_log; sendTemplated('club_announcement', email, ctx); after all recipients: UPDATE club_announcements SET status='sent', email_sent_count=sent, sent_at=now(). New `club_announcement` template in _mailer.js: subject `[{clubName}] {title}`, HTML with title/body/venue byline. 2 JS wrappers: `clubSendAnnouncement(venueToken, clubId, title, body, audience, cohortId?, teamId?)` + `memberListClubAnnouncements(clubId)` in supabase.js + barrel exports. Venue UI: Announcements tab in MembershipsView.jsx (multi-club picker if >1 club; audience toggles with cohort/team sub-pickers; title input; body textarea; isSavingRef double-fire guard; "queued" success message). Member UI: AnnouncementsSection in SessionsScreen.jsx above session list (zero-footprint unless selectedClubId + has sent/loading announcements; shows max 3 with "See all (N)" toggle). EV 7/7 PASS + leak=0. Security sweep 3/3 PASS. Hygiene 7/7 PASS (4 files). Build clean. Next mig = 308. PHASE 11 COMPLETE.) (session 111 — PHASE 12 CLUB STAFF + DBS + ROLE-SCOPED ACCESS SHIPPED (migs 305–306, commit 6b7c100). New `club_staff_dbs` table: UNIQUE(member_profile_id, club_id), check_type IN (basic/standard/enhanced/enhanced_barred), status IN (pending/valid/expired/withdrawn), expiry_date, certificate_number, notes, recorded_by; RLS REVOKE ALL. Mig 305 — 5 RPCs: `venue_assign_team_manager(token, team_id, member_profile_id, role)` — manage_memberships cap, membership-in-club guard, ON CONFLICT DO UPDATE, audit staff_assigned; `venue_remove_team_manager(token, team_id, member_profile_id)` — soft-delete is_active=false, audit staff_removed; `venue_list_club_staff(token, club_id)` — LEFT JOIN club_staff_dbs, read-only, no cap check; `venue_upsert_staff_dbs(token, member_profile_id, club_id, check_type, status, cert?, issued?, expiry?, notes?)` — manage_memberships, ON CONFLICT, audit staff_dbs_recorded; `expire_staff_dbs()` — service_role ONLY (Supabase auto-grant of anon/authenticated caught and explicitly revoked — new pattern: REVOKE anon,authenticated after REVOKE FROM PUBLIC), pg_cron dbsExpiryJob at 08:00 UK with 15min guard. Mig 306 — 2 RPCs: `club_manager_get_team_members` UPDATED with `has_medical_notes` flag (medical_conditions OR allergies OR medications OR gp_details IS NOT NULL); `club_manager_get_member_detail(p_member_profile_id)` authenticated-only, two-tier auth: role='manager' on ANY club_team in member's club → full CPSU+guardian; coach/asst shares a team → same access; else `not_authorised`. Returns all ec1_*/ec2_*, medical fields, guardian (via member_guardians LEFT JOIN). 5 new JS wrappers (venueAssignTeamManager/venueRemoveTeamManager/venueListClubStaff/venueUpsertStaffDbs/clubManagerGetMemberDetail) in supabase.js + barrel exports. MembershipsView (apps/venue): new Staff tab — per-club expandable per-team staff roster, DbsBadge component, assign-staff modal (team/member/role dropdowns), DBS modal (check_type/status/cert/dates/notes); Club tab gains 3 DBS role-requirement toggles (stored in clubs.safeguarding_config jsonb, reuses toggleSafeguardingField). SessionsScreen (apps/inorout): 48h-window medical-alert panel in SessionDetail — gated isManagerOfSession && within48h && teamMembers.length > 0; per-member with has_medical_notes flag expandable amber card; lazy-loads clubManagerGetMemberDetail; MedRow helper. EV 10/10 PASS + leak=0. Hygiene 7/7 PASS. Casual-regression PASS (SessionsScreen not a casual surface, packages/core additive-only). RPC security sweep 7/7 PASS. Next mig = 307. PHASE 12 COMPLETE.) (session 110 — PHASE 10 CLUB ATTENDANCE SLICE 4E SHIPPED (mig 304, commit 55c6180). `club_manager_mark_attendance(p_session_id uuid, p_attendances jsonb)` — final slice of Phase 10. SECDEF/search_path/1-overload/authenticated-only (anon+PUBLIC revoked). Auth: auth.uid()→member_profiles.auth_user_id→club_team_managers(is_active=true, team_id=session.team_id). Guards: not_authenticated / profile_not_found / session_not_found (incl. NULL team_id) / not_manager / session_not_scheduled / invalid_status (per-row). Upserts into club_session_attendance ON CONFLICT (session_id, member_profile_id) DO UPDATE — same idempotent pattern as admin club_mark_attendance (mig 298). Audit: team_id='_system', actor_type='player', action='club_attendance_marked'. EV 8/8 PASS + leak=0 (all zeros). RPC security sweep PASS (SECDEF/search_path/overload_count=1/authenticated-only). JS wrapper `clubManagerMarkAttendance(sessionId, attendances)` added to supabase.js + exported from index.js. SessionsScreen.jsx `SessionDetail`: new Attendance section — zero-footprint when `isFuture || !isManagerOfSession`; shows per-member toggles (✓=attended, ~=late, ✕=absent) for all teamMembers; "Save attendance" button disabled until at least one toggle set; `attendanceMaps` state keyed by session_id persists across detail re-opens; `attendanceSaving` bool + `isMarkingAttendanceRef` double-fire guard. Section placed below Guests, above RSVP-for selector. Build PASS. Hygiene 7/7 PASS. Next mig = 305. PHASE 10 CLUB ATTENDANCE NOW COMPLETE.) (session 109 — PHASE 10 CLUB ATTENDANCE SLICE 4D SHIPPED (mig 303, commit 9ef222a). 6 manager write RPCs all SECDEF/search_path/1-overload/authenticated-only (anon+PUBLIC revoked): `club_manager_create_session` (12 args, one-off session with type/match fields, derives club_id+cohort_id from club_teams, audits manager_session_created), `club_manager_create_session_series` (10 args, DOW stepping loop same pattern as mig 302, inserts club_session_series + all weekly club_sessions, audits manager_series_created), `club_manager_cancel_session` (guards: not_team_session / not_a_manager / session_not_scheduled / session_in_past; UPDATE status='cancelled', audits manager_session_cancelled), `club_manager_get_team_members` (optional p_session_id folds is_session_guest bool via EXISTS on club_session_guests — 6 not 7 RPCs; no audit, read-only), `club_manager_add_session_guest` (INSERT ON CONFLICT DO NOTHING + not_team_session/not_a_manager/session_not_scheduled/guest_not_found guards, audits manager_guest_added), `club_manager_remove_session_guest` (same guards, DELETE, audits manager_guest_removed). Auth pattern all 6: auth.uid() → member_profiles.auth_user_id → club_team_managers.is_active=true for the target team. Audit sentinel: team_id='_system', actor_type='player', actor_identifier=first_name||' '||last_name. 6 JS wrappers clubManager* in supabase.js + barrel exports in index.js. SessionsScreen.jsx fully rewritten (620 lines, apps/inorout/src/views/): zero-footprint manager panel (only renders when isManager=true for selected club); "+ Create" button in header; CreateSessionModal (one-off/recurring toggle, session-type chips, match opponent/home-away fields, team picker for multi-team managers, capacity/notes/location); SessionDetail extended with guests section (current guest list with Remove, + Add guest picker from nonGuest team members) + cancel-session section (reason textarea + Confirm cancel / Keep session); double-fire guards isCreatingRef/isCancellingRef/isGuestActingRef. isManagerOfSession(session) = team_id set + managedTeamsForClub.some(t.team_id). openDetail extended: when isManagerOfSession, also calls clubManagerGetTeamMembers(teamId, sessionId) for guest picker. EV 7/7 + leak=0 (borrowed auth UIDs from auth.users since FK on auth_user_id; club_team_members uses is_active not status). Build PASS. Hygiene 7/7 PASS. RPC security sweep 6/6 PASS (SECDEF/search_path/1-overload/authenticated-only). Next mig = 304. NEXT: Slice 4E TBD.) (session 108 — PHASE 10 CLUB ATTENDANCE SLICE 4C SHIPPED (mig 302, commit 5ee6129). New `club_session_series` table (club_id, cohort_id, team_id, title, session_type, day_of_week 0=Sun…6=Sat, start_time, from_date, to_date, location, notes, capacity; RLS enabled, REVOKE ALL). New `club_sessions.series_id` nullable FK + index. Admin RPC `club_create_session_series` — SECDEF/anon+authenticated, resolve_venue_caller+manage_memberships, generates individual `club_sessions` rows for every weekly occurrence between from_date and to_date using EXTRACT(DOW) stepping, audits `club_series_created`. Admin RPC `club_cancel_session_series` — bulk-cancels all future scheduled sessions (scheduled_at > now()) in a series via GET DIAGNOSTICS ROW_COUNT, audits `club_series_cancelled`. `club_list_sessions` extended with `series_id` + `series_title` (LEFT JOIN club_session_series). `member_list_upcoming_sessions` extended with 4th visibility arm (d): managers (club_team_managers.is_active=true) see their team's sessions even without a club_team_members row. Two new JS wrappers `clubCreateSessionSeries` / `clubCancelSessionSeries` in supabase.js + barrel. SessionsView.jsx (apps/venue): One-off / Recurring block mode toggle in create modal (day picker Mon-default, time, from/to dates, session_type); "Recurring" badge on session cards with series_id; "Cancel remaining series" button in detail modal → CancelSeriesModal. RPC security sweep 4/4 PASS (SECDEF/search_path/1 overload/correct grants). Build PASS. Next mig = 303. NEXT: Slice 4D — manager app write UI (manager RPCs + SessionsScreen manager panel).) (session 107 — PHASE 10 CLUB ATTENDANCE SLICE 4C DESIGNED (no mig, no code). Full audit + product design session. Original scope (5 manager write RPCs) expanded to include session series (recurring blocks). Key decisions: (1) `club_session_series` table — one series = one day-of-week + time + from/to dates; separate series per day of week; individual `club_sessions` rows pre-generated for every occurrence with `series_id` FK back to the series. (2) Dashboard is primary entry point for bulk season scheduling; manager app handles exceptions (cancel individual date, create ad-hoc or new series from phone). (3) Both dashboard AND app can create series or one-offs; cancelling an individual date within a series cancels that session only, rest unaffected. (4) Split into two slices: Slice 4C = series infrastructure + dashboard UI (mig 302); Slice 4D = manager app write UI + guest RPCs (mig 303). (5) `member_list_upcoming_sessions` needs a 4th visibility arm so managers see their team's sessions even without a `club_team_members` row (coaches who don't play). Next mig = 302.) (session 106 — PHASE 10 CLUB ATTENDANCE SLICE 4B SHIPPED (mig 301 + frontend, commit 60809a1). Two parts: (1) mig 301 extended `member_get_self()` with two new jsonb fields — `active_clubs` (all clubs where caller has active/ending venue_membership, one row per club+cohort; was missing before) and `managed_teams` (club_team_managers rows where is_active=true; enables manager write-UI in Slice 4C). Signature unchanged, CREATE OR REPLACE, no DROP. (2) `SessionsScreen.jsx` (330 lines, apps/inorout): zero-footprint (null if no active_clubs), club picker auto-selects when one club, upcoming sessions via memberListUpcomingSessions, session detail sheet with RSVP board via memberGetSessionRsvpBoard, RSVP via memberRsvpSession with forProfileId for guardian-on-behalf-of-child, manager badge when managed_teams contains selected club, isRsvpingRef double-fire guard, TYPE_LABEL/TYPE_STYLE/RSVP_STYLE constants, CSS-vars only. App.jsx: `/sessions` route, memberProfile state, memberGetSelf() on authUser change, pure-parent landing redirect (authUser + active_clubs.length > 0 → replace('/sessions')), My Squads switcher extended with "YOUR CLUBS" section. Build PASS. Hygiene 7/7 PASS both files. RPC security PASS. Next mig = 302. NEXT: Slice 4C — manager write RPCs (mig 302) + SessionsScreen manager write UI.) (session 105 — PHASE 10 CLUB ATTENDANCE SLICE 4A SHIPPED (mig 300, commit 596ab9b). DB-only. (1) Dropped mig-055 dead `club_teams` stub (league-domain join table, 0 rows, no JS refs) and replaced with membership-domain `club_teams` (playing groups within a cohort: club_id, cohort_id, name). (2) New tables: `club_team_members` (seasonal assignment, partial-unique index `WHERE is_active=true` prevents double active assignment), `club_team_managers` (manager/assistant_manager/coach role per team, UNIQUE team+profile), `club_session_guests` (one-game appearance, session-level, UNIQUE session+profile). (3) `club_sessions` extended: session_type (training/match/friendly/other, NOT NULL DEFAULT training), team_id (uuid nullable FK→club_teams; NULL = whole-cohort session), opponent_name, home_away (home/away/neutral), opponent_venue_name, opponent_address, meet_time (arrive-by). (4) `member_list_upcoming_sessions` body rewritten with 3-way visibility OR: (a) team_id IS NULL + active cohort membership; (b) team_id matches active club_team_members row; (c) club_session_guests row. Signature unchanged. Returns 8 new fields: team_id, session_type, meet_time, opponent_name, home_away, opponent_venue_name, opponent_address (in addition to existing 8). RPC security sweep PASS (SECDEF/search_path/1-overload/authenticated-only). EV 5/5 PASS (not_authenticated; membership_required; user_a sees whole-cohort only; user_b sees both; user_c sees team session via guest row) + leak=0. Next mig = 301. NEXT: Slice 4B — SessionsScreen.jsx in apps/inorout.) (session 103 — PHASE 10 CLUB ATTENDANCE SLICE 3 SHIPPED (commits 5d5414f + ae20d32, no new mig). SessionsView.jsx added to apps/venue — new "Sessions" tab under Directory in the venue console. Club picker auto-selects when venue has one club; chip selector for multiple. Cohort filter chips from clubListCohorts. Session list: title, status, RSVP counts (in/maybe/out), location, "Attendance marked" badge. Create session modal: title (required), datetime-local, cohort picker, location, capacity, notes; double-fire guarded. Cancel session: confirmation modal with optional reason. Session detail modal on click: RSVP board bucketed by in/maybe/pending/out (first names only); attendance section — per-member attended/absent/late toggles pre-populated from existing records; bulk save. Bug caught in verify: ✕ (clear) button set status=undefined → Object.entries includes it → RPC raises invalid_status. Fixed by filtering status!=null before payload. All 5 admin club RPCs consumed: clubListSessions/clubCreateSession/clubCancelSession/clubGetSessionRsvps/clubMarkAttendance. Hygiene 7/7 PASS both files. venue build PASS. Next mig = 300. NEXT: Slice 4 — member-facing UI in apps/inorout (SessionsScreen or inline in MemberProfile — TBD at start of s104).) (session 102 — PHASE 10 CLUB ATTENDANCE SLICE 2 SHIPPED (mig 299, commit f30771b). Three member-facing RPCs: member_list_upcoming_sessions (lists upcoming scheduled sessions for a club the caller has an active venue_membership in, with own RSVP status; active = status IN active/ending), member_rsvp_session (upserts club_session_rsvps; guardian check via member_guardians if p_for_profile_id set; only 'scheduled' sessions accepted; 11 neg-path errors covered), member_get_session_rsvp_board (first-name-only board bucketed by status in/out/maybe/pending). All SECDEF/search_path/1-overload/authenticated-only (anon revoked). Audit team_id='_system'/actor_type='player'. JS wrappers memberListUpcomingSessions/memberRsvpSession/memberGetSessionRsvpBoard added to supabase.js + exported from index.js. EV 11/11 PASS + leak=0. Build PASS. Next mig = 300. NEXT: Slice 3 — venue UI for session management (SessionsView in apps/venue). Key design decisions: membership check uses venue_memberships not a separate club_memberships table; 'ending' status treated as active access; rsvp_by_profile_id always set to caller for auditability.) (session 101 — PHASE 10 CLUB ATTENDANCE SLICE 1 SHIPPED (mig 298, commit 6cd2abb). Three tables: club_sessions (status scheduled/cancelled), club_session_rsvps (status in/out/maybe/pending), club_session_attendance (status attended/absent/late, idempotent upsert). Nine admin RPCs all SECDEF/search_path/1-overload, resolve_venue_caller+manage_memberships cap, correct audit schema: club_create/list/update_cohort, club_create/update/cancel_session, club_list_sessions (RSVP counts + attendance_marked flag), club_get_session_rsvps (first names only per operator decision), club_mark_attendance (bulk upsert). Demo seed: Adults + Juniors cohorts + 2 upcoming sessions (Jun 20 + Jun 22) on club_demo. EV 13/13 PASS + leak=0. RPC security sweep 9/9 PASS. Both builds PASS. Next mig = 299. NEXT: Slice 2 (mig 299) — member_list_upcoming_sessions, member_rsvp_session, member_get_session_rsvp_board.) (session 99 — MEMBERSHIP V2 ALL 7 PHASES COMPLETE. mig 295 (fix): audit_events column names wrong in member_register_child/member_update_child/member_accept_consent — wrong cols (actor_id/event_type/payload) + NULL team_id violating NOT NULL. Fixed actor_user_id/action/metadata + '_system' sentinel. mig 296 (Phase 7 /q signup rebuild): customer_id nullable on venue_memberships; member_self_create_profile (SECDEF/authenticated); get_venue_signup_tiers extended (club+documents+audience+price_type); member_enrol_membership (SECDEF/authenticated, self or child via guardian). MembershipSignup.jsx two-path wizard (myself/my child): auth-gated CTA, profile creation, child CPSU fields, tier picker (audience-filtered), sequential consent signing, conditional ID upload, enrolment. VenueLanding.jsx: old MemberSignupForm replaced. policyTemplates.js: 5 CPSU-ready pre-drafted templates. MembershipsView template picker. mig 297 (EV-caught fix): all 5 member RPCs used actor_type='member' — not in audit_events_actor_type_check. Fixed 'member'→'player'. EV 8/8 + leak 0. Casual regression PASS. Next mig = 298. OWED: real-device test on /q signup flow (Hard Rule #13 — VenueLanding has auth gate); venue manual redeploy (platform-venue.vercel.app prebuilt-static). Commits: 9e2497d (mig 295), d980ed7 (mig 296), f44c670 (mig 297), 33e0b79 (docs).) (session 98 — MEMBERSHIP V2 Phase 6 SHIPPED (mig 294, commit 611a6c9). member_id_documents table + private member-id-docs Storage bucket (INSERT: member's own profile-id prefix; SELECT: any authenticated user). 4 RPCs: member_submit_id_document/member_list_id_documents (authenticated-only, anon revoked); venue_list_id_submissions/venue_verify_id_document (token-gated). member_get_self extended with id_mandate_clubs. MemberProfile: ID section (zero-footprint, upload + status per club). MembershipsView: ID docs tab (pending queue, approve/reject, signed URL viewer). Both builds PASS; hygiene 7/7 PASS both files; RPC security PASS. Casual regression PASS. Pre-existing bug found: audit_events column names in Phase 1–5 member RPCs are wrong — those audit inserts fail silently (separate fix needed). policy_documents table empty — no actual document templates created yet. Next mig = 295. Next: Phase 7 — /q signup rebuild.) (session 97 — MEMBERSHIP V2 s96 tech debt (mig 292) + Phase 5 SHIPPED (mig 293, commit fcdf6c9). policy_documents + consent_acceptances + 6 RPCs (anon explicitly revoked on 3 authenticated-only fns after Supabase auto-grant caught in security sweep). MembershipsView Documents tab + MemberProfile Consents section + ConsentModal. Both builds PASS; hygiene PASS; rpc-security PASS. Next mig = 294. Next: Phase 6 — ID & document upload.) (session 95 — MEMBERSHIP V2 Phase 3 SHIPPED (mig 290, commit 5337be6). member_register_child/member_list_children/member_update_child: SECDEF/authenticated-only, auth.uid()-scoped, audit-logged. member_update_child: caller must have accepted member_guardians edge or raises not_guardian. Neg-path EV PASS (6 assertions + leak=0). MemberProfile.jsx: My Children section — child cards, add-child form (first_name/last_name/dob), in-place edit with full CPSU safeguarding fields; zero-footprint when no children. Invite flow deferred (second guardian = ec2_* data fields). Cohort deferred to Phase 4. Casual regression PASS. Build clean. Next mig = 291. Next: Phase 4 — Membership builder rework (apps/venue).) (session 94 — MEMBERSHIP V2 Phase 2 SHIPPED (mig 289, commit 2ea899b). member_update_self RPC: SECDEF/authenticated, auth.uid()-scoped WHERE, jsonb partial-update (CASE WHEN per column), email immutable, medical fields → member_profile_medical_updated audit event. get_member_pass extended with member_profile_id. Phase-1 JS wrappers that were missing from supabase.js added: memberGetSelf, memberClaimProfile, memberCreateProfile, memberUpdateSelf, clubCreate, venueListClubs. /profile route added to App.jsx + new MemberProfile.jsx (zero-footprint: null when found:false; view/edit with 6 sections). MemberPass.jsx shows "Your account" pill → /profile when logged-in user's profile.id matches pass.member_profile_id. Neg-path EV PASS. Casual regression PASS. Build clean. Next mig = 290. Next: Phase 3 — Households / child memberships.) (session 93 — MEMBERSHIP V2 Phase 1 SHIPPED (migs 283–288, commit f774782): member_profiles+guardians, clubs extended, club_venues, club_cohorts, venue_memberships reframed, 5 RPCs, demo backfill. Neg-path EV on member_claim_profile PASS. Casual regression PASS.) (session 90 — MY VIEW header consolidation (frontend-only, no DB/migration). The screen stacked TWO title blocks — `PageHeader` (IN OR OUT wordmark + fixture meta) and `HeroCard` (the green animated pitch banner) — and both printed `schedule.dayOfWeek`; the operator spotted the duplicate after rejecting two fuller redesign explorations (full restructure + a slide-to-commit "team sheet"). Merged into ONE `PageHeader`: extracted the floodlit-pitch animation into new `PitchCanvas.jsx` (colours as `rgb()/rgba()` strings ON PURPOSE so the hygiene hex-check — which flags `#hex` in assignment position and scans only the just-edited file — stays green; reduced-motion-aware: one static frame under `prefers-reduced-motion`). `PageHeader` now renders the pitch as its background + the wordmark + ONE fixture line (`day · venue · time · £price` chip; operator chose the pure-facts variant, dropped "Night Football") + a thin admins line (vice-captains, absorbed from HeroCard). Framer-Motion entrance: staggered row-rise + `IN`/`OUT` scale-pop, ALL gated behind `useReducedMotion`. `PlayerView` drops the standalone `HeroCard`; `HeroCard.jsx` DELETED (only consumer was PlayerView; StatsView's `SeasonHeroCard` is unrelated). ~95px freed above the In/Out decision. Build + 7/7 hygiene clean on both files; rendered in-browser on the dev server (duplicate banner gone, £ chip + pitch present; the 2 console errors were unrelated Supabase 401 probes). commit 4e1ee0d. OWED: real-iPhone home-screen pass per Hard Rule #13 (in-or-out.com auto-deploys from main). See SESSION 90 below.) (session 89 — VENUE MEMBERSHIP PROGRAMME Phases 5–7 finished + operator-driven refinements (migs 274–281; next free=282). Phase 5 member pass completed (bar Wallet): QR on /m pass (react-qr-code), reception check-in (mig 274 `venue_member_checkins` + `member_check_in` venue-bound write + apps/display `CheckInOverlay` BarcodeDetector scanner w/ manual fallback), member self-signup on the existing /q venue_landing rail (mig 275 `member_self_signup`+`venue_approve_customer`, status `pending`). Phase 6 done: reminders (mig 276 `get_membership_reminders_due` service_role + cron `membershipRemindersJob` 10:00 UK + 4 Resend templates), booking-discount auto-apply (mig 277 `pitch_bookings.customer_id` + `_booking_member_discount` applied in `venue_confirm_booking(_series)` — member by explicit link OR active-member email match; 100%→comped; EV 9/9), HQ rollup (mig 278 `hq_get_membership_rollup` + apps/hq AnalyticsView Memberships card — apps/hq has NO standalone Vercel project, card unshipped). Phase 7 STRIPE = keyless DORMANT scaffolding ONLY (mig 279 connect/customer/subscription cols + `payment_state` + persist-then-process `billing_events`; RPCs record_stripe_event(idempotent)/mark_stripe_event_processed/apply_membership_subscription_status(state machine)/set_venue_connect_state/venue_get_billing_status; api/_stripe.js+stripe-webhook.js+stripe-connect.js + cron membershipReconciliationJob — all env-guarded 503/no-op). DECISIONS.md "MONEY-FLOW GATE" = PROPOSED, awaiting operator ratify + test keys (sk_test_/whsec_) + sign-off. Refinements: tiered self-signup (mig 280 — tiers free/paid via benefits.is_free, opt-in /q via benefits.self_signup; `get_venue_signup_tiers`+/q tier picker; FREE→instant auto-member £0 pass; PAID→pending+requested_tier_id; `venue_approve_and_enrol` one-tap; member_self_signup +p_tier_id, old 6-arg DROPPED; Plans Free/Offer-on-signup toggles); free-pass tidy + Payments "N% member" badge (mig 281 `pitch_bookings.member_discount_pct` + `venue_get_charges`). ALL write RPCs EV'd + leak 0 + rpc-security PASS; gotcha: Supabase default-privs auto-grant anon/authenticated → service_role-only fns need explicit REVOKE anon,authenticated. venue+casual+display deployed & live-bundle-verified; member pass/QR + /q tiered signup (free auto-join) + venue Memberships approve + reception check-in ALL browser-verified end-to-end on prod w/ throwaway fixtures (deleted, leak 0). OWED (operator-only): real-iPhone passes (Hard Rule #13); venue setup (build tiers, generate /q QR); Stripe keys+sign-off; HQ deploy + Apple Wallet certs.) (session 88 — RESOLVED (mig 268, commit 3d21539) the SESSION 80 open bug "drawn teams stay mutable after kick-off": a post-kickoff injured-toggle silently dropped a drawn player from a locked team (Footy Tuesdays, Matty, 2026-06-09). Three stacked server-side fixes, no JS change, all six replaced RPC signatures unchanged → grants preserved: (1) un-injure restores a still-drawn player (players.team IN A/B) to 'in' in set_player_injured + admin_set_player_injured; (2) NEW helper `is_lineup_locked(team_id)` (lock point = schedule.game_date_time — casual matches carry no kick-off column) gates the four self-service lineup RPCs → raise 'lineup_locked' post-kickoff for drawn players (set_player_injured/set_player_status scoped to drawn; add_guest_player unscoped; remove_guest_player scoped to drawn guest); admin injured path deliberately NOT locked; (3) admin_save_match_result reconciles orphan player_match rows (attended=true/result NULL) before the flat W/L/D bump so source-of-truth can't diverge. EV 9/9 + leak 0; sweep clean (no overloads, all secdef+search_path). Docs: BUGS open→resolved, RPCS, DECISIONS lock-point, GO_LIVE 11.2 pre-flight. OWED: real-iPhone post-kickoff lock test (Hard Rule #13).) (session 86 — EQUIPMENT HIRE Cycle 5 = the data-product tail SHIPPED + DEPLOYED: one READ-ONLY RPC `venue_equipment_insights` (mig 260) → ROI-per-asset (lifetime cost vs collected), usage over range, procurement signal from `equipment_demand_misses`; new EquipmentView **Insights** tab; no write path → EV N/A, proven via live BEGIN…ROLLBACK revenue-join probe (ALL PASS + leak 0), sweep PASS, both builds clean; commit 36645d3; venue redeployed prebuilt-static & verified live (bundle index-Bm0OHgf7.js contains `venue_equipment_insights`). Operator chose Option A = venue-dashboard-only; venue-Gaffer narrative + HQ multi-venue benchmarking deferred (pilot is one venue). RPC shaped as future venue-Gaffer context source (Hard Rule #14). OWED: logged-in browser pass. See SESSION 86 below.) (session 85 — QR ONBOARDING epic COMPLETE: slice 7 link management shipped (mig 254 — venue_create/set_active/repoint/list_invite_links + venue_owns_entity helper; re-point fully flexible cross-type; EV 15/15 + sweep PASS; commit 612adf7) + apps/venue redeployed prebuilt-static & verified live. All 7 slices done; slices 1–6 shipped session 84 (migs 248–253). OWED: real-device tests (slices 2 iPhone install, 4 TV, 7 dashboard). See SESSION 85 below.) (session 83 — Reception Display broadcast redesign SHIPPED + DEPLOYED end-to-end, migs 244–247, see SESSION 83 below.) (session 82 — RESOLVED: "Paid" carried into the next game — go-live now clears per-game payment flags (paid/self_paid/paid_by/paid_at) on new-match creation, owes untouched (mig 243, commit 4a5fbe4); + retroactive one-off cleanup of 4 stale flags on the live Footy Tuesdays game.) session 81 — Part 2 SHIPPED: live POTM tally revealed only after you vote (mig 242 get_potm_tally_public, counts-only, server-gated; POTMVotingModal voted-state leaderboard, live via team_live broadcast). Operator decision: tally at vote-time only, NO reopen path (no banner exists to reopen the modal). OWED: real-iPhone test (Hard Rule #13). Earlier this session: payment labels reworded "Paid Cash"→"Paid" (commits 2736c1a, c6c2415, UI-only).) session 80 — live firefight on Footy Tuesdays: paid button (debt-state Confirm unreachable), POTM modal re-popping, payment reconciliation, mig 241 post-game lifecycle, POTM window 1h→2h. session 79 — superadmin operator-analytics suite + ops email digest; migs 234–240, ⚠ migration-number COLLISION with parallel session 78 venue work — see SESSION 79.*

## SESSION 118 — Event OS Phase 0 UI + Phase 1 schema (commit 5873d91)

**What shipped:**
- Track A (UI): App.jsx homeScreenType routing, 3 new screens (UnifiedFeedScreen, ParentHomeScreen, FollowLiveView), 3 new routes (/feed, /parent-home, /follow-live/:profileId)
- Track B (schema): rls_migrations/315_phase1_event_os_schema.sql (3 tables + 4 ALTERs) — **written and committed but NOT yet applied to DB**

**Key decisions:**
- homeScreenType is a derived const (not useState), keyed on `relationships !== null` to guarantee zero-footprint for squad-only/unauthenticated users
- FollowLiveView subscribes to `venue_live:${venue_id}` broadcast channel — same pattern as apps/display and apps/ref
- Type corrections applied: clubs.id=text → tournament_events.club_id=text; players.id=text → performance_results.athlete_id=text; playing_areas.id=uuid → performance_events.surface_id=uuid

**Critical owed:**
- Apply rls_migrations/315 in Supabase SQL editor before any Phase 2 work (Hard Rule #1)

---

## SESSION 117 — Event OS Phase 0 RPCs (mig 314, commit 58f2d1f)

**What shipped:**
- Mig 314: 4 Phase 0 relationship RPCs — `get_user_relationships`, `get_unified_home_feed`, `get_guardian_home_feed`, `get_child_live_match`
- JS wrappers + barrel exports in packages/core
- FEATURES.md + RPCS.md + CONTEXT.md updated

**Key decisions:**
- `get_user_relationships` is a routing oracle, called once on auth load; homeScreenType derived client-side from squads/club_memberships/guardian_of presence
- Guardian home feed scoped to club sessions only in Phase 0 — children's competition fixtures deferred to Phase 2 (most children don't have auth-linked players rows yet)
- `get_child_live_match` returns `{ok:false, reason:'no_live_match'}` on all no-data paths — not an error — so guardian screen can render gracefully without try/catch branching
- Type mismatches corrected vs the plan spec: tournament_events.club_id=text (not uuid), performance_results.athlete_id=text (not uuid), performance_events.surface_id=uuid (not text)

**Owed:**
- ~~Phase 0 UI~~ ✅ shipped session 118
- Phase 1: mig 315 schema (tournament_events, performance_events, performance_results + ALTERs) + mig 316 RPCs

---

## SESSION 116 — Demo data reseed (migs 310–313, commit ff1becf)

**What shipped:**
- Mig 310: Casual reseed — 10 in / 4 reserve / 4 out; bibs on; game Jun 18 19:00; 48 POTM votes; 3 bib_history dates; 10 payment_ledger rows
- Mig 311: Venue reseed — 4 named upcoming bookings; cohort_ids on venue_memberships
- Mig 312: Club OS reseed — 2 club_teams; 6 members; 2 DBS-verified managers; 7 RSVPs; past session + attendance; 3 announcements; 4 merchandise items
- Mig 313: League/ref/display reseed — 3 in-progress fixtures on today; 4 upcoming at +7 days; 18 match_events; featured_fixture_id pinned

**Gotchas logged:**
- `0t` UUID prefix is invalid hex — club_team IDs use `c0` prefix
- `payment_ledger` partial-unique index requires `match_id` on every row (even debts)
- `match_events.local_timestamp` is NOT NULL
- `match_events.recorded_by_type` CHECK allows `'referee'` not `'ref'`
- `club_staff_dbs.recorded_by` FK → `auth.users` — tarny's auth UID = `11e35b81-5fa7-4bee-b57d-f6e70449b013`

**Next mig:** 314
**Pilot pitch:** Jun 18 2026

---

## SESSION 115 — Planning / audit (no code shipped)

Full session-start audit. All Club OS phases confirmed complete (migs 283–309). Working tree clean. Build PASS. No new bugs. **Next session = demo data reseed** — replace sparse/stale demo seed rows with a rich, realistic dataset that makes the app demo-ready for the pilot pitch.

---

## SESSION 114 — Club OS Phase 9: club merchandise (mig 309 + 309b, commit 6fc781f)

**What shipped:**
- New tables: `club_merchandise` + `club_purchases`; `venue_charges.source_type` extended to include `'merchandise'`
- Stripe scaffold (dormant): purchases enter `pending_payment` status — same Connect account as memberships; no-op until keys arrive
- 8 RPCs: `venue_upsert_merchandise`, `venue_list_merchandise`, `venue_list_purchases`, `venue_fulfil_purchase`, `venue_cancel_purchase`, `member_get_merchandise`, `member_purchase_merchandise`, `member_list_my_purchases`
- **EV-caught bug:** `venue_list_merchandise` had nested aggregate (COUNT inside jsonb_agg) — PostgreSQL runtime error "aggregate function calls cannot be nested". Fixed with inner subquery in mig 309b before commit.
- Shop tab in `MembershipsView.jsx` (apps/venue): CataloguePanel (item table, upsert modal) + OrdersPanel (status filter, Fulfil/Cancel)
- Club Shop section in `SessionsScreen.jsx` (apps/inorout): zero-footprint; ShopItemRow with qty picker, stock warning, Order button
- My orders section in `MemberProfile.jsx` (apps/inorout): zero-footprint; quantity / total / date / status chip
- 8 JS wrappers + barrel exports

**EV result:** 14/14 PASS, leak=0
**Security sweep:** 8/8 PASS
**Hygiene:** 7/7 PASS
**Build:** clean (both apps/venue and apps/inorout)
**Next mig:** 310

**ALL CLUB OS PHASES COMPLETE (Phases 1–7 + 8 + 9 + 10 + 11 + 12)**

---

## SESSION 113 — Club OS Phase 8: multi-venue activation (mig 308, commit 09ae92f)

**What shipped:**
- `venue_member_checkins.customer_id` made nullable; `member_profile_id` FK added — prerequisite for V2 check-in
- **Live bug fixed:** `get_member_pass` rewrote INNER JOIN → LEFT JOIN on both `venue_customers` + `member_profiles`; COALESCE names; V2 members (customer_id=NULL) now return `ok:true`
- `member_check_in` multi-venue: club_venues EXISTS check replaces hard venue equality; V2-aware INSERT
- `member_get_self` extended: each `active_clubs` row gains `venues[]` (the full club footprint)
- `venue_list_members` made V2+club-scoped (LEFT JOINs + WHERE club_venues EXISTS)
- 4 new venue management RPCs: `venue_list_club_venues`, `venue_add_club_venue`, `venue_remove_club_venue`, `venue_search`
- `MembershipsView.jsx` (apps/venue): VenuesSection per club card — lazy-loads on expand, is_self badge, 30d checkin count, Remove with guard error translation, AddVenueModal with 300ms debounced search
- `MemberPass.jsx` (apps/inorout): ValidVenuesSection (collapsible when >1 venue); single-venue shows inline

**EV result:** 14/14 PASS, leak=0 (all `_e2e_%` counts = 0 post-rollback)
**RPC security sweep:** PASS
**Build:** clean (both apps/venue and apps/inorout)
**Next mig:** 309

---

## SESSION 99 — Membership V2: bug fix + Phase 7 /q signup rebuild (migs 295–297, commits 9e2497d→33e0b79)

**mig 295 (fix):** audit_events INSERTs broken in 3 member RPCs since Phase 1–5. Wrong column names
(`actor_id`/`event_type`/`payload` → `actor_user_id`/`action`/`metadata`) + `NULL` `team_id` (NOT NULL)
→ `'_system'` sentinel. Applied + source written same commit.

**mig 296 (Phase 7):**
- `venue_memberships.customer_id` → nullable (v2 memberships need no v1 customer row)
- `member_self_create_profile(text,text?,text?,date?,text?)` — SECDEF/authenticated; raises
  `not_authenticated`/`first_name_required`/`profile_exists`; audits `member_profile_self_created` (actor_type `player`)
- `get_venue_signup_tiers` extended: now returns `venue_id`, `club` (id_mandate/safeguarding_config),
  `documents[]` (current policy docs), `audience`/`price_type` per tier/price
- `member_enrol_membership(text,uuid,text,uuid?)` — SECDEF/authenticated; self or child (guardian check);
  inserts venue_memberships (customer_id NULL); audits `member_self_enrolled` (actor_type `player`)
- `MembershipSignup.jsx` — two-path wizard: myself / my child; auth-gated via `onStart` prop;
  profile creation if none; child CPSU fields (ec1/ec2, SEND, dietary, emergency-treatment/medication
  consents, may-leave-unaccompanied); tier picker (audience-filtered: adult→adult+family, child→junior+child+family);
  sequential consent signing (typed sig, on_behalf support); conditional ID upload (club.id_mandate);
  `member_enrol_membership` on mount via useRef guard; done state
- `VenueLanding.jsx` — old `MemberSignupForm` component (~190 lines) replaced; `signupData` now
  stores full `{venue_id, club, documents, tiers}` from `getVenueSignupTiers`
- `policyTemplates.js` (apps/venue) — 5 CPSU-compliant pre-drafted templates: child registration
  + safeguarding, photo consent, medical data, T&Cs, junior code of conduct. All use
  `[CLUB NAME]`/`[AGE GROUP]`/`[SPORT]` placeholders. MembershipsView Documents tab gains
  "Start from a template" picker that pre-fills the Create modal

**mig 297 (EV-caught fix):** EV first run failed at `audit_events` INSERT — `actor_type='member'`
violates `audit_events_actor_type_check`. Allowed values do NOT include `'member'`. All 5 member RPCs
affected (migs 290/293/295/296). Fixed `'member'` → `'player'` across all 5. Re-ran EV 8/8 PASS
+ leak 0.

**Casual regression:** PASS — no casual-surface files changed (VenueLanding is /q/ only, not /p/).

**RPC security sweep:** PASS — 3 new RPCs all SECDEF/search_path/1 overload. member RPCs authenticated-only; tiers anon+authenticated.

**Next mig = 308.**

**OWED:** real-device test on /q signup flow (Hard Rule #13); venue manual redeploy.

---

## SESSION 98 — Membership V2 Phase 6: ID & document upload (mig 294, commit 611a6c9)

**Mig 294:**
- `member_id_documents` table (RLS-on/RPC-only): member_profile_id, club_id, document_type (CHECK passport/driving_licence/pass_card/birth_certificate), storage_path, status (pending/approved/rejected), rejection_reason, verified_by, verified_at.
- `member-id-docs` Storage bucket (private, JPEG/PNG/WebP/PDF, 10 MB). Storage policies: INSERT scoped to member's own profile-id path prefix; SELECT for any authenticated user (signed URLs only, time-limited).
- 4 new RPCs — all SECDEF/search_path/1 overload: `member_submit_id_document` (validates id_mandate=true, path prefix, inserts, audits); `member_list_id_documents` (own submissions only); both authenticated-only with anon explicitly revoked. `venue_list_id_submissions` (scoped via club_venues JOIN); `venue_verify_id_document` (approve/reject, scope-checked, audit event); both anon+authenticated (token-gated).
- `member_get_self` extended: adds `id_mandate_clubs` ([{club_id, club_name}]) — clubs where member has active membership AND club.id_mandate=true. No signature change (RETURNS jsonb), no DROP needed.
- 6 JS wrappers + 2 Storage helpers (uploadMemberIdDoc, getMemberIdDocUrl) added to supabase.js; barrel-exported from index.js.
- MemberProfile.jsx: ID section (zero-footprint: hidden unless id_mandate_clubs.length > 0). Per-club status (pending/approved/rejected). Upload form: doc type selector + file picker + submit button. Double-fire guard via isIdUploadingRef.
- MembershipsView.jsx: "ID docs" tab — pending queue first, reviewed list below; review modal (signed URL viewer — image inline / PDF link), approve/reject with mandatory rejection reason.

**Bug found (pre-existing, not fixed this session):** audit_events column names in Phase 1–5 member RPCs (`actor_id`, `event_type`, `payload`) don't match the live schema (`actor_user_id`, `action`, `entity_type`, `entity_id`, `metadata`). Those audit inserts fail at runtime. Casual flow unaffected; member audit trail for register_child, update_child, accept_consent is broken. Needs a dedicated fix.

**Note:** `policy_documents` table is empty — no actual consent document templates have been created yet. Venue admin must create them via Documents tab.
...
### team_admins partial unique index (unified login, session 171, mig 377)
- `team_admins_team_user_active_uniq` ON `team_admins (team_id, user_id) WHERE revoked_at IS NULL` — guarantees one active admin row per (team, user); supports the unified-login `claim_team_admin` auto-enrol (idempotent).
...
## SESSION 129 — Event OS Phase 6 — Performance Events (mig 326)

**Goal:** Athletics/sports-day discipline model. Directors create performance events (disciplines), judges enter results, public page shows ranked leaderboards and overall standings.

**Schema (mig 326):**
- `performance_results.athlete_id` made nullable (sports-day athletes are not casual squad players).
- `performance_results` gains `athlete_name text NOT NULL DEFAULT ''` + `competition_team_id uuid FK → competition_teams`.
- UNIQUE constraint `perf_results_upsert_key (performance_event_id, competition_team_id, athlete_name, attempt_number)` — enables judge re-entry UPSERT.
- `tournament_events.points_config` default changed to athletics 10-8-6-5-4-3-2-1; existing `{}` rows backfilled.

**RPCs (mig 326):** Six new SECDEF, all authenticated-only:
- `club_admin_set_performance_config` — write points_config, blocked once results exist.
- `club_admin_add_performance_event` — director creates a discipline (measurement_type CHECK).
- `club_admin_list_performance_events` — list with per-event result_count.
- `club_admin_record_result` — judge upserts attempt via UNIQUE constraint.
- `club_admin_get_performance_results` — ranked leaderboard (sort direction per measurement_type).
- `club_admin_get_sports_day_standings` — team totals: points/gold/silver/bronze/events_entered.
- `get_tournament_public` extended (4th CREATE OR REPLACE): adds `performance_events[]` + `performance_standings[]`.

**Frontend:**
- `SessionsScreen`: 6 new imports; performance events state; handlers; Performance Events section (expandable event rows with leaderboard + result entry form); Overall Sports Day Standings section; Add Performance Event modal.
- `TournamentScreen`: per-discipline results sections + team standings from existing `getTournamentPublic` call (no new fetch).
...
## SESSION 130 — Event OS Phase 7 Commercial (mig 327)

**Goal:** Monetisation layer for Event OS. Sponsors (logo + website), tournament branding (colour theme + custom logo), Player of Tournament award, and equipment hire bundles linked to a tournament.

**Schema (mig 327):**
- `tournament_events` gains `player_of_tournament_name text`, `player_of_tournament_team text`.
- `equipment_bookings` gains `tournament_event_id uuid FK → tournament_events ON DELETE SET NULL`.
- New table `tournament_sponsors`: id uuid PK, tournament_event_id FK CASCADE, name text NOT NULL, logo_url, website_url, display_order int, active bool. RLS enabled, all direct access revoked.

**RPCs (mig 327):** Nine new SECDEF, all authenticated-only:
- `club_admin_add_sponsor` / `club_admin_list_sponsors` / `club_admin_remove_sponsor` — sponsor CRUD.
- `club_admin_set_branding` — writes primary_colour/secondary_colour/custom_logo_url into `tournament_events.branding` jsonb (column from mig 315).
- `club_admin_set_player_of_tournament` — sets POT name + team, name_required guard.
- `club_admin_get_equipment_for_tournament` — returns active equipment catalogue at tournament's venue.
- `club_admin_book_equipment_for_tournament` — confirmed booking via `_equipment_peak_committed`, status='confirmed', no charge row created.
- `club_admin_list_tournament_equipment_bookings` — active bookings. Field is `qty` (not `qty_booked`).
- `club_admin_cancel_equipment_booking` — cannot_cancel guard for out/returned/cancelled status.
- `get_tournament_public` extended (5th CREATE OR REPLACE): adds branding, sponsors[], player_of_tournament_name/team.
- `club_admin_get_tournament` extended: adds sponsors[], player_of_tournament_name/team.

**Frontend:**
- `SessionsScreen`: Branding, Sponsors, Player of Tournament, Equipment sections in tournament detail panel.
- `TournamentScreen`: 4px colour top border + optional custom logo in header; sponsor strip below header; POT trophy card after standings.

**Bug caught by EV:** All 6 write RPCs used old `audit_events` INSERT pattern missing `entity_type` + `entity_id` (both NOT NULL since mig 003). Fixed before commit.

**Ephemeral verify:** 13/13 PASS. Leak check: all zeros.
...
## SESSION 133 — Payment Infrastructure Phase 2: Stripe Connect venue OAuth flow (mig 330)

**Goal:** Activate the Stripe Connect Express onboarding flow for venue operators via the Integrations tab.

**RPC (mig 330):**
- `venue_stripe_disconnect(p_venue_token text) → jsonb` — SECDEF, search_path locked, anon+authenticated, 1 overload. Validates token via `resolve_venue_caller` + `manage_memberships` cap. Idempotent (returns `{ok:true, already_disconnected:true}` if already disconnected). UPDATEs `venue_integrations` status='disconnected'. Audits: `team_id='_system'`, `actor_user_id=NULL`, `actor_type='venue_admin'`, action='stripe_disconnected'. **EV 5/5 PASS, leak=0.**

**api/stripe-connect.js (inorout Vercel):**
- Added CORS support for cross-origin calls from venue app. `setCors()` helper sets `Access-Control-Allow-Origin` (controlled by `STRIPE_CONNECT_ALLOWED_ORIGIN` env var, defaults to `https://platform-venue.vercel.app`). OPTIONS preflight → 204. Existing `action='onboard'` + `action='refresh'` logic unchanged.

**IntegrationsView.jsx (apps/venue):**
- `API_BASE = import.meta.env.VITE_INOROUT_API_URL ?? ""`
- `callStripeConnect(venueToken, action)` — cross-origin fetch to `${API_BASE}/api/stripe-connect`
- `handleStripeConnect` → onboard action → `window.location.href = json.url`; on `stripe_not_configured` shows friendly error
- `handleStripeDisconnect` → `window.confirm()` guard → `venueStripeDisconnect(venueToken)` RPC → reload billing
- Return URL handler (useEffect): `?connect=done/refresh` → `replaceState` → calls `action='refresh'` → reloads billing
- `StatusBadge` + `ProviderCard` extended with `onConnect/onDisconnect/connecting/disconnecting/error/actionAvailable` props

**Dashboard.jsx (apps/venue):**
- `view` useState initialiser: auto-switches to `"integrations"` when `?connect=*` in URL

**packages/core:**
- `venueStripeDisconnect(venueToken)` wrapper in supabase.js + barrel export in index.js

**Venue app:**
- `apps/venue/.env.local`: `VITE_INOROUT_API_URL=https://in-or-out.com` baked in at build time
- Redeployed prebuilt-static: bundle `index-B3nNwqj1.js` live — `stripe-connect`, `venue_stripe_disconnect`, `in-or-out.com` confirmed in live bundle

**Architecture decision — cross-origin API:**
Venue app (prebuilt-static, no serverless) calls the inorout deployment's `api/stripe-connect.js` via CORS. This activates existing dormant code per FEATURES.md intent, adds no new infrastructure, and establishes the pattern GoCardless Phase 5 will follow.

**Env vars still needed from operator (inorout Vercel project) before Phase 3:**
- `STRIPE_SECRET_KEY`
- `STRIPE_WEBHOOK_SECRET`
- `STRIPE_CONNECT_RETURN_URL=https://platform-venue.vercel.app/?connect=done`
- `STRIPE_CONNECT_REFRESH_URL=https://platform-venue.vercel.app/?connect=refresh`
...
martial-arts clubs. Reuses the club/consent/caps primitives; adds 3 tables only where the domain
genuinely differs.

**Operator decisions confirmed before building (in chat, plain-English):**
- **1A — award authority reuses `manage_facility`.** No dedicated `award_grades` cap in v1
  (reversible; add later if coach-only delegation is wanted).
- **2B + research — ladder shape.** Ordered named grades + `colour_hex` + per-grade `max_stripes`;
  the award carries a capped `stripes` count (BJJ stripes / dan degrees). **Age bands are separate
  schemes** (`age_band` juniors/adults/all), decided after researching real systems — BJJ kids
  (grey→green, can't get blue till 16) and TKD (poom under-15 vs dan) run different ladders; a club
  already supports many schemes, so a "Juniors" scheme + an "Adults" scheme is the natural model. A
  kid ageing up just gets a fresh award in the adults scheme (append-only history preserved). Half/
  tag belts = extra grade rows.

**Schema (mig 357, 3 RLS-walled tables, no policies → SECURITY DEFINER RPCs only):**
`venue_grading_schemes` (club_id, discipline, name, age_band, active); `venue_grades` (scheme_id,
name, rank_order, colour_hex, max_stripes, UNIQUE(scheme_id,rank_order)); `member_grades` APPEND-ONLY
award log (member_profile_id, scheme_id, grade_id, stripes, note, awarded_at, awarded_by,
awarded_by_actor_type, **awarded_seq bigint GENERATED ALWAYS AS IDENTITY**). "Current grade" = latest
per (member, scheme).

**EV caught a real bug pre-commit:** within a single transaction `now()` is constant, so two awards
tied on `awarded_at` and the DISTINCT-ON "current = latest" picked the *first* award. Fixed by adding
the monotonic `awarded_seq` identity column and ordering current by `awarded_at DESC, awarded_seq
DESC` (also hardens production against same-microsecond awards). Re-ran EV → 11/11 + leak 0.

**RPCs (all SECDEF, search_path pinned, audited):** writes `venue_create_grading_scheme` /
`venue_add_grade` (UNIQUE rank → `rank_order_taken`) / `venue_award_grade` (derives club+member from
membership, validates grade.scheme.club==membership.club → `grade_club_mismatch`, caps stripes at
max, returns `at_max`); reads `venue_list_grading_schemes` / `member_get_grade_history`. Extended:
`get_member_pass` → current `grades[]`; `venue_list_members` → +club_id +discipline (additive).
Wrappers in supabase.js + barrel.

**UI.** Operator (apps/venue MembershipsView): new **Grading** sub-tab (club picker filtered to
martial-arts clubs → create scheme + add grades with colour-picker + max-stripes) and an **Award
grade** action on member cards (GradeModal: scheme → grade → stripes 0–max → note; shows promote hint
at max). Member (apps/inorout): rank chip on **MemberPass** + **Progression** history section on
**MemberProfile**, both gated on `disciplineLabels.hasGrading` (martial_arts only). Reused the
existing `cups` icon (no `award` glyph). New venue constant `GRADING_DISCIPLINES=['martial_arts']`
mirrors `disciplineLabels.hasGrading`.

**Gates:** rpc-security-sweep PASS (6 RPCs: SECDEF + search_path + overload=1 + anon/auth grants);
ephemeral-verify 11/11 + leak 0; casual-regression PASS (additive-diff — every new inorout render
gated on hasGrading so a casual football member renders nothing new; core changes purely additive
wrappers; Playwright boot smoke of /m/<bogus> + /profile = 0 app console errors, the 1 /api/manifest
error is a pre-existing dev-only quirk); build inorout+venue + hygiene 7/7 clean; security advisors =
expected posture only (rls_enabled_no_policy INFO is intentional; SECDEF-executable WARNs apply to all
venue-token RPCs). ⛔ **real-iPhone PWA walk OWED** (Hard Rule #13 — Pass chip + Profile history;
needs an authed member on a martial-arts club). **Next free mig = 358.** Phase 3 (PT 1-on-1 booking,
mig 358) is next in GYM_VERTICAL_HANDOFF.md.

## SESSION 145 — Gym/Boxing vertical Phase 1: sparring / open-mat availability (mig 356) (2026-06-17)

Triggered by: "continue the gym/boxing vertical — Phase 1 (sparring / open-mat), mig 356."
Conflict guard clean (main, tree clean, 0 open PRs). Highest-reuse phase: members book In/Out
on a sparring night by REUSING the class-session booking model wholesale.

**Operator decisions (chat):** (1) the "Sparring / open-mat?" tick-box is set **at create**
(and edit) — a class type is EITHER a technical class OR a sparring session; incidental sparring
in a technical class just goes in the description. (2) Classes tab shows for **non-football clubs
only** (football nav byte-identical).

**Shipped (mig 356, branch `gym-vertical-phase1`):**
- **Schema:** `venue_class_types.is_sparring boolean NOT NULL DEFAULT false` (additive, nullable-safe).
- **Write RPCs (modified, not new):** `venue_create_class_type` +`p_is_sparring` (signature change →
  DROP old 9-arg, re-grant anon/auth/service_role); `venue_update_class_type` is_sparring via jsonb patch.
- **Read RPCs:** `venue_list_class_types` + `member_list_class_sessions` return is_sparring (badges);
  `get_member_pass` returns `discipline` (LEFT JOIN clubs) so the Pass-surface ClubNavBar lights the
  Classes tab — nav consistency across /sessions, /profile, /m.
- **Wrapper:** `venueCreateClassType` +`isSparring`. **Operator UI:** ClassesView ClassTypeModal toggle
  (create + edit) + types-table "Sparring" pill. **Member UI:** new `/classes` route (App.jsx parse +
  render + RESUMABLE) → new `ClassesScreen.jsx` (club + venue picker, reuses `ClassesTimetable` which
  gained a sparring badge); `ClubNavBar` Classes tab gated `discipline !== 'football'`; `MemberPass`
  threads discipline into clubEntry.
- **Gates:** rpc-security PASS both write RPCs (SECDEF + search_path + overload=1 + anon/auth grants);
  ephemeral-verify 5/5 (create sparring persists / create defaults false / update flip / update reflip /
  bad-category still rejected) + leak 0; casual-regression PASS (no casual surface in the diff; App.jsx +
  supabase.js changes provably additive); build inorout+venue clean; hygiene 7/7 every changed file;
  Playwright `/classes` boot smoke renders SignIn, 0 console errors.
- **OWED:** real-iPhone PWA walk (Hard Rule #13 — new route + NavBar tab; needs an authed member on a
  non-football club). **Next free mig = 357.**

## SESSION 144 — Gym/Boxing vertical Phase 0: club discipline identity (mig 355) (2026-06-17)

Triggered by: "start the gym/boxing vertical — Phase 0." Operator waived the post-pilot
timing gate ("forget previous instruction and pilot, focus on this build only").

**Scope decision settled first (operator-prompted).** The operator floated a generic
"build-any-sport" config engine (operator-defined sport + resource + schedule + progression).
Surfaced that the booking/resource *engine* is already generic (it's why the vertical is ~80%
reuse) — only the thin per-discipline *identity* is missing; and that a config engine is the
session-84-rejected lookup table writ large, designed blind before a real second-sport customer.
Operator confirmed: **discipline-as-pick-list, NOT a config engine.** Also corrected the plan —
**boxing has NO grades (fight record, Phase 4); grades/belts are martial_arts only.** Confirmed
labels-in-code does not limit reporting (the discipline column is in the DB and fully reportable;
labels are display dressing). New decision recorded in DECISIONS.md (CLUB DISCIPLINE).

**Built (mig 355, branch `gym-vertical-phase0`):**
- `clubs.discipline text NOT NULL DEFAULT 'football'` + CHECK pick-list (football|gym|boxing|
  martial_arts|yoga|dance|fitness|other). text+CHECK, not enum (alterability), not lookup table
  (session-84 posture). All existing clubs → 'football', zero footprint on casual.
- `member_get_self.active_clubs[].discipline` + `venue_list_clubs[].discipline` (read, additive).
- NEW `venue_set_club_discipline(p_venue_token, p_club_id, p_discipline)` — SECURITY DEFINER,
  gated `_venue_has_cap('manage_facility')`, server-side discipline validation + club↔venue link
  check, audited `club_discipline_set` (Hard Rule #9), returns jsonb; grants anon+authenticated.
  Wrapper `venueSetClubDiscipline` (supabase.js + barrel). NO operator UI control yet (set via
  RPC; wire alongside Phase 1).
- `apps/inorout/src/lib/disciplineLabels.js` — member-facing vocabulary map (sessionsTab/classesTab/
  bookCta/rankWord/hasGrading/hasFightRecord), DEFAULT fallback keeps unknown/`other` safe; boxing
  hasFightRecord, martial_arts hasGrading. Threaded `deriveClubContext().discipline` → `ClubNavBar`
  tab labels (football default = byte-identical to before).

**Verify:** rpc-security-sweep PASS (secdef/search_path/overload=1/grants anon+auth); ephemeral-verify
7/7 PASS (default-football, set-boxing persist, reset-martial_arts, audit-2-rows, reject
invalid-discipline/bad-token/club-not-in-venue) + leak-check 0; casual-regression PASS by static
analysis (deriveSquadContext byte-unchanged, ClubNavBar club-context-only, all clubs football→labels
identical); inorout build clean + hygiene 7/7. Migration `_up`/`_down` source landed same commit
(Hard Rule #11). ⛔ real-iPhone PWA walk OWED (flag-gated). **Next free mig = 356.** Phases 1–4 remain
planned in GYM_VERTICAL_HANDOFF.md.

## SESSION 143 — Club/membership system end-to-end audit + 3 fixes (2026-06-17)

Triggered by: "could the system work for gyms / boxing clubs?" → before planning any new
vertical, audited whether the EXISTING club/membership/"multi-sport" system actually works
and is wired live. Four parallel auditors traced every capability UI → supabase.js wrapper →
barrel export → live RPC (pg_proc) → live tables/columns → return mapper.

**Audit verdict — the plumbing is solid.** Verified wired live, no broken chains:
memberships (tiers/prices/family pricing, operator + self enrol, profiles/households,
freeze/cancel, renewals cron, pass, perks, ID docs); class creation/spaces/scheduling;
member booking / cancel / waitlist-claim / QR check-in / no-show / packages / charges
ledger; club sessions + attendance incl. guardian RSVP-on-behalf; context routing
(getUserRelationships mig-352 fix confirmed live, memberGetSelf.active_clubs, deriveContext).
**"Multi-sport" is REAL SCHEMA but DORMANT LOGIC** — `sport` columns + agnostic naming exist
but nothing branches on `sport`; it's scaffolding, not exercised. (This is exactly what the
gym vertical's Phase 0 `discipline` work would make real.)

**3 issues found, all fixed this session:**

1. **BST recurring-session timezone (mig 353, PR #13).** `venue_create_class_series`,
   `club_create_session_series`, `club_manager_create_session_series` used a bare
   `(v_cursor + p_start_time)::timestamptz` cast — `(date+time)` is a wall-clock, the bare
   cast reads it as UTC (DB session tz). Entered 18:00 → stored 18:00Z → displayed 19:00 in
   BST; booking cutoffs / QR windows / `_space_is_available` all an hour off. Fixed to
   `(v_cursor + p_start_time) AT TIME ZONE 'Europe/London'` (repo pattern migs 143/181; same
   class as the mig-207 game-time fix). One-off creators (venue_schedule_class_session,
   club_create_session, club_manager_create_session) take a client timestamptz — correct,
   untouched. Verified by pure-expression proof (BST 18:00→17:00Z displays 18:00; GMT
   unaffected); security sweep 3/3; **0 future series rows existed** so no historical data
   correction needed. Source + down landed same commit (Hard Rule #11).

2. **Multi-club nav residual club[0] (PR #14, no mig).** The s141 `?club=<id>` selection fix
   landed in SessionsScreen but missed two sites: MemberProfile hardcoded
   `active_clubs[0].pass_token` on its ClubNavBar, and ClubNavBar's Sessions/Profile tabs
   navigated to bare `/sessions` `/profile` (dropping the selection). Fixed: MemberProfile
   derives selectedClub from `?club=` (validated, falls back to [0]); ClubNavBar threads
   `?club=<club_id>` into Sessions/Profile hrefs. Front-end only, casual path untouched,
   flag-gated. Build clean, hygiene 7/7.

3. **MemberPass nav club context (mig 354, PR #15).** Third site: MemberPass rendered
   ClubNavBar without a clubEntry because `get_member_pass` didn't return `club_id`. Added
   `club_id` to the get_member_pass return shape (function already loaded `m.club_id`;
   getMemberPass returns payload raw so no mapper change) + `clubEntry={{club_id}}` on
   MemberPass. Runtime-verified the payload carries a non-null club_id. All 3 multi-club nav
   sites now keep the club.

Cloud-session discipline held throughout: the parallel s142 nav session (PRs #10/#11/#12)
was allowed to fully merge before any work started; each fix was its own branch → PR →
merged immediately → main confirmed clean before the next. Commits: PR #13 (00a45f8),
PR #14 (d3b6dcc), PR #15 (daccf38). Next free mig = 355.
