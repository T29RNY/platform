...
This file contains infrastructure details, key tokens, demo environment,
and the full session history archive. For everything else, see the split files:
...
See `SCHEMA.md` for full database schema, constraints, and type conventions.
...
See `FEATURES.md` for all phase tables (Phase 1–4) and the IO unlock grid.
...
See `BUGS.md` for the active bug list with priority order.
Read `BUGS.md` at the start of every session.
