...
- PR #5 — Mobile triage UI — 🟡 BUILT (PR #212), awaiting real-iPhone walk (HR13). See PR #5 branch.
- **PR #6 — HQ escalation inbox UI (apps/hq)** — 🟡 **BUILT, awaiting HQ deploy.** No new backend (reuses `hq_list_escalated_incidents` mig 464 + `hq_resolve_incident` mig 171). New `EscalatedInbox.jsx` = company-scoped cross-venue inbox (priority/category/assignee/reason/escalated-age), resolve reuses App-level `onResolve`→`hq_resolve_incident`, analyst read-only, self-refetches; wired into App.jsx right column above AlertsActions. VenueDetail per-venue rows enriched with priority/escalated badges + category/assignee/reason. Placed company-scoped (not per-venue) since the read RPC is cross-venue — this is the live consumer recorded for `hq_list_escalated_incidents`. Proof: HQ build PASS, check-live-config CLEAR. Gate: ⛔ HQ deploy (its own pipeline).
