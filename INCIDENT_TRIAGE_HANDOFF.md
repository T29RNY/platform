...
- **PR #5 — Mobile triage UI (apps/inorout OperationsTonight, NATIVE)** — 🟡 **BUILT, awaiting real-iPhone walk (HR13).** No new backend (reuses migs 462/465). Each open-incident card gains one-tap Acknowledge / Assign (sheet, venueListAssignableStaff) / Escalate (sheet, optional reason) above Resolve; priority/category/escalated/ack badges + assignee; safeguarding notice on Resolve sheet. Reuses MobileSheet/m-card/MIcon; credential=venueId (auth path). Proof: build PASS, hygiene 7/7, casual-regression file-overlap CLEAN (no casual surface / no packages/core touched), check-live-config CLEAR (not auth-critical). ⛔ **GATE: real-iPhone on-device walk (ack/assign/escalate/resolve persist across reload, no tap-does-nothing) — operator only.** PR: <pending>.
- PR #6 — pending (dep: #6 needs #3 done ✅; independent of #5).
