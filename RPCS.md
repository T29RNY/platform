...
| `admin_go_live_for_team` | _(none — cron-only; service-role `supabase.rpc()` in cron.js `autoOpenGameJob`)_ | Service-role sibling of `admin_go_live` (mig 126); same mig-204 squad reset. **Mig 216:** guest-DELETE removed (dormant-not-delete), matching `admin_go_live`. **Mig 230:** same `inorout.bulk_reset` GUC guard before the reset. |
