...
| SQL function | JS wrapper | Grant | Notes |
|---|---|---|---|
| `search_bookable_venues(p_query)` | `searchBookableVenues(q)` (Stage 5) | anon, authenticated | Typeahead over `venues` WHERE `bookings_enabled`+`active`, name/slug/city ILIKE, LIMIT 20. Returns `[{venue_id,name,slug,city}]`. PII-free. *Consumers: `apps/inorout` ScheduleScreen venue field (Stage 5).* |
