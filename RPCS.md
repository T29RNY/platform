...
| `admin_upsert_schedule` | `upsertSchedule(adminToken, schedule)` | Includes p_game_is_live (added session 27). Mig 207: game_date_time now uses `AT TIME ZONE 'Europe/London'` (was UTC-naive — caused 1hr BST offset on all kickoff-relative cron jobs). Grant corrected to authenticated-only (anon had EXECUTE via stale 13-param REVOKE that never matched the live 14-param signature). |
