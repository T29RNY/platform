...
| SQL function | JS wrapper | Grant | Notes |
|---|---|---|---|
| `team_admin_submit_lineup(p_admin_token, p_fixture_id, p_lineup)` | `submitTeamLineup(adminToken, fixtureId, lineup)` | anon + authenticated | Migration 159 (Cycle 5.6). SECURITY DEFINER, search_path locked. Resolves team from `admin_token`; asserts the team is in the fixture and every picked player is in the squad (`team_players`, else `players_not_in_squad`); rejects starting∩bench overlap (`player_in_starting_and_bench`). **Submit registers**: auto-upserts `player_registrations(active)` for picked players (ON CONFLICT DO NOTHING). Upserts the `fixture_lineups` row. Returns `{ok, starting_count, bench_count, registered_count, warnings[]}` — `warnings` (non-blocking) flag suspended/ineligible or registered-to-another-team. Soft squad size (no min/max). Audits `lineup_submitted` (hard-rule #9). `p_lineup` = `{starting:[player_id], bench:[player_id], shirt_numbers:{player_id:int}}`. **Consumers (hard-rule #14)**: writes `fixture_lineups`, consumed by `get_fixture_state_by_ref_token` (ref pre-match) + `get_team_next_fixture_lineup` (admin); Phase 4 reception display (future). |
| `get_team_next_fixture_lineup(p_admin_token)` | `getTeamNextFixtureLineup(adminToken)` | anon + authenticated | Migration 159 (Cycle 5.6). SECURITY DEFINER, search_path locked. Returns `{team_id, fixture, lineup}` — the team's next upcoming league fixture (`scheduled`/`allocated`, type `league`, ordered by date/kickoff) with opponent/date/venue/competition, plus the existing `fixture_lineups` row (or null). `fixture:null` for casual teams. **Consumers**: AdminView Teamsheet card + `TeamsheetScreen` (gating + prefill). |
|---|---|---|---|
