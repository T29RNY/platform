-- 613_notification_drain_sent_at.sql
-- Fix the queue-and-drain notification bug (BUGS.md CAPTURED; mig 612 discovery).
--
-- THE BUG. notification_log.sent_at DEFAULTs to now(). The cron drains
-- (apps/inorout/api/cron.js: roomHireNotificationsJob, classNotificationsJob part B)
-- only pick up rows with `channel IS NULL AND sent_at IS NULL AND queued_for <= now()`.
-- Every room-hire / class-cancel / class-offer queue-insert OMITS sent_at, so each row
-- is born already-"sent" and NEVER drains. Those acknowledgement / cancellation /
-- waitlist-offer emails have never actually gone out. Verified live 2026-07-20: 0 rows
-- of these types are drain-eligible. mig 612 (club_capture_lead) already did this right;
-- this back-fills the same one-line fix across the 10 functions that predate it.
--
-- THE FIX (identical shape in all 10). In each notification_log queue-insert, add
-- `channel, sent_at` to the column list right after `recipient`, and `NULL, NULL` in the
-- SELECT right before `now()` — the canonical mig-612 ordering
-- (…, recipient, channel, sent_at, queued_for[, queued_payload]). channel already
-- defaults NULL (so the drain's `channel IS NULL` already passed); it is set explicitly
-- here to match the canonical pattern. NOTHING ELSE in any body changes.
--
-- SOURCE OF TRUTH = the LIVE bodies (pg_get_functiondef), NOT the original migrations.
-- These functions were rewritten after their first migration: mig 399 (feature flags —
-- _venue_feature_enabled / _venue_club_feature_enabled gates), mig 344 (class packages —
-- package-credit restore), mig 588 (school-year age guard). Rebuilding from 339/340/342
-- would REVERT all of that. Each CREATE OR REPLACE below is the exact current live body
-- with only its notification_log insert changed.
--
-- SCOPE = 10 functions (the complete set found by a pg_proc sweep for every queue-inserter
-- missing sent_at). The brief named 8; venue_create_room_hire (operator-side direct hire)
-- was the 10th, caught by the sweep. club_capture_lead is already correct (mig 612).
--
-- NO BACKFILL. Zero rows of any affected type exist in production yet (room-hire and
-- classes are unused / demo-only), so there is nothing stuck to migrate and no risk of
-- blasting out stale historical emails. Purely forward-looking.
--
-- SECURITY. No signature or return-shape change on any function => CREATE OR REPLACE with
-- no DROP, no new overload; existing grants are preserved (re-asserted below to match each
-- function's exact current ACL, for explicitness / the rpc-security sweep). All remain
-- SECURITY DEFINER with `SET search_path TO 'public','pg_temp'`. No JS wrapper / consumer
-- change (return shapes untouched).

BEGIN;

-- ── 1. member_request_room_hire (authenticated) ──────────────────────────────
...
BEGIN
  ...
  INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
  SELECT v_space.venue_id, v_profile.id::text, 'room_hire_requested', v_hire_id::text, v_profile.email, NULL, NULL, now(),
...
-- ── 2. public_enquire_room_hire (anon) ───────────────────────────────────────
...
BEGIN
  ...
  INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
  SELECT v_space.venue_id, NULL, 'room_hire_requested', v_hire_id::text, btrim(p_email), NULL, NULL, now(),
         jsonb_build_object('venue_name', vn.name, 'space_name', v_space.name,
                            'starts_at', p_starts_at, 'purpose', btrim(p_purpose))
...
-- ── 3. venue_confirm_room_hire (venue token) ─────────────────────────────────
...
BEGIN
  ...
  IF v_recipient IS NOT NULL THEN
    INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
    SELECT v_hire.venue_id, v_hire.member_profile_id::text, 'room_hire_confirmed', p_hire_id::text, v_recipient, NULL, NULL, now(),
...
-- ── 4. venue_cancel_room_hire (venue token) ──────────────────────────────────
...
BEGIN
  ...
  IF v_recipient IS NOT NULL THEN
    INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
    SELECT v_hire.venue_id, v_hire.member_profile_id::text, 'room_hire_cancelled', p_hire_id::text, v_recipient, NULL, NULL, now(),
           jsonb_build_object('venue_name', vn.name, 'space_name', sp.name, 'starts_at', v_hire.starts_at,
...
-- ── 5. venue_create_room_hire (venue token — operator-side direct hire) ───────
...
BEGIN
  ...
  IF v_recipient IS NOT NULL THEN
    INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
    SELECT v_space.venue_id, p_member_profile_id::text, 'room_hire_confirmed', v_hire_id::text, v_recipient, NULL, NULL, now(),
...
-- ── 6. venue_cancel_class_session (venue token) ──────────────────────────────
...
BEGIN
  ...
  INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
  SELECT v_caller.venue_id, b.member_profile_id::text, 'class_cancelled', p_session_id::text, mp.email, NULL, NULL, now(),
...
-- ── 7. venue_cancel_class_series (venue token) ───────────────────────────────
...
BEGIN
  ...
  GET DIAGNOSTICS v_credits = ROW_COUNT;
  INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
  SELECT v_caller.venue_id, b.member_profile_id::text, 'class_cancelled', cs.id::text, mp.email, NULL, NULL, now(),
...
-- ── 8. venue_reassign_class_instructor (venue token) ─────────────────────────
...
BEGIN
  ...
  IF to_regclass('public.venue_class_bookings') IS NOT NULL THEN
    EXECUTE format($q$
      INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for)
      SELECT %L, b.member_profile_id::text, 'class_instructor_changed', %L, mp.email, NULL, NULL, now()
        FROM public.venue_class_bookings b
        JOIN public.member_profiles mp ON mp.id = b.member_profile_id
...
-- ── 9. member_cancel_class_booking (authenticated) ───────────────────────────
...
BEGIN
  ...
  END IF;
  INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
  SELECT v_venue_id, v_profile_id::text, 'class_booking_cancelled', p_booking_id::text, mp.email, NULL, NULL, now(),
...
-- ── 10. _offer_next_waitlist_spot (internal helper — no client grant) ─────────
...
BEGIN
  ...
  INSERT INTO public.notification_log (team_id, player_id, type, entity_id, recipient, channel, sent_at, queued_for, queued_payload)
  SELECT v_sess.venue_id, b.member_profile_id::text, 'class_spot_offered', b.id::text, mp.email, NULL, NULL, now(),
