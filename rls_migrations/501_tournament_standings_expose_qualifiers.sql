-- 501_tournament_standings_expose_qualifiers.sql
--
-- Surface the group-stage qualification rule to the client so the qualify-tint
-- can highlight the correct number of rows per group.
--
-- Context: mig 498 (self_serve_seed_group_stage) persists
--   competitions.config = { groups_seeded, num_groups, qualifiers_per_group }
-- and mig 500 (venue_seed_knockout) reads config.qualifiers_per_group to pick the
-- top-N of each group. But NO read RPC returned qualifiers_per_group, so the app
-- could not know how many rows to gold-tint (group_rank is written to EVERY team
-- 1..N at KO-seed time, and is NULL entirely during the live group stage). Under
-- the PR#1 scope change qualifiers-per-group is configurable (top-1 OR top-2), so
-- a hardcoded "top-2" tint is wrong for a top-1 tournament.
--
-- This migration adds `qualifiers_per_group` + `num_groups` (read straight from
-- config, NULL for non-group formats) to the two standings-bearing read RPCs:
--   - venue_get_tournament_standings  (organiser Manage view — live group tables)
--   - get_tournament_public.standings[] (public/reception page — the shareable tint)
--
-- Return-shape ADDITION only. No signature change, no column change, no RLS change,
-- no write path. Existing consumers ignore the new fields. Wrappers already return
-- the whole jsonb unchanged, so no supabase.js change is required.
--
-- Consumers (Hard Rule 14): apps/inorout ManageTournament.jsx (group-stage tint)
-- + apps/inorout TournamentScreen.jsx (public qualify-tint). See RPCS.md.

-- ── venue_get_tournament_standings — add qpg + num_groups at top level ──────────
...
BEGIN
  ...
  SELECT COALESCE((config->>'knockout_seeded')::boolean, false),
         (config->>'qualifiers_per_group')::int,
         (config->>'num_groups')::int
    INTO v_knockout_seeded, v_qualifiers_per_group, v_num_groups
  ...
  SELECT jsonb_build_object(
    ...
    'qualifiers_per_group', v_qualifiers_per_group,
...
-- ── get_tournament_public — add qpg + num_groups to each standings[] competition ─
  ...
  RETURN jsonb_build_object(
    ...
    'standings', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'qualifiers_per_group', (comp.config->>'qualifiers_per_group')::int,
        'num_groups', (comp.config->>'num_groups')::int,
        'rows', COALESCE((
