-- 362: add dob + computed age to the class-session attendee roster.
--
-- Context: club training sessions for the gym/boxing vertical run as a single
-- open class session (e.g. "Club Training, Wed 5-6") that any member books into.
-- Coaches split the room into age bands and rotate stations on the day. To group
-- a mixed-age session fast, the operator's session view needs each attendee's age.
--
-- Additive return-shape change to venue_get_class_session_detail: each attendee
-- object gains 'dob' (date) and 'age' (int years, NULL if no dob). Attendees are
-- ordered status, then youngest-first (dob DESC NULLS LAST), so the roster reads
-- top-down by age. Read-only; no booking-path, table, or grant changes. Body is
-- otherwise byte-identical to the mig 360-era live version.
--
-- Hard Rule #12: return-shape addition — the only JS consumer
-- (venueGetClassSessionDetail wrapper → apps/venue ClassesView) is a pure
-- passthrough, so the new keys flow through unmapped; ClassesView renders age.
  ...
  IF to_regclass('public.venue_class_bookings') IS NOT NULL THEN
    EXECUTE format($q$
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
               ...
               'dob', mp.dob,
               'age', CASE WHEN mp.dob IS NOT NULL
                          THEN date_part('year', age(mp.dob))::int ELSE NULL END,
