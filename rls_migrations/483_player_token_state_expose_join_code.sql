-- 483 — expose join_code on get_team_state_by_player_token
--
-- Adds the team's public join_code to the player-token team-state RPC, nested
-- under the `settings` object. Powers the WhatsApp share-sheet /join CTA on My
-- View (PlayerView.buildTeamSheetText): the green IN tile's share message now
-- ends with a "tap to update" /join link when spots are open.
--
-- WHY it's safe to return to a plain player token:
--   * join_code is the non-sensitive PUBLIC invite code — it's how anyone joins
--     the squad (SquadReady.jsx, admin invite). Not a secret, not an identity.
--   * The settings block is built UNCONDITIONALLY (outside the v_privileged
--     branch), so this reaches non-admin player tokens too — the exact person
--     most likely to broadcast the sheet.
--   * SECURITY DEFINER: reads teams as the definer; v_team_id is derived
--     server-side from p_token, never trusted from the client.
--
-- Same-commit mapper update (Hard Rule #12): packages/core/storage/supabase.js
-- getTeamStateByPlayerToken maps data.settings.join_code -> settings.joinCode.
--
-- Return-shape ADDITION only: signature unchanged (p_token text), so no DROP /
-- overload concern. Down-migration restores the mig-349 body verbatim.
--
-- Body below is the live mig-349 function verbatim, with three added lines:
--   * DECLARE v_join_code text;
--   * folded join_code into the existing teams SELECT for v_lckey
--   * v_settings := ... || jsonb_build_object('join_code', v_join_code)
...
BEGIN
  ...
  SELECT live_channel_key, join_code INTO v_lckey, v_join_code
  FROM teams WHERE id = v_team_id;

  -- Surface join_code under settings (mig 483) — public invite code for the
  -- My View share CTA. COALESCE guards the no-settings-row case so the key is
  -- always present even if the settings SELECT returned NULL.
  v_settings := COALESCE(v_settings, '{}'::jsonb)
                || jsonb_build_object('join_code', v_join_code);
