-- ════════════════════════════════════════════════════════════════════════════
-- 125 — deterministic squad ordering in team-state RPCs
-- ════════════════════════════════════════════════════════════════════════════
-- Bug: get_team_state_by_admin_token (mig 070) and
-- get_team_state_by_player_token (mig 080) both build the squad via
-- jsonb_agg(jsonb_build_object(...)) with NO ORDER BY. PostgreSQL's
-- aggregation order is implementation-defined and non-deterministic —
-- every call can return players in a different order.
--
-- Surfaced by: 2026-05-27 incident — rockybram (team admin & creator of
-- team_KPaoX8oJYMQ "Footy Tuesdays") opened his PWA on iOS while the
-- access token had expired client-side (audit_events: app_boot row at
-- 15:33:09 with actor_user_id=null, session_present_client=false,
-- route_type=admin, display_mode=standalone). The admin RPC returned
-- is_self=false on every squad row (auth.uid() was null). App.jsx
-- fell through to its squad[0] fallback at line 1168. squad[0] that
-- millisecond happened to be Pritpal (a regular squad member who
-- joined two days before). Rocky was rendered AS Pritpal on his own
-- admin's PWA.
--
-- This migration is one half of the fix (deterministic ordering at
-- the server). The other half (removing the squad[0] fallback in
-- App.jsx) ships in the same cycle. Either fix alone would resolve
-- the visible bug; together they close the class of failure.
--
-- Ordering choice: tp.created_at ASC, p.id ASC. team_players.created_at
-- is set on row insert (admin_add_player / player_join_team), is never
-- updated, and is stable across all teams. Tie-break by p.id keeps
-- ordering deterministic when two team_players rows share a microsecond.
-- ════════════════════════════════════════════════════════════════════════════
