-- 556: (#1 cont.) venue_get_charges gains per-charge `refunded_pence` (Σ kind='refund' non-voided
-- payments). WHY: `status='refunded'` is OVERLOADED — venue_void_charge sets it (charge cancelled,
-- payments KEPT) AND a real refund sets it (money handed back). Without a discriminator the desktop
-- mislabels every refunded charge "Voided". refunded_pence lets the UI tell them apart (refund → >0,
-- void → 0) AND annotate PARTIAL refunds (refunded_pence>0 while status stays paid/partial).
-- Additive return-shape only (new key on each charge row); all other behaviour byte-identical.
-- Consumer (HR#14): apps/venue PaymentsView (Refunded vs Voided label + partial-refund annotation).
