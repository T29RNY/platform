-- 111 down — revert to mig 086/089 'upcoming' filter (excludes 'allocated').

-- venue_get_state pre-mig-111 body
...
BEGIN
  ...
  WITH
  v_leagues AS (SELECT id, venue_id, name, short_name, sport, format, day_of_week, default_kickoff_time,
  ...
  v_pending AS (SELECT ct.id, ct.competition_id, ct.team_id, ct.status, ct.registered_at, t.name AS team_name
