-- 113_phase2_venue_get_state_teams_directory.sql
--
-- Phase 2 (League Mode) — Cycle 2.7d read-shape extension.
--
-- venue_get_state didn't include team-name resolution — fixture
-- rows carry home_team_id / away_team_id (text PKs) but not
-- display names. Cycle 2.7c's dashboard rendered raw IDs as a
-- result.
--
-- Fix: add a top-level `teams` object keyed by team_id, with
-- `{ id, name, primary_colour, secondary_colour }`. Built from
-- the venue's competitions (any team ever registered into any
-- competition under this venue, regardless of status). Frontend
-- consumes via state.teams[fixture.home_team_id]?.name.
--
-- Same shape addition could be made to league_get_state but isn't
-- needed yet — the league deep-link surface is read-only and
-- displays fewer fixtures. Defer.
--
-- Body otherwise identical to mig 111 (verified via
-- pg_get_functiondef before patching).
