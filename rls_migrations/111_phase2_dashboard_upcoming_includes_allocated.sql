-- 111_phase2_dashboard_upcoming_includes_allocated.sql
--
-- Phase 2 (League Mode) — Cycle 2.7a dashboard read RPC hotfix.
--
-- Latent bug caught while building the demo venue seed (mig 110):
-- venue_get_state.fixtures.upcoming and league_get_state.fixtures.upcoming
-- both filtered `status IN ('scheduled','postponed')`. The intent
-- was "fixtures not yet started"; the reality is once an operator
-- assigned a pitch via venue_assign_pitch (Cycle 2.4), the fixture
-- transitions scheduled→allocated and silently vanished from the
-- upcoming list — even though it IS still upcoming, just pitched.
--
-- Fix: include 'allocated' alongside 'scheduled' and 'postponed' in
-- the upcoming filter on both RPCs. 'in_progress' / 'completed' /
-- 'walkover' / 'forfeit' / 'void' stay excluded.
--
-- Other buckets unchanged: tonight (today only), this_week (no
-- status filter), recent (terminal states < today).
--
-- Function bodies otherwise IDENTICAL to mig 086 (verified by
-- pulling pg_get_functiondef before patching).
...
BEGIN
  ...
  v_fixtures_all AS (
    SELECT f.id, f.competition_id, f.home_team_id, f.away_team_id, f.week_number, f.round_name,
           ...
           f.home_score, f.away_score, f.walkover_winner_id, f.postpone_reason, f.void_reason
...
AS $function$
DECLARE
  v_caller record; v_league_id text; v_venue_id text;
  v_today date := current_date; v_week_end date := current_date + 6;
  v_result jsonb;
BEGIN
  ...
  END IF;
  v_venue_id := v_caller.venue_id; v_league_id := v_caller.league_id;
  IF v_league_id IS NULL THEN
    SELECT jsonb_build_object('requires_league_pick', true, 'venue_id', v_venue_id,
      'leagues', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', l.id, 'name', l.name, 'short_name', l.short_name,
          'league_code', l.league_code, 'league_admin_token', l.league_admin_token, 'active', l.active) ORDER BY l.name)
        FROM leagues l WHERE l.venue_id = v_venue_id), '[]'::jsonb)) INTO v_result;
    RETURN v_result;
  END IF;
  WITH
  v_league AS (SELECT id, venue_id, name, short_name, sport, format, day_of_week, default_kickoff_time,
           default_playing_area_id, league_admin_token, league_code, display_token,
           squad_mode, squad_mode_locked_at, standings_visibility, active, created_at FROM leagues WHERE id = v_league_id),
  v_seasons AS (SELECT s.id, s.league_id, s.name, s.start_date, s.end_date, s.num_weeks, s.status, s.created_at
    FROM seasons s WHERE s.league_id = v_league_id),
  v_competitions AS (SELECT c.id, c.season_id, c.name, c.type, c.format, c.status, c.created_at
    FROM competitions c WHERE c.season_id IN (SELECT id FROM v_seasons)),
  v_fixtures_all AS (SELECT f.id, f.competition_id, f.home_team_id, f.away_team_id, f.week_number, f.round_name,
           f.scheduled_date, f.kickoff_time, f.playing_area_id, f.official_id, f.status,
           f.home_score, f.away_score, f.walkover_winner_id, f.postpone_reason, f.void_reason
    FROM fixtures f WHERE f.competition_id IN (SELECT id FROM v_competitions))
  SELECT jsonb_build_object(
    ...
    'venue', (SELECT jsonb_build_object('id', v.id, 'name', v.name, 'slug', v.slug, 'sport', v.sport,
        'logo_url', v.logo_url, 'primary_colour', v.primary_colour, 'secondary_colour', v.secondary_colour)
