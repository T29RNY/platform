-- 520_get_my_world_club_admin_arm.sql
--
-- Club Console Consolidation — PR #6b, the club-admin /hub routing foundation.
--
-- THE GAP: a club that runs no physical facility lives on a shell venue
-- (mig 518 self_serve_create_club: a venues row with origin='self_serve' + a
-- venue_admins(role='owner') row + exactly one club_venues link). On the desktop
-- console that owner uses the club lens. On the native /hub they resolve, via
-- get_my_world's admin_roles venue arm, into a venue_admin — and nav.js
-- resolveRoles turns EVERY venue_admin into a full venue-OPERATOR hat
-- (Tonight/Bookings/Payments/Setup). A venueless club owner therefore lands in
-- facility-operator screens that don't apply to them, with no club-admin surface.
--
-- THE FIX (Option A, additive — the operator-chosen approach): expose the two
-- facts nav.js needs to tell a club shell apart from a real venue, as TWO NEW
-- ADDITIVE KEYS on each venue_admin entry:
--   * origin  — venues.origin ('self_serve' for a shell/self-serve venue,
--               'superadmin' for a platform-provisioned facility; the column is
--               NOT NULL DEFAULT 'superadmin', CHECK in {superadmin, self_serve}).
--   * club_id — the club a DEDICATED SHELL hosts: a self_serve venue linked to
--               EXACTLY ONE club (the mig-518 signature). A self_serve *facility*
--               venue (mig 484 self_serve_create_venue) has zero clubs → NULL. A
--               multi-club facility → NULL. So club_id is non-null ONLY for a true
--               single-club shell, and nav.js routes exactly those into the new
--               club_admin track; everything else stays an operator hat.
--
-- SAFETY — why this is the lowest-risk possible change to the highest-traffic RPC:
--   * PURELY ADDITIVE. Every pre-existing key (type/entity_id/name/role on the
--     venue arm; every other world.* branch) is byte-identical to mig 494. The
--     ONLY diff vs the live body (verified verbatim == mig 494 at build time) is
--     the two new keys on the venue arm. Existing consumers that read type/
--     entity_id/name/role are unaffected; the getMyWorld() wrapper has no mapper
--     (returns raw data) so the new keys reach nav.js untouched (HR#12: no mapper
--     drops them — confirmed no other mapper reshapes admin_roles).
--   * The membership backfill that makes the owner appear at all is unchanged: the
--     mig-371 trigger trg_venue_admins_person_id sets venue_admins.person_id from
--     user_id on insert, so a self_serve_create_club owner already matches the
--     existing `va.person_id = v_person` filter. No matching-logic change needed.
--   * STABLE SECURITY DEFINER, search_path pinned — identical to mig 494. No grant
--     change (get_my_world's grants are unchanged).
--
-- Whole function reproduced verbatim from the live definition (== mig 494); the
-- ONLY change is the venue_admin arm, marked ← mig 520 below.
...
BEGIN
  ...
  FROM (
    ...
    SELECT jsonb_build_object(
      ...
      'name', v.name,
      'role', va.role,
      'origin', v.origin,                                          -- ← mig 520
      'club_id', CASE WHEN v.origin = 'self_serve' THEN (          -- ← mig 520
