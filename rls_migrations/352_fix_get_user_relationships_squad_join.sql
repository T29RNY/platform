-- Migration 352 — fix get_user_relationships (mig 314) squad block.
--
-- BUG (pre-existing since mig 314, surfaced by multi-context nav testing): the
-- squad-memberships query referenced `p.team_id` (players has no such column —
-- the player→team link is `team_players`) and `p.player_token` (the column is
-- `token`). So the RPC threw `column p.team_id does not exist` (42703) and 400'd
-- for EVERY authenticated user on app load — meaning `homeScreenType` never
-- resolved and the land-on-/feed|/parent-home|/sessions redirects never fired.
--
-- FIX: join players→team_players→teams, select p.token. Everything else is
-- byte-identical to the live mig-314 body. Authenticated-only; CREATE OR REPLACE
...
BEGIN
  ...
  -- Squad memberships: auth.uid() → players.user_id → team_players → teams
  ...
  LEFT JOIN schedule s ON s.team_id = t.id AND s.active = true
