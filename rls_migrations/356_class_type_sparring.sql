-- Migration 356 — Gym/Boxing vertical Phase 1: sparring / open-mat class types.
--
-- A class type is EITHER a technical class OR a sparring/open-mat session
-- (operator decision, s145). `is_sparring` flags the latter so the member Classes
-- timetable can badge it and the "who's in for Thursday sparring?" In/Out booking
-- reuses the class-session model wholesale (capacity, waitlist, QR check-in,
-- no-show) — NO new write RPC. Any incidental sparring inside a technical class
-- is just mentioned in the description, never flagged.
--
-- Additive + nullable-safe: every existing class type (incl. all football) defaults
-- to false, so behaviour is byte-unchanged. Zero footprint on the casual flow.
--
-- Touches:
--   1. venue_class_types          + is_sparring boolean NOT NULL DEFAULT false
--   2. venue_create_class_type     thread p_is_sparring (signature change → DROP old)
--   3. venue_update_class_type     is_sparring in the jsonb patch (no signature change)
--   4. venue_list_class_types      return is_sparring (operator types-table badge)
--   5. member_list_class_sessions  return is_sparring (member timetable badge)
--   6. get_member_pass             return discipline (ClubNavBar Classes-tab gate on
--                                   the Pass surface — nav consistency, mig 355 labels)

-- 1. Schema -------------------------------------------------------------------
ALTER TABLE public.venue_class_types
  ADD COLUMN IF NOT EXISTS is_sparring boolean NOT NULL DEFAULT false;

-- 2. venue_create_class_type — adding a trailing param changes the signature.
--    CREATE OR REPLACE cannot replace a different arg list (both overloads would
--    coexist → "could not choose best candidate function" at call time), so DROP
--    the old 9-arg signature first, then re-grant after recreating.
DROP FUNCTION IF EXISTS public.venue_create_class_type(text, text, uuid, integer, integer, text, integer, boolean, text);
...
GRANT EXECUTE ON FUNCTION public.venue_create_class_type(text, text, uuid, integer, integer, text, integer, boolean, text, boolean)
  TO anon, authenticated, service_role;

-- 3. venue_update_class_type — is_sparring through the jsonb patch (no signature change).
...
-- 4. venue_list_class_types — surface is_sparring for the operator types-table badge.
...
-- 5. member_list_class_sessions — surface is_sparring for the member timetable badge.
...
-- 6. get_member_pass — surface the club's discipline so the ClubNavBar on the Pass
--    surface lights the Classes tab for non-football clubs (mig 355 labels).
