-- 207 DOWN: restore admin_go_live + admin_go_live_for_team to their pre-207
-- bodies (no guest-player deletion on new-week open). Signatures unchanged.
-- Restores to the mig-204 state.
...
BEGIN
  ...
  IF v_match_id IS NULL THEN
    ...
    -- New week: clear last week's in/out status + A/B team assignments so the
    -- board opens fresh. Payment flags carry over intentionally.
    UPDATE players SET
      status          = 'none',
      admin_locked_in = false,
      team            = NULL
