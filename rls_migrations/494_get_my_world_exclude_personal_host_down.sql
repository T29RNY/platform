-- 494_get_my_world_exclude_personal_host_down.sql
-- Reverse of 494: restore get_my_world WITHOUT the is_personal_host exclusion
-- (the hidden self-serve host will again leak into admin_roles as an operator).
--
-- Standalone Tournament Self-Serve epic — PR #5, the GO-LIVE BLOCKER fix.
--
-- THE LEAK (found in the PR #4b audit): self_serve_create_tournament (mig 489)
-- mints a hidden personal-host venue with a venue_admins(role='owner') row so the
-- creator can manage their tournament via the Stage-1b venue_id-as-token path.
-- But get_my_world (mig 372) predates the personal-host concept: its admin_roles
-- venue arm emits EVERY active venue_admins row, so the hidden host leaks in as a
-- venue_admin. resolveRoles (apps/inorout mobile/nav.js) then turns any
-- venue_admin into a full operator hat — so a self-serve tournament creator would
-- land in /hub as a venue OPERATOR (Setup/Bookings/Payments/People). Only
-- reachable once the dark flag flips (no self-serve tournaments exist until then),
-- but it MUST be closed before go-live.
--
-- THE FIX: exclude is_personal_host venues from the admin_roles venue arm. This is
-- the single authoritative point — the hidden host never enters admin_roles, so it
-- can't become an operator hat on any client. get_my_tournaments (mig 492) is the
-- sanctioned way for the creator to reach their tournaments instead.
--
-- Full CREATE OR REPLACE transcribed verbatim from the live definition; the ONLY
-- change is the added `AND COALESCE(v.is_personal_host, false) = false` on the
-- venue_admins UNION arm (marked below).
