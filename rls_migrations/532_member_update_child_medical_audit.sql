-- mig 532 — member_update_child: classify special-category (health) edits as medical in the audit.
--
-- The guardian /hub medical editor (Phase 6) writes dietary_notes (allergies/dietary), send_notes
-- (SEND / conditions / medication), and the two medical consents — all special-category health
-- data under UK GDPR Art 9 — but the audit `v_medical` flag only tripped on medical_conditions/
-- allergies/medications/gp_details, so those saves logged as 'member_child_profile_updated' instead
-- of 'member_child_medical_updated'. A safeguarding/DPA reviewer filtering on the medical event type
-- would miss them. This extends the predicate to also trip on dietary_notes, send_notes, and both
-- consents. AUDIT-CLASSIFICATION ONLY — no change to authz, the whitelist update, or the return shape.
-- Body is the LIVE definition (pulled via pg_get_functiondef) with only the v_medical block changed.
...
BEGIN
  ...
  -- Special-category (health) data under UK GDPR Art 9. dietary_notes = allergies/dietary,
  -- send_notes = SEND/conditions/medication (both surfaced by the guardian medical editor),
  -- plus the two medical consents. Any of these => flag the edit as medical in the audit.
  v_medical := (
    p_updates ? 'medical_conditions'            OR
    p_updates ? 'allergies'                     OR
    p_updates ? 'medications'                   OR
    p_updates ? 'gp_details'                    OR
    p_updates ? 'dietary_notes'                 OR
    p_updates ? 'send_notes'                    OR
    p_updates ? 'consent_emergency_treatment'   OR
    p_updates ? 'consent_administer_medication'
  );
