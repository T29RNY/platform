-- 109 down — restore mig 094 body (no maintenance window check)
...
BEGIN
  ...
  END IF;
  SELECT f.id, f.status, f.playing_area_id, f.competition_id, s.league_id, l.venue_id AS l_venue
  INTO v_fixture
  FROM fixtures f
  ...
  IF v_fixture.status NOT IN ('scheduled','allocated') THEN
    RAISE EXCEPTION 'fixture_status_locks_pitch' USING ERRCODE = 'P0001', DETAIL = v_fixture.status;
  END IF;
  ...
  IF p_playing_area_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      WHERE id = p_playing_area_id AND venue_id = v_venue_id AND active = true AND is_available = true
    ) THEN
      RAISE EXCEPTION 'pitch_unavailable' USING ERRCODE = 'P0001', DETAIL = p_playing_area_id::text;
    END IF;
    v_new_status := 'allocated';
  ELSE
    v_new_status := CASE WHEN v_fixture.status = 'allocated' THEN 'scheduled' ELSE v_fixture.status END;
  END IF;
  UPDATE fixtures SET playing_area_id = p_playing_area_id, status = v_new_status WHERE id = p_fixture_id;
  ...
  VALUES (v_venue_id, auth.uid(), v_caller.actor_type, v_caller.actor_ident,
          ...
          jsonb_build_object('competition_id', v_fixture.competition_id, 'league_id', v_league_id,
                             'previous_playing_area_id', v_prev_pitch, 'playing_area_id', p_playing_area_id,
  ...
  RETURN jsonb_build_object('ok', true, 'fixture_id', p_fixture_id,
                            'playing_area_id', p_playing_area_id, 'status', v_new_status);
END;
$function$;

REVOKE ALL ON FUNCTION public.venue_assign_pitch(text, uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.venue_assign_pitch(text, uuid, uuid) TO anon, authenticated;
