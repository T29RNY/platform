-- Migration 150 — Pitch Booking Stage 6 (venue UI support). Three additive changes:
--   1. venue_get_state — expose venues.bookings_enabled + venues.cancellation_policy
--      in the venue projection (columns exist since mig 134; body otherwise identical
--      to the live mig-135 version).
--   2. get_pitch_occupancy — expose pitch_bookings.series_id in the booking detail so
--      the venue Requests inbox can group a weekly block into one card (body otherwise
--      identical to the live mig-141 version).
--   3. venue_update_booking_settings — new venue-owned write RPC for the bookings_enabled
--      toggle + cancellation_policy text. Follows the venue_update_pitch pattern
--      (SECURITY DEFINER, resolve_venue_caller, audit Phase-2 shape, notify_venue_change
--      with the already-whitelisted 'venue_updated' reason, returns jsonb).
-- All additive; existing keys/consumers unaffected.

-- ──────────────────────────────────────────────────────────────────
-- 1. venue_get_state — expose bookings_enabled + cancellation_policy
-- ──────────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────────
-- 2. get_pitch_occupancy — expose series_id in the booking detail
-- ──────────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────────
-- 3. venue_update_booking_settings — bookings_enabled + cancellation_policy
-- ──────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.venue_update_booking_settings(
  p_venue_token text,
  p_updates     jsonb
)
RETURNS jsonb
