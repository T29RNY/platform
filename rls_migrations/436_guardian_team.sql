-- mig 436 — Guardian "Team" screen reader (More hub → Team).
-- Read-only, guardian-gated (mirrors the 426/428/434 sibling gate). Returns one block per
-- ACTIVE team the child plays in: team + club + league header, the team's OWN W/D/L record
-- (real, computed from completed club_fixtures — there is NO honest league position for these
-- free-text-opponent grassroots leagues, so league_position is reserved NULL until the future
-- FA Full-Time scrape populates it; the header pill stays hidden while it is NULL), the active
-- coaches, and the active squad (child flagged). Team broadcasts are NOT here — the screen reuses
-- guardian_list_child_notices filtered to audience='team' (sender + read-state already resolved).
