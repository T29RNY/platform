-- 381_tournament_info_and_public_venue_details.sql
-- Tournament Hub Phase 1 backend (session 172).
--
-- 1) Adds tournament_events.info jsonb (free-form public info: parking, prices note,
--    rules, what-to-bring, contact override, etc.). DEFAULT '{}' so existing rows are
--    unaffected.
-- 2) Extends get_tournament_public to also return the venue's address/contact (already
--    on the venues table — address/city/postcode/lat/lng/contact_email/contact_phone),
--    the info jsonb, and a derived registration_open flag — so the public Tournament Hub
--    can render an Info/Directions tab (map pin, parking, prices, contact) and a
--    pre-tournament signup state. Everything else returned is byte-identical to the prior
--    version. SECURITY DEFINER + pinned search_path + anon/authenticated grant unchanged.
