-- Down for 143 — drop the 4-arg confirmed-clash versions and restore the
-- mig 138 3-arg venue_assign_pitch / venue_generate_fixtures (pitch_double_booked
-- translation, no clash gate, no p_displace_booking_ids).

DROP FUNCTION IF EXISTS public.venue_assign_pitch(text, uuid, uuid, uuid[]);
DROP FUNCTION IF EXISTS public.venue_generate_fixtures(text, uuid, jsonb, uuid[]);
...
GRANT EXECUTE ON FUNCTION public.venue_generate_fixtures(text, uuid, jsonb) TO anon, authenticated;
