-- DOWN for mig 353 — restore the original (buggy) bare ::timestamptz cast in the three
-- recurring-session generators. Provided for completeness; the bare cast reintroduces the
-- BST off-by-one-hour bug, so this should only be used to revert a bad deploy.
