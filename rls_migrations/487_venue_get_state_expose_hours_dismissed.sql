-- Migration 487: Venue Setup Wizard PR-W3 (read-side completion) — expose
-- opening_hours + setup_dismissed_steps on venue_get_state.
--
-- HR#12 fix. Mig 486 added venues.opening_hours + venues.setup_dismissed_steps and
-- the setters that WRITE them, but nothing surfaced them on the READ path — so the
-- hub's hours step could never tick, the hours editor never pre-filled, and "skip
-- for now" never displayed (the write succeeded server-side; the UI couldn't see
-- it). This adds both keys to the venue object in venue_get_state — same read-shape
-- pattern as mig 485. Body verbatim from the live (485) definition; only the two
-- SW487 keys added.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'venue', (SELECT jsonb_build_object('id', v.id, 'name', v.name, 'slug', v.slug, 'sport', v.sport,
        ...
        'verification_status', v.verification_status, 'origin', v.origin,
        'opening_hours', v.opening_hours, 'setup_dismissed_steps', v.setup_dismissed_steps, /* SW487 */
