-- 610_trial_cta_gate.sql
-- P5 Step 2 of the DF trial-booking epic (docs/epics/df-trial-booking.md).
--
-- The explicit, per-club gate for the public "Book a FREE trial" CTA. Plan-gate decision #2:
-- the CTA is gated per-club, default OFF — an operator must opt a club in, so PA Sports (and
-- every other published club) renders byte-identical until they tick it. A data-driven gate
-- (e.g. "has a free-first-session class") was rejected because it would auto-enable clubs.
--
--   1. club_pages.trial_cta_enabled boolean NOT NULL DEFAULT false — the switch.
--   2. get_club_public CREATE OR REPLACE — expose it inside the `branding` object so the one
--      existing public read carries the flag (no extra round-trip). Return-shape addition:
--      ClubPublicScreen reads branding.trial_cta_enabled and points its CTA at /c/<slug>/trial
--      when true (same commit — Hard Rule 12).
--
-- The rest of get_club_public's body is copied verbatim from the live definition; the ONLY
-- change is the added 'trial_cta_enabled' key in the branding jsonb_build_object. The EV asserts
-- the full payload shape survives the CREATE OR REPLACE.

BEGIN;

ALTER TABLE public.club_pages
  ADD COLUMN IF NOT EXISTS trial_cta_enabled boolean NOT NULL DEFAULT false;
...
BEGIN
  ...
  RETURN jsonb_build_object('found', true,
    ...
    'branding', jsonb_build_object('primary_colour', v_row.primary_colour, 'secondary_colour', v_row.secondary_colour,
      ...
      'trial_cta_enabled', COALESCE(v_row.trial_cta_enabled, false)),
