-- Migration 176 — Prime-time windows (HQ Intelligence Phase 1, Cycle 1).
-- Lets a venue mark its peak hours per pitch, so HQ utilisation (Cycle 2) can
-- split prime vs off-peak. Three additive changes:
--   1. playing_areas.prime_time_windows jsonb (new column, default '[]').
--   2. venue_update_pitch — add a prime_time_windows validate+write block.
--      Built on the LIVE body (mig 135); booking_windows/maintenance unchanged.
--   3. venue_get_state — expose prime_time_windows in the pitches projection.
--      Built on the LIVE body (mig 168); ALL existing keys preserved verbatim
--      (display_token, display_config, bookings_enabled, cancellation_policy …).
-- All additive; existing keys/consumers unaffected.

-- ──────────────────────────────────────────────────────────────────
-- 1. Schema — new column (metadata-only add, no table rewrite)
-- ──────────────────────────────────────────────────────────────────
ALTER TABLE playing_areas
  ADD COLUMN IF NOT EXISTS prime_time_windows jsonb NOT NULL DEFAULT '[]'::jsonb;

-- ──────────────────────────────────────────────────────────────────
-- 2. venue_update_pitch — add prime_time_windows editable key
--    Shape: [{ day_of_week 0-6, start_time "HH:MM", end_time "HH:MM" }]
--    (a band — no slot_lengths, unlike booking_windows)
-- ──────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF p_updates ? 'maintenance_windows' THEN
    ...
    EXCEPTION WHEN exclusion_violation THEN
      -- the maintenance->occupancy trigger (mig 136) hit an existing
      -- fixture/booking occupancy on this pitch. Surface a clear error.
      RAISE EXCEPTION 'maintenance_window_conflicts_occupancy' USING ERRCODE = 'P0001';
    END;
    v_changed := array_append(v_changed, 'maintenance_windows');
  END IF;
  IF p_updates ? 'booking_windows' THEN
    v_bw := p_updates->'booking_windows';
    IF v_bw IS NULL OR v_bw = 'null'::jsonb THEN
      v_bw := '[]'::jsonb;
    END IF;
    IF jsonb_typeof(v_bw) <> 'array' THEN
      RAISE EXCEPTION 'booking_windows_invalid' USING ERRCODE = 'P0001';
    END IF;
    FOR v_w IN SELECT * FROM jsonb_array_elements(v_bw) LOOP
      IF (v_w->>'day_of_week') IS NULL
         OR (v_w->>'day_of_week') !~ '^[0-9]+$'
         OR (v_w->>'day_of_week')::int < 0 OR (v_w->>'day_of_week')::int > 6 THEN
        RAISE EXCEPTION 'booking_window_day_invalid' USING ERRCODE = 'P0001';
      END IF;
      IF (v_w->>'open_time') IS NULL OR (v_w->>'close_time') IS NULL THEN
        RAISE EXCEPTION 'booking_window_times_required' USING ERRCODE = 'P0001';
      END IF;
      IF (v_w->>'open_time')::time >= (v_w->>'close_time')::time THEN
        RAISE EXCEPTION 'booking_window_times_inverted' USING ERRCODE = 'P0001';
      END IF;
      v_sl := v_w->'slot_lengths';
      IF v_sl IS NULL OR jsonb_typeof(v_sl) <> 'array'
         OR jsonb_array_length(v_sl) = 0 THEN
        RAISE EXCEPTION 'booking_window_slot_lengths_invalid' USING ERRCODE = 'P0001';
      END IF;
      IF EXISTS (
        SELECT 1 FROM jsonb_array_elements(v_sl) e
        WHERE jsonb_typeof(e) <> 'number' OR (e::text)::int <= 0
      ) THEN
        RAISE EXCEPTION 'booking_window_slot_lengths_invalid' USING ERRCODE = 'P0001';
      END IF;
    END LOOP;
    UPDATE playing_areas SET booking_windows = v_bw WHERE id = p_pitch_id;
    v_changed := array_append(v_changed, 'booking_windows');
  END IF;
  -- NEW: prime_time_windows — a peak-hours band per weekday (no slot_lengths).
  IF p_updates ? 'prime_time_windows' THEN
    v_pw := p_updates->'prime_time_windows';
    IF v_pw IS NULL OR v_pw = 'null'::jsonb THEN
      v_pw := '[]'::jsonb;
    END IF;
    IF jsonb_typeof(v_pw) <> 'array' THEN
      RAISE EXCEPTION 'prime_time_windows_invalid' USING ERRCODE = 'P0001';
    END IF;
    FOR v_w IN SELECT * FROM jsonb_array_elements(v_pw) LOOP
      IF (v_w->>'day_of_week') IS NULL
         OR (v_w->>'day_of_week') !~ '^[0-9]+$'
         OR (v_w->>'day_of_week')::int < 0 OR (v_w->>'day_of_week')::int > 6 THEN
        RAISE EXCEPTION 'prime_time_window_day_invalid' USING ERRCODE = 'P0001';
      END IF;
      IF (v_w->>'start_time') IS NULL OR (v_w->>'end_time') IS NULL THEN
        RAISE EXCEPTION 'prime_time_window_times_required' USING ERRCODE = 'P0001';
      END IF;
      IF (v_w->>'start_time')::time >= (v_w->>'end_time')::time THEN
        RAISE EXCEPTION 'prime_time_window_times_inverted' USING ERRCODE = 'P0001';
      END IF;
    END LOOP;
    UPDATE playing_areas SET prime_time_windows = v_pw WHERE id = p_pitch_id;
    v_changed := array_append(v_changed, 'prime_time_windows');
  END IF;
...
-- ──────────────────────────────────────────────────────────────────
-- 3. venue_get_state — expose prime_time_windows in the pitches projection
--    LIVE body (mig 168) reproduced verbatim + the one new pitch key.
-- ──────────────────────────────────────────────────────────────────
...
-- Refresh the PostgREST signature cache so the new RPC shape is served immediately.
