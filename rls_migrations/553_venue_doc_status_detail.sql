-- 553: (FIX-3b) operator/club-admin doc-status gains the SAME per-member "which docs missing"
-- detail the coach reader already returns (mig 542). Today venue_get_club_doc_status returns only
-- status FLAGS per member (consents{signed,required,status} / id{status} / medical{status}); the
-- coach reader (club_manager_get_team_doc_status) additionally returns:
--   • consents.items[] — every current policy document {title, version, signed, signed_at}
--   • id.detail        — {document_type, status, uploaded_at, verified_at, rejection_reason}
--   • medical.reviewed_at
-- so the coach can see EXACTLY which consent forms are missing / when ID was verified / when the
-- yearly medical was last confirmed. This ports those three detail arms verbatim into the
-- operator/club-admin reader (v_club_id → p_club_id) so all three roles show identical detail.
--
-- Additive return-shape change only (new nested keys; no existing key changes). STATUS/metadata
-- ONLY — never any medical content (matches the coach reader + Hard Rule for this surface).
-- The id.detail/id_status lookups stay purged_at-agnostic, consistent with mig 548 (an approved
-- proof-of-age that was later purged still reads 'done').
--
-- CREATE OR REPLACE, same 2-arg signature — no DROP, no grant change. Consumers updated same
-- commit (Hard Rule #12/#14): apps/venue SafeguardingBoard.jsx (desktop) + apps/inorout
-- ClubAdminSafeguarding.jsx (club-admin /hub) — both gain a per-member drill-in.
...
BEGIN
  ...
  WITH roster AS (
    SELECT DISTINCT vm.member_profile_id AS pid, NULLIF(btrim(mp.first_name || COALESCE(' ' || mp.last_name, '')), '') AS name
  ...
  consents AS (
    SELECT r.pid, count(ca.id)::int AS signed FROM roster r
  ...
  permbr AS (
    SELECT r.pid, r.name, LEAST(COALESCE(c.signed, 0), v_req_count) AS signed,
      ...
      (CASE WHEN NOT v_id_mandate THEN 'na'
            ELSE (CASE (SELECT d.status FROM member_id_documents d WHERE d.member_profile_id = r.pid AND d.club_id = p_club_id ORDER BY d.uploaded_at DESC LIMIT 1)
    ...
    COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'consents', jsonb_build_object('signed', p.signed, 'required', v_req_count, 'status', p.consent_status,
        'items', (SELECT COALESCE(jsonb_agg(jsonb_build_object('title', pd.title, 'version', pd.version, 'signed', (ca.id IS NOT NULL), 'signed_at', ca.accepted_at) ORDER BY (ca.id IS NOT NULL), pd.title), '[]'::jsonb)
                  ...
                  WHERE pd.club_id = p_club_id AND pd.is_current)),
      'id', jsonb_build_object('status', p.id_status,
        'detail', (SELECT jsonb_build_object('document_type', d.document_type, 'status', d.status, 'uploaded_at', d.uploaded_at, 'verified_at', d.verified_at, 'rejection_reason', d.rejection_reason)
                   FROM member_id_documents d WHERE d.member_profile_id = p.pid AND d.club_id = p_club_id ORDER BY d.uploaded_at DESC LIMIT 1)),
