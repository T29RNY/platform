-- ════════════════════════════════════════════════════════════════════════════
-- 346 — Plus-one / guest approvals
-- ════════════════════════════════════════════════════════════════════════════
-- A player's plus-one no longer joins the lineup unilaterally. When a NON-admin
-- adds a guest via add_guest_player it enters a PENDING state (status='none',
-- pending_approval=true) — it takes no squad spot and is hidden from the board
-- (same mechanism as a dormant guest) until an admin approves it.
--
--   - add_guest_player: new optional p_admin_token. If a valid admin token for
--     the guest's team is passed, the add is an ADMIN add → approved straight
--     in (status='in', pending_approval=false), respecting the squad-full guard
--     exactly as before. If no admin token → PENDING (status='none',
--     pending_approval=true), squad-full guard skipped (no spot is taken yet).
--   - admin_approve_guest: admin clears a pending guest. If the squad is at cap
--     the guest is placed on RESERVE instead of IN; otherwise IN.
--   - admin_decline_guest: admin rejects a pending guest → goes dormant
--     (pending_approval=false, status='none') exactly like a removed guest,
--     recoverable via the returning-guest picker.
--
-- Notifications: add_guest_player fires notify_team_change with reason
-- 'guest_pending_approval' (pending) so the admin's open app surfaces the
-- approvals banner live. approve/decline fire their own reasons.
--
-- State RPCs (get_team_state_by_admin_token / get_team_state_by_player_token)
-- gain 'pending_approval' on every squad player shape so the client can render
-- the admin approvals banner and the player's own "waiting for approval" state.
-- Those two function bodies are reproduced from migration 132 byte-for-byte
-- except for the added pending_approval field. The player's own row (v_player)
-- already carries pending_approval via to_jsonb(p.*).
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── 2. add_guest_player (3-arg: drop old 2-arg signature first) ──────────────
DROP FUNCTION IF EXISTS public.add_guest_player(text, text);
...
BEGIN
  ...
  -- Is this an admin add? A valid admin token for THIS team auto-approves.
  IF p_admin_token IS NOT NULL THEN
    SELECT EXISTS (
      SELECT 1 FROM teams t
       WHERE t.admin_token = p_admin_token AND t.id = v_team_id
    ) INTO v_is_admin;
  END IF;

  IF v_is_admin THEN
    -- Admin add → straight in, but still respects the squad-full guard.
    ...
    v_pending := false;
  ELSE
    -- Player add → pending admin approval. Takes no spot, hidden from board.
    v_status  := 'none';
    v_pending := true;
  END IF;
  ...
  INSERT INTO players (
    ...
    is_guest, guest_of, pending_approval
  ) VALUES (
  ...
  INSERT INTO audit_events (
    ...
    v_team_id,
    CASE WHEN v_is_admin THEN 'team_admin' ELSE 'player' END,
    auth.uid(),
    CASE WHEN v_is_admin THEN 'admin_token:' || md5(p_admin_token)
         ELSE 'player_token:' || md5(p_token) END,
    ...
    jsonb_build_object(
      ...
      'via',              CASE WHEN v_is_admin THEN 'admin' ELSE 'player' END
    )
  );
  ...
  PERFORM notify_team_change(
    v_team_id,
    CASE WHEN v_pending THEN 'guest_pending_approval' ELSE 'guest_player_added' END
  );
...
REVOKE EXECUTE ON FUNCTION public.add_guest_player(text, text, text) FROM public;
GRANT  EXECUTE ON FUNCTION public.add_guest_player(text, text, text) TO anon, authenticated;

-- ── 3. admin_approve_guest ───────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.admin_approve_guest(
  p_admin_token text,
  p_guest_id    text
)
 RETURNS jsonb
...
BEGIN
  ...
  -- Guest must belong to this team, be a guest, and be pending.
...
REVOKE EXECUTE ON FUNCTION public.admin_approve_guest(text, text) FROM public;
GRANT  EXECUTE ON FUNCTION public.admin_approve_guest(text, text) TO anon, authenticated;

-- ── 4. admin_decline_guest ───────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.admin_decline_guest(
  p_admin_token text,
  p_guest_id    text
)
 RETURNS jsonb
...
BEGIN
  ...
  -- Decline → dormant (recoverable via returning-guest picker), like removal.
  UPDATE players
...
REVOKE EXECUTE ON FUNCTION public.admin_decline_guest(text, text) FROM public;
GRANT  EXECUTE ON FUNCTION public.admin_decline_guest(text, text) TO anon, authenticated;

-- ── 5. State RPCs: add pending_approval to squad shapes (from mig 132) ────────
-- ════════════════════════════════════════════════════════════════════════════
...
-- ── 6. remove_guest_player: also clear pending_approval on host cancel ───────
...
BEGIN
  ...
  -- 346: also clear pending_approval so a host cancelling a not-yet-approved
  -- plus-one removes it from the admin approvals queue.
  UPDATE players SET
    status           = 'none',
    admin_locked_in  = false,
    team             = NULL,
