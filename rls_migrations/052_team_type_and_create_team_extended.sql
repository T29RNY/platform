-- Migration 052 — teams.team_type column + create_team RPC accepts p_team_type
-- Phase 0C of venue_league_hq_SCOPE.md.
--
-- Sport-agnostic context flag at the team level:
--   - 'casual'      → friends game, no league context
--   - 'competitive' → league/cup team
--
-- All 3 existing teams default to 'casual' (invisible to existing customers).
-- The onboarding UI step "Casual or Competitive?" is DEFERRED to Phase 2 —
-- there is no league context to attach to in Phase 0, so asking would be
-- UX noise. Phase 0 only lands the column + RPC signature so the data
-- model is ready when Phase 1/2 builds the league flow.
--
-- create_team is resigned to accept optional p_team_type (default 'casual').
-- Per CLAUDE.md "RPC PARAMETER TYPE CHANGES" rule, the old signature MUST
-- be explicitly DROPed first — CREATE OR REPLACE alone treats a different
-- parameter list as an overload, causing "could not choose best candidate"
-- runtime errors. The body is byte-identical to the prior version apart
-- from the new parameter, the new INSERT column, and an extra validation.

ALTER TABLE public.teams
  ADD COLUMN IF NOT EXISTS team_type text NOT NULL DEFAULT 'casual';
...
-- Drop the old create_team signature (no p_team_type) before re-creating
-- with the extended signature. Failure mode without this DROP: PostgREST
-- can't choose between two overloads → 400 on every /create.
...
CREATE OR REPLACE FUNCTION public.create_team(
  ...
  p_priority_lead_mins   integer DEFAULT NULL,
  p_team_type            text DEFAULT 'casual'
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Link authenticated creator as team admin + create player row for admin
