-- 104_phase2_standings_forfeit_support.sql
--
-- Phase 2 (League Mode) — Cycle 2.5b standings update.
--
-- Extends get_league_standings_for_player (mig 087) to count forfeit
-- fixtures alongside walkover fixtures. Default of 3-0 to the
-- forfeit_winner_id mirrors the existing walkover treatment.
--
-- Postponed / voided fixtures remain excluded from standings — they
-- represent fixtures that genuinely didn't happen (either rescheduled
-- elsewhere or cancelled outright by team withdrawal/expulsion).
--
-- Withdrawn / expelled teams stay in the standings list (their
-- accumulated points pre-exit are real and other teams should still
-- be able to see who they beat). The ct_status field already
-- surfaces 'withdrawn' / 'expelled' so the UI can render those rows
-- struck through if desired.
--
-- Return shape unchanged (per rule #12: no consumer mapper change
-- needed if shape is identical).
...
BEGIN
  ...
  comp_teams_all AS (
    SELECT ct.competition_id, ct.team_id, ct.status AS ct_status,
           t.name AS team_name,
           t.primary_colour, t.secondary_colour
    FROM competition_teams ct
    JOIN teams t ON t.id = ct.team_id
    WHERE ct.competition_id IN (SELECT competition_id FROM player_comps)
      AND ct.status IN ('active','withdrawn','expelled')
  ),
  fixture_outcomes AS (
  ...
  team_rows AS (
    ...
    WHERE hs IS NOT NULL AND as_score IS NOT NULL
    UNION ALL
  ...
  standings AS (
    SELECT cta.competition_id,
           cta.team_id,
           cta.team_name,
           cta.primary_colour,
           cta.secondary_colour,
           cta.ct_status,
           COALESCE(ta.played, 0) AS played,
           COALESCE(ta.w, 0) AS w,
           COALESCE(ta.d, 0) AS d,
           COALESCE(ta.l, 0) AS l,
           COALESCE(ta.gf, 0) AS gf,
           COALESCE(ta.ga, 0) AS ga,
           COALESCE(ta.gd, 0) AS gd,
           COALESCE(ta.pts, 0) AS pts
    FROM comp_teams_all cta
    LEFT JOIN team_aggregated ta
      ON ta.competition_id = cta.competition_id
     AND ta.team_id = cta.team_id
  )
  SELECT jsonb_build_object(
    'player_id', v_player_id,
    'competitions', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'competition_id', cm.competition_id,
        'competition_name', cm.comp_name,
        'competition_type', cm.comp_type,
        'competition_format', cm.comp_format,
        'season_id', cm.season_id,
        'season_name', cm.season_name,
        'season_start', cm.season_start,
        'season_end', cm.season_end,
        'league_id', cm.league_id,
        'league_name', cm.league_name,
        'venue_id', cm.venue_id,
        'venue_name', cm.venue_name,
        'standings_visible', cm.standings_visibility = 'public',
        'standings', CASE
          WHEN cm.standings_visibility = 'public' THEN COALESCE((
            SELECT jsonb_agg(jsonb_build_object(
              'team_id', s.team_id,
              'team_name', s.team_name,
              'primary_colour', s.primary_colour,
              'secondary_colour', s.secondary_colour,
              'ct_status', s.ct_status,
              'played', s.played,
              'w', s.w, 'd', s.d, 'l', s.l,
              'gf', s.gf, 'ga', s.ga, 'gd', s.gd, 'pts', s.pts
            ) ORDER BY s.pts DESC, s.gd DESC, s.gf DESC, s.team_name)
            FROM standings s WHERE s.competition_id = cm.competition_id
          ), '[]'::jsonb)
          ELSE '[]'::jsonb
        END,
        'top_scorers', '[]'::jsonb
...
GRANT EXECUTE ON FUNCTION public.get_league_standings_for_player(text)
