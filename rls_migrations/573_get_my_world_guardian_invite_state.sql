-- 573_get_my_world_guardian_invite_state.sql
-- Align get_my_world()'s guardian arm with every sibling RPC: only ACCEPTED
-- guardian links resolve a guardian hat. Mig 571 added invite_state='accepted'
-- to member_accept_consent; the app-wide role resolver get_my_world() was the
-- lone outlier (held back deliberately — it is load-bearing for every role).
--
-- Change is a strict tightening of ONE WHERE clause in the guardian LATERAL
-- block; every other arm (league / casual / ref / clubs / admin / admin_clubs /
-- coaching / conflicts) is reproduced byte-identical to the live body.
--
-- NO-OP on today's data: member_guardians has 21 rows, all invite_state='accepted'
-- (the column is NOT NULL DEFAULT 'accepted'). This only takes effect once a
-- 'pending'/'invited' guardian state is ever introduced — at which point an
-- unaccepted guardian must NOT resolve a guardian hat or see a child's sessions.
...
BEGIN
  ...
  WHERE v_profile IS NOT NULL AND mg.guardian_profile_id = v_profile AND mg.invite_state = 'accepted';
