-- 398 — Coach paid/unpaid roster (Pilot demo sprint, item #4).
-- A team manager (coach) sees, for their own team, who's paid and who owes — read
-- only on the existing venue_charges ledger + venue_memberships. The auto-reminders
-- half is ALREADY built: get_membership_reminders_due (mig 276) emits a 'payment_due'
-- reminder for every unpaid/partial membership charge past its due date, sent by the
-- api/cron.js membershipRemindersJob (no-ops until RESEND_API_KEY). So #4 = this pill.
--
-- Authenticated-only, manager-scoped: mirrors the club_manager_* auth pattern
-- (mig 392/304): auth.uid() → member_profiles → club_team_managers(team_id,is_active).
-- Read-only (STABLE) — no audit. Membership charges join via the mig-271
-- '<membership_id>:<period>' source_id encoding, same as the reminder RPC.
...
REVOKE ALL ON FUNCTION public.club_manager_team_payments(uuid) FROM public;
-- authenticated-only: the manager is identified via auth.uid(). Explicit anon REVOKE
-- because Supabase auto-grants EXECUTE to anon on creation (the mig-175 gotcha).
REVOKE EXECUTE ON FUNCTION public.club_manager_team_payments(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.club_manager_team_payments(uuid) TO authenticated;
