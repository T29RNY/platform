-- mig 533 — Guardian fixture/league detail enrichment: venue + address + referee.
--
-- On-device walk: the League detail (and the Sessions match detail) should carry venue,
-- street address and referee. ADDITIVE only — new keys added to existing readers, no key
-- removed/renamed, so existing consumers are unaffected.
--   • guardian_list_child_leagues — fixtures[] gains venue_name / venue_address / ref_name;
--     results[] gains kickoff_time / pitch_name / venue_name / venue_address / ref_name
--     (joins already proven in guardian_list_child_fixtures: playing_areas→venues, match_officials).
--   • guardian_list_child_fixtures — upcoming[] gains venue_address (it already returns venue_name).
-- Address = the HOME venue only (club_fixtures stores the home playing_area; the opponent is
-- free-text with no stored ground, so AWAY fixtures have no address — venue_* stays NULL there,
-- honestly). Bodies are the LIVE definitions (pg_get_functiondef) with only the additions above.

-- ── guardian_list_child_leagues ──────────────────────────────────────────────
...
-- ── guardian_list_child_fixtures (add venue_address to upcoming) ──────────────
