-- ════════════════════════════════════════════════════════════════════════════
-- 070 — get_team_state_by_admin_token: expose token on every squad row,
--       and add is_self for the admin's own player
-- ════════════════════════════════════════════════════════════════════════════
-- Surfaced by: Admin > Squad > "Copy personal link" was producing
-- /p/<player_id> URLs that don't resolve. Root cause is the fallback in
-- SquadScreen.jsx (`${p.token || p.id}`) combined with migration 061's
-- decision to expose tokens only on the admin's own row. The fallback
-- masked the empty-token state and shipped player_ids to the clipboard.
--
-- Decision: admins should be able to share a player's /p/<token> URL —
-- that's the whole point of the Copy personal link feature. Tokens grant
-- player-self write access (status taps, self-pay, POTM vote), all of
-- which the admin can already do via admin RPCs that attribute correctly
-- to the admin. So token exposure doesn't add power; it enables sharing.
--
-- Add is_self as an explicit flag so callers don't have to infer
-- "the admin's own row" from token-truthiness. App.jsx:499 will be
-- updated in the same commit to use is_self instead of (p => p.token).
-- ════════════════════════════════════════════════════════════════════════════
