-- Migration 485: Venue Setup Wizard PR-W1 — expose verification_status + origin.
--
-- READ-SHAPE ONLY. The setup hub (apps/venue SetupHub + native /hub OperatorSetup)
-- needs the venue's `verification_status` ('verified'|'pending'|'rejected') and
-- `origin` ('superadmin'|'self_serve') — added to the `venues` row in mig 484 but
-- never surfaced to any client read. Without them the hub cannot (a) decide whether
-- to auto-open for a `pending` self-serve venue, nor (b) show honest go-live state,
-- nor (c) degrade gracefully for a superadmin-`verified` venue (Decision #8).
--
-- The ONLY change to each function body below is two added keys in the venue object
-- (marked `-- SW485`). Both bodies are reproduced VERBATIM from live:
--   venue_get_state  from mig 468 (safeguarding read-filter, current live def)
--   venue_whoami     from mig 237 (venue staff logins, current live def)
--
-- HR#12 (same-commit mapper): venueGetState returns the RPC jsonb verbatim (no JS
-- dbToVenue mapper exists), and venue_whoami's venues[] is consumed raw — the hub
-- reads `state.venue.verification_status` / `whoami.venues[].verification_status`
-- directly. There is no intermediate mapper to update; the consumer (SetupHub) lands
-- in the same PR.
--
-- Nothing writes/gates on these fields yet — enforcement (search_bookable_venues
-- filter + the auto-flip) is PR-W5 (mig 487). This migration is purely additive read.

-- ===========================================================================
-- 1. venue_get_state — add verification_status + origin to the venue object.
--    Body verbatim from mig 468; only the two SW485 keys added at line ~96.
-- ===========================================================================
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'venue', (SELECT jsonb_build_object('id', v.id, 'name', v.name, 'slug', v.slug, 'sport', v.sport,
        ...
        'verification_status', v.verification_status, 'origin', v.origin, /* SW485 */
...
-- ===========================================================================
-- 2. venue_whoami — add verification_status + origin to each venue in venues[].
--    Body verbatim from mig 237; only the two SW485 keys added.
-- ===========================================================================
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           ...
           'verification_status', v.verification_status, /* SW485 */
           'origin',     v.origin                        /* SW485 */
