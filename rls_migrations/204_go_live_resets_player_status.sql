-- 204: Opening a new casual week resets the squad's in/out status + A/B team
-- assignments so the board opens fresh. Without this, last week's whole squad
-- stays status='in' (board reads as full → set_player_status throws squad_full;
-- already-in players see the "Locked in" badge). Payment flags carry over.
-- Reset is gated to the new-match-creation path only, so idempotent re-calls
-- (double-taps, reuse of an existing live match) never wipe this week's statuses.
--
...
BEGIN
  ...
  IF v_match_id IS NULL THEN
    ...
    -- New week: clear last week's in/out status + A/B team assignments so the
    -- board opens fresh. Payment flags (paid/self_paid/paid_by/paid_at/owes)
