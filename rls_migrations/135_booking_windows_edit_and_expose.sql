-- Migration 135 — Pitch Booking Stage 2a.
-- 1. Extend venue_update_pitch to validate + write playing_areas.booking_windows.
--    Body otherwise identical to mig 106 (the live version).
-- 2. Expose booking_windows in venue_get_state's pitches projection.
--    Body otherwise identical to mig 113 (the live version).
-- Both additive; existing keys/consumers unaffected.

-- ──────────────────────────────────────────────────────────────────
-- 1. venue_update_pitch — add booking_windows editable key
-- ──────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF p_updates ? 'booking_windows' THEN
    ...
    FOR v_w IN SELECT * FROM jsonb_array_elements(v_bw) LOOP
      -- day_of_week: int 0-6 (0 = Sunday, matches the existing convention)
      ...
      END IF;
      -- open_time / close_time: "HH:MM(:SS)" 24h, open < close (venue-local)
      ...
      END IF;
      -- slot_lengths: non-empty array of positive ints
...
-- ──────────────────────────────────────────────────────────────────
-- 2. venue_get_state — expose booking_windows in the pitches projection
-- ──────────────────────────────────────────────────────────────────
