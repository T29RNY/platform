-- 244: broadcast-redesign enrichment of get_display_state (Reception Display
-- Part A1). Base body = mig 221 (verified identical to the live function modulo
-- one comment line). Additive only — no existing key changes shape:
--
-- 1. upcoming_fixtures[] rows gain: competition_type, round_name, official_id,
--    official_name (LEFT JOIN match_officials on fixtures.official_id).
--    Drives the Coming-Up round line + red "Needs ref" chip (official_id null).
-- 2. NEW top-level `bookings[]` — today's (Europe/London) confirmed casual
--    pitch bookings on this venue's playing areas, backed by an active
--    pitch_occupancy row (source_kind='booking'). Shape:
--    { booking_id, kickoff_time, pitch_name, booked_name, source_kind:'booking' }.
--    booked_name = team name when team-linked, else booked_by_name.
-- 3. competitions[].top_scorers[] rows gain: apps (count of completed fixtures
--    in that competition whose submitted fixture_lineups starting/bench arrays
--    contain the player — 0 until lineups are used), shirt_number
--    (players.shirt_number, may be null). No `position` key: players has no
--    position column (omitted per approved Golden Boot degradation). No mom /
--    g_per_90 in SQL — display computes G/90 from goals÷apps when apps>0.
-- 4. Featured-pin keys (featured_fixture_id / featured_pin_expires_at /
--    featured_pin_story_tag) need no work here: display_config is already
--    returned whole, so they pass through once mig 245 persists them.
--
-- Consumers (hard-rule #14): apps/display broadcast redesign (Part B, this
-- epic); Phase 6 HQ (future); Phase 7 Gaffer briefings (future).
-- Wrapper getDisplayState is a raw jsonb passthrough — no mapper update needed
-- (hard-rule #12): no JS consumer reads the new keys until Part B lands.
...
BEGIN
  ...
  -- 244: appearances = completed fixtures whose submitted lineup (starting or
  -- bench jsonb arrays of player_id text) contains the player ----------------
  player_apps AS (
  ...
  -- 244: today's confirmed casual pitch bookings on this venue, backed by an
  -- active occupancy row. Coming-Up merges these as single-team casual rows. --
  bookings AS (
    SELECT jsonb_agg(jsonb_build_object(
             'booking_id',pb.id,
