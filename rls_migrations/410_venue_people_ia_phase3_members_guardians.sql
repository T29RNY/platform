...
--
-- Venue People & Spaces IA — Phase 3 (Members + Guardians page).
--
-- Extends venue_list_members(p_venue_token) ADDITIVELY: each member row now also
-- carries `dob` (COALESCE member_profile -> venue_customer) and a `guardians` array
-- resolved from member_guardians (child_profile_id = the member's member_profile_id)
-- joined to member_profiles for the guardian's name/contact. Powers the new
-- read-only Members page (Members tab + a Guardians view derived client-side).
--
-- Additive only: the legacy MembersTab (MembershipsView) ignores the new keys, the
-- supabase.js wrapper is unchanged (passes the array through), so no casual surface
-- and no other consumer changes. Read-only; no audit. Single overload; SECURITY
-- DEFINER; search_path pinned; granted anon + authenticated (venue_* gotcha).
--
-- Guardian links only exist for members with a member_profile_id (the structured
-- member spine); customer-only legacy members return an empty guardians array.
--
-- Consumers: apps/venue Members page (MembersView) AND the existing MembershipsView
-- MembersTab (unchanged behaviour).
