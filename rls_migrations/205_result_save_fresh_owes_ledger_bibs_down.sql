-- 205 DOWN: restore admin_save_match_result to its pre-205 body — freshness via
-- player_match row count, no payment_ledger charge rows, no bib_history cascade.
-- (Reinstates the latent bugs; for rollback only.)
