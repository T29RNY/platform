-- 541_membership_reminders_member_id.sql — P11b: surface member_profile_id for the push channel.
--
-- P11a (mig 540) gave the payment-reminder cadence. P11b adds a PUSH leg alongside the email:
-- the cron pushes a payment reminder to the recipient's device (if they're an app member with a
-- registered push subscription). The recipient is a venue_customers.email, but push targets a
-- member_profile_id → member_profiles.auth_user_id → push_subscriptions. venue_memberships already
-- carries member_profile_id (nullable — customer-only memberships have none, so they get email only).
--
-- ADDITIVE: +member_profile_id as the 13th column on every arm (from m.member_profile_id, the same
-- venue_membership the reminder is about — NULL where the membership has no linked app member).
-- Nothing else changes vs mig 540. SQL STABLE SECDEF, search_path pinned, service_role-only grant.
...
AS $function$
  WITH base AS (
    SELECT 'welcome'::text AS kind, m.id::text AS entity_key,
           ...
           NULL::text AS pay_url, NULL::text AS reminder_stage, m.member_profile_id
    ...
    SELECT 'renewal_due', m.id::text || ':' || m.renews_at::text,
           ...
           NULL::text, NULL::text, m.member_profile_id
      FROM public.venue_memberships m
      JOIN public.venue_customers c        ON c.id = m.customer_id
      JOIN public.venue_membership_tiers t ON t.id = m.tier_id
      JOIN public.venues vn                ON vn.id = m.venue_id
