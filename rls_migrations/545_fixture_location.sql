-- 545: per-fixture free-text location/address.
--
-- club_fixtures gains a free-text `location` column. Coaches (team_manager) and
-- club-admins/operators (venue token) set it via the existing fixture-edit RPCs; the
-- guardian + coach fixture readers surface it. The client display rule everywhere is
-- `location || venue_address` — the free-text override wins, else the linked venue's
-- postal address (playing_area → venue). Purely additive: all existing rows are NULL
-- (→ readers fall back to the venue address exactly as before), no existing key removed.
--
-- Both write RPCs gain a trailing `p_location text` param — a NEW overload — so the old
-- signatures are DROPped first (RPC PARAMETER TYPE CHANGES rule) and re-granted after.
--
-- Consumers (Hard Rule #14):
--   write : apps/inorout TeamManagerFixtureEdit.jsx + SessionsScreen.jsx (coach),
--           apps/venue MembershipsView.jsx FixturesTab (club-admin/operator desktop)
--   read  : apps/inorout GuardianMatches.jsx, GuardianLeague.jsx (guardian);
--           TeamManagerLeague.jsx, TeamManagerTonight.jsx, TeamManagerMatchday.jsx (coach)

ALTER TABLE public.club_fixtures ADD COLUMN IF NOT EXISTS location text;

-- ─────────────────────────────────────────────────────────────────────────────
-- WRITE A — coach edit (new overload; DROP old 5-arg sig, re-grant authenticated)
-- ─────────────────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.club_manager_update_home_fixture(uuid, uuid, uuid, text, time without time zone);
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Options reader — return `location` so the coach edit form can prefill (no sig change)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- WRITE B — club-admin/operator desktop upsert (new 17-arg overload; DROP old 16-arg)
-- ─────────────────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.venue_upsert_club_fixture(text, uuid, uuid, uuid, text, text, boolean, date, time without time zone, uuid, uuid, text, integer, integer, text, text);
...
CREATE OR REPLACE FUNCTION public.venue_upsert_club_fixture(
  ...
  p_club_team_id uuid DEFAULT NULL::uuid,
  p_club_team_name text DEFAULT NULL::text,
  p_opponent_name text DEFAULT NULL::text,
  p_is_home boolean DEFAULT NULL::boolean,
...
BEGIN
  ...
  IF p_fixture_id IS NULL THEN
    ...
    END IF;
    INSERT INTO public.club_fixtures (league_id, club_team_id, club_team_name, opponent_name, is_home, scheduled_date, kickoff_time, playing_area_id, official_id, ref_name, home_score, away_score, status, notes, location)
    VALUES (p_league_id, p_club_team_id, NULLIF(btrim(p_club_team_name), ''), btrim(p_opponent_name), COALESCE(p_is_home, true), p_scheduled_date, p_kickoff_time, p_playing_area_id, p_official_id, NULLIF(btrim(p_ref_name), ''), p_home_score, p_away_score, COALESCE(p_status, 'scheduled'), NULLIF(btrim(p_notes), ''), NULLIF(btrim(p_location), ''))
  ...
  ELSE
    ...
    UPDATE public.club_fixtures SET
      ...
      away_score = COALESCE(p_away_score, away_score), status = COALESCE(p_status, status), notes = COALESCE(NULLIF(btrim(p_notes), ''), notes),
      location = COALESCE(NULLIF(btrim(p_location), ''), location), updated_at = now()
    WHERE id = p_fixture_id;
    INSERT INTO public.audit_events (team_id, actor_user_id, actor_type, actor_identifier, action, entity_type, entity_id, metadata)
...
REVOKE ALL ON FUNCTION public.venue_upsert_club_fixture(text, uuid, uuid, uuid, text, text, boolean, date, time without time zone, uuid, uuid, text, integer, integer, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.venue_upsert_club_fixture(text, uuid, uuid, uuid, text, text, boolean, date, time without time zone, uuid, uuid, text, integer, integer, text, text, text) TO anon, authenticated;

-- ─────────────────────────────────────────────────────────────────────────────
-- READ 1 — guardian fixtures (Sessions): +location on upcoming
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- READ 2 — guardian leagues (League tab): +location on fixtures[] and results[]
-- ─────────────────────────────────────────────────────────────────────────────
    ...
    SELECT cl.name AS league_name, jsonb_build_object(
      ...
      'fixtures', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'pitch_name',     pa.name,
          'venue_name',     vn.name,
          'venue_address',  NULLIF(concat_ws(', ', vn.address, vn.city, vn.postcode), ''),
          'location',       cf.location,
          'ref_name',       COALESCE(mo.name, cf.ref_name),
          'status',         cf.status
        ...
        FROM public.club_fixtures cf
        LEFT JOIN public.playing_areas   pa ON pa.id = cf.playing_area_id
        LEFT JOIN public.venues          vn ON vn.id = pa.venue_id
        LEFT JOIN public.match_officials mo ON mo.id = cf.official_id
...
-- ─────────────────────────────────────────────────────────────────────────────
-- READ 3 — coach team fixtures (League tab / Tonight): +location on upcoming
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- READ 4 — coach fixture detail (Matchday drill-in): +location/venue on fixture obj
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  SELECT cf.id, cf.opponent_name, cf.is_home, cf.scheduled_date,
         ...
         cf.location,
         pa.name AS pitch_name, v.name AS venue_name,
         NULLIF(concat_ws(', ', v.address, v.city, v.postcode), '') AS venue_address,
         COALESCE(mo.name, cf.ref_name) AS ref_name
    INTO v_fx FROM club_fixtures cf
    LEFT JOIN club_teams ct ON ct.id = cf.club_team_id
    LEFT JOIN playing_areas   pa ON pa.id = cf.playing_area_id
    LEFT JOIN venues          v  ON v.id  = pa.venue_id
    LEFT JOIN match_officials mo ON mo.id = cf.official_id
   WHERE cf.id = p_fixture_id;
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           ...
           'is_starter', ln.is_starter, 'position', ln.position,
           'selected', (ln.member_profile_id IS NOT NULL)
         ) ORDER BY mp.first_name), '[]'::jsonb)
    INTO v_roster FROM club_team_members cm
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'member_profile_id', s.member_profile_id, 'goals', s.goals, 'assists', s.assists,
           'yellow_cards', s.yellow_cards, 'red_cards', s.red_cards, 'minutes', s.minutes, 'is_potm', s.is_potm
         )), '[]'::jsonb)
    INTO v_stats FROM club_fixture_player_stats s WHERE s.fixture_id = p_fixture_id;
  RETURN jsonb_build_object('ok', true,
    'fixture', jsonb_build_object('fixture_id', v_fx.id, 'opponent_name', v_fx.opponent_name,
      'is_home', v_fx.is_home, 'scheduled_date', v_fx.scheduled_date, 'kickoff_time', v_fx.kickoff_time,
      'home_score', v_fx.home_score, 'away_score', v_fx.away_score, 'status', v_fx.status,
      'our_team', v_fx.our_team, 'team_id', v_fx.club_team_id,
      'location', v_fx.location, 'pitch_name', v_fx.pitch_name, 'venue_name', v_fx.venue_name,
      'venue_address', v_fx.venue_address, 'ref_name', v_fx.ref_name),
    'roster', v_roster, 'stats', v_stats);
END;
$function$;

-- ─────────────────────────────────────────────────────────────────────────────
-- READ 5 — coach/member club fixtures feed (SessionsScreen read view): +location/venue_address
-- ─────────────────────────────────────────────────────────────────────────────
  ...
  RETURN COALESCE((
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'venue_address', NULLIF(concat_ws(', ', v.address, v.city, v.postcode), ''), 'location', cf.location,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- READ 6 — desktop operator/club-admin fixture list (apps/venue FixturesTab): +location
-- (venue_address already returned; needed for the edit-form prefill + list display)
-- ─────────────────────────────────────────────────────────────────────────────
