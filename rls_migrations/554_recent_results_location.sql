-- 554: (FIX-1d) carry the match LOCATION onto RECENT RESULTS too. Mig 545 added per-fixture
-- location + venue to the UPCOMING arms of the guardian + coach fixture readers, but their
-- recent[] (completed-result) arms still returned only the scoreline. A guardian/coach looking
-- back at a result couldn't see WHERE it was played. This adds pitch_name / venue_name /
-- venue_address / location to the recent[] arm of BOTH readers (same keys + display rule as the
-- upcoming arm — location || venue), via the same LEFT JOINs to playing_areas + venues.
--
-- Additive return-shape only (new keys under each recent[] row; no existing key changes).
-- Consumers (Hard Rule #14): apps/inorout GuardianMatches.jsx (guardian recent) +
-- TeamManagerLeague.jsx (coach recent) — both render a compact "· at <where>" on the result row.
-- CREATE OR REPLACE, unchanged signatures — no DROP, no grant change.
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object('fixture_id', cf.id, 'league_id', cf.league_id, 'league_name', cl.name, 'club_team_id', cf.club_team_id,
      'club_team_name', COALESCE(cf.club_team_name, ct.name), 'opponent_name', cf.opponent_name, 'is_home', cf.is_home,
      'scheduled_date', cf.scheduled_date, 'kickoff_time', to_char(cf.kickoff_time, 'HH24:MI'), 'home_score', cf.home_score, 'away_score', cf.away_score,
      'pitch_name', pa.name, 'venue_name', v.name, 'venue_address', NULLIF(concat_ws(', ', v.address, v.city, v.postcode), ''), 'location', cf.location, 'status', cf.status) AS row_obj
    FROM public.club_fixtures cf
    JOIN public.club_team_members ctm ON ctm.team_id = cf.club_team_id AND ctm.member_profile_id = p_child_profile_id AND ctm.is_active = true
    LEFT JOIN public.club_leagues cl ON cl.id = cf.league_id LEFT JOIN public.club_teams ct ON ct.id = cf.club_team_id
    LEFT JOIN public.playing_areas pa ON pa.id = cf.playing_area_id LEFT JOIN public.venues v ON v.id = pa.venue_id
...
BEGIN
  ...
  CROSS JOIN LATERAL (
    SELECT COALESCE(jsonb_agg(fx ORDER BY (fx->>'scheduled_date') DESC), '[]'::jsonb) AS recent FROM (
      SELECT jsonb_build_object('fixture_id', cf.id, 'opponent_name', cf.opponent_name, 'is_home', cf.is_home, 'scheduled_date', cf.scheduled_date, 'kickoff_time', to_char(cf.kickoff_time, 'HH24:MI'),
        'home_score', cf.home_score, 'away_score', cf.away_score, 'league_name', cl.name,
        'pitch_name', pa.name, 'venue_name', v.name, 'venue_address', NULLIF(concat_ws(', ', v.address, v.city, v.postcode), ''), 'location', cf.location, 'source', cf.source, 'status', cf.status) AS fx
      FROM club_fixtures cf LEFT JOIN club_leagues cl ON cl.id = cf.league_id LEFT JOIN playing_areas pa ON pa.id = cf.playing_area_id LEFT JOIN venues v ON v.id = pa.venue_id
      WHERE cf.club_team_id = ct.id AND cf.status = 'completed' ORDER BY cf.scheduled_date DESC NULLS LAST LIMIT 6
    ) rec_inner
  ) rc
