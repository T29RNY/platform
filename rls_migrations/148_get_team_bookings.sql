-- Migration 148 — Pitch Booking Stage 5 (booking-owned): casual read of own bookings.
-- Powers the in-app booking list, the live Requested->Confirmed badge, "recent
-- venues" discovery, and cancel. Auth via auth.uid() -> team_admins (caller must
-- administer p_team_id). Returns the team's bookings (recent + future) joined to
-- venue + pitch detail. authenticated-only.
