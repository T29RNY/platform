-- Down for migration 151. Restores the mig-142 trigger + whitelists, drops the four
-- new columns, restores the pre-151 status CHECK. (Will fail if any 'hold' bookings
-- or renewal series still exist — clear them first in that case.)

ALTER TABLE public.booking_series DROP COLUMN IF EXISTS hold_expires_at;
ALTER TABLE public.booking_series DROP COLUMN IF EXISTS renewal_of_series_id;
ALTER TABLE public.pitch_bookings DROP COLUMN IF EXISTS superseded_at;
...
ALTER TABLE public.pitch_bookings ADD CONSTRAINT pitch_bookings_status_check
  CHECK (status = ANY (ARRAY['requested','confirmed','declined','cancelled','superseded','expired']));

-- Restore tg_sync_fixture_occupancy without the superseded_at stamp (mig 142 body).
...
-- Restore whitelists without the two renewal reasons (mig 142 bodies).
