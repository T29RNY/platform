-- ════════════════════════════════════════════════════════════════════════════
-- 063 DOWN — restore pre-063 bodies (remove audit_events inserts)
-- ════════════════════════════════════════════════════════════════════════════
-- Reverts the 7 player self-write RPCs to their pre-063 bodies. Each loses
-- the audit_events INSERT added by 063. Safe to apply; no data loss. After
-- applying, silent client-side failures on these RPCs will no longer leave
-- a server-side trace.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SET    injured       = p_injured,
         ...
         status        = CASE WHEN p_injured AND status = 'in' THEN 'out' ELSE status END
...
BEGIN
  ...
  INSERT INTO players (
    id, name, token, type, disabled, priority, status, paid, owes,
    goals, motm, attended, total, bib_count, team, w, l, d,
    pay_count, late_dropouts, note, self_paid, is_guest, guest_of
  ) VALUES (
    v_guest_id, trim(p_guest_name), v_guest_token, 'regular',
    false, false, 'in', false, 0, 0, 0, 0, 0, 0, null, 0, 0, 0,
    0, 0, '', false, true, v_player_id
  );

  INSERT INTO team_players (team_id, player_id) VALUES (v_team_id, v_guest_id);
  ...
  INTO v_result FROM players p WHERE p.id = v_guest_id;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM players WHERE id = p_guest_id AND guest_of = v_player_id AND is_guest = true
  ) THEN
    RAISE EXCEPTION USING ERRCODE='P0001', MESSAGE='not_found';
  END IF;

  DELETE FROM team_players WHERE player_id = p_guest_id AND team_id = v_team_id;
...
BEGIN
  ...
  ON CONFLICT (player_id) DO UPDATE
    SET subscription = EXCLUDED.subscription, player_token = EXCLUDED.player_token
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM players WHERE user_id = v_user_id AND id != v_player_id
  ) THEN
