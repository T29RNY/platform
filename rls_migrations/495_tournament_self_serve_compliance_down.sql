-- 495_tournament_self_serve_compliance_down.sql
-- Reverse of 495: drop compliance RPCs + reports table + moderation columns,
-- revert the status CHECK (fails if any 'cancelled' rows exist — clear them first),
-- and restore get_tournament_public to the draft-only visibility gate.

DROP FUNCTION IF EXISTS public.self_serve_cancel_tournament(uuid);
DROP FUNCTION IF EXISTS public.tournament_report(text, text, text);
DROP FUNCTION IF EXISTS public.admin_hide_tournament(uuid, boolean, text);
DROP TABLE IF EXISTS public.tournament_reports;

ALTER TABLE public.tournament_events DROP COLUMN IF EXISTS hidden_at;
ALTER TABLE public.tournament_events DROP COLUMN IF EXISTS hidden_by;
ALTER TABLE public.tournament_events DROP COLUMN IF EXISTS hidden_reason;
...
ALTER TABLE public.tournament_events
  ADD CONSTRAINT tournament_events_status_check
    CHECK (status = ANY (ARRAY['draft','open','closed','live','completed']));
