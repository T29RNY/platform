-- 168_venue_get_state_display_fields.sql
-- League Mode Phase 4 (Reception Display), STAGE C — expose display_token + display_config
-- in venue_get_state so the operator's Reception Display settings editor (apps/venue) can
-- show the display URL and prefill the current panel config. ADDITIVE to the venue object
-- only (display_pin already returned). No consumer breaks (venue app reads state.venue
-- directly; no JS mapper). Body is mig-150's verbatim + two keys.
