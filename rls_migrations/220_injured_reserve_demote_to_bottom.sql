-- 220: Injured reserve auto-demotes to the bottom of the reserve queue.
--
-- Product rule (session 73): an injured player may still offer to play, so an
-- injured player CAN be a reserve — but the moment they go injured they drop to
-- last place in the reserve queue. Admins can re-order them afterward.
--
-- Bug fixed: set_player_injured / admin_set_player_injured only reset status
-- when status='in'. A reserve marked injured kept status='reserve' AND their old
-- queue position. The client optimistically moved them to 'out', so the screen
-- looked right until the next reload pulled the stale 'reserve' row back. The two
-- screens then disagreed (player view kept the injured player in the reserve
-- queue at their old position; admin filtered injured out of reserve entirely).
--
-- This migration leaves status='reserve' intact for an injured reserve (per the
-- new rule) and instead sets their reserve_priority_order to MAX(others)+1 so the
-- existing sort drops them to the bottom. The trigger manage_reserve_priority_order
-- only fires on status changes, so it does not interfere here.
...
BEGIN
  ...
  -- 220: an injured reserve drops to the bottom of the queue (admins can re-order)
  IF p_injured THEN
