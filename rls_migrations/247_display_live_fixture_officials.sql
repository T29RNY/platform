-- 247: live_fixtures[] rows gain round_name / official_id / official_name
-- (Reception Display follow-up). Mig 244 enriched upcoming_fixtures with the
-- round + ref, but the hero bar spec ("R12 · Pitch 1 · Ref Petersen",
-- HANDOVER §6.2) reads them off the FEATURED live fixture — live_fx never
-- carried them, so the hero title showed pitch only. Additive: one LEFT JOIN
-- match_officials + three keys in the live_fx object. Base body = mig 244
-- (applied byte-identical; md5-verified).
--
-- Consumers (hard-rule #14): apps/display Hero.jsx titleBits (already reads
-- f.round_name/f.official_name optimistically — no JS change needed);
-- Phase 6 HQ + Phase 7 Gaffer (future).
