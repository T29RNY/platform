-- 384_tournament_referees.sql
-- Tournament Hub — show who's refereeing each game (makes the public page look official).
-- The model already exists (fixtures.official_id -> match_officials; reception TV already
-- shows ref names). This:
--   1) seeds 3 demo match_officials and assigns them to the Finbar's FC Summer Cup fixtures
--      (Group A -> Sam Cooper, Group B -> Aisha Khan, knockout -> Tom Whitfield);
--   2) re-creates get_tournament_public to also return referee_name on each fixture and
--      knockout fixture (LEFT JOIN match_officials via official_id). Everything else
--      returned is byte-identical to mig 381. SECURITY DEFINER / pinned path / grants kept.

-- 1) Referees -------------------------------------------------------------------
INSERT INTO match_officials (id, venue_id, name, active) VALUES
  ('70000000-0000-4000-8000-000000000601', 'demo_venue', 'Sam Cooper',     true),
  ('70000000-0000-4000-8000-000000000602', 'demo_venue', 'Aisha Khan',     true),
  ('70000000-0000-4000-8000-000000000603', 'demo_venue', 'Tom Whitfield',  true)
ON CONFLICT (id) DO NOTHING;

UPDATE fixtures SET official_id = '70000000-0000-4000-8000-000000000601'  -- Group A: Sam Cooper
WHERE id IN ('70000000-0000-4000-8000-000000000301','70000000-0000-4000-8000-000000000302','70000000-0000-4000-8000-000000000303');
UPDATE fixtures SET official_id = '70000000-0000-4000-8000-000000000602'  -- Group B: Aisha Khan
WHERE id IN ('70000000-0000-4000-8000-000000000304','70000000-0000-4000-8000-000000000305','70000000-0000-4000-8000-000000000306');
UPDATE fixtures SET official_id = '70000000-0000-4000-8000-000000000603'  -- Knockout: Tom Whitfield
WHERE id IN ('70000000-0000-4000-8000-000000000401','70000000-0000-4000-8000-000000000402','70000000-0000-4000-8000-000000000403');

-- 2) get_tournament_public + referee_name ---------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'performance_events', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'results', COALESCE((
          ...
          ranked AS (
            SELECT *, CASE WHEN pe.measurement_type = 'time_asc'
          ...
          SELECT jsonb_agg(jsonb_build_object(
            'athlete_name', r.athlete_name, 'team_name', r.team_name,
            'value', r.best_value, 'rank', r.finish_rank
    ...
    'performance_standings', COALESCE((
      ...
      ranked_results AS (
        SELECT er.*, CASE WHEN er.measurement_type = 'time_asc'
      ...
      SELECT jsonb_agg(jsonb_build_object(
        'competition_team_id', tp.competition_team_id, 'team_name', tp.team_name,
        'points', tp.total_points, 'gold', tp.gold, 'silver', tp.silver,
        'bronze', tp.bronze, 'events_entered', tp.events_entered
