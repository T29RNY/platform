-- 568_manager_pitch_availability_own_session_identity.sql
-- Manager booking calendar — P2a: enrich club_manager_pitch_availability's busy blocks
-- with the team's OWN-session identity so the calendar can render the manager's own
-- sessions distinctly + tappable (edit/cancel), while other occupancy stays a generic
-- "in use" block (privacy — a manager never sees another team's booker detail).
-- ADDITIVE ONLY: existing consumers (windowFree in TeamManagerTraining + desktop
-- SessionsScreen availability) read only playing_area_id/start/end — untouched.
-- The CASE-guarded uuid cast avoids casting non-club_session source_ids (hires/maintenance).
