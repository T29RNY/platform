-- DOWN for mig 354 — remove club_id from get_member_pass return shape (restore the
-- pre-mig-354 body). Harmless to keep club_id, so this down is only for a clean revert.
