-- Down for mig 194 — drop knockout-from-groups + the extracted builder, and restore
-- venue_persist_cup_bracket to its mig-185 monolithic body (no _cup_build_bracket dependency).

DROP FUNCTION IF EXISTS public.venue_seed_knockout_from_groups(text, uuid, date, time, uuid[]);
...
AS $function$
DECLARE
  v_caller record; v_venue_id text; v_comp record; v_league_id text; v_lc_slot int;
  v_active text[]; v_ordered text[]; v_seed text; v_t text; v_n int; v_size int; v_rounds int;
  v_order int[]; v_next int[]; v_pos int; v_r int; v_slot int; v_slots int;
  v_posH int; v_posA int; v_teamH text; v_teamA text;
  v_pitch_n int; v_played_idx int := 0; v_pitch uuid; v_fixture_id uuid; v_tie_id uuid;
  v_round_name text; v_remaining int; v_tie_count int := 0; v_fx_count int := 0; v_bye_count int := 0;
BEGIN
  ...
  INTO v_comp FROM competitions c JOIN seasons s ON s.id=c.season_id JOIN leagues l ON l.id=s.league_id
  ...
  IF v_comp.type <> 'cup' OR v_comp.format <> 'single_elimination' THEN
    RAISE EXCEPTION 'not_single_elim_cup' USING ERRCODE='P0001', DETAIL = v_comp.type||'/'||COALESCE(v_comp.format,'null'); END IF;
  IF p_scheduled_date < v_comp.start_date OR p_scheduled_date > v_comp.end_date THEN
    RAISE EXCEPTION 'date_outside_season' USING ERRCODE='P0001', DETAIL = p_scheduled_date::text; END IF;
  IF EXISTS (SELECT 1 FROM fixtures WHERE competition_id=p_competition_id)
    ...
    RAISE EXCEPTION 'bracket_already_exists' USING ERRCODE='P0001'; END IF;

  v_league_id := v_comp.league_id;
  SELECT lc.slot_minutes INTO v_lc_slot FROM league_config lc WHERE lc.league_id = v_league_id;

  SELECT array_agg(team_id) INTO v_active FROM competition_teams WHERE competition_id=p_competition_id AND status='active';
  IF v_active IS NULL OR array_length(v_active,1) < 2 THEN RAISE EXCEPTION 'competition_has_too_few_active_teams' USING ERRCODE='P0001'; END IF;

  v_ordered := ARRAY[]::text[];
  IF p_seed_team_ids IS NOT NULL THEN
    FOREACH v_seed IN ARRAY p_seed_team_ids LOOP
      IF v_seed = ANY(v_active) AND NOT (v_seed = ANY(v_ordered)) THEN v_ordered := array_append(v_ordered, v_seed); END IF;
    END LOOP;
  END IF;
  ...
  v_rounds := 0; v_pos := v_size; WHILE v_pos > 1 LOOP v_rounds := v_rounds+1; v_pos := v_pos/2; END LOOP;
  ...
  FOR v_r IN 1..v_rounds LOOP
    ...
    v_round_name := CASE WHEN v_remaining=0 THEN 'Final' WHEN v_remaining=1 THEN 'Semi-final'
      WHEN v_remaining=2 THEN 'Quarter-final' ELSE 'Round of '||(2^(v_remaining+1))::int END;
  ...
  v_slots := v_size/2;
  v_round_name := CASE WHEN v_rounds-1=0 THEN 'Final' WHEN v_rounds-1=1 THEN 'Semi-final'
    WHEN v_rounds-1=2 THEN 'Quarter-final' ELSE 'Round of '||(2^((v_rounds-1)+1))::int END;
  FOR v_slot IN 0..(v_slots-1) LOOP
    v_posH := v_order[2*v_slot+1]; v_posA := v_order[2*v_slot+2];
    ...
    IF v_teamH IS NOT NULL AND v_teamA IS NOT NULL THEN
      IF v_pitch_n > 0 THEN v_pitch := p_playing_area_ids[(v_played_idx % v_pitch_n)+1]; ELSE v_pitch := NULL; END IF;
      INSERT INTO fixtures (competition_id, home_team_id, away_team_id, week_number, round_name,
                            scheduled_date, kickoff_time, playing_area_id, status)
      VALUES (p_competition_id, v_teamH, v_teamA, 1, v_round_name, p_scheduled_date, p_kickoff_time, v_pitch, 'scheduled')
      ...
      VALUES (p_competition_id, 1, v_slot, v_round_name, v_fixture_id, v_teamH, v_teamA, 'seed','seed','ready')
      RETURNING id INTO v_tie_id;
      UPDATE fixtures SET cup_tie_id = v_tie_id WHERE id = v_fixture_id;
      v_played_idx := v_played_idx+1; v_fx_count := v_fx_count+1;
    ELSE
      INSERT INTO cup_ties (competition_id, round_number, slot_index, round_name, home_team_id, away_team_id,
                            home_source, away_source, winner_team_id, status)
      VALUES (p_competition_id, 1, v_slot, v_round_name, COALESCE(v_teamH,v_teamA), NULL,
              CASE WHEN v_teamH IS NOT NULL THEN 'seed' ELSE 'bye' END, 'bye', COALESCE(v_teamH,v_teamA), 'decided');
  ...
  FOR v_r IN 2..v_rounds LOOP
    ...
    FOR v_slot IN 0..(v_slots-1) LOOP
      ...
      VALUES (p_competition_id, v_r, v_slot, v_round_name, 'winner','winner', 2*v_slot, 2*v_slot+1, 'pending');
      v_tie_count := v_tie_count+1;
    END LOOP;
  END LOOP;

  INSERT INTO venue_charges (venue_id, source_type, source_id, team_id, competition_id, amount_due_pence, status, due_date)
  SELECT v_venue_id, 'fixture', f.id::text, tm.team_id, f.competition_id, lc.fixture_fee_pence, 'unpaid', f.scheduled_date
  FROM fixtures f JOIN league_config lc ON lc.league_id = v_league_id
  CROSS JOIN LATERAL (SELECT f.home_team_id AS team_id UNION ALL SELECT f.away_team_id WHERE COALESCE(lc.fixture_fee_payer,'both')='both') tm
  WHERE f.competition_id = p_competition_id AND tm.team_id IS NOT NULL AND COALESCE(lc.fixture_fee_pence,0) > 0
  ...
  VALUES (v_venue_id, auth.uid(), v_caller.actor_type, v_caller.actor_ident, 'cup_bracket_generated', 'competition', p_competition_id::text,
          jsonb_build_object('league_id', v_league_id, 'season_id', v_comp.season_id, 'teams', v_n, 'bracket_size', v_size,
            'rounds', v_rounds, 'ties', v_tie_count, 'round1_fixtures', v_fx_count, 'byes', v_bye_count));
  ...
  RETURN jsonb_build_object('ok', true, 'competition_id', p_competition_id, 'teams', v_n, 'bracket_size', v_size,
    'rounds', v_rounds, 'ties', v_tie_count, 'round1_fixtures', v_fx_count, 'byes', v_bye_count);
END;
$function$;

DROP FUNCTION IF EXISTS public._cup_build_bracket(uuid, text[], date, time, uuid[], text, text);

-- restore get_cup_bracket to its mig-188 body (without groups/knockout_seeded)
...
AS $function$
DECLARE
  v_comp record; v_champion jsonb; v_max_round int; v_result jsonb;
BEGIN
  IF p_competition_id IS NULL THEN RAISE EXCEPTION 'competition_id_required' USING ERRCODE='P0001'; END IF;
  ...
  SELECT jsonb_build_object(
    ...
    'rounds', COALESCE((
      ...
      FROM (
        SELECT jsonb_build_object('round_number', ct.round_number, 'round_name', max(ct.round_name),
        ...
        WHERE ct.competition_id = p_competition_id GROUP BY ct.round_number
      ) sub
    ), '[]'::jsonb)
  ) INTO v_result;
  RETURN v_result;
