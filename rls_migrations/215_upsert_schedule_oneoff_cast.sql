-- 215: fix the text→timestamp cast in admin_upsert_schedule's one-off-date branch.
--
-- Session-71 audit: the "UPDATE THIS WEEK" one-off date override never worked — two
-- bugs. (2) here: `(p_one_off_date || 'T' || p_kickoff || ':00') AT TIME ZONE
-- 'Europe/London'` raises `function timezone(unknown, text) does not exist` because a
-- text concatenation is not implicitly cast to timestamp (same issue caught in mig 212
-- create_team). It never fired only because bug (1) — the ScheduleScreen UI never sent
-- p_one_off_date — kept this branch unreached. The matching ScheduleScreen fix (send
-- oneOffDate) lands in the same commit, so this branch now executes.
--
-- Only change vs mig 210: add `::timestamp` to the one-off game_date_time construction.
-- Body otherwise re-applied byte-for-byte (resolver auth + mig-210 anon grant retained).
...
BEGIN
  ...
  IF p_one_off_date IS NOT NULL AND p_one_off_date <> '' THEN
    -- Europe/London wall-clock (DST-aware). Explicit ::timestamp — text→timestamp is
    -- not implicit; without it: timezone(unknown,text) error (mig 215 / mig 212 class).
    v_game_dt := ((p_one_off_date || 'T' || p_kickoff || ':00')::timestamp) AT TIME ZONE 'Europe/London';
  ELSE
