-- Down for migration 150. Drops the new settings RPC and reverts venue_get_state +
-- get_pitch_occupancy to their pre-150 bodies (mig 135 / mig 141 respectively).

DROP FUNCTION IF EXISTS public.venue_update_booking_settings(text, jsonb);

-- Revert get_pitch_occupancy to mig 141 (no series_id in booking detail).
...
-- Revert venue_get_state to mig 135 (no bookings_enabled / cancellation_policy in venue projection).
