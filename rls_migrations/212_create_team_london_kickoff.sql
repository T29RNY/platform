-- 212: create_team builds the first game_date_time in Europe/London, not UTC.
--
-- Session-71 audit: create_team computed the first kickoff from bare now()
-- (UTC on the Supabase server) — date_trunc('day', now()) + kickoff gave e.g.
-- 20:00 UTC = 21:00 BST in summer, so a brand-new squad's first-week cron
-- reminders (lineup lock, 1hr-before, POTM) fired 1hr late until the first
-- advanceGameDateJob/upsert overwrote the date. Same class as mig 207, on the
-- create path. The day-of-week / "already past kickoff today" tests also used
-- UTC, which could pick the wrong week between 00:00–01:00 BST.
--
-- Fix: derive day-of-week, the past-kickoff test, and the kickoff timestamp from
-- now() AT TIME ZONE 'Europe/London' (DST-aware), composing the final instant via
-- AT TIME ZONE 'Europe/London' exactly like admin_upsert_schedule (mig 207).
--
-- Also adds `SET search_path TO 'public', 'pg_temp'` — create_team was SECURITY
-- DEFINER with no search_path set (search-path-injection gap). All table/function
-- refs are unqualified public objects (or schema-qualified auth.uid()), so this is
-- behaviour-preserving. Only the three v_days_ahead/v_game_dt lines + the function
-- header change; the rest is re-applied byte-for-byte.
...
BEGIN
  ...
  -- Day-of-week, the "already past kickoff today" test, and the kickoff instant
  -- are all evaluated in Europe/London (DST-aware) so the first game_date_time
  -- matches the wall-clock kickoff the operator entered (mig 212; was bare UTC now()).
  ...
  v_game_dt := (
    (((date_trunc('day', now() AT TIME ZONE 'Europe/London') + (v_days_ahead * interval '1 day'))::date)::text
     || 'T' || p_kickoff || ':00')::timestamp
