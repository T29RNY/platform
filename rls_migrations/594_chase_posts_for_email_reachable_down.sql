-- 594 DOWN: revert admin_chase_payment to push-only hand-off.
--
-- ⛔ DO NOT "just re-run 591". 591 is a single file that ALSO creates platform_config and
-- _team_debtors — and its _team_debtors predates mig 592's waiver fix. Re-running it would
-- silently restore the version that resurrects forgiven debt, which is the worst bug this
-- epic produced. 594 touched ONE function; the down touches that same one function only.
--
-- This reverts exactly two things:
--   • the hand-off gate: (r.has_push OR r.has_email) → r.has_push
--     i.e. stop posting for email-reachable debtors, so notify.js's email leg goes dark.
--   • drops 'chaseAmount' from the POST body (notify.js's email builder needs it; nothing
--     else reads it, and the push copy carries its own formatted string).
-- Everything else — the audience via _team_debtors, the 24h per-recipient cooldown, the
-- synchronous notification_log bookkeeping, the app.in-or-out.com host, the audit row — is
-- unchanged from 594 and stays correct.
--
-- Reverting leaves the notify.js email code in place but starved of input: no POST is made
-- for a push-less debtor, so no email is attempted. That is the pre-594 behaviour (the
-- debtor is silently missed), not a crash.
...
BEGIN
  ...
  IF NOT p_dry_run THEN
    ...
    LOOP
      ...
      CONTINUE WHEN NOT r.has_push;  -- reverted: push is the only transport again
