-- ════════════════════════════════════════════════════════════════════════════
-- 079 — Restore group_number and group_labels in get_team_state_by_admin_token
-- ════════════════════════════════════════════════════════════════════════════
-- Bug: Players assigned to groups (1–5) via admin_set_player_group reset to
-- "Needs Group" every time the admin exits and returns to TeamsScreen.
-- group_labels saved via saveGroupLabels also disappear on reload.
--
-- Root cause: migration 031 added two fields to get_team_state_by_admin_token:
--   1. 'group_number', tp.group_number   (in the squad jsonb_build_object)
--   2. 'group_labels', s.group_labels    (in the settings jsonb_build_object)
-- Migrations 038, 061, and 070 each did CREATE OR REPLACE without carrying
-- those fields forward. The data is correctly written to team_players.group_number
-- and settings.group_labels — only the read path was broken.
--
-- Fix: full CREATE OR REPLACE of migration 070's body with both fields restored.
-- No schema changes, no signature changes, no grant changes.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        -- 031: group assignment (1–5 or null). Restored after being dropped in 038/061/070.
        'group_number',    tp.group_number,
        'disabled',        p.disabled,
        'disable_reason',  p.disable_reason,
        'admin_locked_in', p.admin_locked_in,
        'team',            p.team,
        -- 070: token now exposed on every row (enables admin share UX).
        'token',           p.token,
        -- 070: explicit flag for the admin's own player row.
  ...
  -- 031: group_labels restored after being dropped in 038/061/070.
