-- ════════════════════════════════════════════════════════════════════════════
-- 079 — Restore group_number + group_labels in get_team_state_by_admin_token
-- ════════════════════════════════════════════════════════════════════════════
-- Source-of-truth repair. The body of this function was applied directly
-- to the live database on 2026-05-26 12:38 UTC via a cloud Claude Code
-- session (no filesystem access — could not write this file at the time).
-- This file recreates the live function verbatim so the rls_migrations/
-- directory matches what's actually deployed.
--
-- Background:
-- Migration 070 (admin_squad_tokens_for_all) rewrote
-- get_team_state_by_admin_token to add token + is_self on every squad row,
-- but accidentally dropped tp.group_number from the squad payload and
-- group_labels from settings — which had been present since migration 031.
-- Result: from 2026-05-14 until 2026-05-26, every admin who tapped a
-- player into a Group Balancer group saw the assignment reset on the
-- next reopen, because the squad row that came back from the read RPC
-- had no group_number. The data was persisted correctly server-side —
-- only the read path was broken.
--
-- This migration restores both fields. No grant or signature change.
-- ════════════════════════════════════════════════════════════════════════════
