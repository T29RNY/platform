-- ════════════════════════════════════════════════════════════
-- Migration 250 — venue_get_state v_pending enrichment (QR slice 3b)
-- The approval card never received a real self-serve registration until
-- slice 3a turned the captain form on. As built it shows only team name —
-- too thin to judge a stranger's request. Enrich v_pending with the
-- competition name + captain (admin) email; registered_at already present.
-- to_jsonb(p) in pending_registrations auto-includes the new columns —
-- this is the ONLY change vs the live mig-227 body. Pulled the live body
-- before REPLACE (feedback_verify_before_commit). Plan: QR_ONBOARDING_SCOPE.md.
-- ════════════════════════════════════════════════════════════
