-- 109_phase2_venue_assign_pitch_maintenance_check.sql
--
-- Phase 2 (League Mode) — Cycle 2.6 maintenance-window enforcement
-- on venue_assign_pitch. Carries the deferral noted in mig 094's
-- header comment.
--
-- Behaviour change vs mig 094:
--   - When p_playing_area_id is non-NULL, also check that the fixture's
--     scheduled_date does NOT fall within any maintenance window of
--     the pitch. Dates inclusive on both ends.
--
-- maintenance_windows shape (set via mig 105/106):
--   [{"start_date":"YYYY-MM-DD","end_date":"YYYY-MM-DD","reason":"..."}]
--
-- Error raised: 'pitch_in_maintenance' with DETAIL = the offending
-- window's start_date..end_date string.
--
-- If the fixture has no scheduled_date set (NULL), the maintenance
-- check is skipped — the operator can assign a pitch before
-- scheduling the date; the check fires on whichever leg comes second
-- (this RPC, or whatever sets scheduled_date — a Phase 3 RPC).
...
BEGIN
  ...
  IF p_playing_area_id IS NOT NULL THEN
    ...
    -- Maintenance window overlap check (only meaningful when fixture
    -- has a date set; otherwise skip — the check will fire later).
