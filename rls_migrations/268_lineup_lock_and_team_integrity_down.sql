-- 268_lineup_lock_and_team_integrity_down.sql
-- Reverts migration 268: restores the pre-268 live bodies verbatim and drops the
-- is_lineup_locked helper. Note: the pre-268 bodies carry the SESSION 80 bug.

DROP FUNCTION IF EXISTS public.is_lineup_locked(text);

-- ── set_player_injured (pre-268: no lock, no un-injure restore) ──────────────
...
-- ── admin_set_player_injured (pre-268: no un-injure restore) ─────────────────
...
-- ── set_player_status (pre-268: no post-kickoff lock) ────────────────────────
...
-- ── add_guest_player (pre-268: no lock) ──────────────────────────────────────
...
-- ── remove_guest_player (pre-268: no lock) ───────────────────────────────────
...
-- ── admin_save_match_result (pre-268: no reconciliation pass) ────────────────
-- Restored from the mig 241 live body. (Identical to the 268 body minus the two
-- Fix-3 reconciliation UPDATEs.) Re-applying 268 re-adds them.
