-- Down for migration 176 — restore the pre-176 function bodies and drop the column.
-- Restores venue_update_pitch (mig 135 body) and venue_get_state (mig 168 body)
-- verbatim, then drops playing_areas.prime_time_windows. Run functions first so
-- nothing references the column at drop time.

-- 1. venue_update_pitch — mig 135 body (no prime_time_windows block)
...
-- 2. venue_get_state — mig 168 body (no prime_time_windows key)
...
-- 3. Drop the column (functions above no longer reference it)
ALTER TABLE playing_areas DROP COLUMN IF EXISTS prime_time_windows;
