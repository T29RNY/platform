-- 546 DOWN: restore get_my_money to its pre-546 body (drop `stripe_available` +
-- `manual_pay_url` from both charge builders). Removing additive keys is backward-safe —
-- BookPaySheet reads them defensively (card gates on truthy stripe_available, bank on
-- truthy manual_pay_url; cash always shows).
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'membership_id', m.id, 'member_profile_id', m.member_profile_id,
    ...
    'period',        m.period, 'amount_pence', m.amount_pence, 'status', m.status, 'renews_at', m.renews_at,
  ...
  WITH mine AS (
    ...
    WHERE v_profile IS NOT NULL
      AND ( vm.member_profile_id = v_profile OR vm.payer_profile_id = v_profile
            OR vm.member_profile_id IN (SELECT mg.child_profile_id FROM public.member_guardians mg WHERE mg.guardian_profile_id = v_profile) )
      AND vm.status <> 'cancelled'
  ),
  ch AS (
    SELECT vc.id, vc.amount_due_pence, vc.status, vc.due_date, vc.created_at, vc.billing_run_id, vc.venue_id, vc.pay_url,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'charge_id', ch.id, 'stream', 'membership', 'member_profile_id', ch.member_profile_id,
    'who_for',   CASE WHEN ch.member_profile_id = v_profile THEN 'self' ELSE NULLIF(btrim(COALESCE(mp.first_name,'') || ' ' || COALESCE(mp.last_name,'')), '') END,
    'is_self',   (ch.member_profile_id = v_profile), 'label', COALESCE(br.label, c.name, 'Membership'),
    'amount_due_pence', ch.amount_due_pence, 'paid_pence', ch.paid_pence, 'status', ch.status, 'due_date', ch.due_date,
  ...
  WITH my_profiles AS (
    ...
    UNION
    SELECT mg.child_profile_id FROM public.member_guardians mg WHERE mg.guardian_profile_id = v_profile
  ),
  clsch AS (
    SELECT vc.id, vc.amount_due_pence, vc.status, vc.due_date, vc.created_at, vc.venue_id, vc.pay_url, b.member_profile_id,
    ...
    WHERE vc.source_type = 'class' AND vc.status <> 'refunded'
      AND b.member_profile_id IN (SELECT pid FROM my_profiles WHERE pid IS NOT NULL)
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'charge_id', clsch.id, 'stream', 'class', 'member_profile_id', clsch.member_profile_id,
    'who_for',   CASE WHEN clsch.member_profile_id = v_profile THEN 'self' ELSE NULLIF(btrim(COALESCE(mp.first_name,'') || ' ' || COALESCE(mp.last_name,'')), '') END,
    'is_self',   (clsch.member_profile_id = v_profile), 'label', COALESCE(clsch.class_name, 'Class'),
    'amount_due_pence', clsch.amount_due_pence, 'paid_pence', clsch.paid_pence, 'status', clsch.status, 'due_date', clsch.due_date,
  ...
  SELECT
    COALESCE(SUM(CASE WHEN (e->>'status') IN ('unpaid','partial') THEN (e->>'amount_due_pence')::int - (e->>'paid_pence')::int ELSE 0 END), 0),
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'paid_by', pl.paid_by, 'paid_at', pl.paid_at, 'note', pl.note, 'created_at', pl.created_at, 'updated_at', pl.updated_at
    ) ORDER BY pl.created_at DESC), '[]'::jsonb)
  INTO v_casual
  FROM public.payment_ledger pl
  WHERE v_person IS NOT NULL
    AND pl.player_id IN (SELECT p.id FROM public.players p WHERE p.person_id = v_person AND COALESCE(p.disabled, false) = false);

  RETURN jsonb_build_object(
    'ok', true, 'person_id', v_person, 'profile_id', v_profile,
