-- ════════════════════════════════════════════════════════════════════════════
-- 080 — get_team_state_by_player_token: full admin parity for VCs + admins
-- ════════════════════════════════════════════════════════════════════════════
-- Surfaced by: Footy Tuesdays game day (2026-05-26). A vice-captain
-- (player_token route) tapped 17 group_assigned events against the same
-- few players over and over because the UI never reflected the persisted
-- group. Server-side: every write succeeded. Client-side: the squad
-- payload returned to the VC's session was missing tp.group_number, so
-- localGroups never picked up the saved state on reload.
--
-- Investigation showed the same payload is also missing every payment
-- field (paid, owes, self_paid, paid_by, pay_count), every stats field
-- (goals, motm, attended, total, w, l, d, late_dropouts), and
-- admin_locked_in — plus the caller themselves is excluded from squad.
-- For ordinary players this was a deliberate privacy choice in mig 071
-- ("squad — no financial/stats"). For VCs and team admins using
-- AdminView via /p/<token>, every admin feature that reads those fields
-- silently breaks on the display side.
--
-- Decision: when v_privileged (VC or team admin) is true, return the
-- same squad shape get_team_state_by_admin_token returns — including the
-- caller's own row with is_self=true. For ordinary players, the existing
-- limited shape is unchanged.
--
-- Also adds group_labels to v_settings unconditionally — labels are not
-- privacy-sensitive and any client that doesn't read them just ignores
-- the field.
--
-- Function signature unchanged. Grants unchanged. SECURITY DEFINER
-- preserved. Schema cache notify at end.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Squad: privileged callers (VCs, team admins) get the same shape that
  -- get_team_state_by_admin_token returns, including the caller's own row
  -- with is_self=true. Ordinary players get the existing limited shape
  -- with the caller excluded (their own row lives in v_player).
