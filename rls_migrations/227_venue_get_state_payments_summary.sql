-- Migration 227: add payments_summary to venue_get_state
--
-- The Operations landing "Outstanding" stat (Dashboard.jsx OpsStatBar) reads
-- state.payments_summary.outstanding_pence, but venue_get_state never built that
-- key → the card always rendered "—" even though the Payments screen had the
-- real figure. Add a v_charges CTE mirroring venue_get_charges EXACTLY:
--   paid_pence = sum of non-voided venue_payments (payment minus refund);
--   owed/collected/outstanding exclude 'refunded' charges.
-- so the Ops card agrees with the Payments screen byte-for-byte.
--
-- Read-only RPC. CREATE OR REPLACE preserves existing grants (anon +
-- authenticated). No JS change required — the venueGetState wrapper already
-- returns the whole blob and the client already reads payments_summary.
