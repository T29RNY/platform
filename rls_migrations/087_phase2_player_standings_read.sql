-- 087_phase2_player_standings_read.sql
--
-- Phase 2 (League Mode) — Cycle 2.2 player-facing standings RPC.
--
--   get_league_standings_for_player(p_token)
--     For every competition the player's teams are registered in
--     (status='active' or 'withdrawn' so finished history surfaces too),
--     compute W/D/L/GF/GA/GD/Pts from completed + walkover fixtures.
--
--     Walkover scoring: if home_score/away_score not set, defaults to
--     3-0 in favour of walkover_winner_id. Once the score-entry flow
--     (Cycle 2.4) lets admins enter explicit walkover scores, those
--     override the 3-0 default automatically.
--
--     Postponed / voided / forfeit fixtures are excluded from
--     standings until the cascade rules land (Cycle 2.5b).
--
--     Top scorers are stubbed to [] — match_events feed lands in
--     Phase 3 ref view. Shape is in place; data fills in later.
--
--     standings_visibility='private' on a league yields an empty
--     standings array for that league's competitions (Round 4 v1
--     ships public-only; this is the forward-compatible gate).
