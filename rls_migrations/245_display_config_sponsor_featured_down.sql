-- 245 DOWN: restore venue_update_display_config to its pre-245 LIVE body —
-- mig 167 logic + the mig-239 manage_facility capability guard (which the 167
-- source file does not contain; this body was pulled from pg_get_functiondef
-- before mig 245 was applied). Drops sponsor/featured-pin validation; the
-- whole-config persist still stores any extra keys unvalidated.
