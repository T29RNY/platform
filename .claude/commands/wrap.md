...
3. **Give the paste-ready next-session kickoff prompt as a slash command** in a fenced code block — a single line, **always starting with `/`**, that the operator can paste into a fresh session to resume exactly where we left off. Use the `/loop /dev-loop <MANIFEST>.md` trigger for a multi-phase epic, or `/dev-loop <specific change>` when the next step is a single narrower change. It must be a runnable slash command — never a prose instruction. Note next free migration number if the next PR adds one.
