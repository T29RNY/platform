...
- 📥 **Scheduled/cloud nightly QA cannot run the e2e regression net against live-demo data** — effort ~S (env config) · the 2026-07-07 nightly QA cloud session hit two harness blockers: the environment's network egress allowlist excludes the demo Supabase project host (`ktvpzpnqbwhooiaqrigm.supabase.co`, `403 Host not in allowlist`), and the pre-installed Playwright browser revision doesn't match this project's pinned `@playwright/test` version. Net effect: an unattended/scheduled `/qa-loop scripted` run in this kind of environment only ever exercises syntax + hygiene + build — it cannot catch a real runtime regression the way a laptop session can. Device-level check once fixed: re-run `/qa-loop scripted` in the same environment type and confirm all 9 Playwright projects actually execute (pass/fail/flake, not blocked) · source: qa-loop:2026-07-07 · captured 2026-07-07 · _open_
- 📥 **`Skills/` vs `skills/` directory-casing mismatch breaks the deterministic check scripts (and the hook-enforced gates that call them) on any case-sensitive checkout** — effort ~S · the real directory is capitalized `Skills/` but `check-build.sh`, `.claude/hooks/post-edit-hygiene.sh`, `.claude/hooks/pre-commit-build.sh`, and `.claude/settings.json`'s allowlist all reference lowercase `skills/scripts/...`. This resolves fine by accident on the operator's Mac (case-insensitive filesystem) but fails with "No such file or directory" on case-sensitive Linux — meaning the hooks CLAUDE.md relies on to make hygiene/build enforcement "deterministic, not advisory" may silently fail to run at all in a cloud/Linux session, with no visible error surfaced to the operator. Needs a deliberate one-time fix (likely `git mv Skills skills`, done as its own reviewed commit — a symlink workaround was tried and reverted this session as unsafe on a case-insensitive filesystem). Device-level check once fixed: run `bash skills/scripts/check-build.sh` from a fresh case-sensitive Linux checkout and confirm it doesn't error before attempting the build · source: qa-loop:2026-07-07 · captured 2026-07-07 · _open_
