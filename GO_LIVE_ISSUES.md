...
- ✅ **club-media image upload 403'd for EVERY user (mig 619)** — bug 🔒 · **RESOLVED 2026-07-24 (mig 619, applied live + prod-verified).** The three `club-media` storage RLS policies (mig 444) ran an EXISTS subquery over `member_profiles`/`club_team_managers`/`club_teams` AS the `authenticated` caller, but those base tables are RLS-deny-all with no SELECT grant to `authenticated` → "permission denied" → every club crest/hero/sponsor upload/update/delete 403'd. **Direct sibling of the mig-572 `_can_read_member_id_object` fix.** Moved the membership check into SECDEF helper `_user_can_manage_club_media(name)` (reads with definer rights; `auth.uid()` still resolves to the caller); the 3 policies call it, predicate byte-identical to mig 444. Verified on prod: security props clean (secdef/search_path/1 overload/authenticated-only), all 3 policies wired, authorization correct (real manager=t / random=f / anon=f). **Pre-flight check (device):** sign in as a club manager → open the club page editor → upload a crest → expect it saves (HTTP 200), NOT 403. ⛔ this real-upload smoke on a live manager account is still owed (the fix's authorization was proven directly on prod, but an end-to-end upload through the app is the confirming walk).
...
- 📥 **Stripe LIVE payment-type test runs — go-live proof checklist (one live txn per type before onboarding paying clubs)** — go-live 💳 · effort S each · Expands the "one live proof owed" item above: all CODE is built + sandbox-proven and the live plumbing (keys/webhook/Connect/ledger) is wired on live acct `acct_1TihcaIAelF0ohDl`; these are the operator's per-type live-account confirmations. Run one live transaction of each type the first clubs will use, then confirm the ledger. **CORE money-in:** (1) **Connect onboarding** — a real venue connects → `venue_integrations` status→'connected' + billing UI shows connected; (2) **Recurring membership** (monthly/quarterly/annual) — member signs up+pays → checkout.session.completed + invoice.paid → `venue_charges` paid + a `venue_payments` method='stripe' row, membership active; (3) **One-off / season membership** (checkout mode=payment) → `stripe_record_season_payment` writes the ledger; (4) **Season-schedule membership** (auto-cancels at season end) → Stripe subscription schedule created + `stripe_set_membership_schedule` persisted; (5) **Bulk / mass invoicing** — invoice a cohort → each Invoice → invoice.paid → `stripe_record_charge_payment` reconciles the pre-existing charge (metadata.iorout_charge_id); (6) **Refund** — refund a paid charge → charge.refunded → `venue_payments` kind='refund' + charge flips to refunded/partially_refunded (the exact gap PR #552 hardened). **LIFECYCLE:** (7) **Failed renewal** (invoice.payment_failed, decline-on-renewal test card) → membership past_due; (8) **Cancellation** (customer.subscription.deleted/updated) → `apply_membership_subscription_status` → cancelled; (9) **Connect deauthorize** (account.application.deauthorized) — THROWAWAY connected account ONLY (destructive — never a venue you want to keep) → status→'disconnected' + `disconnected_at` + a `venue_stripe_disconnected` audit row + drops out of the 04:00 reconciliation loop (PR #552 / mig 579). **SUPPORTING:** (10) **Billing portal** (/api/stripe-billing-portal) — opens + a card update takes effect; (11) **Guardian-pays-for-child** (payer-on-behalf) — `payer_profile_id` recorded + child's membership active + charge shows in the guardian's `get_my_money`; (12) **Drop-in / class / PT online** (only if used at launch) — the charge records to the ledger; (13) **04:00 reconciliation cron** — once a few live subs exist, `membershipReconciliationJob` runs clean (idempotent invoice + refund repair). · source: go-live-check Stripe payments 2026-07-16 · captured 2026-07-16 · _open (live test runs owed)_
