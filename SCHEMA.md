...
- `venues` — text PK. company_id (nullable — independent venues allowed). venue_admin_token. display_pin. Stripe columns. sport DEFAULT 'football'. **Phase 4 (mig 164):** `display_token text NOT NULL DEFAULT gen_random_uuid()::text` (UNIQUE — per-venue READ-ONLY public token for the reception big-screen `/display/TOKEN`; NOT the venue_admin_token) + `display_config jsonb` (panel/layout config: `{zones[],mode,interval_secs,custom_message}`; NULL = app default). White-label `logo_url`/`primary_colour`/`secondary_colour` already existed. **Self-Serve PR3 (mig 484):** `verification_status text NOT NULL DEFAULT 'verified'` (CHECK verified|pending|rejected — self-serve venues written 'pending'; existing rows default 'verified' so nothing flips) + `origin text NOT NULL DEFAULT 'superadmin'` (CHECK superadmin|self_serve) + `created_by_user uuid → auth.users` (the self-serve creator; NULL on existing rows). Written only by `self_serve_create_venue`; every existing venue byte-identical.
...
- `ref_assignment_responses` (mig 442, Referee PR #3) — a ref's accept/decline of a specific assignment. `id uuid PK`, `context text CHECK(league|casual)`, `game_id text` (league = `fixtures.id::text`, casual = `matches.id`), `person_id uuid FK→people ON DELETE CASCADE`, `response text CHECK(accepted|declined)`, `note text NULL`, `responded_at timestamptz DEFAULT now()`, `UNIQUE(context,game_id,person_id)`. RLS on, NO client policies (RPC-gated). Absent row = pending; a re-assignment to a different ref naturally reads pending (no row for the new person — so NO reset logic on the assign RPCs, which stay untouched). Written by `ref_respond_to_assignment`; read by `get_my_ref_status` (ref) + `venue_get_ref_responses` (operator, league only).
- `ref_unavailability` (mig 442, Referee PR #3) — blackout date ranges a ref can't officiate; venues assign around them. `id uuid PK`, `person_id uuid FK→people ON DELETE CASCADE`, `start_date date`, `end_date date` (`CHECK end_date≥start_date`), `note text NULL`, `created_at timestamptz`. RLS on, NO client policies (RPC-gated). Written by `ref_add_unavailability`/`ref_remove_unavailability` (own rows only); read by `get_my_ref_status` + `venue_get_ref_responses` (the operator's assign UI flags a clashing official).
...
- `club_features` (mig 399, Venue OS nav Phase 1) — modular feature switches per CLUB (org-owned, follow the club to every venue it operates from). `club_id text PK FK→clubs ON DELETE CASCADE` + boolean cols `memberships`/`competition`/**`club_leagues` (mig 415)**/`coaching`/`tournaments`/`public_web`, all `NOT NULL DEFAULT true`, `updated_at`. RLS on, NO client policies. No row = all-on. The venue rail shows a club feature if ANY club at the venue has it on (OR via `club_venues`). **mig 415** split the old single `competition` switch in two: `competition` now governs the **Internal League** surface (registered teams + standings + Teams/Players rosters), and the NEW `club_leagues` governs the **Club Leagues** surface (grassroots `club_leagues`/`club_fixtures` external-opponent fixtures + matchday links). The 5 club-league write RPCs gate on `club_leagues`; `_club_feature_enabled`/`get_venue_feature_flags`/`venue_get_feature_settings`/`venue_set_club_feature(s)` all handle it.
...
- `incidents` — venue_id, fixture_id (nullable), reported_by (auth.users), description, severity (info/warning/critical), resolved_at/by/note, `outcome` (mig 437; nullable, CHECK ∈ fixed/safe/contractor/nofault — structured resolution from the mobile Operations screen; NULL for free-text-only/legacy resolutions). **Triage layer (mig 461):** `category` (nullable, CHECK ∈ facility/equipment/safety/medical/conduct/security/weather/safeguarding/other), `priority` (NOT NULL DEFAULT 'normal', CHECK ∈ low/normal/high/urgent), `assigned_to` (uuid, validated in-RPC vs venue_admins — no FK), `acknowledged_at`, `escalated_at`, `escalated_by`, `escalation_reason` (all nullable). Lifecycle stays timestamp-derived (open = `resolved_at IS NULL`); no status enum. Partial indexes `idx_incidents_queue` (venue triage) + `idx_incidents_escalation_inbox` (HQ). `delete_my_account*` NULL reported_by/resolved_by/assigned_to on account deletion (GDPR, mig 461). **Safeguarding module schema (mig 466, PR #1 — Phase 2 of Incident Triage, `SAFEGUARDING_MODULE_HANDOFF.md`):** `is_safeguarding_flagged` (NOT NULL DEFAULT false — the SOLE source of visibility truth, orthogonal to `category`), `safeguarding_flagged_at`, `safeguarding_flagged_by` (nullable). Partial index `idx_incidents_safeguarding` (venue_id, created_at) for the Lead-only list. `idx_incidents_queue`/`idx_incidents_escalation_inbox` both extended with `AND is_safeguarding_flagged IS NOT TRUE` to stay index-only once PR #3 adds the exclusion predicate to the reads. `venue_admins_caps_known` CHECK extended with `'safeguarding_lead'` (grant-only cap — deliberately NOT wired into `_venue_has_cap`'s owner/manager default-pass; see the dedicated `_venue_is_safeguarding_lead` helper, PR #2). GDPR carve-out (Art. 17(3)(b)): `safeguarding_flagged_by` is deliberately EXCLUDED from the `delete_my_account*` NULL-cascade — flagged records survive account deletion.
...
- `venue_charges` — what's owed: `id`, `venue_id`→venues (denormalised), `source_type`
  ...
  `billing_run_id`→venue_billing_runs NULL (mig 405), `pay_url` text NULL (mig 408 — Stripe hosted-invoice URL).
...
- **Holiday Camps — `venue_class_types.is_camp` flavour (mig 534, P9.1, additive)** — a camp is an `is_camp` variant of a class type (mirrors `is_sparring`/`members_only`), reusing the whole class engine (sessions → bookings → `venue_charges source_type='class'` → `get_my_money` → pay path). All columns safe-defaulted so existing classes/gym/football rows are byte-identical. New on `venue_class_types`: `is_camp bool NOT NULL DEFAULT false`, `camp_info text`, `camp_dietary text`, `pickup_time`/`dropoff_time time`, `pickup_location`/`dropoff_location text`, `booking_mode text NOT NULL DEFAULT 'per_day'` CHECK(per_day|block), `audience text NOT NULL DEFAULT 'all'` CHECK(all|team), `target_team_id uuid FK→club_teams ON DELETE SET NULL`; integrity CHECK `audience<>'all' OR target_team_id IS NULL` (an 'all' camp must not name a team). The reverse — team ⟹ target present — is enforced at WRITE time in the create RPC, NOT as a biconditional CHECK, so the FK's ON DELETE SET NULL can leave a team camp with NULL target when its team is deleted (a valid "hidden — target gone" state; the guardian reader's EXISTS-active-membership then matches no child) without aborting the team/club delete. New on `venue_class_sessions`: `end_date date NULL` (NULL = single-day; set = a `block` camp spanning `starts_at::date .. end_date`; CHECK `end_date IS NULL OR end_date >= starts_at::date` — orthogonal to the existing `ends_at > starts_at` time-of-day CHECK). RPCs/UI arrive in P9.2–P9.5; the schema alone is dark.
...
- `venue_team_contacts` (mig 411, Venue People & Spaces IA Phase 4) — a team's settable **contacts**: a **primary ("main") and a secondary** slot, each pointing at an existing person (never free text). Polymorphic on BOTH axes: `id uuid PK`, `venue_id text FK→venues ON DELETE CASCADE`, `team_kind text CHECK (league|club)`, `team_id text` (league = `teams.id`; club = `club_teams.id::text`), `contact_rank text CHECK (primary|secondary)`, `contact_kind text CHECK (customer|member)`, `contact_id uuid` (polymorphic, **no FK** — customer → `venue_customers.id`; member → `member_profiles.id`), `created_at`/`updated_at`. **UNIQUE(venue_id, team_kind, team_id, contact_rank)** = one person per slot (clear = row delete). The contact source is fixed by team kind: **league → a `venue_customers` person**; **club → an active manager/assistant/coach of that team** (`club_team_managers`). RLS on, no policies → all access via SECDEF RPCs. Index on `(contact_kind, contact_id)`. Read via the `main_contact` + `secondary_contact` keys embedded additively in `venue_list_active_teams` (league) + `venue_list_club_teams` (club), resolved by the internal `_venue_team_contact_json` helper; written via `venue_set_team_main_contact`. **mig 411 also relaxed `venue_assign_team_manager`** to allow assigning a club's **member OR a guardian of a member** as staff (so a parent can be made a coach → then a contact).
...
- `club_staff_dbs` — `id uuid PK`, `member_profile_id uuid FK→member_profiles`, `club_id text FK→clubs`, check_type (basic|standard|enhanced|enhanced_barred), status (pending|valid|expired|withdrawn), certificate_number, issued_date, expiry_date, notes. UNIQUE(member_profile_id, club_id). mig 305. DBS is CLUB-LEVEL & team-less by design — recorded via `venue_upsert_staff_dbs` with NO team.
- `club_coaches` (mig 582, DF Sports PR #5) — the **team-less** club coach/staff roster (the coaching-academy session model — coaches are club-level session staff, not one-coach-owns-a-team). `id uuid PK`, `club_id text FK→clubs ON DELETE CASCADE`, `member_profile_id uuid FK→member_profiles ON DELETE CASCADE`, `role text DEFAULT 'coach' CHECK (coach|assistant_coach|session_lead|other)`, `is_active bool DEFAULT true`, `added_by uuid FK→auth.users ON DELETE SET NULL`, created_at, updated_at. **UNIQUE(club_id, member_profile_id)**. Index (club_id). RLS on, REVOKE anon/auth → all access via SECDEF RPCs (`venue_upsert_club_coach` / `venue_remove_club_coach` / `venue_list_club_coaches`). Fits neither `venue_admins` (login staff) nor `club_team_managers` (team-scoped); **forward-designed so DF PR #8's `club_coach` LOGIN identity extends THIS table** (member-profile keyed) rather than a second one.
- **Modular Platform Epic B — public club page (`/c/<slug>`).** All RLS-on, REVOKE anon/authenticated, NO policy → SECDEF-RPC-only. FK→`clubs.id` (text).
  - `club_pages` (mig 444) — 1:1 with clubs. `club_id text PK FK→clubs`, `slug text UNIQUE` (CHECK `^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$`), `published bool DEFAULT false`, `primary/secondary/accent_colour text NULL`, `crest_url/hero_url/tagline/about text NULL`, `socials jsonb DEFAULT '{}'`, `sections jsonb DEFAULT '[]'` ([{key,enabled,order}]), `links jsonb DEFAULT '[]'` ([{label,url}] get-involved, mig 448), created_at/updated_at.
  - `club_sponsors` (mig 444) — `id uuid PK`, club_id, name, logo_url/website_url NULL, display_order int, active bool, `tier text NULL` CHECK headline|match|supporter (mig 448), created_at.
  - `club_posts` (mig 444) — `id uuid PK`, club_id, slug, title, body/hero_url/author_name NULL, author_profile_id uuid FK→member_profiles, status (draft|published), published_at, UNIQUE(club_id,slug).
  - `club_committee` (mig 449) — `id uuid PK`, club_id, role text, name text, email text NULL, `is_welfare bool DEFAULT false` (the welfare/safeguarding officer), display_order int, created_at. Index (club_id, display_order).
  - `club_documents` (mig 449) — `id uuid PK`, club_id, title text, url text, doc_type text NULL, size_label text NULL (display only), display_order int, created_at. Index (club_id, display_order).
  - `club_events` (mig 449) — `id uuid PK`, club_id, title text, event_date date NULL, blurb text NULL, display_order int, created_at. Lightweight social "what's on", NOT a calendar. Index (club_id, event_date).
  - `club_team_potm` (mig 449) — manager-pick Player of the Month, ONE row per team: `team_id uuid PK FK→club_teams`, `club_id text FK→clubs` (denormalised, server-derived, never client-trusted), name text, month text NULL, updated_at. UPSERT on team_id. `get_club_public` SUPPRESSES youth-team rows (cohort category=youth OR max_age<min_public_age) and all stats when hide_public_rosters — under-18s never named publicly. Index (club_id).
  - `club-media` storage bucket (mig 444) — public, image MIMEs only (png/jpeg/webp/gif/svg, 5 MB); club-scoped write policy via the manager chain. (PDFs can't upload here → club_documents stores pasted external links.)
...
- `member_id_documents` — `id uuid PK`, `member_profile_id uuid FK→member_profiles`, `club_id text FK→clubs`, `document_type text` CHECK (passport|driving_licence|pass_card|birth_certificate), `status text` CHECK (**pending|approved|rejected** — NB the verify RPCs write `approved`, NOT `verified`; a stale "verified" note here previously risked an RPC reading every cleared ID as outstanding), storage_path text, rejection_reason NULL, verified_by uuid NULL, verified_at timestamptz NULL, `purged_at timestamptz NULL` (mig 431 — set once the ID file is removed via the Storage API; direct DELETE on storage.objects is blocked by the protect_objects_delete trigger). For a guardian-uploaded child doc the row's member_profile_id is the CHILD but storage_path sits under the GUARDIAN's prefix.
- `member_record_reviews` (mig 431) — `id uuid PK`, `member_profile_id uuid FK→member_profiles`, `review_kind text` CHECK in ('medical'), `reviewed_on_behalf_by uuid NULL FK→member_profiles` (guardian who confirmed for a child), `reviewed_by_auth_user uuid NULL FK→auth.users`, `reviewed_at timestamptz`, `snapshot jsonb`. RPC-only (RLS on, no policies). Auditable, repeatable per-season confirmation that the medical/emergency-contact record is current (consent_acceptances' UNIQUE(doc,member) can't express a repeat). Written by `guardian_confirm_record_review`; read into the Documents manifest by `guardian_list_child_documents`.
- `club_announcement_reads` (mig 434) — `announcement_id uuid FK→club_announcements ON DELETE CASCADE`, `member_profile_id uuid FK→member_profiles ON DELETE CASCADE` (the READER, e.g. the guardian — NOT the child), `read_at timestamptz DEFAULT now()`, PK(announcement_id, member_profile_id). RPC-only (RLS on, no policies). Per-reader seen state for the guardian Club-notices inbox; drives the unread badge. Written by `guardian_mark_notice_read` (ON CONFLICT DO NOTHING = idempotent); read (as a `read` flag + `unread_count`) by `guardian_list_child_notices`.
