...
- `member_id_documents` — `id uuid PK`, `member_profile_id uuid FK→member_profiles`, `club_id text FK→clubs`, type (passport|driving_licence|birth_certificate|other), status (pending|verified|rejected), storage_path text, notes NULL, verified_by uuid NULL, verified_at timestamptz NULL, `purged_at timestamptz NULL` (mig 431 — set once the ID file is removed via the Storage API; direct DELETE on storage.objects is blocked by the protect_objects_delete trigger). For a guardian-uploaded child doc the row's member_profile_id is the CHILD but storage_path sits under the GUARDIAN's prefix.
