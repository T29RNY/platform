...
- `.claude/skills/backlog-capture/SKILL.md` — inbox filler (producer to `/backlog`'s consumer). `/backlog-capture` turns a raw idea, a GitHub issue URL, or a bullet list into compact captured/open rows: classifies each feature/bug/security/compliance, assigns a rough XS–XL effort, adds a source link, dedups, and appends to the right doc's `## 📥 CAPTURED — triage inbox (unbuilt)` section (FEATURES.md / BUGS.md / GO_LIVE_ISSUES.md). It FILES, never ranks (that's `/backlog`) and never writes a `## SESSION N` shipped block. Also callable at the end of `/qa-loop` to auto-file T3 findings. Docs-only, allowlist-safe, no DB/RPC/migration.
...
- `.claude/skills/prod-verify/SKILL.md` — targeted post-deploy verification of the LIVE site, the formal Step 5 (POST-DEPLOY) made invocable. `/prod-verify [PR#]` confirms the merged PR is actually live on Vercel, DERIVES which surfaces to walk from the merged diff (reuses babysit-prs's file→app map — tests what changed, not everything), runs a SUPERVISED Playwright-MCP walk of live-prod DEMO surfaces only (`/p/p_demotoken_01` + `/demoadmin`; never a real team, never scheduled-unattended), does read-only live-DB checks for RPC/schema diffs, then classifies each failure T1 (live functional defect → opens a `/dev-loop` fix phase) or T3 (design/gate decision, incl. the recommend-only `pg_notify` schema-cache flush → surface to operator). Two-bucket triage (no qa-loop "T2 auto-polish" — a prod defect always warrants a real fix cycle). Docs-only diff → "nothing to walk". Also invokable manually with an explicit app/surface list. Read-only — never edits, never merges, never applies a migration; findings land in BUGS.md / GO_LIVE_ISSUES.md (no parallel report system). The post-merge, prod-facing twin of qa-loop; formalizes `skills/post-deploy.md`.
