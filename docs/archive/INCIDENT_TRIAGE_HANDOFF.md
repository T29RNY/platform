...
- **PR #4 — Venue desktop triage UI + assignable-staff read (mig 465)** — ✅ **BUILT + APPLIED LIVE 2026-07-01.** Added an unplanned `venue_list_assignable_staff` read (operator-approved: "if we need Assign, build it") — the Assign picker's user_id source, WHERE clause mirrors triage validation. IncidentActions: Triage (category/priority/assign/acknowledge) + Escalate + Resolve + safeguarding notice (report+resolve). Operations: priority/category/escalated/ack badges + assignee + aging + priority filter chips. Proof: venue build PASS, core/inorout build PASS, hygiene 7/7, QA+Security review CLEAN, rpc-security PASS, **EV PASSED** (assignable-staff scoping; leak-check 0). ⚠️ **browser smoke NOT run** (couldn't stand up venue auth locally) → owed at the manual-deploy eyeball. PR: <pending>. Gate: ⛔ venue MANUAL deploy + eyeball.
- **PR #5 — Mobile triage UI (apps/inorout OperationsTonight, NATIVE)** — ✅ **MERGED (PR #212, 2026-07-01) — operator merged un-walked (HR13 override, informed).** No new backend (reuses migs 462/465). Each open-incident card gains one-tap Acknowledge / Assign (sheet, venueListAssignableStaff) / Escalate (sheet, optional reason) above Resolve; priority/category/escalated/ack badges + assignee; safeguarding notice on Resolve sheet. Reuses MobileSheet/m-card/MIcon; credential=venueId (auth path). Proof: build PASS, hygiene 7/7, QA review CLEAN, casual-regression file-overlap CLEAN, check-live-config CLEAR. ⚠️ **real-iPhone on-device walk still owed** (merged live to App-Store bundle without it — verify ack/assign/escalate/resolve persist across reload on-device).
- **PR #6 — HQ escalation inbox UI (apps/hq)** — ✅ **MERGED (PR #213, 2026-07-01) — DARK until HQ deploy.** No new backend (reuses `hq_list_escalated_incidents` mig 464 + `hq_resolve_incident` mig 171). New `EscalatedInbox.jsx` = company-scoped cross-venue inbox (priority/category/assignee/reason/escalated-age), resolve reuses App-level `onResolve`→`hq_resolve_incident`, analyst read-only, self-refetches; wired into App.jsx right column above AlertsActions. VenueDetail per-venue rows enriched with priority/escalated badges + category/assignee/reason. Proof: HQ build PASS, check-live-config CLEAR, QA review CLEAN. Gate: ⛔ HQ deploy (its own pipeline).
