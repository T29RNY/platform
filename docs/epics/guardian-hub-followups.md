...
| 11 | Payment **reminders**. **P11a DONE (mig 540, APPLIED-live)**: `get_membership_reminders_due` payment_due arm → due−7/−1/0 + overdue, offset-aware entity_key (ch.id:stage) → each stage dedups once, `reminder_stage` field, stage-aware email copy. → **P11b push channel next**. ⚠️ SHIPS-LIVE (real customer emails); operator heads-up: first run → 1 fresh "overdue" nudge per currently-overdue charge. | 3 | — | **building** | **apply(×N)** + merge(×N) |
