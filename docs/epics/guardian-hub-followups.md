...
| 10 | Coach/admin per-player **doc-status**. **P10a coach reader `club_manager_get_team_doc_status` DONE (mig 538, APPLIED-live)** → P10b coach /hub surface next → P10c desktop venue-club-lens (needs its OWN venue-token reader — P10a is coach-auth only). Status flags only (no medical content). DPIA nuance: medical-review flag is person-global (no club_id on member_record_reviews). | 3 | — | **building** | **apply(×N)** + merge(×N) |
