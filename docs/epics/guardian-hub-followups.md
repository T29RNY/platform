...
| 10 | Coach/admin per-player **doc-status** — **COMPLETE**: P10a coach reader (mig 538) + P10b coach /hub surface + P10c venue-token reader (mig 539) + desktop SafeguardingBoard "Player documents" section. Status flags only (no medical content). DPIA nuance: medical-review flag is person-global (no club_id on member_record_reviews). | 3 | — | **DONE (build)** | walks (batched) |
| 11 | Payment **reminders** — **COMPLETE**: P11a (mig 540) cadence due−7/−1/0+overdue, offset-aware dedup, stage-aware email copy; P11b (mig 541) PUSH leg — get_membership_reminders_due +member_profile_id, notify.js typed push, cron pushes payment_due to app members. ⚠️ SHIPS-LIVE (email+push); operator heads-up: first cron run → 1 fresh "overdue" nudge per currently-overdue charge. | 3 | — | **DONE (build)** | operator heads-up |
...
### Walk-gap follow-ups (2026-07-11, post-epic on-device walk findings)
Three gaps surfaced on the authed guardian/coach walk, built as their own PRs after the epic:
- **#3 Per-fixture location/address (mig 545)** — `club_fixtures` gains a free-text `location`;
  coaches (TeamManagerFixtureEdit + SessionsScreen) and club-admins/operators (apps/venue FixturesTab)
  set it; the 6 fixture readers (guardian fixtures + leagues, coach list + detail + member-feed, desktop
  operator list) surface it; every render shows `location || venue_address` (free-text wins, else the
  linked venue's postal address). Additive column NULL on all existing rows. EV 10/10 + leak-0, both
  writes + all readers proven; rpc-security PASS (single overloads, grants preserved); QA+Security reviewed.
  ⛔ owed: authed guardian + coach on-device walk (batched).
- **#5 Unread comms on the guardian Sessions home** — surfaces recent/unread club announcements on
  GuardianMatches (reuse `guardian_list_child_notices` + the shell's existing `unread_count`). No backend.
- **#4 Fees "Pay now" cash option** — the standalone fee "Pay now" opens the shared BookPaySheet
  (card/bank/**cash-always**) instead of dead-ending when Stripe is off (`get_my_money` +stripe_available/manual_pay_url).
...
- 2026-07-11 P11a DONE — mig 540 APPLIED-live: payment-reminder cadence. get_membership_reminders_due payment_due arm now fires at due−7/−1/0 + one overdue nudge, EACH ONCE, via an offset-aware entity_key (ch.id:stage) — the existing notification_log dedup does the rest (no job change). +reminder_stage field (12th col; NULL for the other 3 kinds, byte-unchanged). cron.js passes reminderStage into ctx; _mailer.js membership_payment_due varies subject/copy by stage. Grants LOCKED explicitly (REVOKE public+anon+auth / GRANT service_role — was inherited-only; defense-in-depth per feedback_default_privileges_revoke, security-review catch). EV + leak-0 (5-charge fixture: due_7/due_1/due_0/overdue stage-keyed + due+3 fires nothing + 4 total). rpc-security PASS. build+lint PASS (hygiene N/A — api/ backend, pre-existing direct-supabase+email-hex not in my diff). QA PASS + Security SECURE-TO-SHIP (service_role-only verified, no exposure change). **SHIPS-LIVE** — real customer payment emails. ⚠️ Operator heads-up: first 10am run → 1 fresh "Payment overdue" email per currently-overdue membership charge (bounded, dedups after). P11b = the PUSH channel (needs recipient→push mapping: the payment recipient is a venue_customers.email; push needs venue_memberships.member_profile_id→member_profiles.auth_user_id→push_subscriptions). Split rationale: P11a = cadence+email (core), P11b = push.
- 2026-07-11 P10c DONE → **P10 COMPLETE**. mig 539: venue-token doc-status reader `venue_get_club_doc_status(venue_token, club_id)` — admin/owner twin of the P10a coach reader, per active CLUB member (venue_memberships), same status logic. Venue-token auth (resolve_venue_caller + manage_facility cap + club_venues scope, verbatim from venue_get_club_page). STATUS FLAGS ONLY. Desktop: SafeguardingBoard.jsx gains a "Player documents" section (stat-row + per-member rows, soft-caught → hides if the read fails). Wrapper venueGetClubDocStatus + barrel. EV 7/7 + leak-0 (venue-token path: 3 members + summary + requirements + foreign-club→club_not_in_venue + bad-token→invalid_venue_token). rpc-security PASS. venue+inorout build PASS. QA PASS + Security SECURE-TO-SHIP (two scope layers, no medical/storage_path leak). Design notes (non-blocking): medical-review flag member-scoped not club-scoped (inherited P10a); M:N shared-club → both venues' admins see the board (intentional co-management). ⛔ owed: desktop SafeguardingBoard walk (batched to epic end; venue app manual-deploy).
