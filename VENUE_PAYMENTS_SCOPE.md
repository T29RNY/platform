...
| **V3.1 ✅ SHIPPED** (mig 183, session 64) | per-fixture charge **add** (`venue_add_fixture_charge` — amount = arg or league default; reactivates a voided charge) + **void** (`venue_void_charge` — status→refunded, payments kept) + `payment_link` show/edit (added to `venue_update_booking_settings` whitelist; exposed on `venue_get_state`). apps/venue PaymentsView gains Add-charge + Void + pay-link editor. | yes → ephemeral-verify (13/13) |
