...
*Last updated: Jul 18 2026 (VENUE INTERNAL LEAGUE — operator direct result-entry. New additive RPC `venue_enter_fixture_result` (mig 607) + `venueEnterFixtureResult` wrapper + an "Enter result" action in apps/venue FixtureActions.jsx: an operator can now record the final score of a played `scheduled`/`allocated` league fixture and complete it, without a referee live-scoring it in the ref app (previously the console could only CORRECT an already-completed fixture via `venue_update_fixture_result` mig 127). Reuses only already-whitelisted broadcast reasons → no shipped function edited; standings re-derive live. Desktop-only for now (mobile operator league surface is separately scoped → MOBILE_OPERATOR_LEAGUE_DESIGN_BRIEF.md). Tier-3: migration NOT yet applied — **PR pending human sign-off**. Prior: Jul 15 2026 (STATS MONTH PICKER + STICKY HEADER — ✅ **SHIPPED + LIVE (PRs #540–#543, mig 575)**. Casual **Stats** tab, four changes: (1) the **This-Month** player table drops the 3-game rank minimum — any player with ≥1 game that month is ranked; Season/All-Time keep ≥3 (#540). (2) `get_squad_fitness_leaderboard` gains an optional `p_month_anchor` ('YYYY-MM') windowing the squad-fitness board to a chosen month; time-window-only change, security byte-identical to mig 513, DROP old 2-arg + recreate 3-arg (#541, **mig 575**). (3) Whole-page **month picker** — a `‹ June 2026 ›` stepper walks the months-with-games and retargets the ENTIRE Stats tab (table, top scorers, clinical, POTM, winners, relegation, attendance, tiles, bib duty, own + squad Match Fitness) to that month, plus a personal **"Your <Month>"** summary card (#542). (4) **Sticky collapsing header** — the period switcher + month stepper are pinned while scrolling (extracted into `PeriodSwitcher`), and the STATBOOK banner collapses 104→40px into a slim strip on scroll via an IntersectionObserver sentinel (#543). Design: **Payment Reliability + the Reliability column stay all-time** by design (standing status, not monthly); month form derived **client-side** (the league RPC only knows the current month); reliability fetched from the all-time table in month mode. QA reviews CLEAN, adversarial security review CLEAN (mig 575), all iPhone walks PASSED. **Next free mig = 576.** Prior: Jul 15 2026 (backlog-capture — filed **Android native app** + **Android/Wear-OS watch-tracking** as two inbox features). Prior: Jul 13 2026 (TOURNAMENT in-app create — ✅ **SHIPPED + LIVE (PRs #512–#515, no migration)**. Venue operators & club-admins create a tournament from the native app (`/hub → More → Cups → New`): name + format + optional date + optional entry fee → owned to their venue/club via `venue_create_tournament` / `club_admin_create_tournament` (NOT the self-serve RPC, which mints a hidden personal-host venue), auto-adds one competition (format-mapped) + publishes, so it appears on the desktop venue console + the public page immediately. "+ New" on the operator Cups index; a "Tournaments" row in the club-admin More launcher; public-page block reworked to **Copy link / View page** with unlimited multi-team self-registration. **Team managers do NOT create — operators/club-admins only** (DECISIONS.md 2026-07-13). Reuses the fully-built Event OS engine + venue/club create chain; casual `CreateTournament.jsx` untouched. Prior: Jul 09 2026 (Club Console PR #6 Phase B — adult-member SELF reliability/POTM "Stats" tab in the native /hub member track (mig 519, PR-pending apply). NEW self-scoped reader `club_member_get_self_reliability()` (param-less, auth.uid→own member_profile, NO manager check; DPIA self-scope airtight — reviewer-traced every CTE, other members excluded, guardian-of-child pulls ZERO child rows) + `MemberReliability.jsx` (one card per team: reliability %/P-W-D-L/winRate/goals/POTM/last-5 form). Phase A (schedule/matches/membership/more) shipped in PR #384. EV PASS + leak-0, QA no-defects/Security no-findings/Adversarial could-not-refute, casual-regression STATIC PASS (additive /hub + core), check-live-config PROTECTED. ⛔ mig-519 apply = G1 human gate; real-iPhone /hub Stats walk owed (HR#13, waived for merge per manifest). Next free mig = 520. Prior: Jul 09 2026 (backlog-capture — filed 2 venue-nav IA polish nits: "Programmes" group split + "Club & admin"→"Settings" rename, from CLUB_CONSOLE_CONSOLIDATION Decision 9). Prior: Jul 08 2026 (backlog-capture — filed Gaffer club-context feature to inbox). Prior: Jul 04 2026 (backlog-capture — filed DIS distance-column feature to inbox; earlier today: 2 MyView WhatsApp-share items). Prior: Jun 29 2026 (session 230 — UNIVERSAL AI AGENT — Pillar D (Know-who) foundation SHIPPED (mig 454, PR #150): `ai_agent_access` opt-in canary + daily cost-ceiling table; `resolve_agent_caller` unified caller resolver (5 auth models → one caller-context jsonb; agent.phase=1 = answer-only); `resolveAgentCaller` core wrapper. EV 10/10 + leak-0, rpc-security PASS, live smoke PASS. Four pillars: A Answer (per-domain context RPCs), B Direct/navigation (DEFERRED Ph2), C Act/tool-use (DEFERRED Ph2), D Know-who (DONE). NOT YET: universal-agent edge fn, agent conversation tables, ai_briefings team_id-nullable, non-casual domain context RPCs, Stage-1 casual-canary wiring (ENABLE_GAFFER env-gate + GafferCard + sonnet-4-6 — NEXT PR). Prior Gaffer (migs 033–037) audit: backend built s33, never canaried — the opt-in gate now governs rollout. Next free mig = 455.)*
...
- 📥 **MyView WhatsApp SHARE button → Live Board** — feature · effort XS · relocate the WhatsApp share button into the Live Board (In's), positioned under the current-players-In number; keep the central tick + number exactly as-is, just add the word "SHARE" below it · source: input:2026-07-04 · captured 2026-07-04 · _open_
- 📥 **WhatsApp share message title needs count suffix** — feature · effort XS · the shared WhatsApp message title (e.g. `*Footy Tuesdays* — Tuesday 7 Jul`) should append `- x more needed` until the squad hits its target player count, or `- Full Squad` once reached (e.g. 14/14) · source: input:2026-07-04 · captured 2026-07-04 · _open_
- 📥 **DIS (distance) column in the Stats-tab tables** — feature · effort ~M · add a per-player distance column (`DIS`) to the Stats-tab player league table / stats tables, alongside P/W/D/L/W%. Reads shipped match-fitness data (`getMyMatchHealth` = self only; `get_squad_fitness_leaderboard` = per-player squad distance — a whole-table column needs the squad-fitness rows joined into the league table, so it's more than self-data). DESIGN DECISION owed: distance is Apple Watch data — sparse + auth-gated, so most cells would be empty; decide whether it belongs in the main league table vs staying in the dedicated Match Fitness section (which already shows it). Depends on the shipped match-fitness epic · source: input:2026-07-04 · captured 2026-07-04 · _open_
- ✅ **Match result shows the team's combined distance ran** — feature · effort ~S · **SATISFIED by the Team A vs Team B fitness card** (TEAM_VS_TEAM_FITNESS_HANDOFF, migs 481/482, PR — per-match `PerMatchFitnessCard`): the A + B side totals ARE the combined distance, and the split is richer than a single sum. Same `getMatchHealthForMatch` rows, consent-gated + casual-only, honest `(N tracked)` count (never framed as a complete team total). Closed here — do not build separately · source: input:2026-07-04 · captured 2026-07-04 · _satisfied 2026-07-05_
- 📥 **Show the Apple workout NAME in the attach confirm/picker sheet** — feature · effort XS · when the match-to-game attach flow surfaces a workout (single confirm) or multiple workouts (picker) in `PerMatchFitnessCard`, show Apple's workout name/title (Apple names every workout) so a player can tell which one to pick when several fall in the game's time window. Today the sheet shows only stats, which is ambiguous with multiple candidates · source: input:2026-07-04 (G5 device walk feedback) · captured 2026-07-04 · _open_
- 📥 **Venue nav "Programmes" group is incoherent — split it** — feature · effort ~S · the apps/venue rail's "Programmes" group (Dashboard.jsx TABS) spans 4 feature flags (coaching+equipment+spaces+room_hire) and mixes "activities you run" (Timetable/Classes/Training, Trainers = coaching) with "facility resources you rent" (Equipment, Rooms). Split into an "Activities" group (coaching) + fold Equipment/Rooms near Bookings/Run as facility inventory; ids stay stable for deep-links (session-178-regroup mechanism). Cosmetic IA polish, NOT part of the club-console consolidation epic · source: CLUB_CONSOLE_CONSOLIDATION_HANDOFF.md Decision 9 (2026-07-09) · captured 2026-07-09 · _open_
- 📥 **Venue nav "Club & admin" group is mislabelled → rename "Settings"** — feature · effort XS · the apps/venue rail's "Club & admin" group holds QR codes/Features/Access/Integrations — all console Settings/Admin, none club-specific (the actual club items live in People/Competition). Rename the group to "Settings" (or "Admin"); ids stable for deep-links. Cosmetic IA polish, NOT part of the club-console consolidation epic · source: CLUB_CONSOLE_CONSOLIDATION_HANDOFF.md Decision 9 (2026-07-09) · captured 2026-07-09 · _open_
- 📥 **Android native app (Capacitor Android target)** — feature · effort XL · a native Android build of the In-or-Out app mirroring the shipped iOS native wrap ([[project_native_app_wrap]]); the stack table already names Capacitor as the iOS+Android mechanism (FEATURES.md line ~2691), so this is standing up the Android target of the existing Capacitor wrap, not a rewrite — owes Play Store listing, Android push (FCM) + auth/Google-sign-in parity, and a real-device Android walk (native-app-only discipline, HR#13). Prereq for the Wear-OS item below · source: input:2026-07-15 · captured 2026-07-15 · _open_
- 📥 **Android watch (Wear OS) workout tracking** — feature · effort XL · Wear OS + **Google Health Connect** match-workout capture mirroring the Apple Watch / HealthKit pipeline ([[project_watchos_companion]], [[project_match_workout_tracking]]): the same `match_health_sessions` + per-match attach flow, fed from Health Connect instead of HealthKit. Depends on the Android app existing (row above), and owes **DPIA / consent parity** for Android health data (Health Connect permission model) the way the HealthKit path did (`MATCH_FITNESS_DPIA_ADDENDUM.md`) · source: input:2026-07-15 · captured 2026-07-15 · _open_

### Nightly automation suite (cloud routines, 03:30–04:15 UTC · unattended-safe, read-only/draft-only) — approved 2026-07-02, SCHEDULED + MERGED 2026-07-03
Findings land in **one place**: GitHub issue [#247 "Nightly Ops Digest"](https://github.com/T29RNY/platform/issues/247). Routines A–E each post a raw findings comment; routine F reads all of tonight's comments and rewrites the issue BODY into one short plain-English summary (needs-attention / PRs ready to merge / PRs blocked / routine health). Read only the issue body each morning — comments are history, not the daily read.
- 🏁 **A — `/babysit-prs` (03:30 UTC)** — `trig_01A1JhArCnQnPNZ2bE2xt7ia`. Posts PR digest comment to #247.
- 🏁 **B — Migration/source drift check (03:40 UTC) — SHIPPED, PR #245 MERGED 2026-07-03.** `Skills/scripts/check-drift.sh` (read-only, Hard Rule 11). Routine `trig_01Bfb7NhCJuo4dQ37DQvZT3m` posts finding to #247.
- 🏁 **C — Deploy-freshness sweep (03:50 UTC) — SHIPPED, PR #246 MERGED 2026-07-03.** `Skills/scripts/check-deploy-freshness.sh`, covers 5 manual-deploy apps (venue, hq, display, ref, superadmin) + `packages/core`/`packages/ui` bundling — wider than the originally-captured venue+hq. Degrades gracefully to "last merged commit" info when Vercel CLI isn't authenticated non-interactively. Routine `trig_01NypoLwayE5HEVCHmcP4bHd` posts finding to #247.
- 🏁 **D — Security/perf advisor sweep (04:00 UTC)** — `trig_01A6bYVz4EAf4mt3cmFpCntP`. No baseline-diffing mechanism yet — reports ALL `get_advisors` findings every night, not just new ones; persisting `skills/state/advisors-baseline.json` via a reviewed PR is a follow-up.
- 🏁 **E — `_e2e_` residue leak check (04:00 UTC)** — `trig_017vywm3aBqGiNyDQ9uKgSfX`, wraps `Skills/scripts/check-ev-leak.sh`.
- 🏁 **F — Digest compiler (04:15 UTC) — NEW, added 2026-07-03 after operator feedback that 5 siloed routine outputs weren't usable.** `trig_01YGqs7QJMn7KYkiZVrNs7Ce` reads A–E's comments on #247 each night and replaces the issue body with the digest. Smoke-tested by manually running routine A once same day to confirm `gh issue comment` works from the cloud environment.
- 🏁 **FAQ page + `/faq-sync` nightly maintenance skill — BUILT 2026-07-02.** In-app `/faq` page in `apps/inorout` (searchable FAQ accordion, static content-only data file `apps/inorout/src/data/faq.js`, linked from the profile sheet) — PR #230, OPEN, owed a real-iPhone walk before merge (Hard Rule 13, App.jsx is PROTECTED). `.claude/skills/faq-sync/SKILL.md` — three-job nightly loop (new-feature detection, accuracy sweep, PR-touch sweep) that edits `faq.js` directly; a pure content-only diff auto-merges under a narrow operator-approved exception (see DECISIONS.md 2026-07-02), enforced by `Skills/scripts/check-faq-content-only.sh`; anything wider falls back to normal dev-loop PR + human merge. Nightly schedule not yet created (`CronCreate`, pick a slot in the 03:30–04:00 UTC window alongside the routines above) — owed. Extending to other apps (venue/clubmanager/etc.) needs that app's `/faq` page built first via dev-loop.
- 📥 **Gaffer club-context — coach /hub AI panel** — feature · effort ~M · "ask the gaffer" over the club's own data (form / top-scorer / turnout / roster): new coach-auth reader RPC `gaffer_get_context_club_team_summary` (mirrors casual mig 034; reads club_fixtures / club_fixture_player_stats / availability — #8 landed the data) + `apps/inorout/api/gaffer.js` coach-auth path (currently casual admin-token only) + new surface key + club-tuned system prompt + a GafferCard-equivalent in the coach /hub gated by `VITE_GAFFER_ENABLED` (ships DARK). The edge-fn coach-auth extension is the fiddly bit; provider + ai_briefings audit reused. Deferred from Club Manager epic PR #7 split (7a shipped #367) · source: Club Manager epic PR #7b · captured 2026-07-08 · _open_
