...
| **Equipment Hire** (venue) | Sport-agnostic kit hire for the venue (bibs/balls/goals/nets/AV). Full booking + inventory build; sport-agnostic per the settled multi-sport posture (no `sports` table). Plan: `EQUIPMENT_HIRE_PLAN.md`. **Cycle 1 ✅ shipped (session 85, migs 255–256):** 3 tables (`equipment` catalogue + `equipment_bookings` + `equipment_demand_misses`) with the data foundations locked in (category taxonomy, session-link FKs, demand-miss capture, asset value/condition) + `venue_charges` extended to bill equipment; catalogue RPCs `venue_list_equipment`/`venue_upsert_equipment` (EV 10/10, leak 0); venue dashboard **Equipment** tab + `EquipmentView.jsx`. **Next — Cycle 2:** quantity-aware availability + the hire flow (creates/charges hires, logs demand-misses). Cycle 3: returns/deposits/overdue. Cycle 4: QR self-hire (reuses the QR rail). | Backend+UI | 🟡 Cycle 1/4 done |
