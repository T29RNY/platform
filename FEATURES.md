...
- ✅ **Match result shows the team's combined distance ran** — feature · effort ~S · **SATISFIED by the Team A vs Team B fitness card** (TEAM_VS_TEAM_FITNESS_HANDOFF, migs 481/482, PR — per-match `PerMatchFitnessCard`): the A + B side totals ARE the combined distance, and the split is richer than a single sum. Same `getMatchHealthForMatch` rows, consent-gated + casual-only, honest `(N shared)` count (never framed as a complete team total). Closed here — do not build separately · source: input:2026-07-04 · captured 2026-07-04 · _satisfied 2026-07-05_
