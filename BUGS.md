...
- 📥 **`Skills/scripts/check-workspace-deps.sh` (and `check-build.sh`) hardcode a lowercase `skills/scripts/...` path, but the real directory is capitalized `Skills/`** — bug · effort ~S · works by accident on the operator's Mac (case-insensitive filesystem folds the case), but fails outright (`No such file or directory`) on any case-sensitive Linux checkout — confirmed during the 2026-07-07 nightly QA cloud session, where `check-build.sh` errored before attempting a build. Same casing mismatch is referenced pervasively in lowercase across `.claude/hooks/post-edit-hygiene.sh`, `.claude/hooks/pre-commit-build.sh`, `.claude/settings.json`'s allowlist, and `.claude/hooks/session-start.sh` — meaning the hook-enforced hygiene/build gates CLAUDE.md calls "deterministic, not advisory" may be silently non-functional in any cloud/Linux session. A same-session fix attempt (a `skills → Skills` symlink) was reverted after an independent review caught that a case-only-differing symlink pair can't safely coexist on a case-insensitive filesystem — checking it out on the operator's Mac risked corrupting the real `Skills/` directory. Needs a deliberate fix (most likely a proper `git mv Skills skills` rename, done as its own reviewed commit, not folded into an unattended pass) · source: qa-loop:2026-07-07 · captured 2026-07-07 · _open_
- 📥 **e2e regression net (`qa-suite.sh`) cannot run in this remote/cloud execution environment** — bug · effort ~S (config, not code) · two independent harness blockers hit during the 2026-07-07 nightly QA run: (1) the environment's network egress allowlist does not include the demo Supabase project host (`ktvpzpnqbwhooiaqrigm.supabase.co`) — `403 Host not in allowlist`, blocking `e2e/global-setup.mjs` session-minting for every authed Playwright project (inorout, venue, hq, superadmin, tokens); (2) the pre-installed Playwright browser revision (1194) doesn't match what this project's pinned `@playwright/test` expects (1228), so the two token-only projects (display, ref) fail at browser launch before even reaching the network. Neither is an app defect — no code fix resolves them; both need the remote environment's network policy / browser image reconciled for scheduled cloud QA runs to actually exercise e2e · source: qa-loop:2026-07-07 · captured 2026-07-07 · _open_
