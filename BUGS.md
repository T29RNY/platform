...
- 📥 **`Skills/` vs `skills/` directory casing mismatch breaks check-*.sh cross-references on Linux** — bug · effort ~M · the repo's committed directory is `Skills/` (capital S, confirmed via `git log --diff-filter=A -- 'Skills/*'` — always capitalized, never lowercase), but CLAUDE.md and most skill docs reference lowercase `skills/scripts/...` throughout, and `check-live-config.sh`'s own safe-surface classifier pattern-matches `skills/*` (won't match the real path); masked on macOS (case-insensitive fs) but breaks any executable cross-script call on case-sensitive Linux — one instance already found + fixed (check-build.sh → check-workspace-deps.sh, PR #283). Needs a human call: rename the directory to lowercase repo-wide, or fix every doc reference to `Skills/` — broad blast radius, not auto-fixed · source: qa-loop 2026-07-05-nightly · captured 2026-07-05 · _open_
- 📥 **Nightly QA e2e lane blocked by cloud-session network egress policy (Supabase host not allowlisted)** — bug · effort ~M · outbound access to the live Supabase project host (`ktvpzpnqbwhooiaqrigm.supabase.co`) is blocked by this session's egress policy (confirmed 403 CONNECT at the proxy), breaking both the Node-side auth-session mint (`e2e/lib/auth.mjs`) and the app's live data fetches in-browser — explains why even token-based specs (`tokens.public.spec.js`) failed to render expected content. Not an app regression — needs the platform/operator to allowlist the Supabase host for the nightly QA trigger's environment, or reconsider qa-suite's live-DB dependency for that environment · source: qa-loop 2026-07-05-nightly · captured 2026-07-05 · _open_
- 📥 **Nightly QA e2e lane hit a Playwright browser-revision mismatch (chromium_headless_shell missing)** — bug · effort ~S-M · repo pins `@playwright/test` 1.61.0 which expects Chromium revision 1228, but this session's pre-installed browser cache only has revision 1194, so every headless project fails `browserType.launch: Executable doesn't exist`. Worked around locally for verification only (temporary, non-committed `executablePath` override, reverted) — will recur on every nightly run in a similarly-provisioned environment unless the platform team pre-caches a matching browser revision or the pinned Playwright version is aligned to the trigger environment's cache · source: qa-loop 2026-07-05-nightly · captured 2026-07-05 · _open_
