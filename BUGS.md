...
## BUG (Jul 14 2026) — 🔴 OPEN — new Stripe member's FIRST invoice payment doesn't reconcile to the ledger in real time (parallel-webhook race)
...
**Recommended fix:** in the webhook's `checkout.session.completed` branch (which creates the membership and already holds `subscription` + `customer`), record the first invoice payment there — the deterministic "membership definitely exists" moment — closing the race; the cron then only backstops genuinely dropped events. (Alt: on `invoice.paid` + `no_membership`, return 500 so Stripe retries after enrolment commits — less clean, risks retry storms.) Prior Phase 5/6 dev walks missed this because webhooks were replayed sequentially (Stripe CLI) / against a pre-existing membership.
