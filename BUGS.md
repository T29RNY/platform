...
*Last updated: Jul 20 2026 (✅ **RESOLVED — queue-and-drain notification emails never sent**: mig 613 / PR #614 (commit be8dd858, applied+merged+live) set `channel/sent_at=NULL` on 10 room-hire/class `notification_log` queue-inserts so the cron drains actually pick them up). Prior: Jul 15 2026 (**`venue_list_members` PII gate — FIXED + LIVE (PR #549, mig 577)**: closed a live GDPR data-minimisation leak — a plain reception `staff` venue login received every enrolled child's `dob` + each guardian's name/email/phone in the member list (`venue_list_members`, mig 410, ungated). Now cap-gated behind `manage_memberships` via `_venue_has_cap` → plain staff get `email`/`dob` NULL + `guardians` `[]`; owner/manager byte-identical (keys unchanged, Hard Rule 7 — no wrapper/consumer change). Guardians gate fail-closed/self-contained. Applied live + ephemeral-verify PASS (owner full PII, staff blanked, non-gated identical, leak-check 0) + rpc-security PASS. Direct sibling of the mig-524 fix; DF Sports Phase 0 / PR #0. Pre-flight in GO_LIVE_ISSUES.md. Prior: Jul 14 2026 (STRIPE go-live test walk on the new in-or-out account — Test 1 (club onboarding + connected-accounts webhook→prod) ✅ PASS; Test 2 (member charge → subscription + membership) ✅ PASS but found a bug — now ✅ **FIXED (PR #520, no migration)**: a new subscriber's FIRST invoice payment didn't reconcile to the ledger in real time (parallel-webhook race; `invoice.paid` beat `checkout.session.completed` → `no_membership`; cron repaired at 4am, ~24h lag). Fixed by recording the first paid invoice in the `checkout.session.completed` branch (idempotent reuse of `stripe_record_invoice_payment`). See the `## BUG (Jul 14 2026)` entry. ⛔ owed: post-deploy live-Stripe walk (Test 2 real-time + Test 3 refund) on `acct_1TsshHImDQXNp8aQ`. Prior: Jul 13 2026 (TOURNAMENT in-app create + 4 tournament UX/robustness fixes — ✅ **SHIPPED + LIVE (PRs #512–#515, no migration)**. Operators & club-admins now create a tournament from `/hub → Cups → New`, owned to their venue/club so it syncs to the desktop console; 4 fixes folded in — join dead-end, competition-less-register silent 404, `event_date_required` on venue/club create, desktop panel refresh, + the public-page "Copy link / View page" rework. See the `## BUG (Jul 13 2026)` tournament entry + DECISIONS.md 2026-07-13. Also Jul 13: go-live-check player+guardian onboarding — ✅ **FIXED + LIVE (migs 570–572)**. (1) `member_update_self` was **100% broken**: it INSERTed into non-existent audit_events columns (`actor_id,event_type,payload`) → `undefined_column` → every member/guardian "save my own profile" (name/address/emergency/dietary/**medical**/photo-consent) silently rolled back. Undetected because the member self-serve path had **zero real usage** — all demo data was migration-seeded, so no real authed member ever hit it (0 audit rows for any member-profile event confirmed it). mig 570 fixes it + the two orphaned twins `member_create_profile`/`member_claim_profile` (same latent bug). EV-proven (found=true + 1 audit row). (2) mig 571 — consent on-behalf `invite_state='accepted'` gate + `REVOKE ALL` hardening on people/member_id_documents. (3) mig 572 — the private `member-id-docs` bucket SELECT policy let **any authenticated user read any member's/child's passport/birth-cert**; now scoped to owner + venue_admin via a SECDEF helper (EV-proven). rpc-security PASS ×5, leak-0. Follow-ups (GO_LIVE_ISSUES): venue-scoped ID-doc signed-URL minter; `get_my_world` invite_state filter; `/parent-home`→`/hub` nav gap (child ID-doc upload unreachable for a pure guardian). See RPCS.md 2026-07-13 + DECISIONS.md 2026-07-13.) Prior: Jul 06 2026 (backlog-capture — filed CLAUDE.md hooks-count doc drift to inbox; prior same-day: nightly advisor-sweep no-baseline tech-debt). Prior: Jun 30 2026 (session 232 — ✅ PRODUCTION BUG FIXED + LIVE — ORPHAN-GUEST "KEEP IN" PERSISTENCE (mig 458, PR #183, applied + merged). Reported live from the admin panel: when a guest's host drops out, the "host dropped out — Keep IN / Move to reserve / Remove" banner reappeared on every reload after tapping Keep IN. Root cause: the banner is computed live from `host.status != 'in'`, but Keep IN only mutated an in-memory React Set (`dismissedOrphans`) — nothing persisted, so the condition stayed true forever. "Move to reserve" had the same reload bug (filter ignored guest status); only "Remove" stuck (status='none' → dormant → excluded). FIX: per-week `players.host_dropout_ack` flag — set by new `admin_ack_orphan_guest` RPC (SECURITY DEFINER, search_path locked, team derived from admin token via resolve_admin_caller, guest-membership guarded + team-scoped UPDATE, writes audit_events `guest_host_dropout_acknowledged`); exposed on squad rows by `get_team_state_by_admin_token` (else invisible — the is_self latent-bug class, Hard Rule 12); reset to false on weekly rollover in `admin_go_live` + `admin_go_live_for_team` beside `admin_locked_in` so the ack is "for this one game". Orphan filter gained `status === "in" && !hostDropoutAck` — the status guard also fixes the Move-to-reserve reappearance. Guest stays LINKED to host (guest_of untouched) → returning-guest picker unchanged. Gates: ephemeral-verify ×2 (happy + cross-team + bad-token, leak-clean), build, hygiene 7/7, fresh-context QA + security review both clean, post-apply live verify (column + RPC SECDEF/search_path/single-overload/anon+auth grants + flag exposed + both rollovers reset — all confirmed). Operator confirmed the fix worked on the live admin screen same session. ALSO fixed (separate commit): `check-next-migration.sh` self-conflicted on the highest migration (counted the candidate file itself → every new top migration blocked); now excludes the candidate, real duplicates still caught. **NEXT FREE MIG = 459.** | PRIOR session 212 — 🔴 APP-STORE REJECTION #2 FIXED — iOS AUTH STORM, ROUND 2 (build 1.0(4), iPad Air M4). App Review hit the SAME Sign-in-with-Apple refresh-token storm again. CONFIRMED from `auth.refresh_tokens`: the reviewer's fresh Apple account = ONE session, 47 token rotations in 44s (46 revoked) → 429 → logout. PR #96's native flag FAILED: `Capacitor.isNativePlatform()` returned false in the remote-`server.url` WKWebView on iPad → `__CAP_NATIVE__` false → cookie mode engaged; WKWebView then returned stale/partial cookie reads WITHIN the session, and the ls mirror did NOT save it (`getItem` consults the mirror only when the cookie is fully ABSENT, never when it reads back WRONG — a stale single value or a truncated chunk of a >3000-char Apple session). FIX (detection-independent self-heal, web bundle only — `cookieAuthStorage.js`): every cookie write is read straight back; the first read-back ≠ written value latches storage to localStorage-only for the page session (in-memory/never persisted → a one-off web glitch can't permanently disable SSO; mirror written first → session never lost; healthy browsers round-trip exactly → never latch). Ships in the LIVE bundle → fixes the current 1.0(4) binary too (`server.url = app.in-or-out.com`; SW only precaches /offline.html → no stale-bundle path). Gates: build green, hygiene 7/7, mock logic proof (healthy = no-op/0 storms; broken WKWebView dropping a chunk = latches on first write/0 storms). No migration, no RPC. ⛔ owed: real-iPhone AND real-iPad device walk + Option 2 (`appendUserAgent` UA marker in a fresh 1.0(5), recommended given two strikes). See APP_STORE_REJECTION_HANDOFF.md. | PRIOR session 204 — 🔴 PRODUCTION BUG FIXED — UNIFIED HOME ("Feed") RPC THREW ON EVERY CALL (mig 425). Found via a live browser smoke of the multi-context home screen: `get_unified_home_feed` referenced `players.team_id` + `players.player_token`, but that table has `team` + `token` — so the WHOLE RPC threw 42703 and the client swallowed it, leaving the Feed silently showing "Nothing coming up" for EVERY multi-context user since it shipped. A SECOND bug in the same squad block: it keyed squad membership off `players.team` (NULL for linked players) instead of `team_players` — mirrored the working `get_user_relationships` resolution (players→team_players→teams→schedule) and added `s.active=true` + an `> now()` lower bound to match the other three blocks' upcoming-only semantics. Read-only SECDEF RPC, return shape unchanged → no JS wrapper/mapper change. Gates: rpc-security PASS (SECDEF/search_path pinned/single-overload/authenticated-only — anon not granted), live verify (RPC now returns 200 not 400; the corrected joins resolve Alex's two real squads + games — the feed is correctly empty ONLY because both demo games are currently flagged `live`, which the feed deliberately excludes). EV N/A (read-only). LESSON (reinforces feedback_run_app_playwright): build + hygiene + EV could not see this — only a real browser smoke caught the swallowed 400. ⛔ owed: positive end-to-end render once a non-live upcoming game exists in demo data. **NEXT FREE MIG = 426.** | PRIOR session 194 — 🟢 VENUE "ALL GROUNDS" SINGLE-CALENDAR VIEW (UI-only, NO migration — venue app only). The BookingsView ground switcher gains an "All grounds" option that lays every same-company venue on ONE pitch calendar, grouped by venue (a venue header band → its pitch columns), reusing the existing occ-block visuals + filter chips; `get_operator_pitch_occupancy` ALREADY returned every venue's {pitches,occupancy} so NO backend change. New `apps/venue/src/views/AllGroundsGrid.jsx`; per-column bookability (home `is_self` site tappable-to-book, other sites view-only — empty-tap is a no-op + foreign blocks not actionable, since booking needs that site's own console). Plus TWO operator-feedback polish fixes on the SHARED occ-block styling (so they improve ScheduleGrid + DayAgenda too): (a) Training vs Match now read by a COLOUR-FILLED type badge + filled glyph chip (Training=violet `--train`, Match=blue `--info`) instead of a near-identical 12px mono glyph; (b) pitch-visibility chips moved OFF the loud accent-yellow onto a calm `.cal-pitch` style (showing = quiet raised `--bg-4`, hidden = faded + struck-through). IMPORTANT SCOPE NOTE: this calendar is PITCH-only (renders `pitch_occupancy`: bookings/league/match/training/maintenance) — rooms/classes/equipment are NOT on it (separate rail surfaces, separate schedules). Gates: build venue PASS, hygiene 7/7 (AllGroundsGrid + BookingsView + CalendarFilters), hex-clean (no new hex), Playwright all-grounds verify (both demo venues' 13:30 blocks side-by-side on one grid, switcher single↔all, hidden-pitch struck-through + venue header re-spans, 0 console errors); demo "CAL DEMO" seed removed + leak-check 0; casual-regression N/A (no apps/inorout or packages/core change), EV/rpc-security N/A (no backend). ⛔ OWED: venue MANUAL prebuilt-static deploy + real-device eyeball. **NEXT FREE MIG = 416 (unchanged).** | PRIOR session 193 — 🟢 SPLIT THE "COMPETITION" FEATURE SWITCH IN TWO (mig 415, Cycle 2 of the venue two-leagues work). NO new production bugs; ONE LATENT BUG FIXED: `venue_upsert_club_fixture` had NO feature-flag guard — the Phase-2 (mig-413) rewrite silently dropped the `competition` guard mig-399 added — re-added here keyed on the new flag. NEW column `club_features.club_leagues bool NOT NULL DEFAULT true` (default-all-on → existing venues byte-identical). `competition` now governs the Internal League surface only; new `club_leagues` governs the Club Leagues (grassroots external-opponent fixtures + matchday links) surface. Plumbed through `_club_feature_enabled` (CASE), `get_venue_feature_flags` + `venue_get_feature_settings` (return it), `venue_set_club_feature` + `venue_set_club_features` (allow-list + UPDATE + all-on prune + applied output). The 5 Club-Leagues write guards move competition→club_leagues: 4 via in-place single-literal swap on the live body (mig-401 precedent, each had exactly ONE `competition` literal — verified), upsert via full re-apply (+guard). Venue UI: rail `fixtures` item flag→`club_leagues` (Dashboard + featureRelevance ITEM_KIND + a team-sport discipline branch); FeaturesView gains a "Club Leagues" toggle + relabels "Competition"→"Internal League"; every Quick-setup preset gains an explicit `club_leagues` key. Trigger = operator's explicit setup/preset choice (default both-on), NOT data-inference. Gates: rpc-security PASS (10 fns single-overload/SECDEF/search_path; writes anon+auth, `_club_feature_enabled` stays internal), EV 7/7 + leak 0 (own `_e2e_` venue+club+league: default-on creates / **competition OFF but Club Leagues still writes** [the split] / club_leagues OFF blocks fixture+league create / flags-reader reflects split / re-enable / invalid-feature rejected / bulk all-on prunes the row incl club_leagues), build venue clean + hygiene (my diff hex-clean), Playwright smoke PASS (Features shows Internal League + Club Leagues toggles per club, Club Leagues toggle round-trips [write accepts the new flag], 0 console errors, demo restored — toggle-all-on pruned the row). ⛔ owed venue deploy + device eyeball. **NEXT FREE MIG = 416.** | PRIOR session 192 — 🟢 VENUE "TWO LEAGUES" RENAME + CLUB-LEAGUES FLOW FIX (Cycle 1, venue app only, NO migration). NO new production bugs; a UX/IA cleanup. Venue OS had TWO unrelated "league" tools both saying "league": the **Fixtures** rail item (grassroots `club_leagues`/`club_fixtures`, external opponents + matchday links) is renamed **"Club Leagues"**; the **Leagues** rail item (registered-team `leagues`/`seasons`/`competitions` + standings) is renamed **"Internal League"**. Label-only (route ids `fixtures`/`league` + `competition` flag unchanged → routing/deep-links/gating untouched); renamed in Dashboard rail + `TITLES`, `LeagueView` heading, `LeagueTable` empty-state pointer. FLOW FIX on the Club-Leagues (Fixtures) screen: league now **auto-selects** (first, or last valid pick on reload) so fixtures + "Add fixture" are visible immediately; "New league" demoted to a secondary toggle button beside the league switcher; a club with NO leagues gets one clear "Create your first league" CTA (was: empty form + empty "— choose a league —" dropdown that didn't auto-select). NO DB/backend change → EV/rpc-security N/A; apps/inorout untouched → casual-regression N/A. Gates: build venue clean, hygiene (pre-existing MembershipsView `#3030FF`/`#fff` hex untouched, my diff hex-clean), Playwright smoke PASS (rail reads Club Leagues + Internal League; Finbar's FC auto-selects "U12 Saturday League" + 3 fixtures + Add-fixture immediately; New-league toggle reveals/cancels; Demo Boxing Club [no leagues] shows the Create-first-league CTA; Internal League page title + New-season wizard intact; 0 console errors). QUEUED Cycle 2 (mig 415): split the single "Competition" feature switch into two (Internal League vs Club Leagues) so each shows independently — trigger = operator setup/preset choice, default both-on. ⛔ owed venue deploy + device eyeball. **NEXT FREE MIG = 415.** | PRIOR session 191 — 🏁🏁 MULTI-VENUE PILOT #7 PHASE 3 BUILT (mig 414, pitch occupancy / clash protection + cross-site calendar) — **EPIC COMPLETE (Phases 1–3).** NO new production bugs; ONE STALE DOC FIXED: SCHEMA.md listed a `club_sessions.duration_mins` column that does not exist (and `club_session_series` start/end-date names were wrong) — corrected this commit (occupancy uses a fixed 60-min slot). Club sessions + fixtures with a pitch now reserve `pitch_occupancy` via per-table TRIGGERS (`tg_sync_club_session_occupancy`/`tg_sync_club_fixture_occupancy`) covering every create/update/cancel/void/delete path, so they show busy on the venue calendar and a clashing activity is hard-blocked `slot_unavailable` (existing EXCLUDE constraint fires inside the trigger, re-raised P0001; no displacement). `source_kind` CHECK extended (+`club_session`,`club_fixture`). NEW reader `get_operator_pitch_occupancy` returns per-venue {pitches, occupancy} across all same-`company_id` venues → venue BookingsView ground switcher (others view-only); `get_pitch_occupancy` refactored to a shared `_pitch_occupancy_detail` builder (existing-kind output byte-identical, verified live) + club-kind detail (team/manager-initials). Calendar blocks render an ICON + Training/Match tag + manager initials (not colour-alone); Training/Match filter chips added. inorout manager path passes a venue but NO pitch → never writes occupancy, cannot clash (casual byte-identical; core change additive +10/-0). Gates ALL PASS: rpc-security (2 readers SECDEF/search_path/single-overload + 2 helpers locked anon/auth), EV 8/8 + leak 0 (own `_e2e_` 2-venue same-company club + 1 foreign venue: session reserves → session-vs-session + fixture-vs-session clash both rejected slot_unavailable → cancel/void/delete release → reader scopes to operator, excludes foreign venue), build venue+inorout + hygiene 7/7 on changed files (pre-existing MembershipsView `#3030FF`/`#fff` hex untouched, my diff hex-clean), casual-regression additive-only (no apps/inorout/src touched, inorout never calls get_pitch_occupancy), Playwright smoke 0 console errors (Match block renders icon+tag+"DO" initials, Match filter retains/Training-only excludes, ground switcher loads sibling venue). DEFERRED follow-up: create-a-session/match-from-a-calendar-tap + mobile booking polish for that flow. ⛔ OWED: manual venue deploy + real-device walk + a 2-venue UI walk once pilot multi-venue data exists. **NEXT FREE MIG = 415.** | PRIOR session 190 — 🟢 MULTI-VENUE PILOT #7 PHASE 2 BUILT (mig 413, cross-venue fixtures; PR pending; Phase 1 mig 412 merged PR #84 first). NO new production bugs; ONE LATENT BUG FIXED: `get_club_fixture_matchday` derived the ground from `cl.venue_id` (the league's HOME venue), so a fixture played at the club's OTHER same-operator venue would have shown the WRONG ground on the public matchday link — now derives from `COALESCE(pitch.venue, league.venue)`. Same-operator only (REUSES the live Phase-1 guard `_venue_in_club_operator`). **Function-only — NO schema/column/table/guard change.** `venue_upsert_club_fixture` pitch gate relaxed from "pitch ∈ caller venue" → "pitch ∈ caller venue OR a same-operator club venue" (the `= caller venue` short-circuit keeps single-venue clubs incl. NULL `company_id` byte-identical; the guard is consulted ONLY for a different venue → no regression); `venue_list_club_fixtures` += `venue_id`/`venue_name`/`venue_address` (from `COALESCE(pitch.venue, league.venue)`); `get_club_fixture_matchday` ground = pitch's venue (fallback league venue), return keys UNCHANGED → no consumer change (MatchdayScreen byte-identical for single-site). NO `packages/core` or `apps/inorout` change → casual-regression N/A (only the venue `MembershipsView` FixturesTab touched: same-operator venue picker shown when >1 site + pitch picker now from `venueListClubVenues` `playing_areas[]` + cross-site venue label on cards). Gates: rpc-security PASS (3 fns single-overload/SECDEF/search_path/anon+auth; guard stays internal), EV 9/9 + leak 0 (own `_e2e_` 2-venue same-company club + a different-company venue + a not-in-club venue + a NULL-company single-venue club: authorise venue B / matchday shows venue B ground / list carries venue B / reject diff-operator@X / reject not-in-club@Z / own-venue@A ok / update A→B ok / update→X reject / NULL-company own-pitch ok), build venue + hygiene 7/7 (pre-existing grading `#3030FF` + QR `#fff` hex untouched, my diff hex-clean), Playwright smoke PASS (public MatchdayScreen renders correct ground+address+rules, 0 errors; venue FixturesTab create-fixture with pitch-from-playing_areas, 0 errors, demo left byte-identical), live read regression (both existing demo fixtures still resolve their ground). ⛔ OWED: manual venue deploy + device eyeball + a 2-venue UI walk once pilot multi-venue data exists (demo is single-venue → the venue picker can't render against demo). **NEXT FREE MIG = 414** (Phase 3 pitch occupancy / cross-site calendar). | PRIOR session 189 — 🟢 MULTI-VENUE PILOT #7 PHASE 1 BUILT (mig 412, venue-anchor club sessions; PR pending). NO new production bugs. Same-operator only. `club_sessions`/`club_session_series` += `venue_id`/`playing_area_id` (nullable, backfilled to each club's single current venue → byte-identical, verified 3/3 live); new same-operator guard `_venue_in_club_operator` (internal, REVOKED anon+auth); 5 session write RPCs gain `p_venue_id`/`p_playing_area_id` (old overloads DROPped, re-granted); readers `member_list_upcoming_sessions`/`club_list_sessions` return venue+address; `venue_list_club_venues` += `company_id`/`playing_areas[]` (additive, feeds the picker). UI: venue SessionsView venue/pitch picker + venue filter + labels; inorout manager CreateSessionModal optional venue picker (Option B) + member card/detail show home venue+address. Gates: rpc-security PASS (9 fns), EV 11/11 + leak 0 (own `_e2e_` 2-venue same-company club: authorise venue B / reject diff-company / reject not-in-club / pitch-mismatch / backfill-default), casual-regression PASS (additive-only, no casual surface touched — only SessionsScreen + additive supabase.js wrappers), build inorout+venue + hygiene 7/7 + venue hex clean, Playwright venue smoke PASS (backfill labels render 🏟, pitch picker from playing_areas, create anchors venue+pitch, 0 console errors). ⛔ OWED: manual venue deploy + device eyeball + a 2-venue UI walk once pilot multi-venue data exists (demo is single-venue → the picker/filter can't render against demo). **NEXT FREE MIG = 413** (Phase 2 cross-venue fixtures). | PRIOR session 188 — 🏁🏁 VENUE PEOPLE & SPACES IA **EPIC COMPLETE** — PHASE 5 SHIPPED (final, NO migration — venue app only). NO new production bugs. Drop Customers from the rail + consistency sweep: removed the standalone "Customers" rail item from Dashboard.jsx (TABS/TITLES/render/unused default import) — the `venue_customers` records STAY and stay reachable via the Phase-4 ContactPicker (verified) + the casual-bookings detail/nudge modals; CustomersView.jsx survives (TeamsView still imports its NudgeModal). StaffView rebuilt off bespoke `.staff-grid` cards onto the shared `TabbedPage` + `DataTable` + `ViewSubhead` pattern (Match officials + Venue staff tables, clickable rows → existing RefForm/StaffMemberForm; "Coaches & DBS" reuses the Memberships StaffTab unchanged). Memberships operational overlap deliberately left in place (relocating it is explicit OUT-OF-SCOPE). Backend NONE → EV/rpc-security/casual-regression N/A; apps/inorout untouched. Gates: build venue clean, hygiene 7/7 on both changed files + hex hand-check clean (no hex literals), Playwright smoke PASS (Customers gone from rail, ContactPicker reaches the full customer directory, Staff renders as two tables, tab-switch + row→edit-form work, 0 console errors). The full IA epic (Phases 1–5) is done: Rooms+Timetable combined pages, Teams page, Members+Guardians, settable team contacts, Customers off the rail + Staff on tables. **NEXT FREE MIG = 412 (unchanged).** ⛔ owed manual venue deploy + real-device eyeball. | PRIOR session 188 — 🏁 VENUE PEOPLE & SPACES IA PHASE 4 SHIPPED (mig 411, settable team contacts — venue app only). NO new production bugs introduced; ONE latent bug FIXED: the MembershipsView → Coaches & DBS assign-coach dropdown keyed its `<option>` on `m.id`, but `venue_list_members` rows expose `member_profile_id` not `id` (no `id` key at all) → the dropdown produced empty option values and the assign was effectively unusable; now keyed on `member_profile_id` and the candidate list includes guardians. Phase 4 itself adds two settable contact slots (Main + Secondary) on both Teams tabs: league teams pick from the `venue_customers` directory, club teams from that team's own active manager/assistant/coach (`club_team_managers`). New table `venue_team_contacts` (polymorphic, primary+secondary), write `venue_set_team_main_contact` (gated manage_memberships OR manage_facility, audited), both team readers extended additively, and `venue_assign_team_manager` relaxed to accept a member OR a guardian of a member (so a parent can be made a coach). Gates: rpc-security PASS (5 fns), EV 15-grp + leak 0, build venue + hygiene 7/7 on changed files (pre-existing `#3030FF` grading hex untouched), casual-regression PASS (core additive-only, inorout untouched), Playwright smoke PASS (0 unexpected console errors; demo left byte-identical). ⛔ owed manual venue deploy + real-device eyeball. **NEXT FREE MIG = 412.** | PRIOR session 187 — 🏁 STRIPE FULL BUILD PHASE 6 BUILT (mig 408, collection/chasing/reporting — pay-now links #16, notification de-storm #6.2, operator reconciliation #6.3). NO new production bugs; NO behaviour change until an operator acts. ONE additive NULLable column `venue_charges.pay_url`. #16 — a charge's pay link resolves Stripe-hosted-invoice → else `venues.payment_link` → else manual nudge; `api/stripe-bulk-invoices.js` now persists `hosted_invoice_url` via the new service_role `stripe_set_charge_pay_url`; the #4 reminder email gains a "Pay now" button (`get_membership_reminders_due.pay_url` → `_mailer` template) and the member's in-app My-money pill gains a Pay-now button (`get_my_money.pay_url`). #6.2 — a charge with a Stripe invoice is SUPPRESSED from the cron `payment_due` reminder (Stripe dunns it) + a per-recipient per-tick throttle collapses multi-kind reminders to one email/recipient/tick (deferred kinds not logged → they send a later day). #6.3 — new READ `venue_payment_reconciliation` (raised/paid/outstanding/overdue + collection_rate + Stripe-vs-manual `by_method` split) → venue PaymentsView Reconciliation panel. Gates: rpc-security PASS (4 fns), EV 5-groups + leak 0 (`stripe_set_charge_pay_url` own `_e2e_` fixture; idempotent no-op + audit trail + null-guard proven), build inorout+venue + hygiene 7/7 + venue hex hand-check clean, casual-regression PASS (MemberProfile not in casual inventory, +11 additive, `pay_url` NULL today → byte-identical), Playwright PaymentsView smoke PASS (reconciliation rendered £1,957/£330/£1,048/16.9% + Stripe £0 vs Cash £160/Card £50/Transfer £120, 0 console errors). ⛔ STILL OWED (carried, pre-existing): Phase-3 `invoice.paid` → ledger reconcile on a REAL paying member through the DEPLOYED webhook on a CONNECTED account; real-iPhone walk of the Phase-5 Manage-card button + this Phase-6 Pay-now button (MemberProfile; none of Hard Rule #13's enumerated files touched). **NEXT FREE MIG = 409.** | PRIOR session 186 — 🏁 STRIPE FULL BUILD PHASE 5 BUILT (mig 407, lifecycle — Billing Portal #11, bulk price change #12/#20, Stripe refunds #13/#21). NO new production bugs; NO schema change (4 new functions only). Operator decision "Option A": a mid-cycle price change applies at the NEXT renewal, never a surprise mid-month top-up → `proration_behavior:'none'` (so scope #20 needs no proration maths). Season-schedule members are excluded from the bulk price push (re-priced at next season). Refunds only INITIATE (`api/stripe-refund.js`, amount re-computed server-side: full / pro-rated-unused via `_prorated_first_charge` / custom) — the EXISTING `charge.refunded → stripe_record_refund` loop reconciles to the ledger. Billing Portal (`api/stripe-billing-portal.js`) hands a member to Stripe's hosted card/cancel page; cancel is reconciled by the EXISTING subscription.updated/deleted webhook. Our `amount_pence` only syncs AFTER Stripe accepts a push (`stripe_set_membership_price`). Additive — every existing member byte-identical until an operator acts. Gates: EV 5-groups + leak 0, rpc-security PASS (single overloads / search_path pinned / `stripe_set_membership_price` service_role-only), build inorout+venue + hygiene 7/7, casual-regression PASS (no casual surface; `supabase.js`/`index.js` additive-only). ✅ **STRIPE TEST-MODE WALK PASSED (s186, sandbox `acct_…1maH`, sk_test):** (1) PRICE CHANGE — a test-clock monthly sub (£15) had a new £17 immutable price swapped onto its item with `proration_behavior:'none'`; mid-cycle there was NO proration invoice (Option A held) and after the clock advanced past renewal the next invoice billed **£17** (first invoice stayed £15). Clock deleted (cascade; 0 remaining). (2) REFUND — a real test PI charged £15 (`ch_…`), `stripe.refunds.create(charge,500)` refunded £5 on Stripe (`re_…`, `amount_refunded=500`); the webhook's `charge.refunded` branch was then exercised by calling `stripe_record_refund(ch_…,500,re_…)` against an ephemeral fixture whose `external_ref` was that REAL charge id → refund row landed, charge status → `partial`, net £10, idempotent on the refund id; leak-check 0. ⛔ STILL OWED (narrow, pre-existing): the carried Phase-3 `invoice.paid` → ledger reconcile on a REAL PAYING member through the DEPLOYED webhook on a CONNECTED account (needs a connected-account checkout — the webhook delivery mechanism itself was proven s185). Real-iPhone PWA walk owed on the member "Manage card / cancel" button (MemberProfile; none of Hard Rule #13's enumerated files touched). **NEXT FREE MIG = 408.** | PRIOR session 185 — 🏁 STRIPE FULL BUILD PHASE 4 RUNTIME WALK PASSED + 2 PRODUCTION WEBHOOK BUGS FIXED (no migration — NEXT FREE MIG STAYS 407). The Phase-4 season-schedule + dated-anchor flow was proven end-to-end against LIVE TEST-MODE Stripe: a real `checkout.session.completed` verified (200/Recovered) and created membership `442877d5` with `stripe_schedule_id=sub_sched_…`, `phase_end_at=2027-06-30`, `billing_starts_at=2026-09-01` (the Stripe sub is `status:trialing`, `billing_cycle_anchor`=1 Sept — the future anchor fires). The **auto-stop** was then proven via the Stripe CLI: a throwaway test-clock subscription converted to a schedule mirroring the webhook (`from_subscription`→`end_behavior:'cancel'`→`end_date`), clock advanced past the end → `subscription.status=canceled` + `schedule.status=completed`, clock deleted (verified clean). **TWO stacked webhook bugs, each a 100%-outage, found + fixed (GO_LIVE #19):** (1) RAW-BODY DRAIN (fix `6f4ac8b`) — on the bare `@vercel/node` runtime (this is a Vite app, NOT Next.js) `req.body` is a lazy getter that PARSES the JSON + CONSUMES the request stream on first access; `readRawBody` touched it in a guard, so Stripe signature verification ran against an empty body → `400 bad_signature` on every delivery. The Next.js `config.api.bodyParser=false` export is IGNORED by `@vercel/node`. Fixed by reading the stream directly, never touching `req.body`. (2) WEBHOOK-SECRET MISMATCH (config) — Vercel `STRIPE_WEBHOOK_SECRET` was `whsec_hT…` but the live Connect destination signs with `whsec_zP…`; re-copied to Vercel prod+preview and redeployed. ⚠️ GO-LIVE TRAP recorded: the LIVE Connect destination has its OWN `whsec_…` — must be re-copied once at Phase 7 or this exact 100%-failure returns day one. The webhook is a SINGLE platform-level Connect endpoint receiving all connected accounts' events — clubs/venues connect their own Stripe via Express but never touch the webhook/secret. ⛔ STILL OWED: the carried Phase-3 `invoice.paid` → ledger reconcile walk (needs a real PAYING member — this season sub is trialing/£0 until Sept, so it doesn't exercise a paid invoice). NEXT = Phase 5 (mig 407: Billing Portal, bulk price change, Stripe refunds). **NEXT FREE MIG = 407.** | PRIOR session 184 — 🏁 STRIPE FULL BUILD PHASE 4 (mig 406, fixed-term & dated billing — scope #9/#10/#19). NO new production bugs. Season-schedule plans (tier `pricing_model='season'` on a recurring cadence) now auto-stop at season end via a Stripe Subscription Schedule (`end_behavior:'cancel'`); early joiners are anchored to the season start (`trial_end`, pay nothing until then); late joiners pay ONLY the remaining season as EQUAL instalments computed by us (`_season_instalment_plan`, rounded down — member's favour); season `mode=payment` one-offs now land in the ledger (`stripe_record_season_payment`) so they show in `get_my_money`. Renewal cron extended to skip schedule-backed / future-anchored subs. Today's open-ended monthly + season one-off paths are BYTE-IDENTICAL until a tier opts into a season schedule. Gates: EV 7-groups + leak 0, rpc-security PASS, build + hygiene clean, casual-regression N/A (no apps/inorout/src). ⛔ OWED before merge: Stripe TEST-MODE test-clock walk (prove auto-stop + the future anchor) + the carried Phase-3 invoice→paid→reconciled walk — both need Stripe test keys / a Connect test account. **NEXT FREE MIG = 407.** | PRIOR session 183 — 🏁 STRIPE FULL BUILD PHASE 3 SHIPPED (mig 405, mass invoicing — scope #6/#7/#8/#18). NO new production bugs. NEW: `venue_billing_runs` table + `venue_charges.billing_run_id`; 3 venue write RPCs (`venue_bulk_charge_preview` READ-gated, `venue_bulk_charge_commit`, `venue_void_billing_run`), 1 read (`venue_list_billing_runs`), 1 service_role webhook RPC (`stripe_record_charge_payment`), internal `_bulk_cohort_memberships`. Cohorts (tier/club/team) resolve to active memberships; one charge per included member keyed `source_type='membership'`/`source_id='<mid>:run:<run>'`/`billing_run_id` → surfaces in get_my_money verbatim. Pay-online runs get one Stripe Invoice per member via new `/api/stripe-bulk-invoices.js`; `invoice.paid` one-off webhook branch reconciles via `metadata.iorout_charge_id`→`stripe_record_charge_payment`. `get_my_money`+`venue_get_charges` rebuilt to LEFT JOIN the run label (no new field). UI: apps/venue PaymentsView 4-step wizard + billing-run list w/ Void-run. Gates: EV 8/8 + leak 0 (preview counts+skip reasons, pro-rated £9, commit source_id, stripe-paid+idempotent, re-commit dedupe, void→refund, void-releases-dedupe, 5 error paths), rpc-security PASS (single overloads/secdef/search_path; stripe RPC service_role-only), build inorout+venue + hygiene 7/7 + venue hex hand-check clean, casual-regression N/A (no apps/inorout/src). ⛔ OWED before merge: Playwright wizard smoke + Stripe test-mode invoice→paid→reconciled walk. **NEXT FREE MIG = 406.** | PRIOR session 182 — 🏁 STRIPE FULL BUILD PHASE 2 SHIPPED (mig 404, unified member money view — scope #4+#5). NO new production bugs. ONE new authenticated READ resolver `get_my_money()` (SECDEF STABLE, search_path pinned, single overload, anon REVOKED + authenticated grant per mig-175 gotcha, read-only → no audit). Aggregates the signed-in human's WHOLE money picture across streams: casual match fees (`get_my_payment_history` folded in UNCHANGED, whole-pounds, separate array) + own memberships + memberships paid as guardian (own ∪ payer_profile_id ∪ member_guardians, deduped; membership charges matched from venue_charges via split_part(source_id,':',1), paid_pence from non-voided venue_payments). Pence vs pounds kept in SEPARATE arrays — never summed. UI = additive "My money" section in MemberProfile.jsx (authed surface); casual surfaces (PlayerView/PlayerProfile) byte-unchanged. Gates: rpc-security PASS, EV 8/8 + leak 0 (own `_e2e_` payer w/ own+child membership+casual fee → all three streams aggregate, who_for tagged, unauth rejected), build PASS + hygiene 7/7, casual-regression PASS via additive-diff (packages/core +20/-0, no casual wrapper touched), Playwright authed walk PASS (guardian demo user tarny+family → "My money" renders child's Demo Boxing Club Junior £15/mo membership, correct who_for tag, 0 new console errors). DECISION: season one-off (mode=payment) Stripe payments deferred to Phase 3 (Phase 1 recorded invoice.* only). ⛔ real-iPhone PWA walk OWED (MemberProfile "My money" section; note none of Hard Rule #13's enumerated files — App.jsx/PlayerView/routing/manifest/client-config — were touched). **NEXT FREE MIG = 405.** | PRIOR session 180 — 🏁 VENUE OS NAV EPIC COMPLETE — PHASE 4 SHIPPED (rail modulation wiring, NO migration; closes backlog #10 nav + #11 modularity). NO new production bugs. Phases 1–2 already wired the flag+discipline rail gates; Phase 4 gated the last always-on items that belong to a feature — Teams + Players (league/competition roster) now carry flag:competition (nav+route, auto via VIEW_FLAG) + the competition discipline kind in featureRelevance.js. Read-only directories → no write RPC to guard → no migration. Gates: venue build + hygiene 7/7, discipline 7/7 unit, Playwright collapse proof on demo (all 3 clubs Match-day-only + venue Bookings-only → rail 21→9, Programmes+Competition groups hidden, Teams/Players gone → restored full + 0 rows, 0 console errors), casual-regression clean (apps/venue only). 18→~8-per-configured-club target met. ⛔ owed real-device venue walk (Phases 0–4). NEXT FREE MIG = 403 (unchanged — Phase 4 had no migration). | PRIOR session 180 — VENUE OS NAV PHASE 3 SHIPPED (mig 402, package presets). NO new production bugs. Named preset bundles ("Quick setup") applied via 2 atomic/audited bulk RPCs venue_set_venue_features(token,jsonb) + venue_set_club_features(token,club_id,jsonb) — shortcuts on top of the flags (flags = truth; commercial tier decision deferred, no tier enum). Dependency closure (coaching on → memberships forced on) so a preset can't land invalid; only present keys change; row written only on OFF + pruned all-on (no-row=on preserved); anon+authenticated grant (backdoor). Preset catalogue lives CLIENT-SIDE (FeaturesView VENUE_PACKAGES/CLUB_PACKAGES) so re-bundling = one-line edit, no migration. Gates: rpc-security PASS, EV 6/6 + leak 0, build + hygiene 7/7, Playwright boot smoke on demo (League-club preset applied to Finbar's FC → DB matched + lock lifted + highlight moved → Full-club restored to 0 rows, 0 console errors), casual-regression PASS via additive-diff. ⛔ owed real-device venue walk (Phases 0–3). NEXT FREE MIG = 403. | PRIOR session 180 — VENUE OS NAV PHASE 2.5 SHIPPED (mig 401, membership-scope refactor; cross-VENUE only, cross-club deferred). NO new production bugs. Membership eligibility moved from `venue_memberships.venue_id =` → club SCOPE so a multi-venue club's member is honored across all the club's venues. Surface = 6 gates (member_book_class_session/_appointment, member_purchase_class_package, member_join_club_team, member_list_trainers, member_get_venue_membership_pass); NO new column (scope key = existing club_id, set on all 23 live rows); 2 helpers (_membership_covers_venue + _member_entitled_at_venue, STABLE/SECDEF/internal-only). Predicate swapped via exactly-once-asserted regexp_replace on the live body (mig-075 precedent), baseline-diff verified (post-apply md5==predicted, only the predicate changed). BYTE-IDENTICAL on today's single-venue data (change only ever ADMITS a 2nd venue of the SAME club; never a different club). Gates: rpc-security PASS, EV PASS + leak 0 (fix + no-op + status-filter + reject truth-table), no JS change (return shapes unchanged → casual byte-identical). ⛔ owed real-iPhone walk (member of a multi-venue club books at the 2nd venue — needs a real 2-venue club). NEXT FREE MIG = 402. | PRIOR session 180 — VENUE OS NAV PHASE 2 SHIPPED (mig 400, operator feature toggles A+B+C; default-all-on preserved, NO regression). NO new production bugs. Shipped: A operator toggle UI (new venue rail "Features" item, manage_facility-gated → FeaturesView; RPCs venue_get_feature_settings + venue_set_venue_feature + venue_set_club_feature; a flag row is written only on OFF and pruned once all-on → no-row=on invariant kept literally true); B dependency graph (Coaching→Memberships enforced server-side + reflected as a locked toggle; Payments stays always-on core so its edges are comment-only); C discipline axis (get_venue_feature_flags +`disciplines`; lib/featureRelevance.js hides Classes/Trainers from football-only venues + Leagues/Cups from non-team venues, union, fail-open). GOTCHA CAUGHT BY THE BOOT SMOKE: the shared venue_admin_token backdoor calls venue RPCs as ANON (token is an arg, not an auth session) — an initial authenticated-only lock gave permission denied 42501; fixed to GRANT anon+authenticated (auth enforced inside via resolve_venue_caller + _venue_has_cap), the established venue-RPC shape; mig-175 authenticated-only gotcha applies only to auth.uid()-ONLY RPCs with no token path. Gates: rpc-security PASS, EV 10/10 + leak 0, venue build + hygiene 7/7, discipline logic 15/15 unit, Playwright boot smoke (Bookings OFF → rail+screen+server reject with feature_disabled → ON restores, demo_venue back to 0 rows, 0 console errors), casual-regression PASS via additive-diff (packages/core +34/-0, no apps/inorout touched). The whole demo-runbook-pilot stack (s178 IA + pilot sprint + Phase 1) was PR'd + MERGED to main first (PR #62) per cloud-session discipline. OPEN/OWED: (1) real-device venue walk now owed from Phase 0 AND 1 AND 2 (build/boot-smoke can't catch "tap does nothing"); (2) Phase 2.5 = the membership-scope refactor (`venue_memberships.venue_id =` → membership SCOPE) — ~17 of the 74 guards are on membership/coaching writes that refactor will touch again, expected/accepted; (3) venue-token club-feature writes gate on the UNION of all clubs at the venue (`_venue_club_feature_enabled`) because they only resolve a venue_id today — becomes club-specific once the scope refactor lands. Residual venue-token dead-fallback hex sweep from s178 still open. NEXT FREE MIG = 401. | PRIOR session 179 — VENUE OS NAV PHASE 1 SHIPPED (mig 399, modular feature flags + full 3-layer gate; default-all-on, NO regression — guards inert until a flag is switched off). | PRIOR session 171 — UNIFIED LOGIN SHIPPED (migs 376–377, one-account sign-in for admins AND players, LIVE on main). ONE login-loop scare RESOLVED and confirmed NOT a code regression: a stale Safari resume breadcrumb (`ioo_redirect_to`/`ioo_last_visited` from an earlier demo `/p/` link in the same Safari session) bounced the user back via the App.jsx redirect bridge after sign-in — the OLD code would do the same. Deleting the home-screen PWA does NOT clear Safari localStorage; clearing Safari "History and Website Data" fixed it. Hardened anyway: AuthCallback now clears the resume breadcrumbs on a generic sign-in (`57d563e`) so a fresh sign-in always lands on the account landing. Step 2 reverted as a precaution (`5962813`) then re-shipped (`46807eb`). Device check added to GO_LIVE_ISSUES.md. OPEN follow-ups: rename chooser "MANAGER"→"ADMIN" (optional); native real-device launch/landing/auth walk (owed); Apple "link accounts" flow (deferred until Hide-My-Email dups occur). See SESSION 171 entry below. Next free mig = 378. PRIOR session 149 — CLASSES OPEN/FREE/TRIAL ACCESS SHIPPED (mig 360) — NO new production bugs. Closes the s148 OPT-IN FOLLOW-UP: the classes epic now supports open/free/trial via a per-class-type `venue_class_types.members_only` flag (additive NOT NULL DEFAULT true → existing classes byte-identical). AUDIT CORRECTION to the handoff's "three gate sites": the live `member_claim_waitlist_spot` body has NO membership check (it only validates the existing `offered` row, which inherited the booking gate), so it needed NO change; `member_purchase_class_package` left member-only (operator s149 — packs stay members-only); only `member_book_class_session` changed (membership EXISTS wrapped in `IF COALESCE(members_only,true)`). An ACCOUNT (auth.uid→member_profiles) is NEVER dropped. Flag plumbed through venue_create/update/list_class_type + member_list_class_sessions; operator toggle in ClassesView, member CTA in ClassesTimetable. All gates passed: rpc-security PASS (5 RPCs single-overload/SECDEF/search_path/grants; old 10-arg venue_create_class_type DROPped), EV 5/5 + leak 0 (non-member books open-paid→£10 door charge; open-free→waived/no charge; rejected on member-only; member books member-only; column default true), casual-regression PASS via additive-diff (no casual surface touched — only ClassesTimetable labels + class-type wrapper), boot smoke clean, build inorout+venue + hygiene 7/7. ⛔ real-iPhone PWA walk OWED (a signed-in NON-member books an open class). **Next free mig = 361.** PRIOR session 148 — GYM/BOXING VERTICAL Phase 4 SHIPPED (mig 359, bout / fight record + sparring stats) — **THE VERTICAL IS COMPLETE (Phases 0–4).** NO new production bugs; EV caught + fixed ONE pre-commit consistency gap before any commit: the member-facing headline W-L-D counted sparring rows while the operator list excluded them — folded the `NOT is_sparring` filter into the source as 359b so both reads agree (sparring surfaced separately as `sparring_count`). ONE new RLS-walled table `member_bouts` (soft-void via `voided`, never hard delete; W-L-D-NC derived over non-voided non-sparring rows) + DORMANT additive `player_match.sport_stats`/`matches.sport_stats jsonb` (0 pg_proc refs — football result cascade proven byte-unchanged in casual-regression). 5 new RPCs: operator `venue_record_bout`/`venue_update_bout`/`venue_delete_bout` (soft-void) gated `manage_facility` + audited; reads `venue_list_member_bouts` (operator, incl voided) + `member_get_fight_record` (member via pass_token, excludes voided). Operator: per-member "Fight record" modal in MembershipsView (`FIGHT_RECORD_DISCIPLINES=['boxing']`). Member: MemberProfile "Fight record" section (disciplineLabels.hasFightRecord; boxing only). All gates passed (rpc-security PASS 5 RPCs single-overload/SECDEF/search_path/grants, EV 10/10 + leak 0, casual-regression PASS via additive-diff — only MemberProfile touched + gated on hasFightRecord, sport_stats invisible to all RPCs, no casual football surface touched — Playwright boot smoke 0 app errors, build inorout+venue + hygiene clean). ⛔ real-iPhone PWA walk OWED (MemberProfile Fight record section; needs an authed member on a boxing club). The OPT-IN FOLLOW-UP below (classes free/trial `members_only`+price-0 levers) is now PLANNED for the NEXT session — operator opted in s148; full scope + live audit facts + a paste-ready next-session prompt are in **CLASSES_OPEN_ACCESS_HANDOFF.md** (3 membership-gate sites to modify: `member_book_class_session` / `member_claim_waitlist_spot` / `member_purchase_class_package`; the `members_only` flag goes on `venue_class_types`; an account stays always-required; mig 360 reserved). **Next free mig = 360.** PRIOR session 147 — GYM/BOXING VERTICAL Phase 3 SHIPPED (mig 358, PT / 1-on-1 appointment booking) — NO new production bugs; EV caught + fixed ONE pre-commit gap: the shared `venue_charges_source_type_check` constraint rejected `'pt'`, so the very first priced booking failed — fixed by extending the allow-list (applied live as 358b, folded into the 358 source). 3 RLS-walled tables (venue_trainers / venue_trainer_availability / venue_appointments, partial-unique one-live-booking-per-slot), 11 new RPCs (4 operator CRUD + venue_pt_checkin + venue_mark_appointment_completed + 4 member + member_list_my_appointments). DEDICATED appointments model (NOT capacity=1 classes — locked in handoff). Money rides the existing venue_charges ledger (source_type='pt', door path; settlement DORMANT until live keys). TWO LEVERS decide who can book: an ACCOUNT is always required (auth.uid → member_profiles); `members_only` per-trainer adds the active-membership requirement — members_only=false + price 0 = a free open session (operator answer s147 "A, but B for trials/one-offs"). Trainer = optional staff login (admin_id nullable → also a no-login coach card). Cancellation cutoff per-trainer (0 = free cancel); no-show keeps the charge + bumps member_profiles.no_show_count. Train tab is discipline-gated via disciplineLabels.hasPT (gym/boxing/martial_arts/fitness) so casual football is byte-identical. Operator: new venue TrainersView (Trainers + Appointments tabs, ClassCheckinScanner generalised with an optional `checkin` cb). Member: new `/book` route + BookPT view + ClubNavBar Train tab. All gates passed (EV 9/9 + leak 0, rpc-security PASS 11 RPCs single-overload/SECDEF/search_path/grants, casual-regression PASS via additive-diff — no casual view touched, App.jsx purely additive — Playwright /book boot smoke 0 console errors, build inorout+venue + hygiene 7/7 clean). ⛔ real-iPhone PWA walk OWED (/book Train tab + booking + operator QR check-in; needs an authed member on a PT-discipline club). OPT-IN FOLLOW-UP (no mig yet): the classes epic still hard-requires membership — retrofitting the same `members_only`+price-0 levers to classes would let venues run free/open or trial classes (operator asked s147); not bundled into 358. **Next free mig = 359.** PRIOR session 146 — GYM/BOXING VERTICAL Phase 2 SHIPPED (mig 357, grading / belt progression) — NO new production bugs; EV caught + fixed ONE pre-commit bug: within a single transaction `now()` is constant so two `member_grades` awards tied on `awarded_at` and "current grade = latest" picked the wrong row — fixed by adding a monotonic `awarded_seq` identity column and ordering "current" by `awarded_at DESC, awarded_seq DESC` (also hardens production against same-microsecond awards). 3 RLS-walled tables (venue_grading_schemes / venue_grades / member_grades append-only), 5 new RPCs + get_member_pass (now returns current `grades[]`) + venue_list_members (additive club_id+discipline) extended. Award authority = existing `manage_facility` cap (operator 1A); ladder = ordered named grades + colour_hex + per-grade max_stripes, award carries capped stripes, age bands = separate schemes (operator 2B + martial-arts research). All gates passed (EV 11/11 + leak 0, rpc-security PASS 6 RPCs, casual-regression PASS via additive-diff — every new inorout render gated on disciplineLabels.hasGrading so casual football is byte-identical — + Playwright boot smoke 0 app errors, build inorout+venue + hygiene clean). ⛔ real-iPhone PWA walk OWED (Pass rank chip + Profile Progression; needs an authed member on a martial-arts club). **Next free mig = 358.** PRIOR session 145 — GYM/BOXING VERTICAL Phase 1 SHIPPED (mig 356, sparring/open-mat class-type flag + member `/classes` route reusing the class-session booking model) — NO new bugs; all gates passed (EV 5/5 + leak 0, rpc-security PASS both modified write RPCs, casual-regression PASS via additive-diff proof, build inorout+venue + hygiene clean, Playwright `/classes` boot smoke 0 console errors). ⛔ real-iPhone PWA walk OWED (new route + ClubNavBar Classes tab; needs an authed member on a non-football club). **Next free mig = 357.** PRIOR session 144 — GYM/BOXING VERTICAL Phase 0 SHIPPED (mig 355, club `discipline` identity) — NO new bugs; all gates passed (EV 7/7 + leak 0, rpc-security/casual-regression/build/hygiene PASS). ⛔ real-iPhone PWA walk OWED (flag-gated, club nav labels). **Next free mig = 356.** PRIOR session 143 — CLUB/MEMBERSHIP SYSTEM AUDIT + 3 FIXES (see SESSION 143 entries below): (1) BST recurring-session timezone bug — venue_create_class_series/club_create_session_series/club_manager_create_session_series stored UK wall-clock as UTC, 1h late in summer, fixed AT TIME ZONE 'Europe/London' (mig 353); (2) multi-club nav residual club[0] in MemberProfile + ClubNavBar (PR #14); (3) MemberPass nav club context via get_member_pass club_id (mig 354). Full end-to-end audit found only these — memberships/scheduling/booking/waitlist/check-in/packages/context-wiring all verified live; multi-sport is dormant scaffolding. Next free mig = 355. PRIOR — session 141 — MULTI-CONTEXT NAV Phase 1 — two pre-existing bugs RESOLVED: (1) MULTI-CLUB SELECTION — a multi-club member tapping a specific club always landed on the FIRST club; SessionsScreen auto-selected only when exactly one club existed and never read the tapped one. Fix: carry the chosen club via `?club=<id>` (switcher + legacy club cards append it) and SessionsScreen pre-selects it (validated against active_clubs). (2) STRANDED CLUB/GUARDIAN USERS — `/sessions` and `/profile` had NO bottom nav and ParentHome's bespoke bar had no way back; a club-only member landing on /sessions was stuck. Fix: shared config-driven NavBar via new `ClubNavBar` (Sessions·Pass·Profile — Pass deep-links via the new active_clubs.pass_token; "Classes" deferred until a /classes route exists) on SessionsScreen+MemberProfile; guardian Home rebuilt child-first on `guardian_list_children_sessions` with the unified NavBar; header-avatar `ContextSwitcher` on squad routes (flag-gated). New squad-route nav ships dark behind `teams.multi_context_nav` (default off) so the footballer's app is byte-identical until enabled per team; club/guardian nav is additive. Build: 8/8 monorepo apps clean; hygiene 7/7 every changed file. ⛔ real-iPhone walk OWED before commit (Hard Rule #13 — App.jsx routing/PlayerView/NavBar touched). session 139 — RESOLVED (mig 348, getHeadToHead/getPlayerLeagueTable): Stats → tap a player → Head-to-Head always showed the empty "you haven't played together" state for EVERY player on the normal player route (/p/<token>), even with shared games. Root cause: `getHeadToHead` only had a SECURITY DEFINER path for ADMIN tokens (mig 041); its fallback read `player_match` directly, but that table has RLS on with NO anon/authenticated select policy, so the reads returned zero rows → totalSharedGames 0 → empty state. On a player route `isAdmin` is false so `adminToken` is null → the dead direct path was always taken. The Stats league table itself still rendered because it's derived client-side from match history, which masked the gap (admin showcase route worked, so it was never caught). Fix: new player-token RPCs `get_head_to_head_raw_by_player_token` + `get_player_league_table_raw_by_player_token` (mirror the mig-041/042 admin variants, resolve team via players.token→team_players), threaded `playerToken` through App.jsx→StatsView→HeadToHead→supabase.js. FOLLOW-UP (commit 28821af): the first pass only wired the STANDALONE `view==="stats"` StatsView in App.jsx — but the Stats screen players/admins actually use is the Stats TAB inside PlayerView (PlayerView.jsx:1640, reached via the navbar and via admin `onGoStats`), which passed only adminToken (null on a player route) and no playerToken, so H2H stayed empty after the first deploy. Fixed by passing PlayerView's own `playerToken` (me.token) to that StatsView render. LESSON: when a shared view (StatsView) is rendered from more than one place, grep `<StatsView` and patch EVERY render site, not just the first one found. Verified end-to-end via the live PostgREST endpoint as anon (token vs Ranny → 2 games, HTTP 200) and confirmed new bundle live. Owed: real-iPhone walk on /p/ link. RESOLVED (no mig, AdminView/POTMTiebreakModal): the ADMIN-facing "POTM TIE — YOUR CALL" modal had NO escape — no ✕, no backdrop close, no scroll, no height cap; the only way out was to lock in a winner, so two admins (incl. Rocky) were trapped behind it and couldn't reach admin options. NOTE: there are TWO POTM modals — POTMVotingModal (player, in PlayerView) and AdminView/POTMTiebreakModal (admin, shown only on a vote tie). The admin "too big / can't tap away" complaint is the TIEBREAK one; the first round of fixes wrongly targeted the player voting modal. Fixed the tiebreak modal: capped to calc(100dvh - 40px) flex column (header + footer pinned, list scrolls w/ iOS momentum), always-visible ✕, tap-backdrop-to-close, "Decide later" footer → new onClose (tie re-surfaces next session until resolved). RESOLVED (no mig, POTMVotingModal): voting modal was too tall for small screens — the card was centred with overflow:hidden so on a big squad the footer (skip/close) was clipped off-screen, scrolling didn't work, and there was no backdrop-tap-to-dismiss → admins were trapped. Fixed: card capped to calc(100dvh - 40px) as a flex column (header + footer flexShrink:0, only the player list scrolls) + tap-backdrop-to-close. RESOLVED (no mig, HistoryView/StatsView): an UNPLAYED match (winner NULL) was classified by getResult() as a "draw", so this week's not-yet-played fixture rendered as an amber 0–0 draw — indistinguishable from a real draw. Split "pending" out from "draw" (winner==='D' only is a draw; null→pending, shown neutral grey "NOT PLAYED YET"/"–") and gave real draws a dedicated teal token --draw (distinct from amber, which also means maybe/won-by). Applied across HistoryView result cards + StatsView W/D/L totals + form dots; league-table form pill left amber (PlayerLeagueTable.jsx has pre-existing hardcoded medal hexes that block clean edits — tokenize separately). RESOLVED (no mig, PlayerView): POTM voting modal never reappeared once dismissed without voting. Root cause: a `localStorage` `ioo_potm_seen_<matchId>` flag (added s80 to stop per-open re-popping) was written the instant the modal first showed, and the show-condition suppressed on seen-OR-voted — so tapping away to open the admin panel killed it permanently. Fixed: gate the show purely on `voted` (server-truth) and drop the seen-flag. Modal now reappears on every app open while voting is live UNTIL a vote is cast, then never again; the `nowOpen && !wasOpen` transition guard still prevents within-session re-pop after a dismissal. RESOLVED (no mig, HistoryView): a DRAW (stored as winner='D') rendered wrong in the expanded result drill-down — margin-mode draws showed the "Won by" pill + "?" ("Won by ?"), and the IO prediction chip read "Predicted: Draw · Result: Team D won" (it treated a draw as winner being empty, and interpolated 'D' as a team name). Fixed both to key off getResult()==="draw": margin draws now show "D", and the chip shows "Draw"/marks a draw prediction correct. Also shipped (mig 347): match RESULT note — admins can attach free-text to a result (e.g. "abandoned early due to injury, declared a draw"); see FEATURES/RPCS. session 137 — no new bugs found or fixed. Phase 4 Stripe test clock lifecycle proven: all 5 state-machine scenarios pass, reconciliation cron verified live at 04:00 BST. Critical operational finding: live Stripe keys must go in `platform-clubmanager` Vercel project, NOT `inorout`. System DORMANT pending operator live-key sign-off. session 136 — RESOLVED (mig 332): `get_venue_signup_tiers` used `status='active'` not `'connected'` to detect Stripe connection — tier picker showed no Stripe fork for any venue. Fixed s135, documented s136. RESOLVED (no mig): Stripe webhook endpoint was a platform webhook, not a Connect webhook — `checkout.session.completed` events from the connected account were never delivered. Recreated endpoint in Stripe Dashboard with "Listen to events on Connected accounts" enabled. RESOLVED (mig 335): `stripe_complete_member_enrolment` had `actor_type='member'` in its audit_events INSERT — violated `audit_events_actor_type_check` constraint; every webhook-triggered enrolment silently failed at the audit step. Same pattern as mig 297 fix but this RPC was missed. Fixed 'member'→'player'. RESOLVED (mig 336): after Stripe redirect back to `?checkout=done`, `passToken` was null (component re-mounts fresh after external redirect) so the "Open your membership pass →" link never rendered. New RPC `member_get_venue_membership_pass` + called on MembershipSignup mount — also handles returning-already-enrolled members.) (session 130 — RESOLVED (mig 328): mig 326 write RPCs (club_admin_set_performance_config, club_admin_add_performance_event, club_admin_record_result) all had audit_events INSERTs missing entity_type + entity_id (both NOT NULL since mig 003). Fixed via CREATE OR REPLACE in mig 328. EV 9/9 PASS, leak-clean. Also in session 130 — RESOLVED (mig 327, caught by EV): all 6 write RPCs in mig 327 had the same bug — fixed before commit.) (session 126 — RESOLVED (mig 323): `club_admin_get_standings` crashed at runtime since mig 320 — joined `public.club_admins` which has never existed in any schema. Replaced with `club_team_managers` guard (correct Phase 1-3 pattern). Also shipped: Phase 7B card tracking + auto-suspension. EV 8/8 PASS, leak-clean.) (session 114 — no new bugs found or fixed. Phase 9 Club Merchandise shipped (mig 309); EV caught + fixed a real runtime bug: `venue_list_merchandise` nested-aggregate in PostgreSQL (COUNT inside jsonb_agg) would have silently errored in production — fixed with inner subquery (mig 309b). No change to open tech-debt list.) (session 99 — RESOLVED (mig 297): all 5 member RPCs used actor_type='member' which violates audit_events_actor_type_check constraint. Caught by ephemeral-verify run. Fixed 'member'→'player' across member_register_child, member_update_child, member_accept_consent (from mig 295) + member_self_create_profile, member_enrol_membership (from mig 296). EV 8/8 PASS, leak-clean.) (session 99 — RESOLVED (mig 295): member_register_child, member_update_child, member_accept_consent audit_events inserts were broken — wrong column names (actor_id/event_type/payload vs actual actor_user_id/action/metadata) plus NULL team_id which is NOT NULL. Rewritten in-place, no schema change, all 3 SECDEF/search_path/overload=1 confirmed post-apply.) (session 98 — found pre-existing bug: audit_events column mismatch in Phase 1–5 member RPCs — those audit inserts fail silently at runtime.) (session 89 — STALE-NOTE SWEEP: verified four "open/owed" items against the LIVE function bodies and closed them as already-fixed — (1) "result-save double-charges guests" NOT a code bug: the is_guest=false guard sits on both the owes bump AND the game_fee ledger insert in every admin_save_match_result since mig 206 (predates the s80 report), through 241, to live mig 268; the observed Little K row was historical/hand-created; (2) "result-save wipes the flat paid flag mid-cycle" RESOLVED mig 241 (live in 268) — paid is derived from a LEFT JOIN onto that match's paid game_fee ledger row so it can't diverge; the entry's heading already said RESOLVED, only the body still read "fix owed"; (3) admin_upsert_schedule one-off-date cast RESOLVED by mig 215; (4) superadmin blank screen RESOLVED s79 / GO_LIVE #13 (the BUGS entry still had an "## OPEN" heading). One real fix: removed the shadowed duplicate `minWidth` key in SquadScreen.jsx (kept 220, which already won) — clears the vite warning. Genuinely-open tech debt unchanged: pg_cron bearer secret hardcoded across 7 jobs; Gaffer top-reliable guest filter (defer to Phase 7). session 88 — RESOLVED (mig 268): drawn teams stayed mutable after kick-off — a post-kickoff injured-toggle silently dropped a player from a locked team. Three fixes: un-injure restores a drawn player to 'in'; a kick-off lineup lock (`is_lineup_locked`, lock point = schedule.game_date_time) on the four self-service lineup RPCs; result-save reconciles orphan player_match rows so flat stats can't diverge. EV'd 9 assertions, leak-clean. session 82 — RESOLVED: "Paid" carried into the next game — go-live now clears per-game payment flags (paid/self_paid/paid_by/paid_at), owes untouched, mig 243. session 80 — RESOLVED: debt-state players couldn't finish paying (Confirm button unreachable once "Paid Cash" tapped) + POTM modal re-popped on every app open even after voting — both PlayerView, fixed this session. OPEN: drawn teams stay mutable AFTER kick-off — a post-kickoff injured-toggle silently dropped a player from team B (Footy Tuesdays, this week); two linked bugs filed, live data corrected, no code fix yet. session 79 — RESOLVED ops analytics counted players off players.team (A/B matchday side) not team_players (squad membership), mig 234; RESOLVED apps/superadmin blank screen since first deploy — prebuilt build missing VITE_SUPABASE_* env, see GO_LIVE #13. session 78 — RESOLVED venue Requests inbox confirmed long weekly blocks only partially, mig 236. session 77 — RESOLVED players couldn't save their own nickname, mig 233. session 76 — RESOLVED unreliable "spot opened" reserve notification, mig 230. session 71 — full-codebase bug audit; Batch A COMPLETE (migs 208–211); Batch B COMPLETE (mig 212 create_team TZ + BibsScreen dead-code); Batch C COMPLETE (migs 213–215: notify whitelist, drop cast_potm_vote, update-this-week; + cron DST-safe rollover + dead-code removal). VC parity + guest orphans + HistoryView id-res + self-pay-as-pending-claim all shipped. session 70 — stale guest row RESOLVED e6f9459; session 69 — BST offset RESOLVED 4e351b6; PWA live-update RESOLVED 5edd64f.)*
...
- ✅ **Membership signup is BLOCKED at the "Documents to sign" step for any short policy document** — bug (T1, signup blocker) · effort XS (frontend) · `apps/inorout/src/views/MembershipSignup.jsx:556` gates the "Continue" button behind a scroll-to-bottom read of the `.ms-doc-body` div, but only sets `scrolled=true` inside that div's `onScroll` handler. When the policy text is short enough to fit without scrolling (`.ms-doc-body` scrollHeight == clientHeight, e.g. 90px == 90px — measured live), the `onScroll` event **never fires**, so `scrolled` stays false, "Continue" never renders (only "Back"), and the user is permanently stuck at "Documents to sign 1 of 4". **`member_self_signup` (the enrolment write) is AFTER this step, so no membership is ever created — this blocks the ENTIRE public signup (child AND adult).** Confirmed live via a full throwaway walk of PA Sports' `/q/q_L8hbfm3fXy4`: parent profile + child (via `member_register_child`) were written, then the flow dead-ended at document 1 of 4; PA's 4 policy docs are all short `[Demo placeholder policy text]`. **Fix:** on mount/ref, if `scrollHeight <= clientHeight + 40` call `setScrolled(true)` immediately (content already fully visible = already "read"). Also affects any real club whose policy text is short. · source: PA Sports full-persona walk 2026-07-17 · captured 2026-07-17 · **✅ FIXED 2026-07-17** (branch `fix/membership-signup-short-doc-scroll-gate`): `StepConsent` now measures the `.ms-doc-body` in a `useLayoutEffect` keyed on `idx` and reveals the signature field immediately when `scrollHeight <= clientHeight + 40` (whole doc already visible = already read), while a genuinely long doc stays gated until scrolled (the `onScroll` path is unchanged). Client-only, no migration/RPC/auth change. Proven in a real headless-Chromium browser (390×844): short doc reveals the signature field on mount, long doc gates then reveals after scroll-to-bottom, and each doc in the 1-of-N sequence re-measures. ⛔ owed: supervised end-to-end throwaway-account signup walk on live prod (writes a member+child row — operator/`/prod-verify`). · _resolved_
- ✅ **Coach "Training" screen is broken ("Couldn't load sessions") for any coach who isn't also a paying member** — bug (T1) · effort ~S · the coach/manager `/hub` Training screen (More → Training) lists sessions via `member_list_upcoming_sessions`, which `RAISE`s `membership_required` (P0001 → 400) for a caller with no membership. A volunteer coach has a `club_team_managers` row but no `venue_memberships` row, so the reader hard-fails and the screen shows "Upcoming training · 0 sessions / Couldn't load sessions." Confirmed live: Nihal (PA U7 Dortmund coach) has 1 coaching team, **0 memberships, on 0 rosters** → broken Training tab despite 6 reseeded Dortmund sessions existing. Affects ALL of PA's coaches (none are members). The coach **Tonight** tab is fine (coach-scoped RPCs); only the member-gated Training list breaks. The membership gate is deliberately shared with desktop (manager-hub epic #421), but it makes the mobile coach Training non-functional for coach-only users. **Fix options:** (a) coach Training screen should read via a coach-scoped RPC (`club_manager_*`) instead of `member_list_upcoming_sessions`; or (b) grant PA's coaches a comp membership (data stopgap for the demo). "Add training" still works. · source: PA Sports full-persona walk 2026-07-17 · captured 2026-07-17 · **✅ FIXED (option a — durable) 2026-07-17** (branch `feat/coach-training-sessions-reader`, mig 601): new coach-authorised read RPC `club_manager_list_upcoming_sessions` — byte-identical to `member_list_upcoming_sessions` but the top gate is active `club_team_managers` (coach) status, not `venue_memberships`, so a volunteer coach with no membership loads their team's sessions. Coach screens (TeamManagerTraining/Tonight, ManagerBookings) switched to it; member/guardian screens keep the member reader. EV 5/5 on the unapplied migration (coach sees session; member-reader still raises membership_required; stranger→not_authorised; unauth→not_authenticated; SECDEF+search_path+1-overload+authenticated-only) + leak-check 0 + fresh-context QA & Security reviews CLEAN. ⛔ owed: apply mig 601 + merge (both operator sign-off). · _resolved_
- ✅ **Membership signup routed every returning member through a redundant "create profile" step (silent `profile_exists`)** — bug/UX (T3) · effort XS (frontend) · `apps/inorout/src/views/MembershipSignup.jsx` `handlePathSelect` branched on the `self`/`loadErr` state captured in its render-time closure right after `await loadMemberData()` (which `setSelf`s the profile) — but a setState doesn't update the awaiting closure's local const, so it always read the initial `null` and sent EVERY member who already has a profile to `createProfile`, which no-ops with `profile_exists` (a console 400) then recovers to the child picker. A wasted screen + an error on every signup; the stale `loadErr` also meant a load failure never stopped the flow. **FIXED 2026-07-17** (branch `fix/membership-signup-stale-self-closure`): `loadMemberData` now RETURNS `{self, children, error}` and `handlePathSelect` branches on the returned value; the error path is fixed too (banner shown on the idle screen + `loadErr` cleared per attempt + load-guard reset so a retry re-loads). Found during the #596 post-deploy walk; real-browser walk confirms an existing member now goes straight to the child picker with zero `profile_exists`; fresh-context QA (RESOLVED after a follow-up) + Security reviews CLEAN. · source: #596 signup walk 2026-07-17 · _resolved_
- ✅ **Public venue landing page fires 8 member-only RPC calls that 401 for anonymous visitors** — bug (polish) · effort ~XS · loading the public `/q/<code>` venue landing unauthenticated throws 8 console errors: `member_get_package_balance` and `member_get_venue_membership_pass` each return 401 / `permission denied for function` (42501) for the anon role. Non-fatal — the landing page (classes, hire, "Join as a member") renders fine — but it's noisy failed network calls on the club's public-facing signup entry, and the calls are pointless for a visitor with no membership. Confirmed live on PA Sports' `/q/q_L8hbfm3fXy4`. **Fix:** guard these member-pass reads behind an authenticated-session check before calling. · source: PA Sports full-persona walk 2026-07-17 · captured 2026-07-17 · **✅ FIXED 2026-07-17** (branch `fix/clubpublic-cta-and-anon-401`): both member-only reads — `member_get_venue_membership_pass` (MembershipSignup mount) + `member_get_package_balance` (ClassesTimetable) — now guarded behind `supabase.auth.getSession()`, skipped for anon. Anon-browser walk of `/q/q_L8hbfm3fXy4`: 0 member-only 401s. · _resolved_
- ✅ **Operator People→Members tab shows "No members yet" when a club has memberships but no `venue_customers` rows** — bug/UX (T3) · effort ~S · the operator `/hub` People → **Members** sub-tab reads venue-CRM customers (`venue_customers`), which is empty for PA Sports' `pa_peugeot` (0 rows) even though the venue has **35 `venue_memberships` + 68 `venue_charges`**. So an operator sees "No members yet" while the Payments tab shows £1,175 outstanding and 68 charges — confusing. The club's people ARE visible under People → **Teams** (rosters). This is a data-model seam (club members enrolled as `member_profiles` + `club_team_members` + `venue_memberships` but never written as `venue_customers`). Decide whether the operator Members tab should union in club members / memberships, or whether "Members" is genuinely venue-CRM-only and should show an explainer for club-only venues. · source: PA Sports full-persona walk 2026-07-17 · captured 2026-07-17 · **✅ RESOLVED 2026-07-17 (operator: union + filterable)** (branch `feat/unified-members-list`, mig 603): new shared reader `venue_list_all_members` UNIONs `venue_memberships` (members) + `venue_customers`-with-no-live-membership (pay-as-you-go), deduped, each row carrying tier/status/team/cohort/balance/guardians. Wired into BOTH the mobile /hub `OperatorPeople` Members tab AND the desktop `MembersView` off the SAME reader (can't drift), with client-side filters: name search + person-type + membership status + payment + membership tier + team. Verified live: PA's `pa_peugeot` now returns all **35 members** (was "No members yet"). EV 4/4 + leak 0; fresh-context QA + Security reviews CLEAN; both apps Playwright-walked (mobile + desktop render the unified list + filters, 0 console errors). · _resolved_
only once fixed.*

- ✅ **Club public page fixture countdown is off by one after midday — a game TOMORROW says "TODAY"** — bug · effort ~S · `apps/inorout/src/views/ClubPublic/clubPublicSections.jsx:143-148` computes the hero countdown as `Math.round((fixtureMidnight - Date.now()) / 86400000)`, measuring from the current **time** rather than **today's midnight**. So from ~12:00 local onwards every label shifts a day: a fixture 2 days out reads "TOMORROW", and a fixture tomorrow reads "TODAY". Reproduced deterministically (`TZ=Europe/London`, fixture Sun 19 Jul viewed across Fri 17 Jul: 08:00/11:00 → correct "IN 2 DAYS"; 12:54/18:00/23:00 → wrong "TOMORROW"). **Observed live on `/c/pa-sports` 2026-07-17: hero read "NEXT MATCH · TOMORROW" for a Sunday fixture 2 days away.** Affects **every** club public page, not just PA Sports — this is the flagship marketing surface. Fix = compare midnight-to-midnight (`const t=new Date(); t.setHours(0,0,0,0)`) so the label counts calendar days, not elapsed hours. Frontend-only, no migration, no RPC · source: PA Sports demo readiness audit 2026-07-17 · captured 2026-07-17 · **✅ ALREADY FIXED** — commit `8185af51` ("count calendar days, not elapsed hours", #5) already measures from today's local midnight (`today.setHours(0,0,0,0)`) in `clubPublicSections.jsx` deriveHero; on `main` + live. Fixed by a concurrent PA-readiness pass — this row was just never marked resolved (verify-first, 2026-07-17). · _resolved_
- 📥 **Club public page lists PAST fixtures under "UPCOMING" forever when no result is entered — DECISION NEEDED** — bug/product · effort ~S + product call · `apps/inorout/src/views/ClubPublic/clubPublicSections.jsx:211-213` builds the `upcoming` list with `fx.filter(f => f.status === "scheduled" && f.scheduled_date)` — **status only, no date guard**. Any fixture whose result was never entered stays under "UPCOMING" indefinitely. **Observed live on `/c/pa-sports` 2026-07-17: three 12 Jul fixtures (5 days past) still shown as upcoming**, because `status='scheduled'` with null scores. Affects any grassroots club that forgets to enter a result — i.e. most of them. **Not a straight fix — needs a product call:** a past-dated `scheduled` fixture might be a *postponement* rather than a missing result, so silently hiding it by date could be wrong; a distinct "result pending" state may be truer than either current behaviour or a date filter. Decide the intended semantics before coding · source: PA Sports demo readiness audit 2026-07-17 · captured 2026-07-17 · _open_
- ✅ **Club public page's primary "Play for us" CTA is inert for any club without a website set** — bug · effort ~XS (data workaround) / ~S (durable fix) · `apps/inorout/src/views/ClubPublicScreen.jsx:80` sets `const joinHref = website || "#get-involved"`, where `website = branding?.socials?.website`. When a club has no website configured, the biggest button on its public page — "PLAY FOR US / New players welcome / Get started →" — links to `#get-involved`, **the id of the very section it sits in**, so clicking it scrolls to itself and nothing happens. **Confirmed live on `/c/pa-sports` 2026-07-17** (`/url: "#get-involved"` in the DOM; PA has only an Instagram social set). There is therefore **no path from the public club page into the signup/join flow** for such a club. Data workaround = set `socials.website` to the venue's `/q/<code>` join link. Durable fix = fall back to the club's own join/QR link (a `venue_landing` invite code exists per venue, e.g. PA's `q_L8hbfm3fXy4`) rather than a self-anchor, or hide the CTA when there's genuinely nowhere to send them · source: PA Sports demo readiness audit 2026-07-17 · captured 2026-07-17 · **✅ FIXED (frontend) 2026-07-17** (branch `fix/clubpublic-cta-and-anon-401`): `joinHref` no longer falls back to the `#get-involved` self-anchor — it's null when there's no website, and `GetInvolvedSection` hides the CTA (returns null when there are also no get-involved links). No more button that scrolls to itself. Anon-browser walk: `/c/df-sports-coaching` (no website) renders NO `cp-cta`; `/c/finbars-fc` (has website) shows the real one. ⛔ DURABLE follow-up (separate, tier-2): route a website-less club's CTA to its own `/q/<code>` signup — needs the `venue_landing` code added to `get_club_public` (a migration). Data workaround (set `socials.website` to the /q link) already applied for PA. · _resolved_
- ✅ **Legacy membership tiers created before the `self_signup` flag are invisible on public signup — affects PA Sports AND DF Sports' live £80 tier** — bug/data · effort ~XS per tenant (data) / ~S (durable backfill) · the frontend half of the `self_signup` ambiguity is fixed (see the TierModal fix logged below — checkbox now fail-closed + writes the key explicitly), **but the existing DATA was never backfilled**, so every tier predating the flag stays hidden from its own public join page. Confirmed live 2026-07-17 via `benefits ? 'self_signup'`: **PA Sports** `Junior Membership` + `Adult Membership` (both `flag_present=false`, created 2026-07-08) → `get_venue_signup_tiers('q_L8hbfm3fXy4')` returns **only** the fake `U8 Membership (Free) (demo)` test tier and neither real membership; **DF Sports** `Term Membership` (the live £80 term tier) `flag_present=false`; also `U12 Season (Phase4 walk)` on demo_venue. Net effect: a club publishes its page, a parent taps join, and is offered nothing real. Fix = decide whether to backfill legacy tiers to `self_signup:true` (⚠️ **publishes them to public paid signup** — must be a deliberate per-tier operator choice, not a blanket UPDATE, per the original bug's own reasoning) or surface the absent-flag state in the operator tier list so it's visible rather than silent · source: PA Sports demo readiness audit 2026-07-17 · captured 2026-07-17 · **✅ RESOLVED 2026-07-17 (operator: publish all)**: PA's Junior + Adult were already flipped `self_signup:true` by the demo-readiness pass; DF's £80 Term tier flipped by **mig 602** (branch `feat/df-tier-publish`) — verified `get_venue_signup_tiers` now returns PA's real tiers, and DF's Term tier is `self_signup:true`. ⚠️ PA's tiers have NO `venue_tier_prices` (public signup shows no price) — deliberately left to the operator to set (their commercial data) → pre-demo action. DF has no `venue_landing` invite code yet → DF needs a public /q code provisioned separately before a parent can reach the tier online. · _resolved_

- 📥 **The consumer e2e lane has been SILENTLY SKIPPED — and un-skipping it reveals 7 red tests** — bug/tooling · effort ~M · `qa-suite.sh:46` probes `127.0.0.1:$port`, but `apps/inorout`'s dev script was bare `vite`, which binds **`[::1]` only** — so the probe never connected and `inorout-alex` / `inorout-sam` / `tokens` were reported SKIPPED every run (almost certainly the recurring "env blocker" in the QA log). The consumer app's e2e has therefore not actually been running. The squad-less-owner landing PR pins `vite --host 127.0.0.1` (one IPv4-bound server still serves `localhost` via fallback, so every existing localhost spec is unaffected), which makes the probe pass — and surfaces **7 pre-existing failures, all baselined as red on unmodified `main`, none caused by that PR**: (1) `inorout-alex` ×2 — `classes` "5 class credits on your pass" + `/book` seeded PT appointment (demo-seed drift; same MemberPass family as (3)); (2) `inorout-sam` ×4 — all of `guardian.sam.spec.js`, which still asserts the **retired** `/parent-home` shell + `ParentHomeScreen` that shell-unify PR #6/#6b deleted (stale spec, not an app bug); (3) `tokens` ×1 — `tokens.memberpass-frozen` paused-pass Frozen state. Fix = re-point the guardian specs at the `/hub` guardian track and reconcile the demo MemberPass seed. Until then qa-suite reports FAIL where it used to report SKIP — honest, but red · source: squad-less-owner landing fix (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **`/hub` can spin forever if `get_my_world` fails on the reload — no `myWorldReady` valve** — bug · effort ~S · `App.jsx:1425` gates `/hub` on `myWorld === null`, but the 4s safety valve only sets `myWorldReady`, **not** `myWorld`. Every route INTO `/hub` is a `window.location.replace()` = a full reload that re-runs `getMyWorld()`; if that second call fails (the pitch-side signal flap the loader comments already cite), `/hub` renders `<LoadingScreen/>` indefinitely with **no reachable sign-out** — the App Store 2.1(a) shape. Pre-existing and reachable today for every guardian/`multi` user (via the `:1482` and `:1416` arms); the squad-less-owner arm (PR for Danny's landing fix) newly exposes club owners to it too. Not a one-liner: needs a decision on what `/hub` should render when hats resolve EMPTY (bounce to `/`? an error with sign-out?), and it changes `/hub` for all hub users = PROTECTED. Flagged independently by both the QA and adversarial reviews · source: squad-less-owner landing fix (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **The `club_member` landing arm ignores `hubEligible` — same strand, different arm** — bug · effort ~S · `App.jsx:1484` sends `homeScreenType === "club_member"` to `/sessions` unconditionally, never consulting `hubEligible`. But `nav.js:15` maps `club_memberships[]` → a `member` hat, so that user IS hub-eligible. Not reachable for DF's Danny today (he has no `venue_memberships` row), but **DF PR #6 (public enrol/trial) or any adult-session enrolment on his own club flips him from `squad_only` → `club_member`**, at which point his club-admin hat is un-routed-to again and the landing fix silently stops applying to him. `parent` is already covered at `:1481`; `club_member` is the last uncovered arm. Fix = same shape as the squad-less arm (wait on `myWorldReady`, prefer the /hub hat) · source: squad-less-owner landing fix QA review (2026-07-16) · captured 2026-07-16 · _open_
- ✅ **RESOLVED + LIVE (PR #632, no migration) — 2026-07-24.** `hubResumeTarget()` now resolves the breadcrumb against our own origin and re-checks the NORMALISED path is still under /hub (rejects `..`, backslashes, off-origin); proven against 12 attack/legit vectors, deployed + boot-verified. ⛔ owed: real-iPhone /hub-landing walk (Hard Rule 13). Was: `hubResumeTarget()` accepts a `/hub` breadcrumb by PREFIX, so `/hub/../p/<token>` would pass** — security/tech-debt · effort ~XS · `App.jsx:87` validates the stored breadcrumb with `lc === "/hub" || lc.startsWith("/hub/")`, then hands it to `window.location.replace()`. A value like `"/hub/../p/<someone_else_token>"` satisfies that check but RESOLVES to `/p/<token>` — i.e. it slips past the "Rocky" guard (App Store 2.1(a)) that exists to stop a breadcrumb landing a signed-in user on another identity's token surface. **Not currently exploitable**: the only writer is `MobileShell.jsx:156` (which emits a browser-normalised pathname that cannot contain `..`), and authoring a hostile `localStorage` value needs XSS on the origin — at which point it's already game over. Cheap defence-in-depth: reject `..` (or compare a normalised `new URL(lc, origin).pathname`). Pre-existing, predates the squad-less-owner arm (which is the 3rd caller) · source: squad-less-owner landing fix adversarial review (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **Mig 378 seeds the `team_3v3_*` teams as `casual` while registering them INTO a competition** — bug/seed · effort ~XS · contradicts the platform invariant that a league team is `team_type='competitive'` — mig 158's real registration path hardcodes `'competitive'` and raises `casual_team_cannot_register`, but the 378 seed hardcodes `'casual'` and inserts straight into `competition_teams`, so six casual teams sit in the `3v3 Summer Series` today. Harmless in practice (all six have squad_size 0 and no settings row, and every casual team with a real squad has 0 competitions — incl. Footy Tuesdays), but it makes the data contradict the rule mig 585's casual-gate relies on, and their seeded admin tokens are hardcoded + guessable (`tk_3v3_jag`) — post-585 such a token could relabel a demo league table + the TV display, not just its own board. Fix = seed them `'competitive'`. Found by mig 585's adversarial review · source: PR #567 review (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **`SET search_path TO 'public'` omits `pg_temp` on some SECURITY DEFINER RPCs** — security/tech-debt · effort ~S · e.g. `admin_upsert_settings` pins `search_path=public` only, so `pg_temp` is implicitly searched FIRST for relations — the classic SECDEF search-path shape. Siblings like `resolve_admin_caller` use the safer `'public','pg_temp'`. **Not currently exploitable** (PostgREST exposes no `CREATE TEMP TABLE` path, and function resolution is unaffected), and `check-rpc-security.sh` passes it — its criterion is only "config contains search_path". Worth a sweep across every SECDEF RPC to normalise on `'public','pg_temp'` rather than fixing one arbitrarily; also worth tightening the check's criterion so the gate catches it. Flagged by mig 585's fresh-context security review · source: PR #567 review (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **No season lifecycle — nothing can ever END a season** — bug/feature · effort ~L · `'completed'` is a legal value for both `seasons.status` and `competitions.status`, but **NOTHING in the codebase ever sets it** (0 functions, 0 migrations): every season ever created is still `active`, including two already past their `end_date` (Test Season ended 2026-07-02 with 2 fixtures unplayed; 3v3 Series 2026 ended 2026-07-13 with 5 unplayed). So nothing can finalise standings, archive a table, or roll into a next season, and any rule conditioned on "the season has ended" is unbuildable. **Directly blocks the operator's stated product rule that a league side may rename only at season end** (mig 585 therefore scoped its rename-sync to casual teams only). Needs a real lifecycle + a decision on what "ended" means — end_date passed, all fixtures played, or an explicit operator action (the 3v3 case shows date and fixtures disagree) · source: mig 585 audit (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **~13 admin RPCs hand-inline their own copy of `resolve_admin_caller`'s check** — bug/tech-debt · effort ~M · 40 functions call `resolve_admin_caller`, but ~13 real admin actions (`admin_delete_player`, `add_guest_player`, `admin_approve_guest`, `admin_decline_guest`, `admin_set_vice_captain`, `get_team_state_by_admin_token`, `get_potm_tally`, `get_team_payment_reliability`, the `gaffer_get_context_*` family, …) re-implement the same two-stage admin_token-or-VC-token check inline instead of calling it. So there is no single admin-auth decision point: any future role (the dormant `role_scope` RBAC, the DF Sports model-ii `club_coach`) needs **14 separate edits**, and the copies can drift apart silently. This is also why PR #565 could NOT be fixed server-side — adding a stage-3 to the resolver would have fixed only 40 of ~53, leaving an Admin tab that loads then fails on the writes. Consolidate the 13 onto the resolver (rpc-security-sweep + ephemeral-verify per RPC) · source: PR #565 audit (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **Two columns mean "the squad's name" (`teams.name` + `settings.group_name`)** — tech-debt · effort ~M · the root cause behind mig 585: `create_team` writes both together at birth and `admin_upsert_settings` (the only rename path) now syncs them, but nothing structurally prevents them diverging again, and every reader must know which one it wants — `teams.name` (switcher, My Squads, unified feed, "Your teams" chooser, `guardian_list_child_team`, league tables) vs `group_name` (My View header, Stats, My IO, push titles). Real fix = one field: drop `group_name` and read `teams.name` everywhere (it's the broader one). schema-sync gated (column drop), many readers, not a pilot blocker · source: mig 585 (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **`check-diff-triggers.sh` / `check-live-config.sh` diff against LOCAL `main`, which goes stale** — bug/tooling · effort ~S · both classifiers compare the branch against the local `main` ref rather than `origin/main`. In a worktree (or any checkout where local `main` hasn't been fast-forwarded) they attribute *other people's already-merged* files to your diff — observed 2026-07-16: a 3-file diff (RPCS.md + two 585 migration files) was reported as also touching `584_coach_min_age_16.sql` and `apps/inorout/src/mobile/screens/ClubAdminPeople.jsx` from the just-merged #566, falsely forcing `casual-regression`. Harmless-but-noisy today (it over-reports, never under-reports), but it trains the reader to ignore the classifier — which is exactly the failure mode the mechanical trigger exists to prevent. Fix: diff against `origin/main` (after a fetch) · source: mig 585 cycle (2026-07-16) · captured 2026-07-16 · _open_
- ✅ **RESOLVED 2026-07-24 (operator confirmed `charge.refunded` is now subscribed on the live webhook).** Handler code was already wired (`stripe-webhook.js` case `charge.refunded` → idempotent `stripe_record_refund`); the gap was purely the live-account event subscription. Was: Stripe refunds don't reconcile to the ledger — webhook not subscribed to `charge.refunded`** — money/config · effort XS (Stripe config, no code) · Found in the 2026-07-14 Stripe go-live **Test 3** (live sandbox, demo_venue connected account `acct_1TsshHImDQXNp8aQ`): a `/api/stripe-refund` full refund succeeded in Stripe (`re_3Tt1g1…`, £25) but wrote **no `venue_payments` refund row** (charge still `paid`). The Connect webhook endpoint only enables `account.updated / checkout.session.completed / customer.subscription.created / customer.subscription.updated / invoice.paid` — **0 `charge.*` events ever, 0 refund rows ever** — and there's no cron backstop for refunds, so a refund is invisible in the ledger indefinitely. The code handles it (`charge.refunded` → idempotent `stripe_record_refund`); the event is simply never delivered. **Fix = add `charge.refunded` (+ `invoice.payment_failed`) to the endpoint's enabled events (Stripe Dashboard / API — operator/config, no deploy). Full detail + pre-flight in GO_LIVE_ISSUES.md `## 📥 CAPTURED` (2026-07-14).** · source: prod-verify 520 / Stripe go-live Test 3 (2026-07-14) · captured 2026-07-14 · _open_
- 📥 **(Low severity) `stripe_record_invoice_payment` void-vs-`UNIQUE(external_ref)` poison-event** — bug/tech-debt · effort XS · the RPC's payment-dedupe `EXISTS` filters `voided_at IS NULL`, but the `venue_payments_external_ref_key` UNIQUE constraint does not — so re-recording a first-invoice payment that had been **voided** (`venue_void_payment`) would pass the EXISTS check, attempt the INSERT, hit the unique violation → the webhook 500s and Stripe retries the event indefinitely (poison event). Practically unreachable (a manual void happens long after the webhook retry window). Pre-existing on the `invoice.paid` path; **PR #520 extended the same exposure onto `checkout.session.completed`**. Fix: reconcile the dedup/void semantics with the constraint (include voided rows in the EXISTS, or a partial unique index). Flagged by PR #520's adversarial security review. · source: PR #520 review (2026-07-14) · captured 2026-07-14 · _open_
...
- ✅ **`skills/` vs `Skills/` casing breaks every check-*.sh script on case-sensitive filesystems** — bug · effort ~S · the repo's top-level skills directory was committed as `Skills/` (capital S, confirmed via `git ls-tree`, introduced commit 085dade), but CLAUDE.md and every script under it — including `check-build.sh`'s own internal call to `skills/scripts/check-workspace-deps.sh` — referenced lowercase `skills/`. Silently masked on the operator's Mac (case-insensitive filesystem); on any case-sensitive filesystem (Linux, this cloud sandbox, likely any CI) every check-*.sh script that dev-loop/qa-loop depend on failed immediately with "No such file or directory." **RESOLVED** via `git mv Skills skills` (PR #244, nightly QA 2026-07-03) — also fixed the handful of files whose comments/commands still hardcoded the capitalized form (`survey-backlog.sh`, `check-incident-safeguarding.sh`, `check-diff-triggers.sh`, `.claude/skills/faq-sync/SKILL.md`, `CLAUDE.md`); the `faq-sync` one was a real latent regression caught by fresh-context QA review during that PR · source: qa-loop 2026-07-02 (nightly scripted) · captured 2026-07-02 · resolved 2026-07-03 (PR #244) · **📥 RECURRED 2026-07-06**: new files (`check-advisors.sh`, `check-deploy-freshness.sh`, `check-drift.sh`, `check-plugin-proxy.sh`, `state/advisors-baseline.json`) have since landed under the capitalized `Skills/` path again — same root cause (operator's Mac can't see the drift, being case-insensitive) — and it's live-broken right now: `skills/scripts/check-hygiene.sh:232` calls `skills/scripts/check-plugin-proxy.sh`, which doesn't exist there. Last night's independent nightly run (`fix/check-build-skills-path-case`, still open/unmerged) patched only `check-build.sh`'s one call site pointing it at `Skills/` instead of consolidating; tonight's nightly QA opened a fuller rename+consolidate PR (**#294**, `fix/skills-dir-case-mismatch`) but `main` moved ~80 commits mid-run and it can't merge clean without a rebase that also moves the 5 straggler files — deliberately left unresolved (touching an already-pushed PR's history unattended was blocked by this session's own safety check) rather than force-pushed. The two open branches are mutually exclusive; merging both would reintroduce the bug. Worth a permanent build-time guard against anything ever landing under capitalized `Skills/` again so this doesn't recur a third time — _open_, see PR #294 + its PR comment for full detail. · **re-confirmed 2026-07-16** (nightly scripted, first run in 10 days): identical failure, `check-hygiene.sh` check 8 still errors `No such file or directory` on `check-plugin-proxy.sh`. PR #294 is still open/unmerged and now based on a stale `main` (b5dd0058, ~490 commits behind) — did not attempt a competing fix or rebase per Cloud Session Discipline (one open PR per file surface); needs a human call: rebase #294 or close it and redo against current `main` · _open_
- 📥 **`.claude/hooks/session-start.sh` hardcodes `/Users/tarny/platform`** — bug · effort ~XS · `ROOT="/Users/tarny/platform"` then `cd "$ROOT" 2>/dev/null || exit 0` — on any environment where the repo isn't at that exact path (e.g. a cloud session at `/home/user/platform`), the cd fails and the hook silently no-ops, so cloud/CI sessions never get the session primer CLAUDE.md describes as "no way to skip it." Fix: derive root via `git rev-parse --show-toplevel` instead of a hardcoded path · source: qa-loop 2026-07-02 (nightly scripted) · captured 2026-07-02 · _open_
- 📥 **No Supabase credential provisioning for cloud/headless sessions — blocks the e2e regression net** — bug · effort ~M · a fresh cloud clone has zero Supabase env vars anywhere (`.env`/`.env.local` correctly gitignored; `.env.production` ships `VITE_SUPABASE_URL=""` blank; no injection mechanism found) — apps boot but every page needing the live demo DB renders blank, confirmed via `qa-suite.sh`: all 9 Playwright projects failed on blank-page timeouts (e.g. waiting for "Live Board" that never renders) — not an app regression, purely missing credentials. CLAUDE.md's "Cloud Session Discipline" section encourages cloud sessions but never documents how they're supposed to get demo Supabase creds, so `/qa-loop scripted`'s e2e lane cannot produce real signal on a scheduled/cloud run today. Needs a product decision (secrets-manager injection vs. a checked-in read-only demo anon key vs. something else) more than a pure code fix · source: qa-loop 2026-07-02 (nightly scripted) · captured 2026-07-02 · _open_ · **re-confirmed 2026-07-03**: apps run in dev mode (`npm run dev`) with zero effective `VITE_SUPABASE_*` env — even `apps/inorout`'s one checked-in file is `.env.production`, which Vite's dev server does not load — so `createClient()` never gets a real URL/key client-side, matching the blank-page evidence exactly (see also the two related network-layer findings below, which are a *second*, independent blocker on top of this one).
- 📥 **e2e auth harness's hardcoded demo Supabase project is blocked by this cloud sandbox's network egress policy** — bug/infra · effort ~M · `e2e/lib/auth.mjs` hardcodes a real Supabase URL + anon key (`ktvpzpnqbwhooiaqrigm.supabase.co`) for minting Playwright auth sessions, independent of the app's own (blank) env config above. On the 2026-07-03 nightly QA cloud sandbox, every authenticated `qa-suite.sh` project (`inorout-alex`, `inorout-sam`, `tokens`, `venue-alex`, `venue-sam`, `hq-alex`, `superadmin-alex`) failed identically: `403 Host not in allowlist: ktvpzpnqbwhooiaqrigm.supabase.co. Add this host to your network egress settings to allow access.` Per this environment's own proxy guidance ("do not retry or route around a 403/407 policy denial — report the blocked host"), this was not routed around. The two unauthenticated projects (`display-token`, `ref-token`) also failed, but via the client-side blank-env-var issue above (confirmed: blank white page, no "Host not in allowlist" error — the browser never got far enough to hit the network block). **Net effect: `/qa-loop scripted`'s e2e leg has ZERO valid signal on this cloud environment tonight** — all 9 projects environment-blocked, not app regressions. Needs an infra decision: add `ktvpzpnqbwhooiaqrigm.supabase.co` to this cloud environment's network egress allowlist for scheduled/unattended QA runs, or accept e2e coverage is desktop-only until then · source: qa-loop 2026-07-03 (nightly scripted) · captured 2026-07-03 · **updated 2026-07-04:** reproduced identically — same 7 authenticated projects, same `403 Host not in allowlist` error, zero change. Also newly observed tonight: the two unauthenticated projects (`display-token`, `ref-token`) no longer even reach the blank-white-page state — `browserType.launch` itself now fails first (`Executable doesn't exist at /opt/pw-browsers/chromium_headless_shell-1228/...`). This container's pre-installed Playwright browser cache is pinned at revision 1194 (`chromium-1194`, `chromium_headless_shell-1194`), but the repo's pinned `@playwright/test@^1.61.0` requires revision 1228 for both `chromium` and `chromium-headless-shell`. A second, independent environment-image drift stacked on top of the network-egress gap — the token-based lane's true app-level signal (env-var/blank-page question) still can't be observed until this is fixed too · _open_ · **re-confirmed 2026-07-06**: identical `403 Host not in allowlist` on every authed project again; the two token-based projects (display-token, ref-token) now got far enough to actually launch a browser (a separate sandbox chromium-revision mismatch was worked around locally, not committed) and still timed out waiting for their app's post-token UI, because their Supabase RPC calls silently fail against the same blocked host. Still purely an environment allowlist gap, not an app regression — still unresolved three nightly runs running. · **re-confirmed 2026-07-16** (nightly scripted, first run in 10 days): identical `403 Host not in allowlist: ktvpzpnqbwhooiaqrigm.supabase.co` on all 7 authenticated projects; `display-token`/`ref-token` regressed one step earlier than 07-06 — the browser cache is now at revision 1194 only (`chromium_headless_shell-1194`, no 1228 present at all), so both fail at `browserType.launch` before ever reaching the app. Zero e2e signal produced, fourth consecutive nightly run with this exact pair of environment blockers · _open_ · **re-confirmed 2026-07-20** (nightly scripted, fifth consecutive run with this exact blocker pair): identical `403 Host not in allowlist: ktvpzpnqbwhooiaqrigm.supabase.co` on all 7 authenticated projects — confirmed by direct `curl` to the host from this sandbox returning a bare connection failure (proxy-level block, not an HTTP-layer 403 from Supabase). Browser cache still only has revision 1194 (no 1228); worked around **locally, session-only, not committed** by pointing `use.launchOptions.executablePath` at `/opt/pw-browsers/chromium` (the unversioned 1194 binary) for `display-token`/`ref-token` — both launch cleanly with that override, proving the browser-cache-drift half is real and independently fixable, but both then fail on the SAME Supabase egress block (blank white page, confirmed via screenshot; `getByText('Pre-match')`/`getByText('ENTER PIN')` never render because the anon RPC calls silently fail). So the browser-cache-drift and network-egress gaps are now confirmed as two fully independent, both-still-open blockers — fixing one alone would not restore any e2e signal on this environment. Config reverted before commit, working tree clean · _open_ · **re-confirmed 2026-07-22** (nightly scripted, sixth consecutive run with this exact blocker pair; main at `95bf925`): identical `403 Host not in allowlist: ktvpzpnqbwhooiaqrigm.supabase.co` confirmed via direct `curl` (CONNECT tunnel failed) and via all 7 authenticated `qa-suite.sh` projects failing at `mintSession` with the same 403; `display-token`/`ref-token` still fail at `browserType.launch` (`chromium_headless_shell-1228` missing, only revision 1194 present). No committed workaround attempted. Zero e2e signal, unchanged from 07-20 · _open_ · **re-confirmed 2026-07-24** (nightly scripted, seventh consecutive run with this exact blocker pair; main at `74dafa8`): identical `403 Host not in allowlist: ktvpzpnqbwhooiaqrigm.supabase.co` on all 7 authenticated `qa-suite.sh` projects (`inorout-alex`, `inorout-sam`, `venue-alex`, `venue-sam`, `hq-alex`, `superadmin-alex`, and now also `tokens` — confirmed failing the same way this run); `display-token`/`ref-token` still fail at `browserType.launch` (`chromium_headless_shell-1228` missing, only revision 1194 present, no committed workaround attempted). Deterministic lane (syntax + hygiene + lint + build, all 8 apps via Turbo) ran clean with zero findings — this run's e2e leg produced zero signal, unchanged from prior runs. Also newly observed tonight: a fresh `npm install` (no `--ignore-scripts`) completed with **no** `sharp` postinstall failure — `node_modules/sharp/build/Release` present and built — so the separate `sharp` postinstall 403 logged below did not reproduce this run (worth re-checking on the next nightly before assuming it's durably fixed) · _open_
- 📥 **Fresh `npm install` hard-fails in a network-restricted sandbox — `sharp` postinstall blocked** — bug/infra · effort ~XS (workaround) / ~S (durable fix) · a clean `npm install` on this cloud sandbox exits 1: `sharp`'s postinstall (`prebuild-install`) tries to download `libvips-8.14.5-linux-x64.tar.br` from a GitHub release URL and gets `403 Forbidden` from the network egress proxy. `sharp` is a transitive **dev** dependency of `@capacitor/assets` (iOS/Android app-icon generation only — not needed to run `vite build` or `vite dev`), so `npm install --ignore-scripts` unblocks it with no loss of build capability (verified: all 8 apps still build clean after `--ignore-scripts`). No committed fix applied tonight — a blanket `.npmrc` `ignore-scripts=true` would affect every environment including the operator's own Mac (where the real icon-generation postinstall may be wanted), so this needs a decision, not a silent global flag flip · source: qa-loop 2026-07-03 (nightly scripted) · captured 2026-07-03 · _open_ · **re-confirmed 2026-07-20**: identical `sharp` postinstall 403 on a fresh clone; `--ignore-scripts` workaround still unblocks it cleanly, all 8 apps build clean afterward. Still no durable `.npmrc` decision made · _open_ · **re-confirmed 2026-07-22**: identical `sharp` postinstall 403 on a fresh clone, `--ignore-scripts` workaround still required and still unblocks cleanly (all 8 apps build clean afterward). Still no durable `.npmrc` decision made · _open_ · **not reproduced 2026-07-24**: fresh `npm install` (plain, no `--ignore-scripts`) on this nightly's sandbox completed clean — `sharp`'s postinstall ran and `node_modules/sharp/build/Release` is present. Either this session's egress allowlist now permits the GitHub release URL or the block is intermittent per-sandbox; not yet a durable fix, just a data point. Still no `.npmrc` decision made · _open_
- 📥 **Dead-code audit + decision before any deletion — DECISION NEEDED** — tech-debt · effort ~M (audit) · The new ESLint gate (PR #252) is scoped to ignore `**/_archived_*`, and at least one confirmed-dead file exists: `apps/inorout/src/views/Gaffer/_archived_chatbot.jsx` (not imported anywhere; carries real `react-hooks/rules-of-hooks` violations, which is why it's ignored rather than fixed). **Operator decision (2026-07-03): do NOT delete anything yet.** Before removing any dead/archived code we want a *full, proper audit* of dead code across the monorepo — not a one-off file delete. Scope to define when picked up: (1) enumerate candidates (unimported modules, `_archived_*` files, unreferenced exports/components, dead RPCs/wrappers, orphaned assets); (2) prove each is genuinely unreachable (grep + import graph, not just filename convention); (3) classify keep / delete / needs-owner-call; (4) delete in one reviewed batch with the gate re-run after. Until that audit runs and a decision is made, `_archived_*` stays ignored by the lint gate and nothing is deleted · source: input:2026-07-03 (status-tap session, post PR #252) · captured 2026-07-03 · _open_
- 📥 **Nightly advisor sweep (routine D) has no baseline — re-reports everything every run** — bug · effort ~S · routine D posts ALL advisor findings each night instead of only new/changed ones (e.g. the recurring 115 "RLS enabled + no policy" tables, which are the intended RLS-plus-SECURITY-DEFINER-RPC posture, not a hole). Without `skills/state/advisors-baseline.json` persistence to diff against known-accepted findings, a genuinely-should-be-readable table slipping into the 115 would be camouflaged in the count. Fix: add the baseline file + diff step so D surfaces only deltas. Mirrors issue #229's advisors note ("no prior baseline") · source: Nightly Ops Digest #247 D-sweep 2026-07-06 · captured 2026-07-06 · _open_
- 📥 **CLAUDE.md HOOKS section says "Three hooks" but there are now four** — bug · effort XS · the `## HOOKS — DETERMINISTIC ENFORCEMENT` section opens "Three Claude Code hooks live in `.claude/`", but a fourth was added and merged (PR #304, 2026-07-06): `.claude/hooks/post-merge-prod-verify.sh`, a PostToolUse(Bash) hook that auto-chains `/prod-verify` after an in-session `gh pr merge` (start-anchored match; skips `--auto`/`--help`; never blocks). Doc drift only — update the count to four and add a bullet describing hook #4 alongside the existing three. No code/DB · source: input:2026-07-06 (auto-prod-verify tooling build) · captured 2026-07-06 · _open_
- ✅ **RESOLVED + LIVE (mig 619, PR #633) — 2026-07-24.** Moved the three club-media storage policies' membership check into a SECURITY DEFINER helper `_user_can_manage_club_media` (reads the base tables with definer rights; auth.uid() still resolves to the caller). Applied + verified on prod: security props clean, all 3 policies wired, authorization correct (real manager=true / random=false / anon=false). ⛔ owed: confirming real-upload smoke by an authed club manager. Was: club-media image upload 403s for ALL users — `club_media_insert` policy sub-queries `member_profiles` with no table SELECT grant to `authenticated`** — bug (⚠️ security-sensitive fix) · effort ~S · every club crest/hero/sponsor upload via the `club-media` bucket fails with `permission denied for table member_profiles`. The `club_media_insert`/`update`/`delete` storage policies `EXISTS`-join `club_team_managers⨝club_teams⨝member_profiles`, but `authenticated` has no table-level SELECT on `member_profiles`, so the policy errors before row evaluation — affects all clubs, not just demo. Fix = `GRANT SELECT ON member_profiles TO authenticated` (RLS already row-gates it) **or** move the check into a SECURITY DEFINER helper; run rpc-security-sweep either way. Found during PA Sports demo build 2026-07-08 (worked around by setting `crest_url` inline via the `club_set_page` RPC). Source: storage.objects policies `club_media_insert`/`update`/`delete`.
- ✅ **RESOLVED (mig 613 — 10 room-hire/class RPCs now set `sent_at=NULL` on queue-insert so the cron drains fire)** — was: queue-and-drain notification emails never sent — drains filter `sent_at IS NULL` but the RPC queue-inserts omitted `sent_at` (which defaults `now()`) — bug · effort ~M · `notification_log.sent_at` DEFAULTS to `now()`, but `roomHireNotificationsJob` (room_hire_requested/confirmed/cancelled) and `classNotificationsJob` part-B (class_cancelled / class_instructor_changed / class_waitlist_promoted / class_booking_cancelled / class_spot_offered) in `apps/inorout/api/cron.js` drain on `sent_at IS NULL AND channel IS NULL`. Their queue-inserts in the RPCs — mig 342 (room_hire), mig 339 (class venue-side: `venue_cancel_class_session`, `venue_cancel_class_series`), mig 340 (member-side: `member_cancel_class_booking`, etc.) — **omit `sent_at`**, so every queued row is born with `sent_at=now()` and is NEVER picked up by the drain. Verified live 2026-07-20: **0 of 238 `notification_log` rows have `sent_at IS NULL`; 153 channel-NULL rows sit permanently un-drained.** Net user-facing effect: room-hire acknowledgement/confirm/cancel emails and class cancellation/instructor-change/waitlist-promoted/spot-offered emails have never sent via the drain. Fix = add `sent_at` (NULL) to each affected queue-insert so the row is drain-eligible (the pattern mig 612's `club_capture_lead` owner email-on-lead uses correctly). Touches multiple shipped SECURITY DEFINER RPCs → each needs its own ephemeral-verify. · source: mig 612 / DF trial owner email-on-lead (2026-07-20) · captured 2026-07-20 · ✅ **RESOLVED + LIVE 2026-07-20 — mig 613 APPLIED + PR #614 MERGED (commit be8dd858); all 10 live RPCs carry the fix (fix_live / single-overload / secdef / grants confirmed post-apply, schema cache reloaded).** A `pg_proc` completeness sweep found the true buggy set is **10** RPCs, not the captured ~7: the captured list plus `venue_create_room_hire` (operator direct hire) and `_offer_next_waitlist_spot` (waitlist offer). ⚠️ The LIVE bodies are NOT the original migrations — mig 399 (feature-flag gates) + mig 344 (package-credit) + mig 588 rewrote them, so mig 613 `CREATE OR REPLACE`s from the current `pg_get_functiondef` bodies, changing ONLY the insert (adds `channel=NULL, sent_at=NULL`). NO backfill — 0 rows of any affected type exist in prod (room-hire/classes unused/demo-only), so no stale-email risk. Proof: ephemeral-verify PASS (body-diff all 10 code-identical outside the insert + behavioural `public_enquire_room_hire` → drain-eligible row, leak-check 0) + rpc-security PASS (all 10: secdef/search_path/single-overload/grants unchanged) + fresh-context QA/Security review PASS/SAFE. · _RESOLVED 2026-07-20_
...
**Root cause — a parallel-webhook ordering race.** Stripe delivered the 4 events in one burst; Vercel processed them in parallel serverless invocations. `invoice.paid` was dispatched **before** `checkout.session.completed` finished committing the membership. `stripe_record_invoice_payment` looks the membership up by `stripe_subscription_id`, found none, and returned `{ok:false, reason:'no_membership'}` — **recording nothing**. Because it returns cleanly (no exception), `dispatch()` succeeds and the webhook marks the event **`processed`**, so **Stripe never retries it**. Confirmed against the live RPC body (the `IF v_m.id IS NULL THEN RETURN … 'no_membership'` early-out) and the event timestamps (membership committed 23:33:14.17; `invoice.paid` recorded 23:33:13.73).

**Money is NOT lost — eventually consistent via cron.** `membershipReconciliationJob` ([cron.js ~L1017](apps/inorout/api/cron.js#L1017), 04:00–04:15 UK daily) lists each subscription's PAID invoices and re-runs the idempotent RPC; the membership exists by then, so the £40 records at the next 4am run.

**Impact:** a brand-new member pays, their membership is `active`, but their payment is **absent from the ledger / `get_my_money` for up to ~24h** until the 4am cron repairs it — a real "I paid and it doesn't show" window for the first real members. (No false payment-chase, because no unpaid charge row is created either.)

**Fix applied (PR #520):** in the webhook's `checkout.session.completed` branch — the deterministic "membership definitely exists" moment — after `stripe_complete_member_enrolment` returns the membership, the subscription's latest paid invoice is now recorded into the ledger via the same idempotent `stripe_record_invoice_payment` used by `invoice.paid` and the cron ([stripe-webhook.js `if (subscriptionId && membershipId && latestInvoiceId)`](apps/inorout/api/stripe-webhook.js#L188)). No extra Stripe call — the `latest_invoice` id is read from the subscription already fetched for its price. Race closed: whichever of the two webhooks records the invoice first wins; the other (and the cron) are safe no-ops (idempotent per `membership:inv:invoice_id`, payment external_ref = charge id). Subscription-only, so season one-off (mode=payment, no subscription/invoice) is untouched and still reconciled via `stripe_record_season_payment`. No new RPC / migration / RLS — reuses the live, already-proven `stripe_record_invoice_payment` (mig 403). (Considered + rejected: return 500 on `invoice.paid`+`no_membership` to force a Stripe retry — risks retry storms and is less deterministic.) Prior Phase 5/6 dev walks missed the race because webhooks were replayed sequentially (Stripe CLI) / against a pre-existing membership. **Owed:** post-deploy live-Stripe walk (Test 2 real-time ledger landing + Test 3 refund) against `acct_1TsshHImDQXNp8aQ` once merged + deployed — the fix lives in the deployed webhook, so it can only be exercised end-to-end after deploy.

**Test data left behind:** a live TEST-mode subscription on Alex Demo (`0d…011`) at demo_venue (`sub_1TstLN…`) — cancel it in the Stripe dashboard so it doesn't keep generating monthly test invoices on the demo seed.

---

## BUG (Jul 13 2026) — ✅ FIXED + LIVE (PRs #512–#515, no migration) — in-app tournament create + 4 tournament UX/robustness fixes

Found while shipping **in-app tournament create** (operators & club-admins create from `/hub → Cups → New`, owned to their venue/club so it syncs to the desktop console + public page — see DECISIONS.md 2026-07-13). Fixes folded in across the same set of PRs:

- **#512 — join dead-end.** The join screen (`/tournament/join/<code>`) left the team-name form editable after a used/expired/invalid invite and only errored on submit → now a clear terminal dead-end card ("Can't join this tournament — ask the organiser for a fresh link"). Client-only, reuses the existing RPC error codes.
- **#513 — competition-less register silent 404.** In the mobile `TournamentView`, registering to an OPEN tournament with **no competition** fired `tournament_register_team` with an undefined `competition_id` → silent `PGRST202` 404 (only a generic toast). Now the register form is replaced by a "Registration opening soon" holding state.
- **#513 — `event_date_required` on owned create.** `venue_create_tournament` / `club_admin_create_tournament` REQUIRE `event_date` (unlike `self_serve_create_tournament`, which defaults it) → the in-app create defaults a blank date to today, keeping the field optional in the UI.
- **#513 — desktop panel staleness.** `apps/venue` `TournamentsView` detail panel only refetched on open/mutation, so a team that registered elsewhere wasn't visible without close+reopen → added a **Refresh** button on the detail header. ⚠️ `apps/venue` is a MANUAL deploy — not live until deployed.
- **#514/#515 — public-link clarity + copy.** The app tournament page's public link was labelled "PUBLIC RESULTS PAGE" (hid that it's the register-here link) with a cryptic QR copy button → now **"PUBLIC TOURNAMENT PAGE"** with a clear **Copy link** (native clipboard + `execCommand` fallback for older iOS) + **View page**, plus copy explaining the one link = unlimited multi-team entry (host approves each). Optional **entry-fee** field added to the create form (informational).

Gates per PR: hygiene 8/8 + lint + build (inorout & venue) + casual-regression (zero casual-surface files — all under `mobile/`) + live operator walk (create in-app → DB shows venue-owned + `Main Draw` competition + status `open` → appears on the desktop venue console) + prod-verify on each merge. ⛔ owed: real-iPhone walk (Hard Rule 13); manual `apps/venue` deploy for the desktop Refresh button.

## BUG (Jul 13 2026) — ✅ FIXED (apps/venue, PR pending) — tier "Offer on the QR signup page" checkbox looked ON but saved OFF (editor ↔ public-signup mismatch)
**Found during a Playwright test of the PA Sports self-signup flow:** the public join page (`/q/<code>` → `get_venue_signup_tiers`) showed **no membership to join**, despite PA Sports having two active tiers. Root cause: the tier `benefits.self_signup` flag governs public visibility, and the two sides disagreed on an **absent** flag. The public reader is fail-closed — `COALESCE((benefits->>'self_signup')::boolean, false)` → absent = OFF. But the operator tier editor (`TierModal`, `MembershipsView.jsx`) defaulted the **"Offer on the QR signup page"** checkbox to ON for an absent flag (`self_signup !== false`) AND only wrote the key when true (omitted it when off). So legacy tiers created **before the flag existed** (flag absent — e.g. both PA Sports tiers, confirmed `flag_present=false`) rendered the checkbox as **ticked** while the public page **hid** them. Worse (found by review): opening such a tier and saving it unchanged silently wrote `self_signup:true` → **published it to public paid signup** without the operator choosing to.
**Fix (frontend-only, no migration):** two lines in `TierModal` — (1) default `useState(tier?.benefits?.self_signup === true)` so the checkbox is fail-closed and matches the public reader (absent / new tier → OFF; explicit true/false → as stored); (2) `self_signup: selfSignup` written **explicitly on every save** (true AND false), killing the "absent flag" ambiguity for all future tiers. Net effect is strictly neutral-or-less-exposed — no tier can become public that wasn't. New tiers now default to self-signup OFF (deliberate: publishing a paid tier to public signup is an explicit opt-in). Tier-list "On signup" pill already reads the stored value, so it stays truthful.
**Gates:** hygiene 8/8, lint PASS, `apps/venue` build PASS, `check-live-config` CLEAR, fresh-context QA+security review CLEAN (verified all 4 flag states match the public reader; explicit `false` vs absent identical for every consumer — SQL COALESCE, the `member_self_signup` server enrolment guard, the JS pill). ⚠️ **apps/venue = MANUAL deploy** — merging does NOT push it live; a manual venue deploy is owed. ⛔ OWED: operator console eyeball (batched).

---

## PRODUCTION BUG (Jul 10 2026) — ✅ FIXED (mig 530, APPLIED-live) — Vice Captains can't Approve/Decline a pending +1 ("your admin link may be out of date")
**Reported live (native app):** on Footy Tuesdays (`team_KPaoX8oJYMQ`) the admin panel's **Plus-One Approvals** card showed `Harpreet · added by Gurnam` with the error **"Couldn't update — your admin link may be out of date. Pull to refresh."** on tapping **Approve** (and Decline). The panel itself loaded fine, so the token was clearly valid for reads.
**Root cause:** the reporter is a **Vice Captain**, not the team owner. VCs open AdminView via `/p/<player_token>`, and App.jsx passes `route.token` (their PLAYER token) as `adminToken` to every admin RPC. `admin_approve_guest` / `admin_decline_guest` (from the mig-346 plus-one feature) hand-rolled a plain `SELECT id FROM teams WHERE admin_token = p_admin_token` with **no VC dual-lookup** — so a VC's player token never matched a team and the RPC raised `invalid_admin_token`, which the client maps to the "admin link out of date" copy. This is exactly the documented session-49 class (DECISIONS.md "admin_* RPCs MUST accept VC player tokens via dual lookup"); these two guest RPCs were the last stragglers that predated the `resolve_admin_caller` sweep. The owner path worked, masking it from owner test accounts.
**Fix (SQL-only, mig 530):** both RPCs now use the reference dual-lookup (mig 116 `admin_delete_player`): team `admin_token` → `team_admin`, else a Vice Captain's `players.token` **scoped to the target guest's team** → `vice_captain`, else `invalid_admin_token`. The resolved `actor_type`/ident is recorded in `audit_events`. No JS change — the supabase.js wrappers already forward the caller token unchanged.
**Gates:** ephemeral-verify (5/5: non-VC token rejected · cross-team VC rejected · VC approve → status=`in` · admin decline ok · VC decline ok; `_e2e_%` leak-check = 0), rpc-security PASS on both (SECDEF + `search_path=public,pg_temp` + single overload + anon/auth grants), live post-apply verify (both bodies carry the VC path + audit the VC actor). **Swept the other 33 `admin_*` RPCs — all already VC-safe via `resolve_admin_caller`; no remaining VC exposure.** ⛔ OWED: on-device VC walk (Hard Rule 13 — admin-token-gated, un-prod-verifiable).
...
DECISIONS.md / RPCS.md s171.

**RESOLVED (no code regression — stale Safari resume breadcrumb).** Symptom: operator on iPhone,
after Google sign-in, saw "Signed in — redirecting", then the page refreshed ~5 times and dumped
them back at the login screen. Root cause: a STALE Safari "resume" breadcrumb — earlier in the
same Safari session the operator had opened the demo player link (`/p/p_demo_alex_token`), which
the app saves as `ioo_redirect_to` / `ioo_last_visited` (on iOS Safari); after sign-in the redirect
bridge (App.jsx `getRoute`, ~lines 117–149) bounced them to that stale page. NOT a code
regression — the old code would do the same. **Deleting a home-screen PWA does NOT clear Safari's
localStorage**; clearing Safari → "History and Website Data" resolved it on the device. Resolution:
reverted Step 2 (`5962813`) as a precaution, confirmed a clean private-tab login worked, then
re-shipped Step 2 (`46807eb`) + AuthCallback breadcrumb-clear hardening (`57d563e` — clears the
resume breadcrumbs on a generic sign-in so a fresh sign-in always lands on the account landing,
not a stale page). Verified on local dev against a throwaway mirror admin: clean sign-in → admin
view; planted demo breadcrumb + sign-in via `/auth/callback` → breadcrumb cleared, lands on admin
view (not demo). Device-level check recorded in GO_LIVE_ISSUES.md.

**OPEN follow-ups / tech-debt (s171):**
- Optional: rename the chooser tile label "MANAGER" → "ADMIN" for clarity (currently "Manager"
  matches the ContextSwitcher badge).
- Owed: native-app real-device walk of the launch/landing/auth path before the next App Store
  build goes to review (PWA hard-rule device test).
- Apple deeper option (deferred): a full "is this you? link accounts" flow if Hide-My-Email
  duplicates ever actually occur (none so far).
