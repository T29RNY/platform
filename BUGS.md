...
- 📥 **`skills/` vs `Skills/` casing breaks every check-*.sh script on case-sensitive filesystems** — bug · effort ~S · the repo's top-level skills directory is committed as `Skills/` (capital S, confirmed via `git ls-tree`, introduced commit 085dade), but CLAUDE.md and every script under it — including `check-build.sh`'s own internal call to `skills/scripts/check-workspace-deps.sh` — reference lowercase `skills/`. Silently masked on the operator's Mac (case-insensitive filesystem); on any case-sensitive filesystem (Linux, this cloud sandbox, likely any CI) every check-*.sh script that dev-loop/qa-loop depend on fails immediately with "No such file or directory." Worked around for one nightly run with a local uncommitted `ln -s Skills skills` symlink — not a durable fix · source: qa-loop 2026-07-02 (nightly scripted) · captured 2026-07-02 · _open_
