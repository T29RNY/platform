...
## SESSION 182 — ✅ NIGHTLY QA (2026-07-23, scripted) — check-hygiene/nightly-routine case-sensitivity bug FIXED (PR #629). No app-code defects found.

Scripted deterministic lane (syntax + hygiene + 8-app build + e2e attempt) found `bash
skills/scripts/check-hygiene.sh` failing on check 8 (the Capacitor plugin-proxy
thenable-await footgun, PR #278) with "No such file or directory" — not a Capacitor
regression, a broken check. Root cause: PR #580 committed `check-plugin-proxy.sh`,
`check-advisors.sh`, `check-drift.sh`, `check-deploy-freshness.sh`, and
`advisors-baseline.json` under capital-S `Skills/` instead of `skills/`. Invisible on
case-insensitive macOS (normal dev machine), but every lowercase reference — the
hygiene hook, and the nightly drift-check/deploy-freshness/advisor-sweep routines
(FEATURES.md routines B/C/D) — silently failed on any case-sensitive filesystem (this
cloud session, and presumably every cron/CI run since). `BACKLOG_CAPTURE_HANDOFF.md`
had already documented the casing split as a "gotcha" to route around rather than a
bug to fix. **Fixed:** all five files `git mv`'d to their documented `skills/` paths,
`check-advisors.sh`'s internal `Skills/state` self-references corrected, stale
workaround note retired. `check-live-config` CLEAR (no app/RPC/migration/auth/money
surface). Merged via dev-loop, PR #629.

e2e suite (`qa-suite.sh`) could not produce a real signal this run: all 9 Playwright
projects errored in setup, not on test assertions — 6 login-based projects blocked by
this sandbox's network egress policy (`403 Host not in allowlist:` the live Supabase
project host), the other 3 blocked by a Playwright browser-cache version mismatch
(`chromium_headless_shell-1228` expected, `-1194` installed) specific to this
container image. Both are session/infrastructure limits, not app regressions — flagged
here for visibility since they'll block e2e on every future scripted run against this
same sandbox until the environment's network allowlist and pre-installed browser cache
are updated. No BUGS.md/GO_LIVE_ISSUES.md entry otherwise — not a code defect.
