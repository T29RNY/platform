...
- 🔴 **⚠️ SECURITY — Supabase "Confirm email" is OFF: anyone can mint a `confirmed` session for ANY address with the PUBLIC key, so every email-match claim is spoofable** — security/config · effort XS (a Supabase Auth setting, no code) · **This invalidates `email_confirmed_at` as a security signal platform-wide.** Proven live: `POST /auth/v1/signup` with only the app-bundle publishable key returns a working `authenticated` session with `email_confirmed_at` **already stamped**, for an address that cannot receive mail (`@example.invalid`). Corroborated read-only across all 50 real users: **`confirmation_sent_at` IS NULL for every one — not a single confirmation email has EVER been sent**; 45/45 confirmed users have `email_confirmed_at − created_at < 2s`; and `m-jassal@hot` (a real typo'd address with no TLD) is confirmed. The app's own UI is fine (`signInWithOtp`/`verifyOtp` genuinely proves ownership) — but an attacker never touches the UI; the GoTrue REST API is public. **Live impact today: `venue_claim_memberships` (mig 237/564, LIVE since 2026-07-16) claims a pending venue/club invite on `lower(auth.users.email)` with NO confirmation gate at all → anyone who signs up as an invited owner's email BEFORE they do takes that club (owner/manager/staff), silently, with no warning to the real invitee.** Currently **0 pending invites**, so nothing is exploitable *right now* — but it arms the instant an owner is invited, which is exactly the DF onboarding plan (⛔ **do not invite Danny until this is fixed**). Blocks mig 586 (PR #4b) entirely: its `email_confirmed_at` gate is a no-op, so it would let anyone claim a guardian identity → read+write on a child's `medical_conditions`/`allergies`/`medications` (`member_update_child`), ID documents (`guardian_*_id_document`), consent (`member_accept_consent`) and live location (`get_child_live_match`), auto-triggered on boot. **Fix = turn ON "Confirm email" in Supabase Auth settings — but ⚠️ NOT FIRST. Do the template step BEFORE the flip or you break signup for every new user.** ORDER: **(1)** In Supabase → Auth → Email Templates → **"Confirm signup"**, confirm it is the repo's `email-templates/confirm-signup.html` (must contain `{{ .Token }}`; NO `{{ .ConfirmationURL }}` button). This template has **NEVER been exercised** — `signInWithOtp` currently sends the *Magic Link* template, and GoTrue only switches a NEW email to *Confirm signup* once "Confirm email" is ON — so if it was never pasted, the flip silently activates Supabase's **default link-only** template and every new native signup dies (the link hits `/auth/callback`, not an AASA universal-link path → opens in Safari → the session never reaches the app; `email-templates/README.md:20-21` warns exactly this, and `project_native_app_wrap` still lists the paste + a fresh-email code walk as OWED). 4 live call sites do `signInWithOtp({shouldCreateUser:true})` (SignIn, AuthGateModal, EmailCaptureOverlay, JoinTeam) — i.e. every real sign-up route. **(2)** Then flip "Confirm email" ON. **(3)** Re-run the `/auth/v1/signup` probe and assert NO `access_token` comes back. **(4)** Fresh-email code walk on the native app (HR#13). **Impact of the flip itself is nil (measured):** all 45 real users are already confirmed; the 5 unconfirmed rows are inert seed fixtures (`d0d00000-…`, no email/identities/profiles/admin rows — they cannot sign in); **there is NO `signUp()` call anywhere in the codebase** (grepped apps+packages), so the setting gates only the raw REST path an attacker uses; `signInWithPassword` exists solely in `apps/venue` VenueSignIn = sign-IN for already-confirmed users. Belt-and-braces worth considering: don't trust `email_confirmed_at` at all (it is a historical fact about a row, not evidence about the caller) — prefer the session's provider/`amr`; and notify the claimed address on every successful claim so a takeover is visible. Found by the mig-586 adversarial security review · source: PR #4b review (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **Apple "Hide My Email" defeats the roster claim for ~16% of users** — bug · effort ~M · 8 of 50 live users sign in via `@privaterelay.appleid.com`. A DF parent told "sign in with the email we hold" who taps Sign in with Apple + Hide My Email gets a relay address that **is** legitimately verified (so it passes every gate) but can match NO roster email → `no_match` → no claim → they self-signup and DUPLICATE — the exact bug mig 586 exists to fix, unfixed on the most common iOS sign-in route. `App.jsx:1613` already detects the relay domain, so the signal exists. Same class: **email drift** (roster `danny@x.com` vs sign-in `danny+df@x.com` or a work address). Neither `no_match` nor `ambiguous` has ANY operator-facing surface — the audit row is the only trace and nothing reads it, so a stranded family is invisible. Needs a product answer (a "claim by invite code/link" path? an operator "link this account" screen? surface unclaimed shells on the club board?) before the real roster lands · source: PR #4b QA review (2026-07-16) · captured 2026-07-16 · _open_
- 📥 **`member_profiles.auth_user_id` has no UNIQUE index — two profiles per user is possible** — bug/tech-debt · effort ~XS · only `member_profiles_pkey` exists on the table. `member_claim_shell_on_signin`'s `already_linked` guard is TOCTOU against `member_self_create_profile`'s unconditional INSERT, so a concurrent signup could give one user TWO profiles — and `get_user_relationships` resolves the caller with `WHERE auth_user_id = v_uid LIMIT 1`, so it would then pick one arbitrarily and silently (the mig-070 `is_self` class of latent bug). Nobody has two today. A `UNIQUE` index on `auth_user_id` closes it for free; also worth an index on `lower(btrim(email))` (mig 586 does 3 seq scans per boot — harmless at 50 users, not at 500) · source: PR #4b adversarial review (2026-07-16) · captured 2026-07-16 · _open_
- 🔴 **An imported/admin-created member can NEVER claim their profile — and self-signup DUPLICATES them instead** — bug · effort ~M · **hard blocker for PR #4's import (and, once anyone is imported, for PR #6)**. Two halves: (1) `member_claim_profile` (mig 570) — the email-match claim — is written, wrapped (`supabase.js:5938`), exported (`core/index.js:246`) and called by **ZERO** app code; verified at the DB too (it is the ONLY function that can set `member_profiles.auth_user_id` on an existing row). (2) `member_self_create_profile` never looks for a claimable shell: it checks only `EXISTS(... WHERE auth_user_id = v_uid)` then INSERTs a NEW row. So an imported parent who signs up gets a **duplicate person** while their real child stays orphaned on the unclaimed shell — silently. `superadmin_import_club_roster`'s own header asserts the opposite ("a guardian claims their profile later via the existing email-match sign-in flow (member_claim_profile)") — that flow does not exist in any UI, and **no PR in DF_SPORTS_ONBOARDING_HANDOFF.md addresses it** (grep: no claim/dedup/match). Fix ≈ PR #564's shipped pattern (claim-on-sign-in, but for members). **THREE traps found in audit — do not skip any:** (a) `MembershipSignup.jsx:158` pre-fills the login email but lets the user EDIT it, and the RPC trusts `p_email` — so a claim MUST match `auth.users.email` (OTP-verified), NEVER `p_email`, or anyone could claim a stranger's child by typing their address; (b) a family email is on the PARENT **and** the CHILD (live today: `bennett.family@example.com` → Claire Bennett + Leo Bennett, 7) so an email match is genuinely AMBIGUOUS — an arbitrary `LIMIT 1` could claim a mother AS her own child, inheriting his medical/consent data; (c) ⛔ the obvious discriminator ("has a `member_guardians` row → is a child → not claimable") is **BANNED** — it is the exact rule mig 583 shipped and mig 584 operator-corrected, because DF's 16-yo coaches legitimately have guardians on file and it silently minted DUPLICATE people. Needs an operator decision on the claimable-shell rule before any code · source: DF demo-setup audit (2026-07-16) · captured 2026-07-16 · _open_
