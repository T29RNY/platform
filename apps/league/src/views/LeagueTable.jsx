...
import { leagueGetStandings } from "@platform/core/storage/supabase.js";

// Standings for the league's round-robin competitions (league_get_standings).
export default function LeagueTable({ state, leagueToken }) {
  const comps = useMemo(
  ...
  useEffect(() => {
    ...
    let alive = true;
    setError(null);
    leagueGetStandings(leagueToken, compId)
  ...
  if (comps.length === 0) {
    return (
      <main className="content mgmt">
        ...
        <div className="panel mgmt-empty"><p className="muted">No round-robin competition yet.</p></div>
      </main>
    );
  }
    ...
    <main className="content mgmt">
      ...
      {!error && (
        <motion.div className="panel lt-panel"
          ...
          {rows.length === 0 && <p className="muted" style={{ padding: "16px 18px" }}>No teams yet.</p>}
        </motion.div>
      )}
    </main>
  );
