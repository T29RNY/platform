...
export default function ScoreScreen({
  ...
  const bibEligible = inPlayers.filter(p => !p.isGuest);
  ...
  return (
    <div style={{ padding:18 }}>
      ...
      <button onClick={() => setWinner("D")} style={{ width:"100%", padding:"12px 0", borderRadius:6,
        ...
        fontFamily:"Inter,sans-serif", fontSize:14, fontWeight:700, cursor:"pointer", marginBottom:20 }}>
        🤝 Draw
      </button>

      {/* Bib holder — inline dropdown, optional */}
      <div style={{ marginBottom:22 }}>
        <div style={{ fontFamily:"Inter,sans-serif", fontSize:11, fontWeight:700, color:C.muted,
          letterSpacing:1, textTransform:"uppercase", marginBottom:8 }}>
          🟡 Who took the bibs? <span style={{ color:C.faint, fontWeight:500 }}>Optional</span>
        </div>
        <select value={bibHolder} onChange={e => setBibHolder(e.target.value)} style={{
          width:"100%", padding:"11px 13px", borderRadius:6,
          border:`1px solid ${bibHolder ? C.amber+"80" : C.border}`,
          background:"#0a0a0a", color: bibHolder ? C.amber : C.muted,
          fontFamily:"Inter,sans-serif", fontSize:13, fontWeight:500,
          outline:"none", appearance:"none", cursor:"pointer" }}>
          <option value="">— not selected —</option>
          {bibEligible.map(p => (
            <option key={p.id} value={p.name}>
              {p.name}{bibCounts[p.name] ? ` (taken ${bibCounts[p.name]}×)` : ""}
            </option>
          ))}
        </select>
      </div>
