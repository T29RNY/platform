...
//
// Compose a club announcement from the phone and queue it to the whole club or a
// single cohort. Delivery is the existing outbound broadcast cron (email + member
// push, within ~5 min) — this screen queues the row and shows a session-local
// "sent" list. Team-scoped comms + the inbound / two-way channel stay on the
// desktop console (documented deferrals, mirrored from the desktop composer).
//
...
//     desktop AUDIENCES const agrees). This screen sends only 'club' | 'cohort'
//     and always passes teamId = null.
...
import { clubSendAnnouncement, clubListCohorts } from "@platform/core";
...
export default function ClubAdminComms({ venueToken, clubId, clubName, toast }) {
  ...
  const send = useCallback(async () => {
    ...
    const label = audience === "club" ? "Whole club" : (selectedCohort?.name || "Cohort");
    try {
      await clubSendAnnouncement(
        venueToken, clubId, title.trim(), body.trim(), audience,
        audience === "cohort" ? cohortId : null, null,
      );
  ...
  }, [venueToken, clubId, title, body, audience, cohortId, selectedCohort, toast]);
