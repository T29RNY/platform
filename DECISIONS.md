...
## A squad rename syncs `teams.name` — for CASUAL squads only; league sides wait for a season lifecycle (2026-07-16)

**Decision (operator, 2026-07-16):** `admin_upsert_settings` — the product's only rename path — now writes **both** of a squad's names (`settings.group_name` **and** `teams.name`), restoring the invariant `create_team` already establishes by writing both at birth. Before mig 585 the rename wrote `group_name` alone, so `teams.name` could never be renamed, only drift: the My View header showed the new name while the context switcher, My Squads, the unified feed, the "Your teams" chooser and `guardian_list_child_team` (every `teams.name` reader) kept the original, **unfixable from the app** — it took a direct DB edit to correct. Only 2 squads had ever diverged, because renaming is rare.

**Scoped to `team_type='casual'`.** Operator's rule: *"league team can only rename at the end of a season."* For a competitive side `teams.name` is its identity in the league table and in already-published fixtures, so a squad-settings edit must not relabel it mid-season in front of its opponents. **That rule is currently unbuildable** — `'completed'` is a legal value for `seasons.status`/`competitions.status` but nothing in the codebase ever sets it, so every season is still `active` (two are already past their `end_date`, one of them with 5 fixtures unplayed — proving "end_date passed" and "actually finished" disagree). Gating the rename on season-end would mean a league team could **never** rename. So competitive teams keep the pre-585 behaviour (header only) until a season lifecycle exists, at which point this decision should be revisited rather than assumed. Chose casual-only over sync-everything because the failure modes aren't symmetrical: too-cautious costs a briefly inconsistent league name we can widen later; too-eager relabels a live league table mid-season, in public, irreversibly-ish.

**Not fixed at the resolver.** Making a player token an admin credential (or adding a stage-3 to `resolve_admin_caller`) was rejected for the sibling PR #565 for the same structural reason: ~13 admin RPCs hand-inline their own copy of that check, so a resolver-only change half-works. See BUGS.md 2026-07-16 (both entries) + RPCS.md mig 585.

## Payment reliability = ledger-derived, admin-confirmed-paid only; `players.pay_count` is dead (2026-07-15)

**Decision (operator, 2026-07-15):** the Stats "Payment Reliability" card is computed from `payment_ledger` — the single payment source of truth — **not** the flat `players.pay_count` column, which no server RPC has incremented since payments moved to the ledger (see BUGS.md 2026-07-15 + RPCS.md mig 576). Per-player reliability = **admin-confirmed paid** game_fee rows ÷ total game_fee rows, all-time. A player's unconfirmed **self-pay claim** (mig 211: "I've paid", ledger row stays `unpaid` until an admin confirms) deliberately does **NOT** count as paid — the card reflects money *actually settled*, matching how `gaffer_get_context_payment_summary` (mig 035) already defines `always_paid_players`. Operator picked confirmed-only over also-crediting pending claims. Reliability stays **all-time / period-independent** (like attendance reliability), never period-filtered — consistent with the existing CLAUDE.md rule. `players.pay_count` is now confirmed-dead in the app (a later schema-sync-gated column drop candidate; left in place for this read-only fix).

## Shell-unify: in-hat switcher rows route IN-APP to /hub — NO "Open full console" link-out (2026-07-15)

**Decision (operator, 2026-07-15):** in the casual context switcher, a person's non-casual hats (operator / club-admin / team-manager / referee / guardian) open their **in-app `/hub` home** for that hat. The original D8 scope added a secondary **"Open the full console"** link out to the external `platform-venue` / `platform-ref` desktop web app; that is **scrapped**. Rationale (operator's challenge): *"why are we giving users a button to open console outside of the app? we want users and features to be in app. what will the mobile web console do on a phone that an app won't?"* — nothing better: on a phone the external console is a cramped desktop-web layout, and the native app **can't carry its session** to a separate Vercel app (it would bounce the user to a fresh login). The desktop console stays a **laptop** tool the operator opens themselves. Any deep feature genuinely needed on mobile becomes a targeted **"build it into `/hub`"** backlog item, never a link-out. Shipped in PR #537 (`24efc867`); removed the now-unused `REF/VENUE/CLUB_APP_BASE` constants. Consequence (accepted): the referee lands on the `/hub` fixtures list (officiating via the in-app iframe) rather than a per-assignment deep-link, and a multi-team coach lands on the coaching home (per-team precision awaits a MobileShell `team` selector). See `SHELL_UNIFY_HANDOFF.md`, [[project_shell_unification]].

## Tournament creation = operators + club-admins ONLY (never team managers) (2026-07-13)

**Decision (operator, 2026-07-13):** creating a tournament is a **venue-operator / club-admin** action, not a team-manager one. On `/hub` the create surface lives on the **operator** hat (`venue_admin` → "Operator") and the **club-admin** hat (`admin_clubs` → "Admin"); the **team_manager** hat ("Manager", e.g. "U7 Milan Manager") deliberately has **no** create-tournament entry. A team is a unit *inside* a club (U7 Milan is a team in the PA Sports club, which the same person Admins), so running a club-wide cup is done by whoever admins the club/venue — a single-team manager creating tournaments is out of scope. Operator's words: *"happy for PA Sports operators only to create tournaments."* The `team_manager` role also carries no venue credential, so wiring create there would need backend work — explicitly declined.

**In-app create ownership (2026-07-13, PRs #513–#515):** the in-app create OWNS the new tournament to the caller's venue/club via `venue_create_tournament` / `club_admin_create_tournament` — **not** `self_serve_create_tournament`, which mints a hidden personal-host venue that would never surface in the operator's console. Owning it to `role.entityId` (venue) / `role.clubId` is what keeps an app-created tournament in sync with the desktop venue console + the public page. It auto-adds one competition (format-mapped: `cup/single_elimination` · `league` · `cup/group_stage`) + publishes (`open`) so it's immediately shareable, matching the casual one-shot. **Entry fee is informational** (shown as "£X per team"; registering takes no payment). **Multi-team entry = the public tournament page link** (unlimited self-registration, host approves each pending team); the single-use invite (desktop console only) is for inviting one specific team. Casual `CreateTournament.jsx` left untouched (casual-regression).
...
Full plan: `~/.claude/plans/once-the-ios-app-dapper-marshmallow.md`; design brief
`WATCH_DESIGN_BRIEF.md`; tracker FEATURES.md "## WATCHOS COMPANION APP". Build starts only AFTER
iOS App Store approval. Settled questions:

1. **Architecture.** ONE native SwiftUI watch target added inside the existing Capacitor iOS Xcode
   project (`apps/inorout/ios`). The iPhone app stays the untouched webview wrap → **one App Store
   record, the watch is the only native code**, runs independently (watchOS 10+). NOT a second iOS
   app; NOT Capacitor on the watch.
2. **Scope = full app, all contexts, football-first.** League ref (fixtures), casual ref (one squad
   member assigned per game — net-new "casual ref slot"), club cohort officiating (net-new
   cohort→official binding). Not a cut-down v1.
3. **Identity-first, NOT token-handoff-only.** Net-new ref/official identity layer (link
   `match_officials`↔auth user) + a single resolver `get_my_next_assignment` keyed on `auth.uid()`
   returns the user's next relevant game across all contexts incl. the `ref_token` to drive it.
   Resolver must disambiguate a multi-role user with same-day games (in-progress > soonest kickoff
   > role priority) + a "you have N games" chooser — never silently pick.
4. **Provider-agnostic watch auth** (the user base signed up via Google / email+password / company
   SSO / Apple — one Supabase identity pool). **(1) primary = WatchConnectivity session handoff
   from the paired iPhone** (works for every original method), **(2) standalone fallback = email
   OTP** (resolves to the same account by email), **(3) bonus = native Sign in with Apple** (Apple
   users + review compliance). Direct Google/company OAuth on watchOS is NOT viable (no
   `ASWebAuthenticationSession` on watchOS) — covered by (1)+(2).
5. **Health = refs only; auto start/stop; no manual controls.** `HKWorkoutSession` `.soccer`
   starts when the ref taps "Start Match" ON THE WATCH and ends on "Full Time" (warn on watch +
   iPhone that health only runs if the match is started from the watch). Metrics MIRROR Apple's
   **Outdoor Football** (duration, active energy, distance, avg+max HR) **+ HR Workout Zones
   (watchOS 27)** live + post-match; graceful fallback to avg/max HR on watchOS 26. Non-ref players
   just use Apple's own Outdoor Football (identical data quality — same activity type).
6. **Health data = UK-GDPR special category.** Store the SUMMARY only (never the raw stream); the
   new `match_health_sessions` table MUST be added to `api/delete-account.js` purge in the SAME
   migration it is created. Triggers a privacy-policy update + App Store "Health & Fitness" label.
7. **2026 / watchOS-27 features committed to v1** (design-load-bearing): Always-On display state,
   haptics (half-time/full-time/sin-bin), Double Tap to confirm, complication + Smart Stack widget,
   and **Live Activity + Dynamic Island on iPhone** (live score/clock/HR, driven by the existing
   realtime broadcast — PROMOTED to core). Backlog: Siri/App Intents voice-start, Ultra Action
   Button mapping, iPhone home/lock widgets, SharePlay, advanced vitals.
8. **Tech.** SwiftUI + official `supabase-swift` SDK (auth + RPC + realtime), straight to the
   existing `ref_*` RPCs — no new API layer. Port the `apps/ref` offline/idempotency engine
   (`client_event_id`) verbatim — do not reinvent.
9. **Sequencing + velocity.** After iOS approval; run solo (cloud-session discipline); migrations
   from 369. ETA recalibrated to AI-build velocity: the bottleneck is operator real-device testing
   + Apple review, not code. MVP ~1–2 wks, full app ~2–4 wks + review. The HealthKit privacy/review
   package (Phase 7) is the one thing NOT to rush (a rejection costs a full review cycle).
...
## Apple App Store first; Google Play parked until after Apple approval (session 160)

**Decision (operator, s160).** Ship the **iOS App Store first**. The Google Play submission and the
entire Android build/console leg are **PARKED until after Apple approves the iOS app**. This is a
sequencing/focus decision, not an architecture change: the Capacitor wrap stays **cross-platform in
code** (`apps/inorout` builds for both; nothing to remove or undo). Only the Play-side work waits —
0.4 Play Console enrolment, 3.2 Firebase `google-services.json`, the Android half of 3.3
(`assetlinks.json` + SHA-256), 3.7 Android upload keystore, 4.2 Play graphics, 4.5 Google IARC age
rating, the Android `.aab` in 5.4, and Play's half of Stage 6.

**Why.** Single-store focus gets the product live fastest and avoids the Android-specific friction up
front — notably a **personal** Play account hitting the 12-tester / 14-day closed-test gate (an org
account avoids it; revisit at resume). Apple's Guideline 4.2 "just a website" challenge is the main
review risk and is worth the team's full attention on one store first. Android resumes after Apple
approval as a fast follow (the wrap is already cross-platform). See `APP_STORE_CHECKLIST.md` item 0.4
+ the Apple-first banner.

## watchOS companion app — ref view + lightweight football workout tracker (session 160, NOTED only)

**Direction (operator, s160) — NOT scoped/started.** A future **watchOS** companion app with two
surfaces: (1) a **ref view on the wrist** — wearable companion to `apps/ref` (Ref V2 backend, migs
261–267) so a referee runs the clock/score/sin-bin/added-time from the watch, almost certainly a
glanceable control + live-state mirror over the existing `ref_*` RPCs + realtime broadcast; (2) a
**lightweight football-specific workout tracker** (metrics TBD with the operator — keep it light,
a companion habit not a Strava). **Key constraint:** a watch app is a **separate native target**
(Swift/SwiftUI), **NOT** part of the Capacitor wrap — it's a new platform/epic, not an extension of
the iOS shell. **Sequencing:** starts AFTER the App Store epic reaches Apple approval; gets its own
handoff doc when it begins. Full scope + open questions (companion-to-which, data path, which metrics,
HealthKit/privacy-label implications, bundle ID / same Apple Team `JCC44FW6XR`) live in FEATURES.md
"## WATCHOS COMPANION APP". Logged now so the intent survives; do not build until prioritised.
