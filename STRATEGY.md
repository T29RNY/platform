...
> **🏁🏁 #5 + #6 — EPIC COMPLETE (3 phases, migs 416–418, P3 SHIPPED s197) → `PITCH_PRIORITY_HANDOFF.md`.** P3 (mig 418) = calendar surfacing: ⭐/#n rank badge on club blocks across all 3 venue grids + Accept/Decline bump prompt for operator (venue banner) AND club manager (player app). UI-only + one additive `priority_rank` field on the occupancy-detail helper.
...
> (s195):** bumping is **club-team-vs-club-team ONLY** (never auto-bumps a paying outside hire —
> protect club time up front with reserved windows); rank decides not arrival order; **suggest-and-
...
> gate + rank bump/reallocate (touched `book_pitch_*` → casual-regression PASS) ✅; P3 (mig 418) calendar surfacing — ⭐ badge + Accept/Decline bump UI ✅. **Epic complete.**
...
> shipped `tournaments` flag). Next free mig = 425.
...
| 5 | Internal vs external pitch booking + reserved/priority times | **🏁 EPIC COMPLETE (migs 416–418)** → `PITCH_PRIORITY_HANDOFF.md`. P1 = `pitch_reserved_windows` table + config + advisory shading. P2 = external gate LIVE (`book_pitch_*` → `slot_reserved`; `venue_create_booking*` warn-only override via `_pitch_window_blocks`). **P3 (mig 418) = calendar surfacing: ⭐ rank badge on club blocks + Accept/Decline bump prompt (operator + manager).** Linked pair with #6 | Med (done) | 🏁 done |
| 6 | Team prioritisation system (some teams rank above others) | **NOW DRIVES CONTENTION (mig 417)** — `club_teams.priority_rank` ⭐ (mig 389, was display-only) makes higher-ranked club teams auto-bump lower-ranked ones off a contested slot (never a paying hire): bumped event → `tentative` + suggest-and-confirm closest free slot across the operator's venues (`pitch_bump_proposals` + `_reserve_club_occupancy`/`_closest_available_slot`) → manager/venue Accept moves it / Decline stays tentative. **P3 (mig 418): ⭐ rank badge on club calendar blocks + Accept/Decline UI surfaced for operator (venue banner) + manager (player app) → epic complete.** | Med (done) | 🏁 done |
| 7 | Multi-venue (train one site, play another) | **🏁 EPIC COMPLETE (Phases 1–3, migs 412–414, PRs #84–86→main)** → `MULTI_VENUE_HANDOFF.md`. Same-operator club (via `venues.company_id`) trains/plays at either of 2 pitch venues: P1 venue-anchored `club_sessions` (mig 412), P2 cross-venue fixtures (mig 413), P3 cross-site pitch clash-protection (`pitch_occupancy` triggers → `slot_unavailable`, mig 414) + `get_operator_pitch_occupancy` ground switcher + `AllGroundsGrid` all-grounds calendar (s194, UI-only). cross-CLUB still deferred (settlement+safeguarding, DECISIONS s180). ⛔ owed venue deploy + 2-venue device walk | Med (done) | 🏁 done |
...
| 18 | **Unified resource calendar** (one calendar: pitches + rooms + classes + equipment, filterable) | 🏁 **P1–P3 BUILT** → `UNIFIED_RESOURCE_CALENDAR_HANDOFF.md`. P1 (mig 419, live s198) = ONE calendar across all operator venues (pitches+rooms+classes+trainers + equipment strip) via `get_venue_resource_occupancy` + shared `GroupedColumnGrid`; P2 = book-from-calendar (reuse-only, no mig); Phase 2b (mig 423) = operator create room-hire + PT appt from the calendar (DEPLOYED live s202); **Phase 3 (mig 424, PR #106→main) = hard room/trainer clash protection** (`resource_occupancy` ledger + btree_gist EXCLUDE + 3 sync triggers; EV re-verified s203, 10/10, leak 0). Phase 2c deferred. ⛔ owed venue deploy + operator device walk | Med (done) | 🏁 done |
